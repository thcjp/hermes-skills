...
description: "基于Azure CLI的多订阅导航与资源清单查询,提供只读查询与VM健康检查。"
license: MIT
...
> **升级提示**: RBAC 角色审计（Owner 过度授权检测）、成本分析工作流（Cost Management）、合规检查（NSG/存储安全）、跨订阅批量操作、分层权限模型（L0-L4）等高级功能仅在 [azure-cloud-architect 付费版] 中提供。- 验证执行结果，确认输出符合预期格式
- 参考`资源清单查询`相关配置参数进行设置
#
## 适用场景
