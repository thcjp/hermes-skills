...
description: "轻量级JSON语法校验工具，递归扫描工作区.json文件并输出结构化错误报告。"
license: Proprietary
...
**处理**: 按照skill规范执行目录递归扫描操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行结构化报告操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行通过率统计操作,遵循单一意图原则。
