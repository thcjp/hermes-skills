...
description: "使用python-docx读写与编辑DOCX文档，支持基础格式化与视觉校验，适合日常文档处理。"
license: Proprietary
...
**处理**: 按照skill规范执行文档操作矩阵操作,遵循单一意图原则。
**输出**: 返回文档操作矩阵的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行支持的文档元素操作,遵循单一意图原则。
**输出**: 返回支持的文档元素的执行结果,包含操作状态和输出数据。
