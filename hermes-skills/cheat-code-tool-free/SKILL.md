...
description: "为AI代理提供训练数据之外的外部知识检索能力,扩展问题解决边界"
license: Proprietary
...
**输入**: 用户提供外部知识检索所需的指令和必要参数。
**处理**: 按照skill规范执行外部知识检索操作,遵循单一意图原则。
**输出**: 返回外部知识检索的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供无状态架构所需的指令和必要参数。
**处理**: 按照skill规范执行无状态架构操作,遵循单一意图原则。
**输出**: 返回无状态架构的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供加密传输所需的指令和必要参数。
**处理**: 按照skill规范执行加密传输操作,遵循单一意图原则。
**输出**: 返回加密传输的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供令牌授权所需的指令和必要参数。
**处理**: 按照skill规范执行令牌授权操作,遵循单一意图原则。
**输出**: 返回令牌授权的执行结果,包含操作状态和输出数据。
