...
description: "通过 Notion API 操作页面与数据库,支持读取、创建、更新与架构管理"
license: Proprietary
...
**输入**: 用户提供读取页面内容并追加块所需的指令和必要参数。
**处理**: 按照skill规范执行读取页面内容并追加块操作,遵循单一意图原则。
**输出**: 返回读取页面内容并追加块的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 创建页面并设置父级与标题
创建页面并设置父级与标题

**输入**: 用户提供创建页面并设置父级与标题所需的指令和必要参数。
**处理**: 按照skill规范执行创建页面并设置父级与标题操作,遵循单一意图原则。
**输出**: 返回创建页面并设置父级与标题的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查询数据库并过滤排序
查询数据库并过滤排序

**输入**: 用户提供查询数据库并过滤排序所需的指令和必要参数。
**处理**: 按照skill规范执行查询数据库并过滤排序操作,遵循单一意图原则。
**输出**: 返回查询数据库并过滤排序的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 创建和更新数据库行
创建和更新数据库行

**输入**: 用户提供创建和更新数据库行所需的指令和必要参数。
**处理**: 按照skill规范执行创建和更新数据库行操作,遵循单一意图原则。
**输出**: 返回创建和更新数据库行的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查数据库架构
检查数据库架构

**输入**: 用户提供检查数据库架构所需的指令和必要参数。
**处理**: 按照skill规范执行检查数据库架构操作,遵循单一意图原则。
**输出**: 返回检查数据库架构的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
