...
description: "轻量级Telegram Bot配置与跨实例聊天工具,支持用户艾特、消息收发与基础群组管理。Use when 需要消息发送、通知推送、邮件短信、通信集成时使用。不适用于垃圾信息群发。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
**输出**: 返回Bot 创建与配置的执行结果,包含操作状态和输出数据。
...
**输出**: 返回用户艾特(@提及)的执行结果,包含操作状态和输出数据。
...
- **分类**: MD+execute(纯 Markdown 指令,核心功能通过配置文件与 Telegram API 调用实现)
