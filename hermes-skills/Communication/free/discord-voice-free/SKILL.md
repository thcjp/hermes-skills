...
description: "Discord语音助手免费版,提供基础加入/离开/状态查询与本地Whisper转写"
license: MIT
...
**输出**: 返回仅支持本地 Whisper STT的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回仅支持默认配置的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
