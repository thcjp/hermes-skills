---
name: "slack-free"
description: "通过slack工具发送与读取Slack消息,支持频道通知与历史消息查询。Use when 需要消息发送、通知推送、邮件短信、通信集成时使用。不适用于垃圾信息群发。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
license: MIT
allowed-tools: read
...
### 可用性分类
- **分类**: MD+execute(纯 Markdown 指令,消息操作需要 exec 调用 slack 工具)
