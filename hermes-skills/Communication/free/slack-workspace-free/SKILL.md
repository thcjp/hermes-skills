...
description: "Slack基础消息发送与频道列表，ClawLink OAuth托管连接"
license: MIT
...
**输入**: 用户提供频道列表所需的指令和必要参数。- 验证执行结果，确认输出符合预期格式
- 参考`频道列表`相关配置参数进行设置
### 3. 用户列表
列出工作区内全部用户，返回用户ID、姓名、邮箱、是否在线等。可用于成员查询与用户ID获取。

> **升级提示**：线程回复、定时消息、频道创建/归档/重命名、文件上传/下载、表情反应、提醒、画布、用户组管理、企业Grid审计日志等高级功能仅在 slack-workspace 付费版中提供。- 验证执行结果，确认输出符合预期格式
- 参考`用户列表`相关配置参数进行设置
#
## 适用场景
