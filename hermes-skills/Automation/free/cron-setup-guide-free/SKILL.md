...
description: "Agent Gateway定时任务设置完全指南免费版，覆盖三种调度类型、会话模式、投递配置与基础作业管理。Use when 需要项目管理、任务规划、进度跟踪、团队协作时使用。不适用于实际人员绩效评估。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Agent、Gateway、定时任务设置完全、指南免费版、覆盖三种调度类型、会话模式、投递配置与基础作、业管理、定时任务设置指南、免费版是面向、的定时调度配置手、不同于优选实践类、本技能聚焦、如何正确配置定时、的技术细节、调度类型选择、会话目标决策、投递通道设置、JSON、Schema、作业生命周期管理、Use、when、需要项目管理、任务规划、进度跟踪、团队协作时使用、不适用于实际人员、绩效评估等。
