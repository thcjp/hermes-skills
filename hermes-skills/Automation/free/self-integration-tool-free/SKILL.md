...
description: "通过统一网关连接任意外部应用并执行操作，适合个人开发者快速打通多平台。Use when 需要API集成、接口对接、Webhook配置、系统连接时使用。不适用于逆向工程闭源API。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
license: Proprietary
...
当需要将外部应用能力作为工具接入Agent时，本工具提供统一连接层：协议端点通过本工具建立连接并执行动作，无需为每个应用单独实现协议适配层适配。
