...
summary: "企业级 Notion 技能系统,支持多配置文件、架构变更管理、安全审计与批量数据操作。核心能力: 知识管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
...
**处理**: 解析多配置文件管理(persona的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多配置文件管理(persona的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 数据库架构变更与差异对比
数据库架构变更与差异对比

**处理**: 解析数据库架构变更与差异对比的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回数据库架构变更与差异对比的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 批量数据导入导出与格式转换
批量数据导入导出与格式转换

**处理**: 解析批量数据导入导出与格式转换的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量数据导入导出与格式转换的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 安全审计
安全审计:操作日志与权限审查

**处理**: 解析安全审计的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回安全审计的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Notion、技能系统、支持多配置文件、架构变更管理、安全审计与批量数、据操作、核心能力、知识管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | Notion 技能专业版处理的输入数据或指令 |
...
```bash
export NOTION_API_KEY=secret_未指定
...
```json
{
  "success": true,
  "data": {
    "result": "Notion 技能专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "notion skill pro"
    }
  },
...
- **自动化执行**: 企业级 Notion 技能系统,支持多配置文件、架构变更管理、安全审计与批量数据操作。核心能力: 知识管理领域的专业化 
