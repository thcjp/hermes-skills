...
license: Proprietary
edition: pro
description: "Word文档工具V1 - （专业版）。可生成提升工作效率。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
Word文档工具V1是针对文档处理领域的专业化AI辅助工具。专业版面向企业用户，提供完整的功能体系，包含高级特性、批量处理与企业级集成能力.
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Word、文档创建与编辑工、具基础版本、支持段落、列表与基本格式化、文档工具等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
# ...
    def add_paragraph(self, text: str,
        ...
        if fmt:
            ...
            para.alignment = align_map.get(fmt.alignment, WD_ALIGN_PARAGRAPH.LEFT)
...
# ...
    def set_page(self, width: float = 21, height: float = 29.7,
        ...
        if landscape:
            ...
            section.page_width = Cm(height)
            section.page_height = Cm(width)
        else:
...
# ...
    def batch_create(self, specs: List[dict],
        ...
        for i, spec in enumerate(specs):
            engine = WordV1Engine()
            if spec.get("page"):
                engine.set_page(**spec["page"])
            for section in spec.get("sections", []):
                if section["type"] == "heading":
add_heading(section["text"], section.get("level", 1))
                elif section["type"] == "paragraph":
add_paragraph(section["text"])
                elif section["type"] == "list":
add_list(section["items"], section.get("ordered", False))
                elif section["type"] == "table":
add_table(section["data"])
...
engine.add_heading("项目方案", level=0,
    fmt=FormatConfig(font_name="SimHei", font_size=22, color="1F4E79"))
engine.add_paragraph("本方案详细说明了项目实施计划。",
    fmt=FormatConfig(font_size=12, alignment="justify", line_spacing=1.5))
engine.add_list(["需求分析", "方案设计", "开发实施", "测试验收"], ordered=True)
engine.add_table([["阶段", "时间", "负责人"], ["分析", "1周", "张三"], ["设计", "2周", "李四"]])
engine.save("project_plan.docx")
print("项目方案已创建")
```
...
```yaml
word_v1:
  features: [heading, paragraph, list, table, image, header_footer]
  formatting: advanced
  page_setup: true
  batch:
    max_files: 50
...
```json
{
  "success": true,
  "data": {
    "result": "Word文档工具V1（专业版）处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "word docx v1 pro"
    }
  },
...
- **自动化执行**: Word文档创建与编辑工具基础版本，支持段落、标题、列表与基本格式化.,支持多种使用场景和自动化处理
