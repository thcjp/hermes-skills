...
license: Proprietary
edition: pro
description: "XML读取器工具专业版，面向数据团队与企业的高阶XML读取与分析平台。核心能力:。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
  - 流式读取大文件（GB级）
...
```json
{
  "success": true,
  "data": {
    "result": "XML读取器专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "xml reader pro"
    }
  },
