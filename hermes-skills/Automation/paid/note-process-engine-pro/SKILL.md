...
license: Proprietary
edition: pro
description: "笔记处理引擎(专业版)是面向研究团队与企业的全功能笔记分析Skill,在免费版基础上新增语义检索、自动标签、跨库联合检索、可视化、导出报告、定时分析等高级能力。核心能力:。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。"
...
note_process_engine.py similar --note-id note_未指定 --top-k 5
```
...
note_process_engine.py sync-to-warehouse \
  --source research_db \
  --destination 数据库://user:pass@host:5432/knowledge_db \
  --mode incremental
```
...
note_process_engine.py schedule add --cron "0 9 * * 1" \
  ...
  --notify wecom:https://qyapi.weixin.qq.com/cgi-（请参考skill目录中的脚本文件）?key=未指定
```
...
## 已知限制
- 语义检索准确率受向量模型质量影响，专业领域术语可能匹配不准，建议结合关键词检索互补使用
- 跨库联合检索性能随库数量增长而下降，超过5个库时建议限定检索范围或启用分布式模式
- 自动标签推荐依赖聚类算法，标注样本不足时准确率较低，建议先提供50条以上人工标注样本
- 可视化功能依赖matplotlib，中文字体需额外安装（思源黑体），否则图表中文显示为方块
- PDF导出依赖wkhtmltopdf或weasyprint引擎，未安装时PDF生成会失败，可改用HTML格式
- 定时分析依赖cron服务，Windows环境需额外配置cron或使用任务计划程序替代
- 分布式处理需Docker环境支撑，单机部署处理能力上限约10万条笔记
- 免费版数据无法自动迁移到专业版向量索引，升级后需手动触发索引重建
...
```json
{
  "success": true,
  "data": {
    "result": "笔记处理引擎(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "note process engine pro"
    }
  },
...
| 对比维度 | 笔记处理引擎(专业版) | 传统手动方式 | 通用脚本工具 |
|---------|------------|-------------|------------|
| 自动化程度 | 全流程自动 | 完全手动 | 部分自动 |
| 错误处理 | 内置错误恢复 | 依赖人工经验 | 基本try-catch |
| 可复用性 | 参数化配置 | 一次性脚本 | 模板化 |
| 安全合规 | 内置安全检查 | 无安全保障 | 无安全保障 |
| 适用场景 | 企业级研究笔记分析平台,支持语义检索、自动标签、跨库联合、可视化与导出报告,适合 | 通用场景 | 通用场景 |

## 核心功能

- **自动化执行**: 企业级研究笔记分析平台,支持语义检索、自动标签、跨库联合、可视化与导出报告,适合研究团队与企业规模化使用.
