...
license: Proprietary
edition: pro
description: "XML处理工具专业版，面向数据团队与企业的高阶XML处理平台。核心能力:。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
  - 流式解析大文件（SAX/StAX）
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：流式解析、格式互转与批量处、适合数据团队与企、数据治理、处理工具专业版、面向数据团队与企、业的高阶、处理平台、核心能力、流式解析大文件、批量处理与目录递、Use、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
```json
{
  "success": true,
  "data": {
    "result": "XML处理工具专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "xmlkit pro"
    }
  },
