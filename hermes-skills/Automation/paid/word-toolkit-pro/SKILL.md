...
summary: "批量文档处理、模板工作流、修订审计与跨平台支持，适合团队与企业文档自动化.。Word文档控制工具专业版，面向团队与企业的高阶Word文档自动化平台。核心能力:"
license: Proprietary
edition: pro
description: "Word文档控制工具专业版，面向团队与企业的高阶Word文档自动化平台。核心能力:。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
  - 批量文档处理与目录递归
...
**Q：模板支持哪些变量？**
A：支持文本、日期、数字等变量。在模板中用 `` 标记，通过 CSV 提供数据.
**Q：修订审计数据存储在哪里？**
