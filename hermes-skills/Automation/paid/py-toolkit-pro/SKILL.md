...
license: Proprietary
edition: pro
description: "企业级 Python 开发工具专业版,面向团队与生产环境。核心能力:。可自动提升工作效率。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
  - 性能分析与优化深度指导
...
# Python 工具包 - 专业版
## 概述
Python 工具包专业版是企业级 Python 开发平台,在免费版常见陷阱与优选实践基础上扩展性能优化、异步编程、安全编码、CI/CD 集成与内存分析。适合高性能服务开发、异步 Web 应用与企业级 Python 项目规范建设.
...
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 3. 多进程与分布式计算
multiprocessing 优选实践,concurrent.futures 线程/进程池选择,分布式任务队列(Celery/RQ)集成.
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、开发平台、支持性能优化、代码规范与、开发工具专业版、面向团队与生产环、核心能力、性能分析与优化深、度指导、异步编程完整方案、算优选实践、企业级代码规范与、安全编码与漏洞预、集成与自动化测试、包管理与发布流水、内存分析与泄漏检等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
### 场景一:性能优化分析
...
- **自动化执行**: 企业级Python开发平台,支持性能优化、异步编程、代码规范与CI/CD集成。企业级 Python 开发工具专业版,面向
