...
license: Proprietary
edition: pro
description: "面向连锁品牌、电商运营公司与代运营机构的企业级店铺运营平台。可生成提升工作效率。Use when 需要电商运营、商品管理、订单处理、支付集成时使用。不适用于虚假交易和刷单。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
### Step 2: 配置凭证
```bash
export SHOP_CULTURE_ADMIN_KEY="sk_pro_admin_未指定"
