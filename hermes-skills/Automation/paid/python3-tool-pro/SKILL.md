...
summary: "企业级 Python 环境管理系统,支持多版本管理、依赖审计、CI/CD 集成与安全扫描。核心能力: 开发工具领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 开发工具领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要安全检测、合规审计、漏洞扫描、加密防护时使用。不适用于渗透测试未授权目标。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
本工具是 **开发工具** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析多 Python 版本管理与切的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多 Python 版本管理与切的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 依赖安全审计与漏洞扫描
依赖安全审计与漏洞扫描

**处理**: 解析依赖安全审计与漏洞扫描的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回依赖安全审计与漏洞扫描的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析依赖许可证合规检查的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回依赖许可证合规检查的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 性能分析与内存 profili
性能分析与内存 profiling

**处理**: 解析性能分析与内存 profili的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回性能分析与内存 profili的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：环境管理系统、支持多版本管理、依赖审计、集成与安全扫描、核心能力、开发工具领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | Python 环境工具专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "Python 环境工具专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "python3 pro"
    }
  },
...
- **自动化执行**: 企业级 Python 环境管理系统,支持多版本管理、依赖审计、CI/CD 集成与安全扫描。核心能力: 开发工具领域的专业
