...
license: Proprietary
edition: pro
description: "面向游戏工作室、教育与商业展示场景的专业 3D 游戏开发平台。可生成提升工作效率。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
...
### Step 2: 配置专业版凭证
```bash
export GAME_BUILDER_ADMIN_KEY="sk_pro_admin_未指定"
...
### Q3: 支持哪些 3D 模型格式?
GLTF 2.0 (推荐)、GLB、FBX、OBJ。建议使用 GLTF 以获得优选 web 兼容性.
### Q4: 商业授权范围?
...
export CDN_DISTRIBUTION_ID="未指定"
...
```json
{
  "success": true,
  "data": {
    "result": "3D游戏构建器专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "game builder pro"
    }
  },
