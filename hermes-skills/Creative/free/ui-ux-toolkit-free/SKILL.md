...
description: "可搜索的UI/UX设计数据库,提供风格、配色、字体、UX规则的基础查询,适合个人开发者快速生成设计系统。Use when 需要设计创作、UI设计、海报制作、品牌视觉时使用。不适用于3D建模和动画制作。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
```bash
# 步骤1生成基础设计系统
py "SaaS productivity tool minimal" --design-system -p "TaskFlow"

# 步骤2补充字体搜索
py "modern clean sans-serif" --domain typography

# 步骤3获取Tailwind实现指引
py "dashboard card layout" --stack html-tailwind
```
...
```bash
# 搜索适合播客/媒体类产品的风格
py "podcast media creative bold" --domain style
...
# 检查布局和响应式规则
py "responsive viewport" --domain ux
```
...
py "soft pastel" --domain color

# 第3步:获取技术栈指引
py "layout responsive" --stack html-tailwind
```
...
# 推荐:具体到产品类型和行业
py "healthcare SaaS dashboard minimal" --design-system
```
...
```bash
# 风格 + 字体 + 配色 = 完整设计系统
py "glassmorphism" --domain style
py "modern sans-serif" --domain typography
py "fintech" --domain color
```
...
```bash
# 开发完成后必查
py "accessibility focus keyboard" --domain ux
py "animation performance" --domain ux
```
