...
description: "轻量短视频一键生成工具，支持画面规划、AI素材生成、TTS配音与视频渲染，适合个人创作者快速出片。Use when 需要视频处理、音频编辑、媒体转换、配音生成时使用。不适用于版权受保护的媒体内容处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
export TTS_API_KEY="${API_KEY:?请设置环境变量}"
```
