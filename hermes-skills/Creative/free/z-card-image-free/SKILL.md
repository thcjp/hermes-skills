...
description: "将短文案渲染为 PNG 海报,支持公众号配色与整行高亮,基础渲染能力"
license: MIT
...
**输出**: 返回渲染管线的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`渲染管线`相关配置参数进行设置
### 环境检测
- `python3 --version` 失败则提示未检测到 Python 3,渲染可能失败
- 检查 Chrome 路径,失败则提示安装
...
**输出**: 返回场景识别的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`场景识别`相关配置参数进行设置
### 渲染输出
...
**输出**: 返回案例的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`案例`相关配置参数进行设置
### 案例:金句海报渲染
