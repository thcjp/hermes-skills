...
description: "轻量数据可视化面板，聚合邮件、会话、任务多源数据，token保护，5秒自动刷新，一站式监控。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
# 3. 浏览器访问（使用打印的token）
# http://127.0.0.1:19195/?token=未指定
...
**输出**: 返回功能一：多源数据聚合的执行结果,包含操作状态和输出数据。
...
node server.js
0.0.1:19195/?token=my-secret-token
```

**token认证方式**（三选一）：
- URL参数：`?token=未指定`
- 请求头：`X-Dashboard-Token: 未指定`
- Bearer令牌：`Authorization: Bearer 未指定`
...
**输出**: 返回功能二：token保护Web UI的执行结果,包含操作状态和输出数据。
...
**输出**: 返回功能三：5秒自动刷新的执行结果,包含操作状态和输出数据。
...
**输出**: 返回功能四：时间筛选的执行结果,包含操作状态和输出数据。
...
**输出**: 返回功能五：基础图表展示的执行结果,包含操作状态和输出数据。
