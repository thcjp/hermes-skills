...
description: "轻量级搜索任务分解与调度助手,支持子任务拆分与通用搜索代理调用,适合个人研究查询。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
免费版最多5个子任务。子任务过多会导致每个任务搜索深度不足。建议3-4个子任务为优选平衡点:
