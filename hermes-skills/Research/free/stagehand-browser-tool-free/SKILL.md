...
description: "通过自然语言指令驱动本地 Chrome 浏览器的轻量级自动化工具,适合个人开发者日常网页交互。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
**输出**: 返回自然语言指令驱动的执行结果,包含操作状态和输出数据。
