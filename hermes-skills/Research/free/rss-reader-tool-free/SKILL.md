...
description: "轻量级RSS/Atom订阅阅读器,支持分类管理、关键词过滤与多种输出格式,适合个人内容研究。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
**输出**: 返回订阅源数量建议的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 不支持内置定时调度(需手动或系
不支持内置定时调度(需手动或系统crontab)

**输出**: 返回不支持内置定时调度(需手动或系的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 不支持邮件/IM推送
不支持邮件/IM推送

**输出**: 返回不支持邮件/IM推送的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输出**: 返回输出格式固定为三种(列表/id的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
```bash
# 添加竞品订阅源
node scripts/rss.js add "https://competitor-a.com/blog/feed" --category competitors
node scripts/rss.js add "https://competitor-b.com/feed.xml" --category competitors
...
# 检查最近24小时更新
node scripts/rss.js check --since 24h
```
...
# 按分类+关键词组合过滤
node scripts/rss.js check --category tech --keywords "Rust,async,tokio" --since 48h
```
...
```bash
node scripts/rss.ycombinator.com/rss" --category tech
```
...
# 检查特定分类
node scripts/rss.js check --category tech --since 24h
```
...
```bash
node scripts/rss.js check --since 24h --format ideas > daily-ideas.md
```
...
```bash
# 好:聚焦当前关注方向
node scripts/rss.js check --keywords "向量数据库,RAG,embedding"

# 不好:关键词过多,几乎不过滤
node scripts/rss.js check --keywords "AI,tech,software,code,dev,tool"
```
...
```bash
# 每日8:00检查并生成内容灵感
0 8 * * * cd /path/to/skill && node scripts/rss.js check --since 24h --format ideas > /reports/daily-ideas.md
```

### 5. 定期清理订阅列表

通过`node scripts/rss.js list`查看所有订阅源,移除长期不更新或不再关注的源,保持订阅列表精简。
