---
name: "prod-helper-v4-tool-free"
description: "增强版效率助手,支持智能任务分类、进度追踪、数据分析与自动化提醒"
license: Proprietary
allowed-tools: read exec
compatibility: "Requires LLM with tool-use capability"
metadata:
  displayName: "效率助手 v4 基础版"
  version: "1.0.0"
  summary: "增强版效率助手,支持智能任务分类、进度追踪、数据分析与自动化提醒"
  tags:
    - "效率助手"
    - "智能分类"
    - "数据分析"
    - "自动化"
  source: "SkillHub"
  converted_at: "2026-07-22T17:58:36"
---

# 效率助手 v4 基础版

## 概述

本工具是 **效率管理** 领域的 **FREE 版本** AI Skill,专为个人用户与轻量级场景设计。通过自然语言指令驱动 AI Agent 执行任务,提供核心功能与简洁易用的操作体验。

FREE 版本与 PRO 版本完全兼容,可在需要时升级至 PRO 版本获取高级功能、批量操作与团队协同能力。

### 版本定位

| 维度 | FREE 版本 | PRO 版本 |
|:-----|:----------|:---------|
| 目标用户 | 个人用户 | 企业团队 |
| 功能范围 | 核心功能 | 全部功能 |
| 批量操作 | 不支持 | 支持 |
| 团队协作 | 不支持 | 支持 |
| 技术支持 | 社区支持 | 优先响应 |
| 数据分析 | 基础统计 | 高级分析 |

## 核心能力

FREE 版本提供以下能力:

### 智能任务分类与优先级推荐
智能任务分类与优先级推荐

**输入**: 用户提供智能任务分类与优先级推荐所需的指令和必要参数。
**处理**: 按照skill规范执行智能任务分类与优先级推荐操作,遵循单一意图原则。
**输出**: 返回智能任务分类与优先级推荐的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 可视化进度追踪与状态看板
可视化进度追踪与状态看板

**输入**: 用户提供可视化进度追踪与状态看板所需的指令和必要参数。
**处理**: 按照skill规范执行可视化进度追踪与状态看板操作,遵循单一意图原则。
**输出**: 返回可视化进度追踪与状态看板的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 工作效率数据分析与图表
工作效率数据分析与图表

**输入**: 用户提供工作效率数据分析与图表所需的指令和必要参数。
**处理**: 按照skill规范执行工作效率数据分析与图表操作,遵循单一意图原则。
**输出**: 返回工作效率数据分析与图表的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自动化提醒与日程建议
自动化提醒与日程建议

**输入**: 用户提供自动化提醒与日程建议所需的指令和必要参数。
**处理**: 按照skill规范执行自动化提醒与日程建议操作,遵循单一意图原则。
**输出**: 返回自动化提醒与日程建议的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多维度工作统计报表
多维度工作统计报表

**输入**: 用户提供多维度工作统计报表所需的指令和必要参数。
**处理**: 按照skill规范执行多维度工作统计报表操作,遵循单一意图原则。
**输出**: 返回多维度工作统计报表的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 功能详情

本版本提供 5 项核心能力,覆盖 效率管理 的常见使用场景。如需批量操作、团队协作、数据分析等高级功能,可升级至 PRO 版本。

**FREE 版本核心功能清单:**

- 智能任务分类与优先级推荐
- 可视化进度追踪与状态看板
- 工作效率数据分析与图表
- 自动化提醒与日程建议
- 多维度工作统计报表

**PRO 版本扩展功能预览:**

- AI 驱动的工作流优化建议
- 团队效率分析与基准对比
- 预测性任务规划:基于历史数据预测工期
- 智能日程优化:自动安排最优工作顺序
- 与企业系统集成:日历/邮件/IM/项目管理
- 效率目标管理:OKR 追踪与达成分析

**输入**: 用户提供功能详情所需的指令和必要参数。
**处理**: 按照skill规范执行功能详情操作,遵循单一意图原则。
**输出**: 返回功能详情的执行结果,包含操作状态和输出数据。
**技术参数**：使用`input_params`和`output_format`参数控制执行行为,支持`json`/`text`/`csv`输出格式。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：增强版效率助手、支持智能任务分类、数据分析与自动化、效率管理领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。

## 使用场景

### 场景 1: 智能任务分类

自动分析任务内容并推荐分类与优先级

```bash
# 输入: "优化数据库查询性能"
# AI 分析
分类: 技术优化
优先级: 高 (影响用户体验)
预估时间: 4小时
建议时段: 上午 9-11 点
关联任务: 性能监控部署
```

### 场景 2: 效率数据分析

生成工作效率分析报告

```bash
# 周报分析
- 总任务数: 23
- 完成率: 87%
- 平均完成时间: 2.3天
- 高效时段: 9-11am, 2-4pm
- 改进建议: 下午会议过多影响专注
```

## 不适用场景

以下场景效率助手 v4 基础版不适合处理：

- 实时流数据处理
- 小规模数据手动分析
- 非结构化文本情感分析

## 触发条件

需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于非本工具能力范围的需求。

## 快速开始

### 1. 环境准备

确保已安装并配置好 AI Agent 环境(Claude Code / Cursor / Codex / Gemini CLI 等),本 Skill 通过 SKILL.md 指令驱动 Agent 执行任务。

**系统要求:**

- 操作系统: Windows / macOS / Linux
- Agent 平台: 支持 SKILL.md 格式的任意 AI Agent
- 运行时: Python 3.8+ 或 Node.js 18+(视具体操作需求)

### 2. 配置参数

```bash
# v4 配置
{
  "version": "4.0",
  "ai_classification": true,
  "data_analysis": true,
  "auto_reminder": true,
  "smart_scheduling": true,
  "report_frequency": "weekly"
}
```

### 3. 验证配置

配置完成后,可通过以下方式验证是否正常工作:

```bash
# 验证环境变量是否设置
echo "配置检查:"
env | grep -E "API|KEY|TOKEN|SECRET" | sed "s/=.*/=***/"  # Linux/macOS
# 或 PowerShell
# Get-ChildItem Env: | Where-Object {$_.Name -match "API|KEY|TOKEN"} | Format-Table
```

### 4. 开始使用

在 AI Agent 对话中描述你的需求,Agent 会根据本 Skill 的指令自动执行对应操作。

```text
请帮我智能任务分类
```

Agent 将自动:
1. 解析你的自然语言指令
2. 调用相应的工具或 API
3. 执行操作并返回结果
4. 返回执行结果供你确认

#
## 示例

### 基础配置

```bash
# v4 配置
{
  "version": "4.0",
  "ai_classification": true,
  "data_analysis": true,
  "auto_reminder": true,
  "smart_scheduling": true,
  "report_frequency": "weekly"
}
```
### 可选配置

```json
{
  "edition": "free",
  "auto_retry": false,
  "max_concurrent": 1,
  "log_level": "info",
  "cache_enabled": true,
  "timeout": 30
}
```

**FREE 配置项说明:**

| 配置项 | 类型 | 默认值 | 说明 |
|:-------|:-----|:-------|:-----|
| auto_retry | bool | false | 失败后自动重试 |
| max_concurrent | int | 1 | 最大并发数(FREE 限制为1) |
| log_level | string | info | 日志级别 |
| cache_enabled | bool | true | 启用结果缓存 |
| timeout | int | 30 | 操作超时时间(秒) |

## 最佳实践

1. **利用 AI 分类减少手动整理时间**
2. **关注数据分析结果找出效率瓶颈**
3. **采纳智能建议但保持人工判断**
4. **定期回顾效率趋势持续改进**

### 个人使用建议

- 从简单任务开始熟悉工具行为,逐步扩展到复杂场景
- 定期备份配置文件与数据,防止意外丢失
- 遇到问题先查阅常见问题章节,再寻求社区帮助
- 保持配置简洁,只启用必要的功能选项
- 定期更新工具版本以获取最新功能与修复
- 记录常用操作命令,提高日常使用效率

## 常见问题

### Q: 与 v1 版本有什么区别?

A: v4 增加了 AI 智能分类、数据分析、自动化提醒与智能日程优化。

### Q: AI 建议准确吗?

A: AI 基于历史数据分析提供建议,准确率随使用时间提升,建议人工审核后采纳。

### Q: 如何升级到 PRO 版本?

A: 升级到 PRO 版本非常简单:
1. 获取 PRO 版本授权
2. 安装 PRO 版本 Skill
3. 原有配置自动迁移,无需额外操作
4. 即可使用批量操作、团队协作等高级功能

### 已知限制

A: FREE 版本主要限制:
- 不支持批量操作(每次只能处理一个任务)
- 不支持团队协作(仅限个人使用)
- 不支持高级数据分析
- 技术支持依赖社区
如需这些功能,建议升级至 PRO 版本。

### Q: FREE 版本的数据安全吗?

A: FREE 版本所有数据本地存储,不上传云端,确保隐私安全。建议定期备份配置文件与数据目录。

## 依赖说明

### 运行环境

- **Agent 平台**: 支持 SKILL.md 的任意 AI Agent(Claude Code / Cursor / Codex / Gemini CLI 等)
- **操作系统**: Windows / macOS / Linux
- **运行时**: Python 3.8+ 或 Node.js 18+(视具体操作需求)
- **网络**: 部分功能需要网络连接访问外部 API

### 依赖详情

| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| LLM API | API | 必需 | 由 Agent 内置 LLM 提供 |
| curl | CLI 工具 | 推荐 | 系统自带或包管理器安装 |
| jq | JSON 处理 | 推荐 | apt install jq / brew install jq |
| Python 3.8+ | 运行时 | 视需求 | python.org 下载 |
| Node.js 18+ | 运行时 | 视需求 | nodejs.org 下载 |

### API Key 配置

FREE 版本支持单一 API Key 配置,满足个人使用需求:
- **环境变量**:通过环境变量配置 API Key
- **配置文件**:支持配置文件方式存储 API Key
- **安全提醒**:切勿将 API Key 硬编码到脚本或提交到版本控制系统

### 可用性分类

- **分类**: MD+EXEC(纯 Markdown 指令,部分功能需要 exec 命令行执行能力)
- **说明**: 基于 Markdown 的 AI Skill,通过自然语言指令驱动 Agent 执行任务
- **FREE 特性**: 支持单次执行、基础配置与社区支持
- **安全等级**: 基础,数据本地存储,建议定期备份
- **SLA**: 社区支持,尽力响应

---

**版本信息**

| 项目 | 值 |
|:-----|:---|
| 版本号 | 1.0.0 |
| 版本类型 | FREE |
| 许可证 | MIT |
| 兼容性 | 可升级至 PRO 版本 |

## 错误处理


| 错误场景 | 原因 | 处理方式 |
|---------|------|---------|
| 配置错误 | 参数缺失或格式错误 | 检查依赖说明中的配置要求 |
| 运行时错误 | 运行环境不满足 | 确认运行环境符合依赖说明 |
| 网络错误 | 连接超时或不可达 | 执行ping命令测试网络连通性,检查防火墙和代理设置连接后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令，参考国内替代方案 |
