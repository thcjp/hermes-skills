...
summary: "企业级数据质量管理系统,支持自动化检查流水线、异常告警与数据血缘追踪。核心能力: 数据质量领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 数据质量领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
本工具是 **数据质量** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析自动化数据质量检查流水线的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自动化数据质量检查流水线的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 异常检测与实时告警通知
异常检测与实时告警通知

**处理**: 解析异常检测与实时告警通知的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回异常检测与实时告警通知的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 数据血缘追踪与影响分析
数据血缘追踪与影响分析

**处理**: 解析数据血缘追踪与影响分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回数据血缘追踪与影响分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多数据源交叉验证与一致性检查
多数据源交叉验证与一致性检查

**处理**: 解析多数据源交叉验证与一致性检查的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多数据源交叉验证与一致性检查的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 数据质量评分卡与趋势分析
数据质量评分卡与趋势分析

**处理**: 解析数据质量评分卡与趋势分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回数据质量评分卡与趋势分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级数据质量管、理系统、支持自动化检查流、异常告警与数据血、核心能力、数据质量领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 数据质量检查专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "数据质量检查专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "quant data quality pro"
    }
  },
...
- **自动化执行**: 企业级数据质量管理系统,支持自动化检查流水线、异常告警与数据血缘追踪。核心能力: 数据质量领域的专业化 AI 辅助工具,
