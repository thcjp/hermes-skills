...
6. **记忆备份约束**:禁止直接用裸 `bdpan upload/download` 命令操作记忆目录;必须通过 `bash ${CLAUDE_SKILL_DIR}/（请参考skill目录中的脚本文件）` 脚本执行,以确保 manifest 生成、路径安全检查、safety net 备份等机制正常运行
## 前置检查
每次触发时按顺序执行:
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 网盘指挥官专业版处理的输入数据或指令 |
...
bash ${CLAUDE_SKILL_DIR}/（请参考skill目录中的脚本文件） restore 2026-03-16 --force
bash ${CLAUDE_SKILL_DIR}/（请参考skill目录中的脚本文件） restore 2026-03-16 --yes
```
...
2. 执行 bash ${CLAUDE_SKILL_DIR}/（请参考skill目录中的脚本文件） backup
...
A1: 命令式网盘管理,含记忆备份恢复、大文件后台下载、批量操作、增量同步、完整性校验.。网盘指挥官专业版是一个以"命令式高效操作"为核心视角的百度网盘文件管理工具,所。支持文本指令和结构化参数输入，具体格式参考使用流程章节。

### Q2: 使用网盘指挥官专业版需要什么前置条件？

A2: 请确认运行环境满足依赖说明中的要求。网盘指挥官专业版基于Markdown指令驱动，无需额外安装包。
...
- **自动化执行**: 命令式网盘管理,含记忆备份恢复、大文件后台下载、批量操作、增量同步、完整性校验.。网盘指挥官专业版是一个以"命令式高效操
