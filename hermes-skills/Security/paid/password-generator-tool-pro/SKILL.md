...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 密码生成器(专业版)处理的输入数据或指令 |
...
- 泄露检测依赖HaveIBeenPwned等第三方API，API不可用时泄露检测功能降级为跳过
- 密码强度评估基于熵值计算，不包含针对特定攻击模式（如字典攻击、模式识别）的额外评估
- 批量生成密码时随机数质量依赖系统熵池，低熵环境（如容器）可能影响密码随机性
## 安全注意事项
...
- **自动化执行**: 企业级密码生成工具,支持8-128位自定义、批量生成、泄露检测、策略模板与多格式导出,支持多种使用场景和自动化处理
