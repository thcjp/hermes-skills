---
name: "doc-guard-free"
description: "端到端加密 Markdown 文档协作工具，支持创建、查询、同步加密笔记，适合个人隐私场景。Use when 用户需要doc-guard-free相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
```bash
claude 协议适配层 add --transport http doc-guard-api <SERVER_URL>
```
...
将相同的 protocol server 配置写入 Windsurf 的 协议适配层 工具配置文件即可。
...
### 运行环境
- **Agent 平台**：支持 SKILL.md 与 工具协议的 AI Agent（Claude Code / Cursor / Windsurf 等）
...
- **说明**：基于 Markdown 的 AI Skill，通过 工具协议与加密文档服务端通信
