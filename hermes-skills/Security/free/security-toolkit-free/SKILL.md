...
description: "AI Agent安全扫描与风险检测,24条规则覆盖注入、密钥泄露、恶意代码等,适合个人开发者。Use when 需要安全检测、合规审计、漏洞扫描、加密防护时使用。不适用于渗透测试未授权目标。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
| # | 规则 | 严重等级 | 文件:行号 | 证据 |
|---|------|----------|-----------|------|
| 1 | REMOTE_LOADER | critical | index.js:42 | JSON.parse(remoteData) |
...
```bash
# 扫描整个项目
node scripts/scan.js /path/to/project --full
```
...
```bash
# 只检测密钥泄露
node scripts/scan.js /path/to/project --rule PRIVATE_KEY_PATTERN

# 只检测Prompt注入
node scripts/scan.js /path/to/project --rule PROMPT_INJECTION
```
...
```bash
# 安装任何新Skill前执行安全扫描
node scripts/scan.js ~/.skills/new-skill/
```
...
```bash
# 每周扫描工作区
node scripts/scan.js ~/workspace/ --full --output weekly_scan.json
```
...
```bash
# 只显示严重问题
node scripts/scan.js /path/to/project | grep CRITICAL
```
