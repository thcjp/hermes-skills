...
description: "OWASP Top 10代码审计,含安全编码模式、输入验证、认证安全检查,适合开发者自检。Use when 需要安全检测、合规审计、漏洞扫描、加密防护时使用。不适用于渗透测试未授权目标。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
**输出**: 返回OWASP Top 10 检查清单的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：代码审计、含安全编码模式、输入验证、认证安全检查、适合开发者自检、核心能力、安全审计检查清单、安全编码模式与反、模式对比、输入验证与文件上、传安全、认证与、安全优选实践、HTTP、头配置指南、依赖安全扫描命令等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
