...
description: "基础协作平台卡片发送，支持文本、标题和颜色设置"
license: MIT
...
通过 `node skills/feishu-card/send_safe.js` 包装器安全发送原始文本。自动处理临时文件创建和清理，避免shell转义问题。支持 `--text` 直接传入含特殊字符的内容，配合 `--title` 设置卡片标题。适用于自动化流程中的安全消息发送。- 验证执行结果，确认输出符合预期格式
- 参考`安全发送`相关配置参数进行设置
#
## 升级提示
