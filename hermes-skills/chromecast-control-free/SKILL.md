...
description: "基础投屏控制，设备发现、媒体投放、播放和音量管理"
license: MIT
...
通过 `catt scan` 命令扫描局域网内的所有投屏设备。返回设备名称和IP地址列表。使用 `-d <device>` 参数通过设备名称或IP地址指定目标设备。适用于初次配置和设备定位。- 验证执行结果，确认输出符合预期格式
- 参考`设备发现`相关配置参数进行设置
### 2. 媒体投放
...
**输出**: 返回媒体投放的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`媒体投放`相关配置参数进行设置
### 3. 播放控制
...
**输出**: 返回音量控制的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`音量控制`相关配置参数进行设置
### 输出格式
