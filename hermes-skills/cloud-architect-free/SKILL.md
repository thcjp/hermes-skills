...
description: "基础云架构设计,覆盖 AWS、Azure、GCP 服务选型与迁移策略框架"
license: MIT
...
**处理**: 按照skill规范执行单云架构设计操作,遵循单一意图原则。
**输出**: 返回单云架构设计的执行结果,包含操作状态和输出数据。### 迁移策略框架(6Rs)
...
**处理**: 按照skill规范执行迁移策略框架(6Rs)操作,遵循单一意图原则。
**输出**: 返回迁移策略框架(6Rs)的执行结果,包含操作状态和输出数据。### 基础成本优化
- 资源 Right-sizing:基于利用率调整实例规格
- Reserved Instances 承诺消费折扣入门
- 存储分层:热数据与冷数据的存储选型

**输入**: 用户提供基础成本优化所需的指令和必要参数。
**处理**: 按照skill规范执行基础成本优化操作,遵循单一意图原则。
**输出**: 返回基础成本优化的执行结果,包含操作状态和输出数据。### 基础安全与合规
- 数据加密(静态加密与传输加密)
- IAM 最小权限策略入门
- 网络分段与安全组基础配置

**输入**: 用户提供基础安全与合规所需的指令和必要参数。
**处理**: 按照skill规范执行基础安全与合规操作,遵循单一意图原则。
**输出**: 返回基础安全与合规的执行结果,包含操作状态和输出数据。### 基础设施即代码
- Terraform 跨云资源编排入门
- IaC 代码版本管理基础

**输入**: 用户提供基础设施即代码所需的指令和必要参数。
**处理**: 按照skill规范执行基础设施即代码操作,遵循单一意图原则。
**输出**: 返回基础设施即代码的执行结果,包含操作状态和输出数据。
...
### 成本超出预期
原因:未及时释放资源或未使用预留折扣。
处理:清理未挂载的存储卷与闲置资源;评估 Reserved Instances 覆盖稳定负载;执行ping命令测试网络连通性,检查防火墙和代理设置出口流量费用;设置预算告警。
