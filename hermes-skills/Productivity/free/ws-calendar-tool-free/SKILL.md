...
description: "日历管理与同步工具,支持创建事件、设置提醒与跨平台同步"
license: Proprietary
...
**输入**: 用户提供查看今日/本周/本月日程所需的指令和必要参数。
**处理**: 按照skill规范执行查看今日/本周/本月日程操作,遵循单一意图原则。
**输出**: 返回查看今日/本周/本月日程的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供事件搜索与过滤所需的指令和必要参数。
**处理**: 按照skill规范执行事件搜索与过滤操作,遵循单一意图原则。
**输出**: 返回事件搜索与过滤的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 重复事件管理
重复事件管理

**输入**: 用户提供重复事件管理所需的指令和必要参数。
**处理**: 按照skill规范执行重复事件管理操作,遵循单一意图原则。
**输出**: 返回重复事件管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
