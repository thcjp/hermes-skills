...
description: "日历管理与调度工具,支持创建事件、安排会议与跨平台同步,适合个人用户"
license: Proprietary
...
**输入**: 用户提供创建日历事件并设置提醒所需的指令和必要参数。
**处理**: 按照skill规范执行创建日历事件并设置提醒操作,遵循单一意图原则。
**输出**: 返回创建日历事件并设置提醒的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 安排会议并查看参与者可用时间
安排会议并查看参与者可用时间

**输入**: 用户提供安排会议并查看参与者可用时间所需的指令和必要参数。
**处理**: 按照skill规范执行安排会议并查看参与者可用时间操作,遵循单一意图原则。
**输出**: 返回安排会议并查看参与者可用时间的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查看今日/本周日程安排
查看今日/本周日程安排

**输入**: 用户提供查看今日/本周日程安排所需的指令和必要参数。
**处理**: 按照skill规范执行查看今日/本周日程安排操作,遵循单一意图原则。
**输出**: 返回查看今日/本周日程安排的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 支持重复事件设置
支持重复事件设置

**输入**: 用户提供支持重复事件设置所需的指令和必要参数。
**处理**: 按照skill规范执行支持重复事件设置操作,遵循单一意图原则。
**输出**: 返回支持重复事件设置的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 跨 Google/Apple/
跨 Google/Apple/Outlook 平台同步

**输入**: 用户提供跨 Google/Apple/所需的指令和必要参数。
**处理**: 按照skill规范执行跨 Google/Apple/操作,遵循单一意图原则。
**输出**: 返回跨 Google/Apple/的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
