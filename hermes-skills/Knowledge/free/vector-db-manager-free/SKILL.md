...
description: "管理向量数据库的索引、嵌入与相似度检索，支持本地与云端部署。Use when 需要数据库操作、SQL查询、数据存储管理时使用。不适用于数据库架构设计决策。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
license: Proprietary
...
| 多引擎适配 | `数据库` pgvector/Milvus/ChromaDB | 是 |
...
conn = psycopg2.connect("数据库://user:pass@localhost/db")
cur = conn.cursor()
...
A：根据规模和现有技术栈选择：(1) 已有 `数据库` 且向量数<100万，选pgvector；(2) 需要高性能且向量数>100万，选Milvus；(3) 快速原型验证，选ChromaDB；(4) 追求精细性能，选Qdrant。
...
- **Python**: 3.8+
- **数据库**: `数据库` 14+ / Milvus 2.0+ / ChromaDB 0.4+ / Qdrant 1.0+
