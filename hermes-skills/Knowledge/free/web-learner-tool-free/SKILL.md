...
description: "让 AI 自主上网搜索和学习,主动获取最新信息,整合互联网知识,适合个人知识获取。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
- **分类**: MD+execute(纯 Markdown 指令,通过 exec 执行搜索和抓取工具)
