...
license: Proprietary
edition: pro
description: "企业级多引擎聚合搜索工具，支持批量查询、自定义引擎、结果导出、定时任务与搜索分析，适合专业研究与数据采集。核心能力:。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
  - 批量关键词查询，并行执行数十个搜索任务
  - 自定义搜索引擎配置与组合
