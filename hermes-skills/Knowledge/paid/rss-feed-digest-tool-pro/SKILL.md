...
license: Proprietary
edition: pro
description: "RSS聚合摘要专业版为企业团队提供高阶RSS/Atom订阅聚合与分发能力。核心能力:。Use when 需要消息发送、通知推送、邮件短信、通信集成时使用。不适用于垃圾信息群发。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
  - 大规模多源并行抓取(50+源)
  - 定时调度与自动化任务管理
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、Atom、聚合摘要系统、支持定时调度、多模板定制与团队、聚合摘要专业版为、企业团队提供高阶、订阅聚合与分发能、核心能力、大规模多源并行抓、定时调度与自动化、任务管理、报告生成、自定义摘要模板与、高级过滤引擎等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | RSS聚合摘要专业版处理的输入数据或指令 |
...
# ...
# 查看任务执行历史
python3 （请参考skill目录中的脚本文件） schedule logs "每日行业简报" --last 7
```
## 不适用场景
...
# ...
# 手动触发测试运行
python3 （请参考skill目录中的脚本文件） schedule run "每日技术简报"
# ...
# 查看运行结果
python3 （请参考skill目录中的脚本文件） schedule logs "每日技术简报"
```
...
```bash
# 批量导入OPML
python3 （请参考skill目录中的脚本文件） sources import industry.opml --group "行业媒体"
# ...
# 批量健康检查
python3 （请参考skill目录中的脚本文件） sources health-check --all --timeout 10
# ...
# 导出当前源列表
python3 （请参考skill目录中的脚本文件） sources export --format opml --output backup.opml
# ...
# 按组管理
python3 （请参考skill目录中的脚本文件） sources list --group "行业媒体" --healthy-only
```
...
```bash
python3 （请参考skill目录中的脚本文件） schedule update "每日行业简报" \
  --retry 3 \
...
```json
{
  "success": true,
  "data": {
    "result": "RSS聚合摘要专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "rss feed digest pro"
    }
  },
...
- **自动化执行**: 企业级RSS/Atom聚合摘要系统,支持定时调度、HTML报告、邮件分发、多模板定制与团队协作。RSS聚合摘要专业版为企
