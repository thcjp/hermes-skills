...
license: Proprietary
edition: pro
description: "RSS采集器专业版为企业团队提供高阶RSS订阅采集与管理系统。核心能力:。Use when 需要系统监控、日志分析、运维告警、部署管理时使用。不适用于物理硬件维修。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
  - 多源并行抓取(50+ workers)
  - 交互式HTML报告生成
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | RSS采集器专业版处理的输入数据或指令 |
| options | object | 否 | 附加配置选项,如模式选择、格式偏好等 |
| callback_url | string | 否 | 异步处理完成后的回调通知URL |

```bash
# 批量导入100+订阅源
python3 （请参考skill目录中的脚本文件） import industry-media.opml --group "行业媒体"
# ...
# 并行抓取(50 workers)
python3 （请参考skill目录中的脚本文件） --workers 50
# ...
# 生成HTML报告
python3 （请参考skill目录中的脚本文件） --template corporate --brand "行业内容库"
# ...
# 导出CSV供数据分析
python3 （请参考skill目录中的脚本文件） --format csv --output /exports/articles-$(date +%Y%m%d).csv --since 7d
```
...
```bash
# 使用自定义标签规则抓取
python3 （请参考skill目录中的脚本文件） --tag-rules config/tag_rules.yaml --workers 50
# ...
# 查看标签分布
python3 （请参考skill目录中的脚本文件） --by-tag --top 20
```
...
# ...
# 查看执行日志
python3 （请参考skill目录中的脚本文件） logs "每日全量抓取" --last 7
```
## 不适用场景
以下场景RSS采集器专业版不适合处理：
...
# ...
# 2. 验证免费版数据库可用
python3 （请参考skill目录中的脚本文件） --dry-run
...
```bash
# 导出为CSV
python3 （请参考skill目录中的脚本文件） --format csv --output articles.csv --since 30d
# ...
# 导出为JSON
python3 （请参考skill目录中的脚本文件） --format json --output articles.json --category tech
# ...
# 导出订阅源为OPML(便于迁移到其他阅读器)
python3 （请参考skill目录中的脚本文件） --format opml --output sources.opml
```
## 配置示例
### 并行抓取配置
```bash
...
# ...
# 指定源抓取
python3 （请参考skill目录中的脚本文件） --sources openai huggingface arxiv-ai --workers 10
# ...
# 已知限制
python3 （请参考skill目录中的脚本文件） --hours 48 --workers 50
# ...
# 全量刷新(忽略增量,重新拉取全部)
python3 （请参考skill目录中的脚本文件） --mode full --workers 50
```
...
```bash
python3 （请参考skill目录中的脚本文件） --workers 50 && python3 （请参考skill目录中的脚本文件）
```
...
```bash
# 查看无标签文章
python3 （请参考skill目录中的脚本文件） --no-tags --limit 20
# ...
# 查看标签分布
python3 （请参考skill目录中的脚本文件） --by-tag --top 30
```
...
```bash
python3 （请参考skill目录中的脚本文件） --dedup content --workers 50
```
...
```bash
# 每周日凌晨自动导出(crontab)
0 0 * * 0 cd /path/to/skill && python3 （请参考skill目录中的脚本文件） --format csv --output /backups/articles-$(date +\%Y\%m\%d).csv
```
## 已知限制
...
```json
{
  "success": true,
  "data": {
    "result": "RSS采集器专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "rss fetcher pro"
    }
  },
...
- **自动化执行**: 企业级RSS采集管理系统,支持并行抓取、HTML报告、自定义标签规则、数据导出与源健康监控告警。RSS采集器专业版为企业
