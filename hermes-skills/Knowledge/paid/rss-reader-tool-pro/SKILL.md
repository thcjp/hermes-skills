...
license: Proprietary
edition: pro
description: "RSS阅读器专业版为企业团队提供高阶RSS订阅阅读与监控能力。核心能力:。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
  - 大规模订阅源管理(100+源)
  - 内置Cron定时调度引擎
  - 多渠道推送(邮件/IM/Webhook)
  - 多用户配置与权限管理
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | RSS阅读器专业版处理的输入数据或指令 |
| options | object | 否 | 附加配置选项,如模式选择、格式偏好等 |
| callback_url | string | 否 | 异步处理完成后的回调通知URL |
```bash
# 批量导入竞品订阅源
node （请参考skill目录中的脚本文件） sources import competitors.opml --category competitors
# ...
# 配置每小时监控(关键信息推送到IM)
...
```bash
# 创建用户并配置独立订阅
node （请参考skill目录中的脚本文件） user create product-team --config-dir /users/product/
node （请参考skill目录中的脚本文件） user create tech-team --config-dir /users/tech/
node （请参考skill目录中的脚本文件） user create market-team --config-dir /users/market/
# ...
# 产品团队:关注产品设计与用户体验
node （请参考skill目录中的脚本文件） user exec product-team -- add "https://www.smashingmagazine.com/feed/" --category design
node （请参考skill目录中的脚本文件） user exec product-team -- schedule add --cron "0 9 * * *" --format ideas --distribute "email:product@corp.com"
# ...
# 技术团队:关注技术架构与开源项目
node （请参考skill目录中的脚本文件） user exec tech-team -- add "https://news.ycombinator.com/rss" --category tech
node （请参考skill目录中的脚本文件） user exec tech-team -- schedule add --cron "0 9 * * *" --keywords "架构,性能,开源" --distribute "email:tech@corp.com"
# ...
# 市场团队:关注行业媒体与竞品
node （请参考skill目录中的脚本文件） user exec market-team -- add "https://techcrunch.com/feed/" --category news
node （请参考skill目录中的脚本文件） user exec market-team -- schedule add --cron "0 8 * * *" --distribute "email:market@corp.com"
```
### 场景三:全文搜索与趋势分析
研究团队需要检索历史归档中的特定主题,并分析关键词热度趋势.
```bash
# 全文搜索
node （请参考skill目录中的脚本文件） search "向量数据库" --since 30d --format ideas
# ...
# 关键词趋势分析
node （请参考skill目录中的脚本文件） analyze trend \
...
# ...
# 热度排行
node （请参考skill目录中的脚本文件） analyze hot \
  --since 7d \
  --top 20 \
  --by engagement
```
...
# ...
# 查看调度任务
node （请参考skill目录中的脚本文件） schedule list
# ...
# 手动触发测试
node （请参考skill目录中的脚本文件） schedule run "每日内容灵感"
# ...
# 查看执行历史
node （请参考skill目录中的脚本文件） schedule logs "每日内容灵感" --last 7
```
### 配置推送渠道
```yaml
...
```bash
# 检索最近30天关于某主题的所有文章
node （请参考skill目录中的脚本文件） search "竞品A定价策略" --since 30d
```
### 4. 趋势分析辅助内容决策
定期运行趋势分析,发现上升中的关键词,提前布局内容创作:
```bash
# 每周趋势分析
node （请参考skill目录中的脚本文件） analyze trend --since 7d --top 10 --output weekly-trends.json
```
### 5. 推送模板按角色定制
不同角色关注不同粒度。为管理层配置精简模板(只看高优先级),为执行层配置详尽模板(包含分析数据):
```bash
# 管理层:精简版
node （请参考skill目录中的脚本文件） schedule add --name "管理层简报" --template executive-brief --distribute "email:c-level@corp.com"
# ...
# 执行层:详尽版
node （请参考skill目录中的脚本文件） schedule add --name "执行层详报" --template detailed-report --distribute "email:team@corp.com"
```
## 常见问题
### Q: 如何从免费版迁移到专业版?
A: 专业版完全兼容免费版。安装专业版依赖后,免费版的`feeds.json`、命令行参数、输出格式全部继续可用。运行`node （请参考skill目录中的脚本文件） pro enable`激活专业版功能,原有订阅配置自动识别.
### Q: 多用户模式下如何保证配置隔离?
...
### API Key 配置
- **专业版授权Key**: 运行`node （请参考skill目录中的脚本文件） pro enable --license "YOUR-KEY"`激活
...
### 可用性分类
- **分类**: MD+execute(纯Markdown指令,核心功能需要exec命令行执行Node.js脚本)
...
## 核心功能
- **自动化执行**: 企业级RSS阅读与监控系统,支持定时调度、邮件推送、多用户配置、全文搜索与高级内容分析。RSS阅读器专业版为企业团队提供
