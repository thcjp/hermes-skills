...
license: Proprietary
edition: pro
description: "文件浏览器助手专业版是面向企业级场景的完整文件管理与操作工具。在免费版基础操作能力之上，新增批量操作、高级搜索、文件监控、压缩解压、二进制查看、权限管理、文件比较、云存储集成八大高级能力。Use。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。适用于独立开发者、企业团队和自动化工作流场景。"
...
# ...
    def download(self, remote_path, local_path):
        """下载文件"""
        if self.provider == "s3":
_download_s3(remote_path, local_path)
        return {"error": "不支持的provider"}
# ...
...
- **自动化执行**: 企业级文件浏览器专业版，含批量操作、高级搜索、文件监控、压缩解压与云存储集成.。文件浏览器助手专业版是面向企业级场景的完
