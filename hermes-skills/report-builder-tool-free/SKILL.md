...
description: "基于已有日报自动汇总生成周报和月报,遵循结果导向与闭环原则"
license: Proprietary
...
**输入**: 用户提供读取已有日报 Markdown所需的指令和必要参数。
**处理**: 按照skill规范执行读取已有日报 Markdown操作,遵循单一意图原则。
**输出**: 返回读取已有日报 Markdown的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 基于真实日报内容生成周报或月报
基于真实日报内容生成周报或月报

**输入**: 用户提供基于真实日报内容生成周报或月报所需的指令和必要参数。
**处理**: 按照skill规范执行基于真实日报内容生成周报或月报操作,遵循单一意图原则。
**输出**: 返回基于真实日报内容生成周报或月报的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 合并重复事项强调成果与进展
合并重复事项强调成果与进展

**输入**: 用户提供合并重复事项强调成果与进展所需的指令和必要参数。
**处理**: 按照skill规范执行合并重复事项强调成果与进展操作,遵循单一意图原则。
**输出**: 返回合并重复事项强调成果与进展的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日期缺失时明确标注
日期缺失时明确标注

**输入**: 用户提供日期缺失时明确标注所需的指令和必要参数。
**处理**: 按照skill规范执行日期缺失时明确标注操作,遵循单一意图原则。
**输出**: 返回日期缺失时明确标注的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 生成可直接发送的摘要
生成可直接发送的摘要

**输入**: 用户提供生成可直接发送的摘要所需的指令和必要参数。
**处理**: 按照skill规范执行生成可直接发送的摘要操作,遵循单一意图原则。
**输出**: 返回生成可直接发送的摘要的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
