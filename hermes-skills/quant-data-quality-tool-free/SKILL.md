...
description: "量化策略数据质量检查工具,检查数据完整性、准确性、一致性与时效性"
license: Proprietary
...
**输入**: 用户提供检查价格数据完整性与缺失所需的指令和必要参数。
**处理**: 按照skill规范执行检查价格数据完整性与缺失操作,遵循单一意图原则。
**输出**: 返回检查价格数据完整性与缺失的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查因子数据 nan 值与极端
检查因子数据 nan 值与极端值

**输入**: 用户提供检查因子数据 nan 值与极端所需的指令和必要参数。
**处理**: 按照skill规范执行检查因子数据 nan 值与极端操作,遵循单一意图原则。
**输出**: 返回检查因子数据 nan 值与极端的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查数据准确性(零价格/负价格
检查数据准确性(零价格/负价格/异常收益)

**输入**: 用户提供检查数据准确性(零价格/负价格所需的指令和必要参数。
**处理**: 按照skill规范执行检查数据准确性(零价格/负价格操作,遵循单一意图原则。
**输出**: 返回检查数据准确性(零价格/负价格的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查价格成交量一致性
检查价格成交量一致性

**输入**: 用户提供检查价格成交量一致性所需的指令和必要参数。
**处理**: 按照skill规范执行检查价格成交量一致性操作,遵循单一意图原则。
**输出**: 返回检查价格成交量一致性的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查数据时效性与滞后天数
检查数据时效性与滞后天数

**输入**: 用户提供检查数据时效性与滞后天数所需的指令和必要参数。
**处理**: 按照skill规范执行检查数据时效性与滞后天数操作,遵循单一意图原则。
**输出**: 返回检查数据时效性与滞后天数的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
