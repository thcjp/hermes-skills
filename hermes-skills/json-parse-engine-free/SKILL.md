...
description: "轻量级JSON解析、展平与提取工具，支持文件与字符串解析、嵌套结构展平、路径提取。"
license: Proprietary
...
**处理**: 按照skill规范执行字符串解析操作,遵循单一意图原则。
