...
edition: "pro"
description: |-
  面向专业交易者的多策略交易分析系统。支持维加斯通道、MACD、布林带 等多种技术策略组合，提供批量标的分析、历史回测验证、自动信号触发 与风险控制功能。Use when 用户需要trading-tool-pro相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。
tags:
  - Finance
  - 交易分析
  - 量化策略
...
- **自动化执行**: 多策略交易分析系统，支持批量分析、策略回测、自动交易与风险控制。。面向专业交易者的多策略交易分析系统。支持维加斯通道、M
