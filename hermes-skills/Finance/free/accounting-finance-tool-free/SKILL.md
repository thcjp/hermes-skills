...
description: "企业财务分析与估值建模基础工具集，涵盖DCF、可比估值、财务比率等核心技能。Use when 用户需要accounting-finance-tool-free相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
