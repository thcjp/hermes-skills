...
description: "通过加密电路隐藏源IP，提供SOCKS5代理与流量转发，保护Agent通信隐私"
license: Proprietary
...
**处理**: 按照skill规范执行能力1：本地SOCKS5代理服务操作,遵循单一意图原则。
**输出**: 返回能力1：本地SOCKS5代理服务的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力2：多层加密电路操作,遵循单一意图原则。
**输出**: 返回能力2：多层加密电路的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力3：多协议支持操作,遵循单一意图原则。
**输出**: 返回能力3：多协议支持的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力4：跨请求会话复用操作,遵循单一意图原则。
**输出**: 返回能力4：跨请求会话复用的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力5：流量按需路由操作,遵循单一意图原则。
**输出**: 返回能力5：流量按需路由的执行结果,包含操作状态和输出数据。
