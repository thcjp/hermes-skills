...
description: "提供文件加密、密码哈希、算法选择指南与基础安全审计,适合个人开发者保护数据。"
license: Proprietary
...
**处理**: 按照skill规范执行加密算法选择指南操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行文件加密与解密操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行基础代码安全审计操作,遵循单一意图原则。
**输出**: 返回基础代码安全审计的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行TLS证书检查操作,遵循单一意图原则。
