...
description: "通过托管OAuth安全调用Slack API，支持消息收发与频道管理，适合个人快速集成。Use when 需要API集成、接口对接、Webhook配置、系统连接时使用。不适用于逆向工程闭源API。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
```bash
export SGW_API_KEY="${API_KEY:?请设置环境变量}"
```
