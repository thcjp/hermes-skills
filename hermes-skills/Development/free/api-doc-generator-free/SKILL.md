...
description: "从代码注释或接口描述自动生成OpenAPI 3.0文档与Markdown接口手册，60秒出稿。"
license: Proprietary
...
**处理**: 按照skill规范执行自动推断字段类型（`id` → inte操作,遵循单一意图原则。
**输出**: 返回自动推断字段类型（`id` → inte的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自动补充 `required`操作,遵循单一意图原则。
**输出**: 返回自动补充 `required`的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行POST/PUT/PATCH默认生成20操作,遵循单一意图原则。
**输出**: 返回POST/PUT/PATCH默认生成20的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行列表接口自动生成分页参数（pa操作,遵循单一意图原则。
**输出**: 返回列表接口自动生成分页参数（pa的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

**输入**: 用户提供功能1：自然语言→OpenAPI转换所需的指令和必要参数。
**处理**: 按照skill规范执行功能1：自然语言→OpenAPI转换操作,遵循单一意图原则。
**输出**: 返回功能1：自然语言→OpenAPI转换的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能2：Markdown接口手册生成操作,遵循单一意图原则。
**输出**: 返回功能2：Markdown接口手册生成的执行结果,包含操作状态和输出数据。
