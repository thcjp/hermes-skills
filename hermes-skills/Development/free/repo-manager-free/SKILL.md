...
description: "通过工具协议管理GitHub仓库、Issue与PR,支持工具发现、参数预览与安全执行,适合个人开发者日常使用。Use when 需要项目管理、任务规划、进度跟踪、团队协作时使用。不适用于实际人员绩效评估。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
metadata:
  ...
  version: "1.0.0"
  summary: "通过工具协议管理GitHub仓库、Issue与PR,支持工具发现、参数预览与安全执行,适合个人开发者日常使用。"
  tags:
    - "GitHub"
    - "仓库管理"
    - "工具"
...
基于工具协议管理GitHub仓库、Issue与Pull Request,通过统一的工具调用接口自动处理认证,提供"发现-预览-确认-执行"的安全流程。

## 概述

管理GitHub仓库通常需要配置API token、记忆大量端点、处理认证流程,门槛较高。本Skill基于工具协议,将GitHub能力封装为标准化工具,Agent可通过统一接口调用,无需关心认证细节。用户只需用自然语言描述意图,Agent自动选择合适工具、预填参数、确认后执行。
...
- **集成插件**: repo-manager-plugin(通过工具协议提供GitHub能力)
- **网络**: 可访问GitHub API
...
### 可用性分类
- **分类**: MD+EXEC+工具(Markdown指令+命令行工具+工具协议)
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent调用工具执行GitHub操作
