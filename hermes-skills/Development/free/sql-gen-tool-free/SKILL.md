...
description: "自然语言转SQL的生成器免费版，支持查询生成、SQL解释、建表语句、测试数据生成等核心场景。Use when 需要数据库操作、SQL查询、数据存储管理时使用。不适用于数据库架构设计决策。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
A：默认生成标准SQL。若指定数据库类型（如 `数据库`、MySQL），会自动适配方言差异（如LIMIT vs TOP、UPSERT语法）。
