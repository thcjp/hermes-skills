...
description: "PostgreSQL数据库管理与优化助手，通过MCP工具实现健康检查、索引调优、查询计划分析等。"
license: Proprietary
...
**处理**: 按照skill规范执行能力一：MCP工具可用性前置检查操作,遵循单一意图原则。
**输出**: 返回能力一：MCP工具可用性前置检查的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力二：六类意图智能路由操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力三：写操作安全确认操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力四：只读模式兼容操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力五：长时间查询性能保护操作,遵循单一意图原则。
