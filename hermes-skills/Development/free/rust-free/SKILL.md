---
name: "rust-free"
description: "规避所有权、借用、生命周期、字符串、错误处理、并发与内存的常见陷阱。(免费版)。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: MIT
