...
description: "面向AI Agent的轻量级本地SQLite数据库管理工具，免部署、低内存，覆盖建表、查询、索引、备份等核心场景。Use when 需要数据库操作、SQL查询、数据存储管理时使用。不适用于数据库架构设计决策。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
SQLite是一种嵌入式关系型数据库，数据存储于单一文件中，无需启动服务进程即可使用。相比 `数据库`、MySQL等需要独立服务端的传统数据库，SQLite在部署成本、跨平台移植、内存占用方面具有天然优势，尤其适合集成在AI Agent、CLI工具、桌面应用中。
...
A：WAL模式下支持任意数量读并发，但同一时刻只能有一个写操作。若写并发是瓶颈，建议改用 `数据库` 等客户端-服务端架构数据库。
