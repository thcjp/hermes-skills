...
description: "面向个人开发者的Web开发辅助工具,覆盖HTML/CSS、JavaScript模式、框架选择与部署基础。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
| --- | --- | --- |------|
| 静态内容,构建快 | Astro 或纯 HTML | 零 JS 默认,性能优选 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
...
工具会输出问题分析、修复代码与简要解释,并标注是否符合优选实践。
...
纯内容站选 Astro(默认零 JS,性能优选);需要 React 组件的博客选 Next.js(App Router);文档站选 VitePress 或 Astro。
