...
description: "基于`PostgreSQL`的轻量任务队列，支持优先级调度、批量领取与进度跟踪，单机与小型团队适用。"
license: Proprietary
...
**处理**: 按照skill规范执行能力一：队列表结构设计操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力二：基于 SKIP LOCKED 的批量领取操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力三：优先级调度操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力四：失败重试与超时回收操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力五：进度可视化操作,遵循单一意图原则。
