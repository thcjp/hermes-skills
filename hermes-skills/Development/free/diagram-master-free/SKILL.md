...
description: "专业SVG图表生成工具，支持架构图、流程图、时序图等九种图表类型，暗色主题自包含输出。Use when 用户需要diagram-master-free相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
license: Proprietary
...
<g transform="translate(X, Y)">
  ...
  <text x="60" y="40" fill="white" font-size="11" font-weight="600" text-anchor="middle">`数据库`</text>
</g>
```
