...
license: Proprietary
edition: pro
description: "面向研发团队的高级Git版本控制工具,提供交互式变基、历史重写、子模块批量管理、仓库性能优化与团队协作工作流。核心能力:。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。"
  - 高级交互式变基与历史重写
...
**处理**: 解析高级交互式变基的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回高级交互式变基的响应数据,包含状态码、结果和日志.
### 2. 历史重写
安全地重写Git历史.
```bash
...
**处理**: 解析历史重写的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回历史重写的响应数据,包含状态码、结果和日志.
### 3. 子模块批量管理
```bash
#!/bin/bash
echo "=== 子模块管理 ==="
...
**处理**: 解析子模块批量管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回子模块批量管理的响应数据,包含状态码、结果和日志.
### 4. 二分查找调试
使用bisect定位问题引入的提交.
```bash
...
**处理**: 解析二分查找调试的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回二分查找调试的响应数据,包含状态码、结果和日志.
...
**处理**: 解析仓库性能优化的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回仓库性能优化的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 6. 批量标签与版本管理

**处理**: 解析批量标签与版本管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量标签与版本管理的响应数据,包含状态码、结果和日志.
