...
license: Proprietary
edition: pro
description: "面向企业运维团队的高级Docker管理工具,提供镜像深度优化、安全漏洞扫描、多节点编排、批量运维与CI/CD流水线集成。核心能力:。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。"
  - 镜像深度优化与体积分析
...
# ...
docker inspect --format='{{range .RootFS.Layers}}{{println .}}' myapp:1.0
```
...
**处理**: 解析容器安全漏洞扫描的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回容器安全漏洞扫描的响应数据,包含状态码、结果和日志.
