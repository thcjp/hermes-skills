...
license: Proprietary
edition: pro
description: "面向团队的多文件 Markdown 站点与文档规范治理专业工具。核心能力:。可自动提升工作效率。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。"
  - 多文件站点与目录（TOC）自动生成
