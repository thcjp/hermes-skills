...
from project_config import TOOLS_DIR, DB_PATH, PACKAGED_SKILLS_DIR, MAX_SKILL_MD_LINES, get_db_connection  # V125 W1: 移除unused OPENSOURCE_SKILLS_DIR
...
def check_platform_health() -> Dict[str, Any]:
    ...
    try:
        ...
        c.execute("""
            SELECT workflow_state, COUNT(*) as cnt
            FROM skills
            WHERE workflow_state LIKE 'step8%' OR workflow_state LIKE 'step9%' OR workflow_state = 'step10_completed'
            GROUP BY workflow_state
            ORDER BY cnt DESC
        """)
        for r in c.fetchall():
        ...
        c.execute("""
            ...
            WHERE pu.platform = 'skillhub'
              ...
              AND s.workflow_state != 'step10_completed'
              AND s.workflow_state != 'deprecated'
        """)
...
# [V131 B5: 与market_monitor.generate_recommendations不同(本版接受3个health参数, 对方无参数)]
