...
# V144 G2: 导入差异化padding函数(替代固定文案)
from auto_differentiate import _get_padding

from skill_core import db as db_module  # V116 W1: 统一db入口(替代import db)
# Q5修复: 统一从skill_core/rules.py导入夸大词列表，消除line 623的硬编码副本
# V118 W7修复: 不再绕过skill_core包,直接使用包导入
from skill_core.rules import EXAGGERATION_WORDS as _EXAGGERATION_WORDS
...
def parse_original_skill(content: str) -> Dict[str, Any]:
    ...
    if content.startswith('---'):
        ...
        if len(parts) >= 3:
            ...
            # 解析章节 (## 标题) — [V132 C1a] 使用skill_core.parser.CHAPTER_HEADING_PATTERN
            chapter_matches = list(CHAPTER_HEADING_PATTERN.finditer(body))
...
# [V135 F1] 模块级常量: 从fill_remaining_placeholders提取(TD-252)
_DEFAULTS_BY_KEYWORD = [
    # (关键词列表, 默认值)
...
def fill_remaining_placeholders(content: str, template_name: str = None,
    ...
    # 按模式分类的默认值
    defaults_by_keyword = _DEFAULTS_BY_KEYWORD  # [V135 F1] 已提取为模块级常量
...
# [V133 D4] 去标识化公共函数: 消除frontmatter/无frontmatter两个分支的重复替换逻辑(TD-245)
# [V135 F4] 合并generate_direct内的_brand_words_fm/brand_words的额外条目(clawdhub/clawhut/clawhob/clawhvb)
_BRAND_WORDS = [
...
def _apply_brand_replacements(text: str) -> str:
    ...
    for word in _BRAND_WORDS:
        text = re.sub(rf'(?i)\b{word}\b', 'SkillHub', text)
    return text


def _apply_internal_system_replacements(text: str) -> str:
    """替换内部系统词为中性表达"""
    for old_word, new_word in _INTERNAL_SYSTEM_WORDS:
        text = re.sub(rf'(?i)\b{old_word}\b', new_word, text)
    return text


def _remove_brand_urls(text: str) -> str:
    """移除含烙印词的URL和GitHub仓库链接"""
    text = re.sub(
...
def _apply_source_tracking_replacements(text: str) -> str:
    """替换溯源词为中性表达"""
    for pattern, replacement in _SOURCE_TRACKING_PATTERNS:
        text = re.sub(pattern, replacement, text)
    return text


def _clean_brand_and_internal_ascii(text: str) -> str:
    """[V135 F4] ASCII-only lookarounds替换品牌词和内部系统词
    
    不使用\\b(Python3的\\w匹配Unicode中文导致\\b在中英文边界失效),
    改用ASCII-only lookarounds, 与check_debranding.py一致。
    用于generate_direct的frontmatter和body清理。
    """
    for word in _BRAND_WORDS:
...
def _load_and_strip_template(template_name: str) -> str:
    """Step 0+1: 加载模板并去除HTML注释"""
    ...
    # Step 1: 删除所有HTML注释(写作指引、质量检查点)
    template_content = re.sub(r'<!--[\s\S]*?-->\s*\n*', '', template_content)
    return template_content


def _process_frontmatter(template_content: str, skill_data: Dict[str, Any]) -> str:
...
def _fix_slug_name(template_content: str, target_slug: str) -> str:
    """Step 2.5: 修正frontmatter中的slug和name字段"""
    if not target_slug:
        return template_content
...
def _replace_chapters(template_content: str, chapters: Dict[str, str]) -> str:
    """Step 3: 替换章节内容(模糊匹配)"""
    for template_chapter, original_variants in _CHAPTER_MAPPING.items():
...
def _clean_exaggeration_and_placeholders(template_content: str) -> str:
    """Step 5.5+5.6: 夸大词清除 + 占位符文本清除 + 模板模式清除"""
    # Step 5.5: 夸大词清除
...
def _debrand_content(template_content: str) -> str:
    """Step 5.7: 去标识化清除 (统一frontmatter/无frontmatter两个分支)

    [V133 D4] 重构: 提取公共替换函数, 消除两个分支的重复逻辑(TD-245)
    """
    ...
    if fm_match:
        ...
        for i, line in enumerate(fm_lines):
            if line.startswith('slug:') or line.startswith('name:'):
                continue
            fm_lines[i] = _apply_brand_replacements(fm_lines[i])
            fm_lines[i] = _apply_internal_system_replacements(fm_lines[i])
        fm_content = '\n'.join(fm_lines)

        # 在正文中: 全部替换
        body_content = _apply_brand_replacements(body_content)
        body_content = _apply_internal_system_replacements(body_content)
        body_content = _remove_brand_urls(body_content)
        body_content = _apply_source_tracking_replacements(body_content)
        fm_content = _apply_source_tracking_replacements(fm_content)

        template_content = f'---\n{fm_content}\n---\n{body_content}'
    else:
        # 无frontmatter, 全部替换
        template_content = _apply_brand_replacements(template_content)
        template_content = _apply_internal_system_replacements(template_content)
        template_content = _remove_brand_urls(template_content)
        template_content = _apply_source_tracking_replacements(template_content)
    return template_content


def _clean_remaining_placeholders(template_content: str, target_slug: str) -> str:
    """Step 5.8: 激进占位符清理"""
...
def _enforce_line_limit_and_cleanup(template_content: str) -> str:
    """Step 5.9+6+7: 行数控制 + 清理空行 + 确保frontmatter开头"""
    lines = template_content.split('\n')
...
def generate_from_template(template_name: str, skill_data: Dict[str, Any],
    ...
    [V133 D3+D4] 重构: 290行→~30行流水线, 8步提取为函数, 去标识化重复逻辑统一(TD-245)
    """
    # Step 0+1: 加载模板并去除HTML注释
    template_content = _load_and_strip_template(template_name)

    chapters = skill_data.get('chapters', {})

    # Step 2: 处理frontmatter
    template_content = _process_frontmatter(template_content, skill_data)

    # Step 2.5: slug/name修正
    template_content = _fix_slug_name(template_content, target_slug)

    # Step 3: 替换章节内容
    template_content = _replace_chapters(template_content, chapters)
    ...
    # Step 5.5+5.6: 夸大词 + 占位符文本清除
    template_content = _clean_exaggeration_and_placeholders(template_content)

    # Step 5.7: 去标识化清除 (D4: 统一替换逻辑)
    template_content = _debrand_content(template_content)

    # Step 5.8: 激进占位符清理
    template_content = _clean_remaining_placeholders(template_content, target_slug)

    # Step 5.9+6+7: 行数控制 + 清理 + 确保frontmatter开头
    template_content = _enforce_line_limit_and_cleanup(template_content)
...
def run_l1_check(skill_md_path: Path) -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(TOOLS_DIR / "quality_gate.py"),
        ...
        if result.returncode == 0 or result.stdout:
            ...
            except json.JSONDecodeError as e:  # [V132 C2] 有意降级: JSON解析失败,回退到returncode检查  V144: 添加警告日志
                print(f"[WARN] JSON解析失败,回退到returncode检查: {e}")
...
# [V131 B5: 与dependency_verifier.run_dependency_verification不同(本版无output参数, 简化版)]
def run_dependency_verification(slug: str) -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(TOOLS_DIR / "dependency_verifier.py"),
...
# V130 A9: 与llm_validator.run_l2_validation不是重复定义。
# 差异: 本函数签名(slug), 通过subprocess调用llm_validator.py脚本, 读取报告文件,
#       返回含status/passed/trace_total/trace_grade的汇总; llm_validator版签名含
#       output_json/output_file, 进程内直接执行静态检查+生成prompt, 返回详细验证数据。
# 保留: 与llm_validator.run_l2_validation行为不同(本函数通过subprocess调用llm_validator.py脚本,
# 读取报告文件并检查final_report, 返回含trace_total/trace_grade的汇总结果;
# llm_validator.py的版本是进程内直接执行, 返回详细验证数据, 结构不同)
def run_l2_validation(slug: str) -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(TOOLS_DIR / "llm_validator.py"),
...
# ============ [V135 F4] generate_direct 拆分为3个子函数 ============

def _fix_frontmatter_direct(slug: str, fm: dict, skill_data: dict, body: str) -> str:
    """[V135 F4] Step 1: 修正frontmatter必需字段
    
    修正slug/name/displayName/version/summary/description/license/tools,
    清理品牌词和内部系统词, 构建合规frontmatter YAML。
    """
    from skill_core.rules import (
        MAX_DISPLAY_NAME_LEN, MAX_SUMMARY_LEN, VERSION_PATTERN,
    ...
    summary = _clean_brand_and_internal_ascii(summary)
    ...
    if len(description) < MIN_DESCRIPTION_LEN:
        ...
        if len(description) < MIN_DESCRIPTION_LEN:
            description = description.rstrip('。') + f'。{_get_padding(summary)}。'  # V144 G2
    description = _clean_brand_and_internal_ascii(description)
    if len(description) < MIN_DESCRIPTION_LEN:
        description = description.rstrip('。') + f'。{_get_padding(summary + "_pad2")}。'  # V144 G2
...
def _clean_body_direct(body: str) -> str:
    """[V135 F4] Step 2: 清理body中的保留词/夸大词/占位符/品牌词/溯源词/URL
    
    清理顺序: 保留词→夸大词→占位符残留→品牌烙印词+内部系统词→溯源词→URL→空行
    """
    from skill_core.rules import RESERVED_WORDS, EXAGGERATION_WORDS, PLACEHOLDER_PATTERNS
    ...
    # 2e+2f: 品牌烙印词+内部系统词清除 (使用统一_BRAND_WORDS和_INTERNAL_SYSTEM_WORDS)
    cleaned_body = _clean_brand_and_internal_ascii(cleaned_body)
...
def _ensure_dependency_section(body: str) -> str:
    """[V135 F4] Step 3: 补充缺失的依赖说明章节"""
    if '## 依赖说明' in body or '## 依赖' in body:
        return body
...
def generate_direct(slug: str, original_content: str) -> str:
    ...
    """
    # [V135 F4] 重构: 委托给3个子函数, 消除~240行重复内联代码
    # 导入规则常量(单一来源) - 子函数内部各自导入所需常量
    ...
    fixed_frontmatter = _fix_frontmatter_direct(slug, fm, skill_data, body)

    # === Step 2: 清理body ===
    cleaned_body = _clean_body_direct(body)

    # === Step 3: 补充依赖说明章节 ===
    final_body = _ensure_dependency_section(cleaned_body)
...
# ============================================================
# 流水线阶段辅助函数 (run_generation_pipeline 拆分)
# ============================================================

def _init_pipeline_result(slug: str) -> Dict[str, Any]:
    """[V137 I11] 初始化流水线结果字典"""
    return {
        'slug': slug,
...
def _read_original_skill(slug: str) -> str:
    """[V137 I11] 读取原始skill内容 (Step 1)"""
    print(f"\n[1/7] 查找原始skill: {slug}")
...
def _resolve_template(template_name: Optional[str], direct: bool, original_content: str,
                      result: Dict[str, Any], pipeline_start: float):
    """[V137 I11] 选择模板或直接增强模式 (Step 2)

    返回 (template_name, early_return) - early_return 非 None 时主函数应直接返回它
    """
...
def _generate_skill_content(slug: str, template_name: Optional[str], direct: bool,
                            original_content: str, result: Dict[str, Any],
                            pipeline_start: float):
    """[V137 I11] 生成SKILL.md内容 (Step 3)

    返回 (generated_content, early_return) - early_return 非 None 时主函数应直接返回它
    """
    print(f"\n[3/7] 生成SKILL.md内容")
    if direct:
...
def _save_skill_md(slug: str, generated_content: str, result: Dict[str, Any]) -> Path:
    """[V137 I11] 保存SKILL.md到磁盘 (Step 4), 返回输出路径(Path)"""
    print(f"\n[4/7] 保存SKILL.md")
...
def _run_l1_check_stage(output_path: Path, result: Dict[str, Any], pipeline_start: float):
    """[V137 I11] 运行L1静态检查 (Step 5)

    返回 (l1_result, early_return) - early_return 非 None 时主函数应直接返回它
    """
    print(f"\n[5/7] 运行L1静态检查")
...
def _run_dependency_verification_stage(slug: str, skip_dep_verify: bool,
                                       result: Dict[str, Any]) -> None:
    """[V137 I11] 运行依赖验证 (Step 6)"""
...
def _run_l2_validation_stage(slug: str, skip_l2: bool, result: Dict[str, Any]) -> None:
    """[V137 I11] 运行L2 LLM验证 (Step 7)"""
    if not skip_l2:
        print(f"\n[7/7] 运行L2 LLM验证")
...
def _finalize_and_report(result: Dict[str, Any], l1_result: Dict[str, Any],
                         pipeline_start: float, slug: str, template_name: str) -> None:
    """[V137 I11] 汇总流水线结果并保存报告"""
    # 汇总
    ...
    # 保存报告
    report_path = TOOLS_DIR / f"generation_report_{slug}.json"
...
def run_generation_pipeline(slug: str, template_name: str = None,
    ...
    result = _init_pipeline_result(slug)

    # Step 1: 读取原始skill
    original_content = _read_original_skill(slug)

    # Step 2: 选择模板 或 直接增强模式
    template_name, early = _resolve_template(template_name, direct, original_content, result, pipeline_start)
    if early is not None:
        return early

    # Step 3: 生成SKILL.md内容
    generated_content, early = _generate_skill_content(slug, template_name, direct, original_content, result, pipeline_start)
    if early is not None:
        return early

    # Step 4: 保存
    output_path = _save_skill_md(slug, generated_content, result)

    # Step 5: L1静态检查
    l1_result, early = _run_l1_check_stage(output_path, result, pipeline_start)
    if early is not None:
        return early

    # Step 6: 依赖验证
    _run_dependency_verification_stage(slug, skip_dep_verify, result)

    # Step 7: L2验证
    _run_l2_validation_stage(slug, skip_l2, result)

    # 汇总
    _finalize_and_report(result, l1_result, pipeline_start, slug, template_name)
...
def cmd_batch(args):
    ...
    report_path = TOOLS_DIR / f"batch_generation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def generate_skill_with_agent(skill_data: Dict[str, Any], context: dict = None) -> str:
    """E13: 使用LLM生成SKILL.md内容 — 双路径(Trae AI代理/外部API)

    V138 A2: 修复断点 — 从"只返回prompt"改为"调用llm_bridge执行+返回结果"。
    LLM不可用时走generate_from_template()规则降级(真实降级, 非mock)。
    """
    from llm_bridge import get_bridge
    bridge = get_bridge()
    result = bridge.execute('generate', skill_data, context)
    if result.get('status') == 'success':
        return result['result']
    # fallback: 无LLM可用时走模板降级(真实降级, 非mock)
    from generate_skill import generate_from_template
    return generate_from_template(skill_data, context or {})
...
def generate_skill_spec(
    ...
    """E14: Spec驱动Skill生成 — 在生成Skill前创建轻量spec

    借鉴OpenSpec的SDD(Spec-Driven Development)理念,
    在生成SKILL.md前先创建spec, 明确:
    - 目标用户(target_user)
    - 核心功能(core_features)
    - 差异化要点(differentiation)
    - 预期评分(expected_score)

    依赖E13的AI代理能力(Phase 1已完成)。

    参数:
        name: skill名称
        description: skill描述
        category: skill分类(默认'Other')
        source: 来源标识
        skill_content: 已有skill内容(差异化时传入, 全新生成时为空)
        use_agent: 是否使用AI代理路径(默认True, False则用规则生成spec)
    ...
    返回:
        {
            'spec': {
                ...
                'core_features': list,    # 核心功能列表
                'differentiation': str,   # 差异化要点
                'expected_score': float,  # 预期评分(0-5)
                'source': str,            # 来源
            },
            'agent_prompt': str,          # AI代理prompt(use_agent=True时)
            'spec_valid': bool,           # spec是否通过校验
            'validation_issues': list,    # 校验问题列表
            'created_at': str,            # 创建时间(ISO)
        }
    """
    # 1. 生成预期slug(复用auto_differentiate的generate_slug逻辑)
    import re as _re
    raw_slug = name.lower()
    raw_slug = _re.sub(r'[\s_]+', '-', raw_slug)
    raw_slug = _re.sub(r'[^a-z0-9-]', '', raw_slug)
    raw_slug = _re.sub(r'-+', '-', raw_slug).strip('-')
    slug = raw_slug[:60] if raw_slug else 'unnamed-skill'

    # 2. 规则生成spec基础信息(不依赖LLM)
    target_user = _infer_target_user(category, description)
    core_features = _extract_core_features(description, skill_content)
    differentiation = _infer_differentiation(category, source, description)
    expected_score = _estimate_expected_score(core_features, differentiation)
    ...
    if use_agent:
        ...
        context = {
            ...
            'source': source,
            'spec_mode': True,  # 标记为spec生成模式
        }
        agent_prompt = generate_agent_prompt('generate', skill_data, context)
    ...
    # 4. Spec完整性校验
    completeness_issues = _validate_spec_completeness(spec)
...
def _infer_target_user(category: str, description: str) -> str:
    """从分类和描述推断目标用户"""
    category_user_map = {
        'Finance': '财务人员、会计、企业财务管理者',
        'Creative': '内容创作者、设计师、营销人员',
        'Developer': '软件开发者、全栈工程师、技术团队',
        'Productivity': '项目经理、知识工作者、效率追求者',
        'Data': '数据分析师、数据工程师、BI团队',
        'Communication': '运营人员、客服团队、社群管理者',
        'Research': '研究人员、学术工作者、情报分析师',
        'Security': '安全工程师、DevSecOps团队、合规审计员',
        'DevOps': '运维工程师、SRE团队、平台工程团队',
        'AI': 'AI工程师、ML研究员、AI产品经理',
        'Other': '独立开发者、一人公司、效率工具使用者',
    }
    return category_user_map.get(category, category_user_map['Other'])


def _extract_core_features(description: str, skill_content: str) -> list:
    """从描述和已有内容提取核心功能列表"""
    features = []
    text = description + '\n' + skill_content
    ...
    # 从描述中提取功能关键词
    feature_keywords = [
        '自动化', '处理', '分析', '生成', '转换', '验证', '监控',
        '批量', '解析', '提取', '计算', '格式化', '检查', '优化',
        'automation', 'process', 'analyze', 'generate', 'convert',
    ]
    for kw in feature_keywords:
        if kw.lower() in text.lower() and kw not in features:
            features.append(kw)
    ...
    return features[:5]  # 最多5个


def _infer_differentiation(category: str, source: str, description: str) -> str:
    """推断差异化要点"""
    parts = []
    if source:
        parts.append(f'基于{source}来源验证')
    parts.append(f'专注{category}领域深度优化')
    if '自动化' in description or 'automatic' in description.lower():
        parts.append('端到端自动化流程')
    parts.append('结构化输入输出支持批量操作')
    return '；'.join(parts)


def _estimate_expected_score(core_features: list, differentiation: str) -> float:
    """根据核心功能和差异化要点估算预期评分"""
    base_score = 3.5
    # 核心功能数量加分
    feature_bonus = min(len(core_features) * 0.2, 0.8)
    # 差异化要点加分
    diff_bonus = 0.3 if len(differentiation) > 20 else 0.1
    expected = base_score + feature_bonus + diff_bonus
    return round(min(expected, 5.0), 1)


def _validate_spec_completeness(spec: dict) -> list:
    """校验spec字段完整性"""
    issues = []
    required_fields = ['name', 'slug', 'category', 'target_user',
                       'core_features', 'differentiation', 'expected_score']
    for field in required_fields:
        val = spec.get(field)
        if val is None or val == '' or val == []:
            issues.append(f"spec缺少必填字段: {field}")
    # expected_score范围校验
    score = spec.get('expected_score', 0)
    if not (0 <= score <= 5):
        issues.append(f"expected_score超出范围(0-5): {score}")
    # core_features至少2个
    features = spec.get('core_features', [])
    if len(features) < 2:
        issues.append(f"core_features至少需要2个, 当前{len(features)}个")
    return issues
