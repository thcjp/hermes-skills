...
license: Proprietary
edition: pro
description: "桌面自发化中枢（专业版）是AI Agent的完整桌面操控方案，在免费版五大核心模块基础上解锁图像识别定位、高级多显示器管控、窗口状态控制、审批模式、性能调优编排五大高级能力。兼容基于OpenCV的模板匹配，让Agent能\"看见\"屏幕元素并精准点击. 适用于需要desktop automation hub相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 桌面自动化
  - 图像识别
  - 多显示器
...
```json
{
  "success": true,
  "data": {
    "result": "桌面自动化中枢(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "desktop automation hub pro"
    }
  },
