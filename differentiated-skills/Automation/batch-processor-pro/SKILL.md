...
license: Proprietary
description: |-
  批处理专家解决大批量数据处理的OOM、中断无法恢复、缺乏幂等、进度不可见四大痛点,提供流式分块、检查点恢复、幂等设计、进度报告、并行决策矩阵、五级错误分级六大核心能力。适用于批量文件/API/数据清洗/媒体转换等"对N个东西做同样操作"的场景。触发关键词:批处理、批量、检查点、断点续跑、幂等、并行、流式、OOM、进度
tags:
- 自动化
- 批处理
- 效率工具
...
1. **流式分块**:按行/按文件/按批次分块处理,内存恒定不OOM。大文件按行读、目录按文件处理、API按批次调用
2. **检查点恢复**:每N项(默认50)存检查点到磁盘,中断后从断点续跑而非从头重跑,避免80%进度丢失
3. **幂等设计**:每项有唯一键(文件路径+mtime/记录主键/业务ID),处理前查重跳过已处理项,防止重复扣款/发邮件
4. **进度报告**:按总项数自适应报告频率(<100每项/100-1000每10项/1000+每100项),含完成百分比与剩余时间预估
5. **并行决策矩阵**:按任务特征(CPU/IO密集、顺序依赖、数据量、限流)选串行/并发/分片并行,附ThreadPoolExecutor与分片示例
6. **五级错误分级**:L1瞬时重试退避、L2数据错误跳过记录、L3可恢复排队、L4系统中止告警、L5数据完整性立即中止人工介入
...
| 全部API调用失败 | API限流或凭证过期 | 加令牌桶限流、退避重试(1s/2s/4s)、检查Token有效性 |
