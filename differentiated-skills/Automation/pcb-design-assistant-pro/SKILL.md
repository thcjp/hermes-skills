...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | PCB设计助手(专业版)处理的输入数据或指令 |
...
**处理**: 解析多页分层原理图设计（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多页分层原理图设计（专业版独有）的响应数据,包含状态码、结果和日志.
...
**处理**: 解析PCB布局与布线（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回PCB布局与布线（专业版独有）的响应数据,包含状态码、结果和日志.
### 3. 设计规则检查DRC（专业版独有）
完整DRC检查覆盖八大类规则：
...
**处理**: 解析设计规则检查DRC（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回设计规则检查DRC（专业版独有）的响应数据,包含状态码、结果和日志.
### 4. 多层板设计（专业版独有）
支持4/6/8层板，提供标准叠层结构与阻抗控制建议：

**输入**: 用户提供多层板设计（专业版独有）所需的指令和必要参数.
**处理**: 解析多层板设计（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多层板设计（专业版独有）的响应数据,包含状态码、结果和日志.
### 5. 自定义元件库与封装管理（专业版独有）

**输入**: 用户提供自定义元件库与封装管理（专业版独有）所需的指令和必要参数.
**处理**: 解析自定义元件库与封装管理（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自定义元件库与封装管理（专业版独有）的响应数据,包含状态码、结果和日志.
### 6. 设计模板与复用库（专业版独有）
将常用电路沉淀为可复用模板，新项目直接调用：

**输入**: 用户提供设计模板与复用库（专业版独有）所需的指令和必要参数.
**处理**: 解析设计模板与复用库（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回设计模板与复用库（专业版独有）的响应数据,包含状态码、结果和日志.
### 7. 扩展电气规则检查ERC（专业版独有）
在基础质量门之上，新增深度电气检查：
...
**处理**: 解析扩展电气规则检查ERC（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回扩展电气规则检查ERC（专业版独有）的响应数据,包含状态码、结果和日志.
### 8. BOM自动生成与立创下单（专业版独有）

**输入**: 用户提供BOM自动生成与立创下单（专业版独有）所需的指令和必要参数.
**处理**: 解析BOM自动生成与立创下单（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回BOM自动生成与立创下单（专业版独有）的响应数据,包含状态码、结果和日志.
### 9. Gerber输出与打板文件包（专业版独有）

**输入**: 用户提供Gerber输出与打板文件包（专业版独有）所需的指令和必要参数.
**处理**: 解析Gerber输出与打板文件包（专业版独有）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回Gerber输出与打板文件包（专业版独有）的响应数据,包含状态码、结果和日志.
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：EasyEDA、全流程、设计专业版、支持多页原理图、自定义库与、覆盖从设计到打板、设计助手、面向硬件团队与专、业电子工程师、在免费版基础上解、锁全部高级能力、设计复用模板、单对接、覆盖从需求到打板、的全流程、Use、when、、品牌视觉时使用、不适用于、建模和动画制作等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 快速开始
...
```json
{
  "success": true,
  "data": {
    "result": "PCB设计助手(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pcb design assistant pro"
    }
  },
