...
**输入**: 用户提供中英文自然语言捕获所需的指令和必要参数。
**处理**: 按照skill规范执行中英文自然语言捕获操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行自定义提醒偏移操作,遵循单一意图原则。
**输出**: 返回自定义提醒偏移的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供农历生日支持所需的指令和必要参数。
**处理**: 按照skill规范执行农历生日支持操作,遵循单一意图原则。
**输出**: 返回农历生日支持的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供多渠道投递所需的指令和必要参数。
**处理**: 按照skill规范执行多渠道投递操作,遵循单一意图原则。
**输出**: 返回多渠道投递的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行语义搜索操作,遵循单一意图原则。
**输出**: 返回语义搜索的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行事件生命周期管理操作,遵循单一意图原则。
**输出**: 返回事件生命周期管理的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：自然语言事件捕获、与提醒助手专业版、含自定义偏移、语义搜索与完整重、智能提醒助手专业、版是在免费版基础、上的全功能升级、Agent、提供企业级自然语、言事件管理能力、专业版解锁自定义、事件更新与归档等、高级特性、实现复杂事件场景、的智能管理、when、模型调用、智能对话、LLM、应用时使用、不适用于需要、确定性的关键决策等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
