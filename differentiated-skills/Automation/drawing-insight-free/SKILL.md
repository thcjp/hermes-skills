...
tags: 图纸解析,pdf,row,用户提供,result,print
tools:
  - read
  - exec
  - write
...
**处理**: 解析PDF图纸文本抽取的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回PDF图纸文本抽取的响应数据,包含状态码、结果和日志.
### 2. 标题栏八要素识别
...
**处理**: 解析标题栏八要素识别的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回标题栏八要素识别的响应数据,包含状态码、结果和日志.
### 3. 尺寸标注解析
...
**处理**: 解析尺寸标注解析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回尺寸标注解析的响应数据,包含状态码、结果和日志.
...
**处理**: 解析注释分类识别的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回注释分类识别的响应数据,包含状态码、结果和日志.
### 5. 明细表解析为构件符号
...
**处理**: 解析明细表解析为构件符号的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回明细表解析为构件符号的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析图纸质量自动检查的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回图纸质量自动检查的响应数据,包含状态码、结果和日志.
