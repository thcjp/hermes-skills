...
**处理**: 按照skill规范执行批量文档翻译（专业版独有）操作,遵循单一意图原则。
**输出**: 返回批量文档翻译（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自定义术语库管理（专业版独有）操作,遵循单一意图原则。
**输出**: 返回自定义术语库管理（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行翻译记忆库TM（专业版独有）操作,遵循单一意图原则。
**输出**: 返回翻译记忆库TM（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行多语言互译（专业版独有）操作,遵循单一意图原则。
**输出**: 返回多语言互译（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行文档级上下文记忆（专业版独有）操作,遵循单一意图原则。
**输出**: 返回文档级上下文记忆（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行领域专用术语库（专业版独有）操作,遵循单一意图原则。
**输出**: 返回领域专用术语库（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行API集成与CI/CD自动化（专业版独有）操作,遵循单一意图原则。
**输出**: 返回API集成与CI/CD自动化（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行翻译质量评估（专业版独有）操作,遵循单一意图原则。
**输出**: 返回翻译质量评估（专业版独有）的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级多语言翻译、支持批量文档翻译、多语言互译与、覆盖本地化全流程、中英翻译中枢、面向翻译团队、本地化工程师与跨、国企业、在免费版基础上解、锁全部高级能力、无上限批量文档翻、自定义术语库与团、中英日韩法德西俄、垂直领域专用术语、覆盖从单段翻译到、企业级本地化流水、线的完整工作流等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
| API调用超时 | LLM响应慢或并发过高 | 降低并发数；增加超时时间；启用执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令机制 | 中 |
