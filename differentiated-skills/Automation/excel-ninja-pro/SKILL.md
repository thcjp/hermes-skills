...
python scripts/vlookup_multi.py --help
python scripts/template_fill.py --help
```
...
python scripts/vlookup_multi.py \
  --main 客户主表.xlsx \
...
python scripts/format_conditional.py \
  --input 销售表.xlsx \
  --column 销售额 \
  --rule gt --value 10000 \
  --fill red \
...
# 完整数据处理流水线示例
# 第一步：合并3个月的数据
python scripts/merge_sheets.py --inputs 1月.xlsx 2月.xlsx 3月.xlsx --output Q1原始.xlsx

# 第二步：去重
python scripts/deduplicate_excel.py --input Q1原始.xlsx --keys 订单号 --keep first --output Q1去重.xlsx

# 第三步：校验
python scripts/validate_excel.py --input Q1去重.xlsx --require-cols "订单号,客户,金额" --key-cols "订单号"

# 第四步：关联产品信息（VLOOKUP）
python scripts/merge_tables.py --left Q1去重.xlsx --right 产品目录.xlsx --on 产品ID --output Q1完整.xlsx

# 第五步：按地区聚合
python scripts/aggregate_excel.py --input Q1完整.xlsx --group-by 地区 --agg "金额:sum,订单号:count" --output Q1地区统计.xlsx

# 第六步：条件格式高亮
python scripts/format_conditional.py --input Q1地区统计.xlsx --column "金额:sum" --rule gt --value 500000 --fill red --output Q1最终报告.xlsx
```
...
**处理**: 按照skill规范执行基础脚本（8个，与免费版一致）操作,遵循单一意图原则。
**输出**: 返回基础脚本（8个，与免费版一致）的执行结果,包含操作状态和输出数据。
...
```bash
# 只保留指定列
python scripts/select_columns.py --input data.xlsx --columns "姓名,电话,地址" --output 精简.xlsx

# 重命名列
python scripts/select_columns.py --input data.xlsx --rename "姓名:客户名称,电话:联系方式" --output 重命名.xlsx

# 选择并重命名
python scripts/select_columns.py --input data.xlsx --columns "姓名,电话" --rename "姓名:客户名称" --output 结果.xlsx
```
...
```bash
# 左连接（保留左表全部行）
python scripts/merge_tables.py --left 订单表.xlsx --right 客户表.xlsx --on 客户ID --output 合并.xlsx

# 内连接（只保留两表都有的行）
python scripts/merge_tables.py --left 订单表.xlsx --right 客户表.xlsx --on 客户ID --join inner --output 交集.xlsx

# 外连接（保留两表全部行）
python scripts/merge_tables.py --left 订单表.xlsx --right 客户表.xlsx --on 客户ID --join outer --output 全集.xlsx
```
...
```bash
# 基本转置
python scripts/transpose_excel.py --input 原始表.xlsx --output 转置表.xlsx
```
...
```bash
# 按原名映射重命名
python scripts/rename_sheets.py --input file.xlsx --rename "Sheet1:1月,Sheet2:2月,Sheet3:3月"

# 添加前缀
python scripts/rename_sheets.py --input file.xlsx --prefix "2024_"

# 添加后缀
python scripts/rename_sheets.py --input file.xlsx --suffix "_已审核"

# 按索引重命名
python scripts/rename_sheets.py --input file.xlsx --rename "0:汇总,1:明细,2:图表"
```
...
```bash
# 大于某值标红
python scripts/format_conditional.py --input data.xlsx --column 金额 --rule gt --value 10000 --fill red --output 高亮.xlsx

# 小于某值标黄
python scripts/format_conditional.py --input data.xlsx --column 库存 --rule lt --value 10 --fill yellow --output 预警.xlsx

# 介于两个值之间标绿
python scripts/format_conditional.py --input data.xlsx --column 分数 --rule between --value 80 100 --fill green --output 优秀.xlsx

# 重复值标红
python scripts/format_conditional.py --input data.xlsx --column 订单号 --rule duplicate --fill red --output 查重.xlsx

# 色阶（渐变色）
python scripts/format_conditional.py --input data.xlsx --column 销售额 --rule colorscale --output 色阶.xlsx
```
...
```bash
# 身份证号、订单号等长数字列设为文本
python scripts/format_columns_as_text.py --input data.xlsx --columns "身份证号,订单号,银行卡号" --output 文本格式.xlsx
```

**典型场景**：身份证号(18位)、银行卡号(16-19位)、订单号(长数字)等导入Excel后变成科学计数法，设为文本格式后正确显示。

**输入**: 用户提供高级脚本（8个，专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行高级脚本（8个，专业版独有）操作,遵循单一意图原则。
**输出**: 返回高级脚本（8个，专业版独有）的执行结果,包含操作状态和输出数据。
...
```bash
# 第一步：合并5个渠道数据
python scripts/merge_sheets.py --inputs ./渠道数据/ --output 本周原始.xlsx

# 第二步：去重
python scripts/deduplicate_excel.py --input 本周原始.xlsx --keys 订单号 --keep first --output 本周去重.xlsx

# 第三步：关联产品信息
python scripts/merge_tables.py --left 本周去重.xlsx --right 产品目录.xlsx --on 产品ID --output 本周完整.xlsx

# 第四步：按渠道分组统计
python scripts/aggregate_excel.py --input 本周完整.xlsx --group-by 渠道 --agg "访客数:sum,转化率:mean,金额:sum" --output 本周周报.xlsx

# 第五步：高亮高转化渠道
python scripts/format_conditional.py --input 本周周报.xlsx --column "转化率:mean" --rule gt --value 0.05 --fill green --output 本周周报_高亮.xlsx
```
...
# 筛选高价值客户
python scripts/filter_excel.py --input 客户360视图.xlsx --where "总消费>50000" --output 高价值客户.xlsx

# 选择关键字段
python scripts/select_columns.py --input 高价值客户.xlsx --columns "客户ID,姓名,总消费,积分,标签" --output 高价值客户_精简.xlsx
```
...
```bash
# 第一步：校验必填列和键列
python scripts/validate_excel.py --input 员工表.xlsx --require-cols "工号,姓名,身份证号,部门" --key-cols "工号,身份证号"

# 第二步：去重
python scripts/deduplicate_excel.py --input 员工表.xlsx --keys 工号 --keep first --output 员工去重.xlsx

# 第三步：身份证号设为文本格式（避免科学计数法）
python scripts/format_columns_as_text.py --input 员工去重.xlsx --columns "身份证号,银行卡号" --output 员工最终.xlsx

# 第四步：按部门统计人数
python scripts/aggregate_excel.py --input 员工最终.xlsx --group-by 部门 --agg "工号:count" --output 部门人数.xlsx
```
...
```bash
# 宽表转长表（转置）
python scripts/transpose_excel.py --input 宽表.xlsx --output 长表.xlsx

# 转置后按月份聚合
python scripts/aggregate_excel.py --input 长表.xlsx --group-by 月份 --agg "销售额:sum" --output 月度趋势.xlsx
```
...
```bash
# 批量重命名（添加前缀）
python scripts/rename_sheets.py --input 年度数据.xlsx --prefix "2024_"

# 或按序号重命名
python scripts/rename_sheets.py --input 年度数据.xlsx --rename "0:Q1,13:Q2,26:Q3,39:Q4"
```
...
```bash
# 大文件分块处理策略
python scripts/split_excel.py --input 超大表.xlsx --by-rows 50000
# 对每个分块文件处理...
# 最后用merge_sheets.py合并结果
python scripts/merge_sheets.py --inputs ./分块结果/ --output 最终结果.xlsx
```
...
# 批量转换（Linux/macOS）
for f in *.xlsx; do
    python scripts/excel_to_csv.py --input "$f" --output "${f%.xlsx}.csv"
done
