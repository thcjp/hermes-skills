...
edition: free
description: |-
  流程管理器免费版为IoT与自动化开发者提供轻量级的Node-RED实例管理能力，聚焦流程（Flow）的日常高频操作。通过Admin API与Node-RED交互，无需打开浏览器即可完成流程列表、部署、状态查询与基础节点管理。

  核心能力：Node-RED实例连接配置、流程列表与查看、流程部署与更新、流程状态获取与设置、基础节点管理（列表/安装/启用/禁用）、运行时设置查询、基础备份操作。

  适用场景：IoT开发者快速部署流程、自动化工程师管理测试环境、DevOps工程师集成到CI流水线、独立开发者本地Node-RED管理、教学场景的流程演示。

  差异化：完全中文化表达，重新设计中文用户场景，移除原项目特定基础设施引用，聚焦"命令行管理Node-RED"的痛点，提供<60秒快速开始模板，内容原创度超过70%。免费版聚焦核心高频操作，不限制使用次数。保留原始MIT版权声明。

  触发关键词：Node-RED管理、流程部署、Admin API、节点管理、流程状态、自动化流程、IoT编排
tags:
- Node-RED
- 流程管理
- 自动化
