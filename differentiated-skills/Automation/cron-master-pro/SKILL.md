...
**输入**: 用户提供定时大师是平台级定时系统的深度使用指南所需的指令和必要参数.
**处理**: 解析定时大师是平台级定时系统的深度使用指南的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回定时大师是平台级定时系统的深度使用指南的响应数据,包含状态码、结果和日志.
...
**输入**: 用户提供定位为"精通层"所需的指令和必要参数.
**处理**: 解析定位为"精通层"的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回定位为"精通层"的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

**技术实现要点**：核心能力基于`input_params`参数与`output_format`配置实现,支持创建/查询/修改/删除等操作模式,通过`config_options`进行运行时配置.
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：平台级定时系统精、通指南、heartbeat、决策矩阵、推送策略、自唤醒规则全覆盖、它提供、的决策矩阵、payload、类型选择、agentTurn、systemEvent、严格指令模板、自唤醒规则、时区锁定规范与历、史模式迁移指南等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
### 核心功能执行
