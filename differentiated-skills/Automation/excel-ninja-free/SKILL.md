...
summary: "快速完成Excel合并、拆分、筛选、去重、聚合、校验等高频操作，告别手工处理表格的繁琐.。Excel忍者为AI Agent提供专业的Excel文件自动化处理能力。免费版开放8个核心脚本，覆盖"
license: MIT
edition: free
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：快速完成、校验等高频操作、告别手工处理表格、的繁琐、忍者为、Agent、提供专业的、文件自动化处理能、覆盖合并、列选择等高频场景、高手一样批量处理、Use、when、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
