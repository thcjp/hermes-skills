...
**处理**: 按照skill规范执行CAD 洞察专家为 AI Agent 提操作,遵循单一意图原则。
**输出**: 返回CAD 洞察专家为 AI Agent 提的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行它通过可配置的标题栏模板适配不同公司/标操作,遵循单一意图原则。
**输出**: 返回它通过可配置的标题栏模板适配不同公司/标的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行从"能解析单张 PDF"升级为操作,遵循单一意图原则。
**输出**: 返回从"能解析单张 PDF"升级为的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
