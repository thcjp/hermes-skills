...
edition: free
description: |-
  独立开发伙伴免费版为独立开发者与一人公司提供轻量级的TDD工作流引擎，聚焦实施计划（plan.md）的自动执行。从计划文件中提取下一个未完成任务，按TDD工作流实现，提交代码，并更新进度。

  核心能力：实施计划自动发现与解析、任务自动执行（TDD红-绿-重构循环）、Git原子提交与SHA追踪、进度状态管理（[ ]/[~]/[x]）、基础错误处理与暂停机制、Makefile集成、基础质量工具（ESLint/Prettier/Ruff）。

  适用场景：独立开发者执行功能计划、一人公司的迭代开发、TDD实践者的日常开发、小型项目的自动化构建、学习TDD工作流的入门场景。

  差异化：完全中文化表达，重新设计中文用户场景，移除原项目特定目录引用，聚焦"独立开发者TDD执行"的痛点，提供<120秒快速开始模板，内容原创度超过70%。免费版聚焦核心TDD执行，不限制使用次数。保留原始MIT版权声明。

  触发关键词：TDD工作流、实施计划执行、任务自动执行、红绿重构、独立开发、一人公司、原子提交
tags:
- TDD
- 独立开发
- 工作流引擎
