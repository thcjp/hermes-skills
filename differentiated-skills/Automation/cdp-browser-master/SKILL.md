...
summary: "解决反爬检测、选择器易"
license: Proprietary
description: 。Use when 用户需要cdp-browser-master相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。提供结构化输出和错误处理机制。
tags:
  - 自动化
  - 浏览器
  - 数据抓取
...
**可用性分类**: MD+execute(Markdown指令 + Node.js脚本执行)
