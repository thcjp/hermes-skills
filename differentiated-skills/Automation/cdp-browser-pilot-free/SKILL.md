...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | CDP浏览器领航(免费版)处理的输入数据或指令 |
...
本免费版在CDP连接、等待时间、端口管理和Cookie处理方面存在以下已知限制，使用前请了解：
...
以下是一个完整的 JS 渲染页面数据抓取示例，展示从启动浏览器到提取数据的全流程：

```javascript
// 1. 启动 Edge 远程调试（PowerShell）
// Start-Process "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --remote-debugging-port=9222
...
| 错误场景 | 原因 | 处理方式 |
|:---:|:---:|:---:|
| CDP连接失败 | 浏览器未启动远程调试或端口被占用 | 确认浏览器以 --remote-debugging-port 启动，检查端口占用 |
| 页面超时 | JS渲染时间超过默认等待 | 手动增加 edge.wait() 毫秒数至 8000-10000 |
| 元素找不到 | CSS选择器不匹配或元素未加载 | 增加等待时间，或用 evaluate 检查 DOM 结构 |
| 截图为空白 | 页面未渲染完成 | 在 screenshot 前加 edge.wait(3000) |
| 配置错误 | 参数缺失或格式错误 | 检查依赖说明中的配置要求 |
| 运行时错误 | 运行环境不满足 | 确认运行环境符合依赖说明 |
| 网络错误 | 连接超时或不可达 | 检查网络连通性和防火墙设置 |
# ...
## 输出格式
```json
{
  "success": true,
  "data": {
    "result": "CDP浏览器领航(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "cdp browser pilot"
    }
  },
