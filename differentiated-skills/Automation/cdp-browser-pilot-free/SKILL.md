...
edition: free
description: |-
  CDP浏览器领航免费版帮助你通过已登录的Edge或Chrome浏览器，利用Chrome DevTools Protocol执行JS渲染页面的自动化操作。解决web_fetch无法处理登录态与JS渲染的痛点，实现导航、点击、截图与基础数据提取。

  核心能力：浏览器远程调试启动（Edge端口9222/Chrome端口9223）、页面导航（goto）、元素点击（click）、页面截图（screenshot）、显式等待（wait）、基础JS执行（evaluate）、标签页管理（tabs）。提供3个真实场景示例与基础FAQ。

  适用场景：JS渲染页面数据抓取、需登录态的网站操作、简单页面交互自动化、截图监控、表单填写、页面信息提取。

  差异化：完全中文化重写，移除原始平台标识，适配标准Agent平台。新增何时使用CDP决策表、浏览器启动模板、基础API参考、已知限制说明。免费版聚焦基础CDP操作，帮助用户处理web_fetch无法胜任的场景。

  触发关键词：CDP、浏览器自动化、Chrome DevTools、JS渲染、登录态、页面交互、Edge自动化、Chrome自动化
tags:
- 浏览器自动化
- CDP
- 数据提取
