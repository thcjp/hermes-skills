...
description: |-
  ...
  核心能力：多云统一管理（AWS+GCP+Azure跨云编排）、Terraform+Ansible+CloudFormation三工具集成、CI/CD部署流水线（Git Push自动触发）、合规策略（CIS基准检查）、灾备恢复（跨区域备份+自动故障切换）、成本优化（资源分析+成本告警+自动缩容）、安全加固（网络审计+密钥轮换+加密配置）、多角色场景指南、多平台集成示例、版本迁移指南。
...
**处理**: 按照skill规范执行功能一：多云统一管理 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能一：多云统一管理 — 专业版启用的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能二：IaC三工具集成 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能二：IaC三工具集成 — 专业版启用的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能三：CI/CD部署流水线 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能三：CI/CD部署流水线 — 专业版启用的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能四：合规策略审计 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能四：合规策略审计 — 专业版启用的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能五：灾备恢复 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能五：灾备恢复 — 专业版启用的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能六：成本优化 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能六：成本优化 — 专业版启用的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能七：安全加固 — 专业版启用操作,遵循单一意图原则。
**输出**: 返回功能七：安全加固 — 专业版启用的执行结果,包含操作状态和输出数据。
