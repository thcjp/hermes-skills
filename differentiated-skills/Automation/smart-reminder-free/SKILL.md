...
summary: "自然语言事件捕获与提醒助手免费版，含中文解析、事件存储、默认提醒偏移与基础查询.。智能提醒助手免费版是面向AI Agent的自然语言事件捕获与提醒系统。不同于提醒创建引擎，本技能聚焦"如何理"
license: Proprietary
edition: free
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | 智能提醒助手(免费版)处理的输入数据或指令 |
...
**处理**: 解析自然语言事件捕获的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自然语言事件捕获的响应数据,包含状态码、结果和日志.
### 事件结构化存储
...
**处理**: 解析事件结构化存储的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回事件结构化存储的响应数据,包含状态码、结果和日志.
### 默认提醒偏移
| 偏移 | 说明 | 适用场景 |
|:---:|:---:|:---:|
...
**处理**: 解析默认提醒偏移的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回默认提醒偏移的响应数据,包含状态码、结果和日志.
### 最小化澄清原则
**仅当关键信息模糊时才提问**：
...
**处理**: 解析最小化澄清原则的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回最小化澄清原则的响应数据,包含状态码、结果和日志.
### 事件查询接口
```python
...
**处理**: 解析事件查询接口的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回事件查询接口的响应数据,包含状态码、结果和日志.
...
```json
{
  "success": true,
  "data": {
    "result": "智能提醒助手(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "smart reminder"
    }
  },
