...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | 笔记同步CLI(免费版)处理的输入数据或指令 |
...
**处理**: 解析Vault发现与默认设置的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回Vault发现与默认设置的响应数据,包含状态码、结果和日志.
...
**处理**: 解析笔记搜索的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回笔记搜索的响应数据,包含状态码、结果和日志.
### 3. 笔记创建
...
**处理**: 解析笔记创建的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回笔记创建的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析Frontmatter基础操作的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回Frontmatter基础操作的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析移动与重命名（安全重构）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回移动与重命名（安全重构）的响应数据,包含状态码、结果和日志.
### 6. 删除笔记
...
**处理**: 解析删除笔记的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回删除笔记的响应数据,包含状态码、结果和日志.
...
```json
{
  "success": true,
  "data": {
    "result": "笔记同步CLI(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "notes sync cli"
    }
  },
