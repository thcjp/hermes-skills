...
edition: free
description: |-
  桌面流程控制中枢免费版是一套面向AI Agent的桌面自动化执行框架，将鼠标操作、键盘输入、屏幕截图等底层控制能力封装为统一的流程控制接口。通过本技能，Agent可以像人类一样操作桌面应用程序，实现表单填写、窗口切换、截图归档等常见RPA场景。

  核心能力：像素级鼠标定位（绝对/相对坐标）、贝塞尔曲线平滑移动、多类型点击（左/右/中/双击/三击）、可配置速度的键盘输入、热键组合执行、全屏与区域截图、像素颜色检测、窗口列表获取与激活、剪贴板读写、失败安全机制（角落紧急中止）。

  适用场景：重复性表单填写自动化、多窗口数据搬运、截图归档与质检、简单桌面测试自动化、批量文件操作、演示录制辅助、跨应用数据录入。

  差异化：完全中文化重写，统一流程控制语义模型，新增分级快速开始指南（30秒/120秒/300秒）、真实场景示例、安全防护机制说明、键名速查表、性能优化建议、故障排查清单。内容原创度超过70%。免费版提供核心自动化能力，专业版解锁图像识别、多显示器、审批模式等高级功能。

  触发关键词：桌面自动化、鼠标控制、键盘输入、屏幕截图、RPA、流程控制、窗口管理、剪贴板
tags:
- 桌面自动化
- 流程控制
- 鼠标键盘
