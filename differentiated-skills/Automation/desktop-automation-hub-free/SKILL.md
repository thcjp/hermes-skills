...
edition: "free"
description: |-
  桌面自动化中枢（免费版）为AI Agent提供桌面级精准操控能力，覆盖鼠标定位、键盘输入、屏幕截图、窗口激活与剪贴板操作五大核心模块。采用failsafe紧急停止机制，确保自动化任务可控可中断。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。
tags:
  - 桌面自动化
  - 鼠标键盘控制
  - 屏幕截图
