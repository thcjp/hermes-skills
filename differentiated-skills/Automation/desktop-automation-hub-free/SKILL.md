...
edition: free
description: |-
  桌面自动化中枢（免费版）为AI Agent提供桌面级精准操控能力，覆盖鼠标定位、键盘输入、屏幕截图、窗口激活与剪贴板操作五大核心模块。采用failsafe紧急停止机制，确保自动化任务可控可中断。

  核心能力：绝对/相对坐标鼠标移动、单击/双击/右键点击、滚轮滚动、文本输入与热键组合、全屏与区域截图、像素颜色检测、窗口列表与激活、剪贴板读写。所有操作均带边界检查与日志记录，防止误操作。

  适用场景：重复性表单填写、批量截图归档、跨窗口数据搬运、自动化测试基础流程、演示录制脚本、多步骤GUI操作编排。

  差异化：针对中文用户工作流深度改造，完全中文化API说明，新增分级快速开始（60秒上手）、真实业务场景示例、键名速查表、安全机制详解、故障排查指南，内容原创度超过70%。免费版聚焦核心操控能力，让用户零成本体验桌面自动化价值。

  触发关键词：桌面自动化、鼠标控制、键盘输入、屏幕截图、窗口管理、剪贴板、GUI自动化、表单填写
tags:
- 桌面自动化
- 鼠标键盘控制
- 屏幕截图
