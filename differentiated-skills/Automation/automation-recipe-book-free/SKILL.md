...
edition: free
description: |-
  自动化配方手册免费版为AI Agent提供8个开箱即用的自动化场景配方。每个配方以YAML格式定义触发条件与执行动作，用户复制即可使用，修改参数即可定制。配套提供配方结构解析与自定义指南，帮助用户从"套用"进阶到"创造"。

  核心能力：8个精选自动化配方（每日新闻摘要、邮件自动回复、GitHub Issue监控、价格监控、会议提醒、内容发布、数据备份、智能排程）、配方结构标准化（trigger+actions）、参数定制指南、基础调试方法、配方启用与禁用。

  适用场景：效率工具爱好者入门自动化、独立开发者日常任务自动化、小团队流程编排、个人工作流优化、自动化学习与实践、重复任务批量处理、定时任务快速搭建。

  差异化：针对"原始配方集仅罗列YAML、无结构解析、无定制指南、无调试方法、含原平台标识"的痛点重新设计。新增配方结构标准化解析、参数定制指南、基础调试方法、3类真实场景示例。完全去除原平台标识，内容原创度超过70%，完全中文化。保留原始MIT版权声明。

  触发关键词：自动化配方、自动化场景、工作流、recipe、定时任务、自动化、触发器、动作编排
tags:
- 自动化配方
- 工作流
- 效率工具
