...
# ...
# 或反过来：把手动资源纳入 IaC
（请参考skill目录中的脚本文件） import prod aws_instance.bastion-2 i-xxxxx
```
...
**处理**: 解析云运维编排器为 AI Agen的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回云运维编排器为 AI Agen的响应数据,包含状态码、结果和日志.
...
**处理**: 解析所有销毁类操作默认 dry-run的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回所有销毁类操作默认 dry-run的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
```bash
# 1. 修改 terraform.tfvars 中的 instance_count
#    例如从 3 改为 5
# 2. 预演变更，确认只增不减
（请参考skill目录中的脚本文件） plan prod
# 3. 检查风险等级，确认无 destroy 操作
...
```bash
# 1. 验证 Terraform 版本 >= 1.5
terraform version

# 2. 验证 Ansible 版本 >= 2.15
ansible --version
...
# 输出摘要：add=12, change=0, destroy=0, cost_delta=+$180/月
