...
edition: free
description: |-
  浏览器代理免费版为AI Agent提供基础的浏览器自动化能力。用户用自然语言描述"打开某网页并截图"、"填写这个表单"、"抓取表格数据"，本技能负责把自然语言转化为浏览器操作指令，自动执行并返回结果。

  核心能力：网页打开与导航、整页截图与区域截图、元素点击与文字输入、页面滚动与等待、基础数据抓取（表格/列表/文本）、操作日志可追溯、robots.txt合规检查、敏感操作确认机制。

  适用场景：网页信息采集、表单自动填写、网页内容存档、价格信息监控、学习资料整理、基础UI验证、截图分享与汇报、批量重复操作自动化。

  差异化：针对"原始浏览器技能仅列举功能、无操作流程、无安全机制、无数据存储规范"的痛点重新设计。新增分级快速开始指南、操作流程标准化、敏感操作确认机制、robots.txt合规检查、结构化数据存储、操作日志追溯。内容原创度超过70%，完全中文化。保留原始MIT版权声明。

  触发关键词：浏览器、截图、自动化、填表、抓取、网页操作、数据采集、页面截图
tags:
- 浏览器自动化
- 数据抓取
- 网页操作
...
```bash
# 1. 抓取页面表格数据并导出CSV
#    对Agent说："抓取 https://example.com/data 页面的表格，导出CSV"
