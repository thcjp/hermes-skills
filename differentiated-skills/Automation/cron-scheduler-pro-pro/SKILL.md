...
description: |-
  ...
  适用关键词：企业调度、cron表达式、任务依赖、失败重试、执行统计、优先级队列、DAG编排、分布式调度
tags:
- 企业调度
- 任务编排
- cron表达式
...
**输入**: 用户提供cron表达式调度（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行cron表达式调度（专业版）操作,遵循单一意图原则。
**输出**: 返回cron表达式调度（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供任务依赖DAG编排（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行任务依赖DAG编排（专业版）操作,遵循单一意图原则。
**输出**: 返回任务依赖DAG编排（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供失败重试与熔断（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行失败重试与熔断（专业版）操作,遵循单一意图原则。
**输出**: 返回失败重试与熔断（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供执行统计（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行执行统计（专业版）操作,遵循单一意图原则。
**输出**: 返回执行统计（专业版）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供任务优先级（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行任务优先级（专业版）操作,遵循单一意图原则。
**输出**: 返回任务优先级（专业版）的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级定时任务调、度引擎专业版、优先级控制、定时调度引擎专业、版是面向企业级场、景的完整定时任务、调度解决方案、在免费版四种调度、模式之上、专业版新增、表达式精确调度、任务依赖编排、失败自动重试与熔、执行统计分析、任务优先级与并发、控制五大高级引擎、满足复杂业务场景、的调度需求等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
