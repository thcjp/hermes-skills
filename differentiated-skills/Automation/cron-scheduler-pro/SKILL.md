...
license: MIT
description: "定时调度专家为 AI Agent 包含本地优先、无云依赖的周期任务调度能力。它把\"每隔X做Y\"的意图固化为可信任的执行契约，兼容每日/每周/每月/自定义间隔四种调度类型，并内置时区锁定、一次性任务自发清理、并发写安全、下次运行预览与失败重试。Use. 适用于需要cron scheduler相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 自动化
  - 定时调度
  - 任务管理
