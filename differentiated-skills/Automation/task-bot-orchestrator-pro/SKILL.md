...
**输入**: 用户提供一、条件触发（专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行一、条件触发（专业版独有）操作,遵循单一意图原则。
**输出**: 返回一、条件触发（专业版独有）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回三、数据库集成（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行四、错误恢复（专业版独有）操作,遵循单一意图原则。
**输出**: 返回四、错误恢复（专业版独有）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回五、多渠道通知（专业版独有）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供六、监控仪表盘（专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行六、监控仪表盘（专业版独有）操作,遵循单一意图原则。
**输出**: 返回六、监控仪表盘（专业版独有）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供七、团队协作（专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行七、团队协作（专业版独有）操作,遵循单一意图原则。
**输出**: 返回七、团队协作（专业版独有）的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能任务编排、含条件触发、数据库集成与监控、种角色场景、任务编排机器人、是在免费版基础上、的全功能升级、Agent、提供完整的任务编、排与自动化能力、在数据处理、定时调度、通知推送核心能力、解锁条件触发、团队协作七大高级、when、需要消息发送、邮件短信、通信集成时使用、不适用于垃圾信息等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- **说明**: 通过自然语言指令驱动Agent完成操作编排与工作流管理
