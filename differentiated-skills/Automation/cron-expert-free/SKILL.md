...
version: "1.0.0"
displayName: cron最佳实践专家(免费版)
summary: cron定时系统最佳实践指南免费版，含自唤醒规则、时区锁定、基础提醒模式、常见陷阱规避。
license: MIT
edition: free
description: |-
  cron最佳实践专家免费版是面向AI Agent的定时系统使用最佳实践指南。不同于表达式编写工具，本技能聚焦"如何正确使用定时系统"的方法论与经验法则，帮助Agent建立可靠的定时行为模式。

  核心能力：自唤醒规则（会话开始时检查待执行任务）、时区锁定策略（统一时区避免混乱）、基础提醒模式（单次/重复/递增提醒）、常见陷阱规避（DST夏令时/月末日期/并发竞争）、定时任务命名规范、执行结果确认机制。

  适用场景：Agent需要建立定时行为模式、提醒系统搭建、定时任务可靠性提升、团队定时规范建立、新人cron教学、定时任务故障预防。

  差异化：完全中文化重写，聚焦"最佳实践与方法论"而非表达式语法，新增自唤醒规则框架、时区锁定策略、三类提醒模式、常见陷阱清单、命名规范建议。内容原创度超过70%。免费版提供基础实践指南，专业版解锁高级调度模式、遗留系统迁移、并发控制规则等。

  触发关键词：cron最佳实践、定时提醒、自唤醒、时区策略、定时陷阱、提醒系统、定时规范
tags:
- cron最佳实践
- 定时提醒
- 时区策略
