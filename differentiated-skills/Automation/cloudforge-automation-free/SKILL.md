...
version: "1.0.0"
displayName: 云锻造自动化(免费版)
summary: 单云基础设施即代码工具，含Terraform模板、基础资源配置与一键部署，支持AWS/GCP/Azure。
license: MIT
edition: free
description: |-
  云锻造自动化免费版帮助你使用基础设施即代码（IaC）管理云资源。通过Terraform模板定义、一键部署与基础配置管理，告别手动控制台操作，实现云资源的可重复、可版本化、可追溯管理。

  核心能力：单云基础设施初始化（AWS/GCP/Azure任选其一）、Terraform模板生成（Web应用/数据库/基础网络）、一键部署与销毁、资源配置管理、基础状态管理。提供3个真实场景示例与基础FAQ。

  适用场景：个人项目云资源管理、初创公司基础架构搭建、开发测试环境快速创建、单一云平台资源标准化管理、学习IaC实践。

  差异化：完全中文化重写，移除原始作者署名行，适配标准Agent平台。新增单云初始化流程、四类基础Terraform模板、一键部署脚本、状态管理说明。免费版聚焦单云IaC基础能力，帮助用户快速上手基础设施即代码。

  触发关键词：云基础设施、Terraform、IaC、基础设施即代码、云资源管理、AWS、GCP、Azure、云部署
tags:
- 云基础设施
- Terraform
- IaC
