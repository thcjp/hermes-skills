...
edition: free
description: |-
  工作流催化器为AI Agent提供"识别重复→抽象模式→设计自动化→实现脚本"的四步思维框架。免费版提供核心方法论与基础模板，帮助Agent主动发现用户的重复操作并构建自动化方案。

  核心能力：重复任务识别、操作模式抽象、自动化方案设计、脚本快速实现。提供周报自动化、数据同步、文件整理三个基础模板，覆盖最常见的重复操作场景。

  适用场景：周报自动生成、定时数据同步、文件批量整理、邮件自动回复、报表自动汇总、社交媒体定时发布。

  差异化：将抽象的"自动化思维"转化为可操作的四步流程，每步都有明确的输入输出和检查清单。免费版聚焦方法论与3个基础模板，专业版解锁6步完整框架、模式库、多角色场景与故障排查。

  触发关键词：自动化、重复任务、工作流、脚本、批处理、效率提升、定时任务
tags:
- 工作流自动化
- 效率提升
- 脚本生成
...
### API Key 配置
- 本Skill基于Markdown指令，无需额外API Key（脚本中涉及的外部API需自行配置）
