...
license: MIT
edition: free
description: "工作流催化器为AI Agent包含\"识别重复→抽象模式→设计自发化→达成脚本\"的四步思维框架。免费版包含核心方法论与基础模板，帮助Agent主动发现用户的重复操作并构建自发化方案。Use. 适用于需要workflow catalyst相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 工作流自动化
  - 效率提升
  - 脚本生成
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | 工作流催化器(免费版)处理的输入数据或指令 |
...
**处理**: 解析用户是否在重复执行相同的操作？的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回用户是否在重复执行相同的操作？的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 操作步骤是否每次都一样？
操作步骤是否每次都一样？

**处理**: 解析操作步骤是否每次都一样？的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回操作步骤是否每次都一样？的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 操作频率如何（每日/每周/每月
操作频率如何（每日/每周/每月）？

**处理**: 解析操作频率如何（每日/每周/每月的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回操作频率如何（每日/每周/每月的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 单次操作耗时多少？
单次操作耗时多少？

**处理**: 解析单次操作耗时多少？的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回单次操作耗时多少？的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析四步自动化框架的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回四步自动化框架的响应数据,包含状态码、结果和日志.
### 基础模板（3个）
...
**处理**: 解析基础模板（3个）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回基础模板（3个）的响应数据,包含状态码、结果和日志.
...
```json
{
  "success": true,
  "data": {
    "result": "工作流催化器(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "workflow catalyst"
    }
  },
