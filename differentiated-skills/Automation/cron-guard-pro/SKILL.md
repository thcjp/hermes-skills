...
description: |-
  ...
  适用关键词：企业安全防护、沙箱隔离、POSIX兼容、git安全、端口检测、分布式锁、安全审计
tags:
- 企业安全防护
- 沙箱隔离
- POSIX兼容
...
**输入**: 用户提供沙箱隔离执行（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行沙箱隔离执行（专业版）操作,遵循单一意图原则。
**输出**: 返回沙箱隔离执行（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供POSIX兼容性检测（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行POSIX兼容性检测（专业版）操作,遵循单一意图原则。
**输出**: 返回POSIX兼容性检测（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供git自动化安全（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行git自动化安全（专业版）操作,遵循单一意图原则。
**输出**: 返回git自动化安全（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供端口冲突检测（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行端口冲突检测（专业版）操作,遵循单一意图原则。
**输出**: 返回端口冲突检测（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供分布式锁（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行分布式锁（专业版）操作,遵循单一意图原则。
**输出**: 返回分布式锁（专业版）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供安全审计（专业版）所需的指令和必要参数。
**处理**: 按照skill规范执行安全审计（专业版）操作,遵循单一意图原则。
**输出**: 返回安全审计（专业版）的执行结果,包含操作状态和输出数据。

### 完整故障模式库（12+类）

在免费版6类基础上新增：锁竞争失败、git合并冲突、端口占用、磁盘空间不足、网络中断、容器OOM。

**输入**: 用户提供完整故障模式库（12+类）所需的指令和必要参数。
**处理**: 按照skill规范执行完整故障模式库（12+类）操作,遵循单一意图原则。
**输出**: 返回完整故障模式库（12+类）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供高级恢复策略所需的指令和必要参数。
**处理**: 按照skill规范执行高级恢复策略操作,遵循单一意图原则。
**输出**: 返回高级恢复策略的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、cron、任务安全防护专业、含沙箱隔离、端口检测、定时任务安全防护、专业版是面向企业、级场景的完整、任务安全防护解决、在免费版四层防护、体系之上、专业版新增沙箱隔、离与资源限制、深度兼容、分布式锁与多节点、协调五大高级防护、满足生产环境对定、时任务安全性和可、靠性的严苛要求等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
