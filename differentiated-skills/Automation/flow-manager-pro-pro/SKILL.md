...
license: Proprietary
edition: pro
description: "流程管控器专业版是在免费版基础上的全功能升级，为IoT团队与自发化运维包含企业级Node-RED实例管控能力。除核心流程操作外，解锁多实例管控、完整备份恢复、Docker容器编排、性能监控告警、成批节点操作、流程版本对比回滚、审计日志七大高级功能. 适用于需要flow manager相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - Node-RED
  - 流程管理
  - 自动化
...
```json
{
  "success": true,
  "data": {
    "result": "流程管理器(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "flow manager pro pro"
    }
  },
