...
**处理**: 按照skill规范执行安全情报雷达为 AI Agen操作,遵循单一意图原则。
**输出**: 返回安全情报雷达为 AI Agen的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行它聚合 NVD CVE、GitHub S操作,遵循单一意图原则。
**输出**: 返回它聚合 NVD CVE、GitHub S的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
