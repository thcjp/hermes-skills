...
**处理**: 按照skill规范执行Obsidian 多端同步引擎操作,遵循单一意图原则。
**输出**: 返回Obsidian 多端同步引擎的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行obsidian/` 目录混乱操作,遵循单一意图原则。
**输出**: 返回obsidian/` 目录混乱的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行支持 Git 同步、选择性同步操作,遵循单一意图原则。
**输出**: 返回支持 Git 同步、选择性同步的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
