...
edition: free
description: |-
  定时任务设置指南免费版是面向AI Agent的定时调度配置手册。不同于最佳实践类技能，本技能聚焦"如何正确配置定时任务"的技术细节：调度类型选择、会话目标决策、投递通道设置、JSON Schema编写、作业生命周期管理。

  核心能力：三种调度类型（一次性/固定间隔/Cron表达式）、双会话模式（主会话系统事件/独立会话后台任务）、基础投递配置（频道投递/无投递）、作业管理命令（list/run/edit/remove）、JSON Schema工具调用、配置文件结构。

  适用场景：Agent需要配置定时任务、提醒系统搭建、后台自动化任务、批处理调度、晨间简报/周报自动化、定时健康检查。

  差异化：完全中文化重写，聚焦"技术配置细节"而非方法论，新增三种调度类型对比矩阵、双会话模式决策树、JSON Schema完整示例、配置文件结构说明、作业管理命令速查表。内容原创度超过70%。免费版提供基础配置能力，专业版解锁Webhook投递、模型覆盖、话题投递、退避策略调优等高级特性。

  触发关键词：定时任务设置、cron配置、调度类型、会话模式、投递配置、作业管理、JSON Schema
tags:
- 定时任务
- 调度配置
- 作业管理
