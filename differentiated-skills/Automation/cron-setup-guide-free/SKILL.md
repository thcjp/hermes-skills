...
license: MIT
edition: free
description: "定时任务设置指南免费版是面向AI Agent的定时调度配置手册。不同于优秀实践类技能，该技能聚焦\"如何正确配置定时任务\"的技术细节：调度类型选择、会话目标决策、投递通道设置、JSON，可处置提升工作效率. 适用于需要cron setup guide相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 定时任务
  - 调度配置
  - 作业管理
...
**处理**: 解析三种调度类型的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回三种调度类型的响应数据,包含状态码、结果和日志.
### 双会话模式
...
**处理**: 解析双会话模式的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回双会话模式的响应数据,包含状态码、结果和日志.
### 基础投递配置
...
**处理**: 解析基础投递配置的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回基础投递配置的响应数据,包含状态码、结果和日志.
### 作业管理命令
...
**处理**: 解析作业管理命令的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回作业管理命令的响应数据,包含状态码、结果和日志.
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Agent、Gateway、定时任务设置完全、指南免费版、覆盖三种调度类型、会话模式、投递配置与基础作、业管理、定时任务设置指南、免费版是面向、的定时调度配置手、不同于优秀实践类、本技能聚焦、如何正确配置定时、的技术细节、调度类型选择、会话目标决策、投递通道设置、JSON、Schema、作业生命周期管理、Use、when、需要项目管理、任务规划、进度跟踪、团队协作时使用、不适用于实际人员、绩效评估等.
...
```json
{
  "success": true,
  "data": {
    "result": "定时任务设置指南(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "cron setup guide"
    }
  },
