...
edition: free
description: |-
  流程锻造器为AI Agent提供代码化的工作流构建能力。通过JSON定义触发器、操作步骤和错误处理，将跨平台自动化流程转化为可版本控制、可复用的工作流配置。免费版支持定时触发、文件监控、手动触发三种触发方式，以及文件操作、网络请求、命令执行三类基础操作节点。

  核心能力：JSON工作流定义、三种触发器（cron/watch/manual）、三类基础操作节点（文件/网络/命令）、工作流模板（数据同步、内容发布）、条件判断基础支持。

  适用场景：定时数据抓取与保存、文件变化自动处理、多步骤数据同步、跨平台流程编排。

  差异化：采用JSON声明式工作流定义，便于版本控制和团队协作。免费版聚焦三种基础触发器和基础操作节点，专业版解锁API webhook触发、多条件组合、数据处理转换、通知发送、工作流模板库和定制开发指南。

  触发关键词：工作流构建、自动化流程、JSON工作流、触发器、定时任务、文件监控
tags:
- 工作流构建
- 流程自动化
- JSON配置
