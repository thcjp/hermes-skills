...
**输入**: 用户提供支持的AWS服务所需的指令和必要参数。
**处理**: 按照skill规范执行支持的AWS服务操作,遵循单一意图原则。
**输出**: 返回支持的AWS服务的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、全服务管理平台、支持多区域、合规审计与成本优、面向企业运维团队、等全量、提供基础设施即代、成本优化与安全扫、描功能、Use、when、需要安全检测、漏洞扫描、加密防护时使用、不适用于渗透测试、未授权目标等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
