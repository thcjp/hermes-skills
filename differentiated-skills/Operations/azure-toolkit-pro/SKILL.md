...
**处理**: 按照skill规范执行支持的Azure服务操作,遵循单一意图原则。
**输出**: 返回支持的Azure服务的执行结果,包含操作状态和输出数据。
...
# 部署基础设施
python3 scripts/infra.py deploy \
...
```bash
# IaC部署
python3 scripts/infra.py generate --template "web_app_ha" --engine bicep --regions "eastus,westus2"
python3 scripts/infra.py deploy --template ./bicep/main.bicep

# 合规审计
python3 scripts/audit.py run --standards "CIS,Azure-Security-Benchmark"
