...
description: |-
  ...
  数据、批量查询与导出、跨链桥接监控、链上行为追踪与深度分析。
  内置代币安全审计与套利机会识别，全面满足专业跨链需求。
  ...
  触发关键词: 跨链, 代币, 全链, 批量查询, 桥接, 鲸鱼, 套利, 安全审计, 监控, cross-chain, bridge
tags:
- Operations
- 加密货币
- 跨链
...
```bash
# 批量跨链查询
python3 scripts/token_pro.py batch-query \
  --chain ethereum \
  --top 50 \
  --sort-by liquidity \
  --cross-chain all \
...
```bash
# 桥接监控
python3 scripts/token_pro.py bridge monitor \
  --token USDC \
...
```bash
# 跨链套利扫描
python3 scripts/token_pro.py arbitrage scan \
...
```bash
# 批量查询
python3 scripts/token_pro.py batch-query --chain ethereum --top 50 --cross-chain all --export

# 桥接监控
python3 scripts/token_pro.py bridge monitor --token USDC --routes "ethereum->arbitrum"
python3 scripts/token_pro.py bridge compare --token USDC --routes all

# 鲸鱼追踪
python3 scripts/token_pro.py whale track --token UNI --chain ethereum --threshold 100000

# 套利扫描
python3 scripts/token_pro.py arbitrage scan --tokens "UNI,AAVE" --chains all --min-spread 0.5%

# 安全审计
python3 scripts/token_pro.py audit --token 0x... --chain ethereum

# 关联分析
python3 scripts/token_pro.py correlation --tokens "UNI,AAVE,COMP" --period 30d

# 价格监控
python3 scripts/token_pro.py monitor add --token UNI --chains all --alert "spread>1%"
```
