...
description: |-
  ...
  Docker/K8s容器监控、日志聚合分析、可视化仪表盘与智能告警。
  内置容量规划与异常检测，全面满足企业级运维监控需求。

  核心能力:
  - 多节点分布式监控（Agent/Agentless）
  ...
  触发关键词: 监控, 分布式, 容器, Docker, K8s, 日志, 仪表盘, 告警, 容量规划, monitor, dashboard
tags:
- Operations
- 系统监控
- 企业级
...
```bash
# 添加监控节点
python3 scripts/nodes.py add \
  --name "web-01" --host 192.168.1.10 --ssh-key ~/.ssh/id_rsa
python3 scripts/nodes.py add \
  --name "db-01" --host 192.168.1.20 --ssh-key ~/.ssh/id_rsa

# 批量添加
python3 scripts/nodes.py batch-add --file servers.csv

# 统一状态查看
python3 scripts/nodes.py status --all
...
```bash
# Docker容器监控
python3 scripts/docker_monitor.py watch \
  --host 192.168.1.10 \
  --interval 30
...
```bash
# 日志聚合搜索
python3 scripts/logs.py search \
  --query "ERROR" \
  --nodes all \
  --time-range "1h" \
...
# 日志分析
python3 scripts/logs.py analyze \
  --pattern "exception" \
...
# 初始化数据库
python3 scripts/init.py --db postgresql --host localhost

# 配置Grafana
python3 scripts/grafana.py setup --url http://localhost:3000
```
...
```bash
# 节点管理
python3 scripts/nodes.py add --name "web-01" --host 192.168.1.10
python3 scripts/nodes.py status --all

# Docker监控
python3 scripts/docker_monitor.py watch --host 192.168.1.10

# 日志分析
python3 scripts/logs.py search --query "ERROR" --nodes all --time-range "1h"

# 仪表盘
python3 scripts/dashboard.py open --url http://localhost:3000

# 容量规划
python3 scripts/capacity.py forecast --metric disk --months 6
```
