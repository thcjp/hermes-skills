...
edition: "pro"
description: "|-
  定时提醒工具（专业版）面向团队与企业用户，在免费版基础定时能力之上新增循环定时、提醒模板库、多级升级提醒、指定时间点触发、实时监控面板与定时器链。支持从简单提醒到复杂定时策略的完整工作流。Use when 需要系统监控、日志分析、运维告警、部署管理时使用。不适用于物理硬件维修。减少重复劳动，提升工作效率。触发关键词：自动化、部署、监控、转换、机器学习、邮件"
