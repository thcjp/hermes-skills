...
**处理**: 按照skill规范执行一、13 个平台全覆盖(专业版独有 10 个平台)操作,遵循单一意图原则。
**输出**: 返回一、13 个平台全覆盖(专业版独有 10 个平台)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行二、批量配文生成(专业版独有)操作,遵循单一意图原则。
**输出**: 返回二、批量配文生成(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行三、品牌风格定制(专业版独有)操作,遵循单一意图原则。
**输出**: 返回三、品牌风格定制(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行四、编辑分析(专业版独有)操作,遵循单一意图原则。
**输出**: 返回四、编辑分析(专业版独有)的执行结果,包含操作状态和输出数据。
...
**输出**: 返回五、多语言完整支持(专业版独有)的执行结果,包含操作状态和输出数据。
...
| 对话优先(X/Bluesky/Threads) | 口语化、即兴感、一句话抓人 |
| 搜索优先(Pinterest) | SEO关键词、描述性、可被发现 |
...
### 3. 品牌配文严格遵循禁用词
品牌场景下,确保配文不出现禁用词,保持品牌调性一致:

```text
- "captured" -> 改用 "shot" / "took" / "made"
- "golden hour doing its thing" -> 具体描述光线行为
- "timeless" / "fleeting moment" -> 删除或替换
- "There's something about..." -> 直接进入正题
```

### 4. 编辑分析辅助提升作品质量
每次生成配文时,编辑分析章节会提供照片技术层面的改进建议,帮助摄影师持续提升:
...
### 5. Pinterest 配文注重 SEO
Pinterest 是发现引擎,配文需融入可搜索关键词:
