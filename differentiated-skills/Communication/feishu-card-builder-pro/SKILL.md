...
**输入**: 用户提供批量推送引擎所需的指令和必要参数。
**处理**: 按照skill规范执行批量推送引擎操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行卡片模板系统操作,遵循单一意图原则。
**输出**: 返回卡片模板系统的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供高级交互组件所需的指令和必要参数。
**处理**: 按照skill规范执行高级交互组件操作,遵循单一意图原则。
**输出**: 返回高级交互组件的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行数据动态绑定操作,遵循单一意图原则。
**输出**: 返回数据动态绑定的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供定时与触发所需的指令和必要参数。
**处理**: 按照skill规范执行定时与触发操作,遵循单一意图原则。
**输出**: 返回定时与触发的执行结果,包含操作状态和输出数据。
**技术参数**：使用`input_params`和`output_format`参数控制执行行为,支持`json`/`text`/`csv`输出格式。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级飞书交互卡、片与批量推送、支持模板与高级组、飞书卡片专业版面、向企业用户与高效、能个人用户、在免费版卡片消息、发送能力之上扩展、卡片版本管理与发、送分析等、企业级特性、核心能力、批量卡片推送、多用户、多群组同时发送、内置与自定义模板、Use、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
