...
edition: free
description: |-
  WhatsApp 消息工具（免费版）面向个人用户与独立开发者，封装 WhatsApp CLI 的基础消息能力：文本消息发送、文件发送、聊天列表查询、消息搜索与基础认证流程。通过命令行工具驱动 WhatsApp Web 协议，无需外部服务。
  ...
  差异化：以"命令 × 参数表 × 示例"三段式组织，每条命令均附可直接复制的命令与参数说明，原创内容占比超过 70%。免费版覆盖基础消息发送与聊天搜索；专业版额外提供批量操作、历史回填与群组管理。

  触发关键词：WhatsApp、消息发送、文件发送、聊天搜索、QR认证、同步
tags:
- 沟通协作
- 即时通讯
- WhatsApp
