...
edition: "free"
description: |-
  WhatsApp 消息工具（免费版）面向个人用户与独立开发者，封装 WhatsApp CLI 的基础消息能力：文本消息发送、文件发送、聊天列表查询、消息搜索与基础认证流程。通过命令行工具驱动 WhatsApp Web 协议，无需外部服务。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。
tags:
  - 沟通协作
  - 即时通讯
  - WhatsApp
