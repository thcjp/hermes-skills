...
**处理**: 按照skill规范执行一、Gmail 高级邮件管理操作,遵循单一意图原则。
**输出**: 返回一、Gmail 高级邮件管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行二、Calendar 高级日程管理操作,遵循单一意图原则。
**输出**: 返回二、Calendar 高级日程管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行三、Drive 文件管理操作,遵循单一意图原则。
**输出**: 返回三、Drive 文件管理的执行结果,包含操作状态和输出数据。
...
**输出**: 返回四、Sheets 表格操作(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行五、Docs 文档操作(专业版独有)操作,遵循单一意图原则。
**输出**: 返回五、Docs 文档操作(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行六、Contacts 联系人管理(专业版独有)操作,遵循单一意图原则。
**输出**: 返回六、Contacts 联系人管理(专业版独有)的执行结果,包含操作状态和输出数据。
**技术参数**：使用`input_params`和`output_format`参数控制执行行为,支持`json`/`text`/`csv`输出格式。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能、Google、Workspace、命令行工具、覆盖六大服务与批、量操作、支持企业级多租户、场景与自动化工作、谷歌办公命令行工、具专业版、面向企业与专业用、全六大服务的深度、操作能力、核心能力、六大服务全覆盖、批量邮件发送与草、清空与元数据查询、内容查看与文档复、联系人批量管理、企业级多账户与自、动化工作流支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
企业级批量操作涉及大量数据,务必对每条记录做错误捕获与执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令,避免单点失败导致整体中断。
