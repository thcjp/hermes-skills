...
description: |-
  ...
  （先上传获取 file_key/image_key，再发送消息）确保文件稳定投递，支持 HTML、ZIP、
  PDF、代码文件等各类普通文件以及图片的可靠发送。

  核心能力:
  - 飞书普通文件发送（HTML/ZIP/PDF/代码文件等）
  - 飞书图片稳定发送（解决路径文本问题）
  - 两步上传流程（上传获取 key → 发送消息）
  - 自定义文件名支持
  - 国际版 Lark 兼容
  - 脚本化与手动两种操作方式
  ...
  差异化: 免费版聚焦个人文件发送需求，提供稳定的两步上传流程解决路径文本问题，
  操作可靠。专业版在此基础上增加批量分发、文件管理、审计日志与企业级特性。

  触发关键词: 飞书文件, 飞书发送, 文件上传, 图片发送, file_key, image_key, 飞书附件, Lark文件
tags:
- 沟通协作
- 飞书
- 文件发送
