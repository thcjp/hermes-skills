...
edition: free
description: |-
  Telegram 消息工具箱（免费版）面向个人用户与独立开发者，封装 Telegram Bot API 的基础消息能力：文本消息、图片、文件、投票发送，聊天信息查询与基础 Bot 命令管理。通过 REST API 直接调用 Telegram Bot 接口，无需额外中间件。

  核心能力：
  - 6 类消息发送：文本、图片、文件、位置、投票、转发
  - 聊天信息查询：群组信息、成员状态、管理员列表
  - Bot 命令管理：设置与获取命令列表
  - 消息编辑与删除（Bot 自身消息）
  - 回调查询响应（内联键盘）
  - 邀请链接生成与撤销
  ...
  适用场景：
  - 个人自动化发送通知、提醒、报告
  - 独立开发者向 Telegram 群组推送部署状态
  - 小团队内部群组快速广播

  差异化：以"消息类型 × 参数表 × curl 示例"三段式组织，每类消息均附可直接复制的命令与参数说明，原创内容占比超过 70%。免费版覆盖 6 类基础消息与聊天查询；专业版额外提供批量操作、群组管理、审核与 Webhook。

  触发关键词：Telegram、消息发送、Bot、群组、投票、聊天、invite、频道
tags:
- 沟通协作
- 即时通讯
- Telegram
