...
**输入**: 用户提供无限订阅源批量管理所需的指令和必要参数。
**处理**: 按照skill规范执行无限订阅源批量管理操作,遵循单一意图原则。
**输出**: 返回无限订阅源批量管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行AI深度摘要与关键词提取操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行定时调度与任务队列操作,遵循单一意图原则。
...
**输入**: 用户提供内容趋势分析所需的指令和必要参数。
**处理**: 按照skill规范执行内容趋势分析操作,遵循单一意图原则。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、RSS、聚合平台、支持多源批量抓取、多渠道推送、定时调度与数据分、智能阅读器、面向团队和企业的、全功能信息聚合平、核心能力、无限订阅源批量抓、取与分类管理、内容趋势分析与阅、团队订阅共享与权等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- **说明**: 基于Markdown的AI Skill，通过自然语言指令驱动Agent完成操作，专业版支持定时调度、并发处理与数据分析
