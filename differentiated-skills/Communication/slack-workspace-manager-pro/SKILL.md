...
**输出**: 返回Canvas文档管理的执行结果,包含操作状态和输出数据。

- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 4. 用户组管理
创建、启用、禁用用户组（子团队），管理组成员与权限。

**输入**: 用户提供用户组管理所需的指令和必要参数。
**处理**: 按照skill规范执行用户组管理操作,遵循单一意图原则。
**输出**: 返回用户组管理的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、工作区管理平台、支持企业、用户组管理与批量、工作区管理器、专业版、面向企业的全功能、核心能力、多团队管理与审计、文档创建与编辑、用户组管理与权限、批量频道操作与成、员管理、自定义表情管理与、完整的工作区安全、与权限审计等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- **说明**: 基于Markdown的AI Skill，通过自然语言指令驱动Agent完成操作，专业版支持企业Grid管理、审计日志与批量操作
