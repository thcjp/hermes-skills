...
**输入**: 用户提供X风格帖子分享长图(x-like-posts)所需的指令和必要参数。
**处理**: 按照skill规范执行X风格帖子分享长图(x-like-posts)操作,遵循单一意图原则。
**输出**: 返回X风格帖子分享长图(x-like-posts)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自定义模板创建操作,遵循单一意图原则。
**输出**: 返回自定义模板创建的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行批量生成操作,遵循单一意图原则。
**输出**: 返回批量生成的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行高级高亮功能操作,遵循单一意图原则。
**输出**: 返回高级高亮功能的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行全平台配色预设与品牌定制操作,遵循单一意图原则。
**输出**: 返回全平台配色预设与品牌定制的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行水印与版权信息操作,遵循单一意图原则。
**输出**: 返回水印与版权信息的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级卡片图生成、支持全模板、卡片图生成器专业、面向企业和专业内、容团队的全功能卡、片图片生成平台、核心能力、全模板支持、长文卡片、风格长图、公众号封面、批量生成与目录递、归处理、自定义模板创建与、水印与版权信息嵌、模板版本管理与团、队共享等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
