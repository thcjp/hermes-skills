...
**输入**: 用户提供批量分发引擎所需的指令和必要参数。
**处理**: 按照skill规范执行批量分发引擎操作,遵循单一意图原则。
**输出**: 返回批量分发引擎的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行多文件管理操作,遵循单一意图原则。
**输出**: 返回多文件管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行发送队列操作,遵循单一意图原则。
**输出**: 返回发送队列的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行重试与恢复操作,遵循单一意图原则。
**输出**: 返回重试与恢复的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行审计与合规操作,遵循单一意图原则。
**输出**: 返回审计与合规的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行发送分析操作,遵循单一意图原则。
**输出**: 返回发送分析的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行大文件支持操作,遵循单一意图原则。
**输出**: 返回大文件支持的执行结果,包含操作状态和输出数据。
**技术参数**：使用`input_params`和`output_format`参数控制执行行为,支持`json`/`text`/`csv`输出格式。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级文件批量分、发与稳定投递、支持审计与重试机、飞书文件发送专业、版面向企业用户与、高效能个人用户、Use、when、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
