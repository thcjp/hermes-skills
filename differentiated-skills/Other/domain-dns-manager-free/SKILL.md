...
edition: free
description: |-
  域名DNS管理免费版是一款面向个人开发者与小团队的域名管理Skill，封装Cloudflare、DNSimple、Namecheap三大平台的常用操作，提供标准化的域名接入与DNS记录管理工作流。

  核心能力：
  - 单域名DNS记录管理（A/AAAA/CNAME/MX/TXT）
  - Cloudflare zone接入与nameserver切换
  - DNS记录验证（dig确认生效）
  - HTTPS重定向配置（Page Rule方式）
  - 基础域名信息查询与状态确认
  ...
  差异化：相比手动在各平台控制台操作，本Skill提供统一的命令行工作流与验证步骤，确保每步操作可追溯可回滚。免费版覆盖单域名的日常管理需求，专业版提供批量域名与多供应商编排能力。

  触发关键词：域名、DNS、Cloudflare、DNSimple、Namecheap、DNS记录、nameserver、重定向
tags:
- 域名管理
- DNS配置
- Cloudflare
