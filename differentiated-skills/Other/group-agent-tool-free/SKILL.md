...
| `拉成员进群` | 邀请Agent加入 | `group-agent invite --group proj-q3 --agent researcher` |
| `群列表` | 查看群成员 | `group-agent list --group proj-q3` |
| `退群` | 退出或解散 | `group-agent leave --group proj-q3` |

**处理**: 解析能力说明:极简群组管理四件套的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力说明:极简群组管理四件套的响应数据,包含状态码、结果和日志.
### 能力说明:@提及定向通信
...
**处理**: 解析能力说明:@提及定向通信的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力说明:@提及定向通信的响应数据,包含状态码、结果和日志.
...
**处理**: 解析能力说明:群公告与广播的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力说明:群公告与广播的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析能力说明:消息归档查询的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力说明:消息归档查询的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析能力说明:群组主题频道的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力说明:群组主题频道的响应数据,包含状态码、结果和日志.
