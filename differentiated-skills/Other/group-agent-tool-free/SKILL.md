...
edition: free
description: |-
  Agent群组工具是一套面向多Agent协作的轻量级群组通信方案，借鉴即时通讯软件"拉群-@提及-广播"的成熟交互模式，让多个Agent能像同事群一样协作，无需学习复杂的消息总线或事件驱动架构。

  核心能力：一行指令创建群组、邀请Agent加入、查看成员、退出解散；支持@特定成员定向通信、群公告广播、消息归档查询；群组数据轻量化、内存优先、按需持久化。

  适用场景：多Agent项目协作、临时任务小组、跨职能Agent协同、Agent团队日常沟通、并行任务分发汇总。

  差异化：相比重量级消息队列或事件总线，本工具强调"零学习成本"——任何会用微信群的人都能在10秒内上手管理Agent群组。极简API设计让Agent开发者专注业务逻辑而非通信协议。

  触发关键词：Agent群组、多Agent协作、群组通信、@提及、群公告、Agent团队、拉群
tags:
- 多Agent协作
- 群组通信
- 轻量级
...
| 现象 | 可能原因 | 排查步骤 |
|------|----------|----------|
| Agent加入失败 | Agent未注册 | 先注册Agent身份 |
| 消息发送失败 | 群组不存在 | 检查 `--group` 参数 |
| @提及未送达 | Agent未订阅 | 检查Agent是否在线监听 |
| 群组创建失败 | 群组ID重复 | 更换群组ID |
| 历史消息查询慢 | 数据量大 | 添加索引或归档旧消息 |
| 数据库锁死 | 并发写入冲突 | 重启服务 |
