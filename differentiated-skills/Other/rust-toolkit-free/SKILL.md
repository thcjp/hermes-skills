...
summary: Rust开发最佳实践助手,避免所有权、借用与生命周期常见陷阱,适合个人学习
license: MIT
edition: free
description: |-
  Rust 开发最佳实践助手免费版,面向个人开发者与学习者。

  核心能力:
  - 所有权与借用陷阱检测
  - 生命周期标注指导
  - 错误处理(Result/Option)最佳实践
  ...
  差异化:免费版覆盖核心陷阱与最佳实践。PRO版扩展性能优化、unsafe 安全审计、嵌入式开发与企业级 CI/CD。

  触发关键词: rust, ownership, borrow, lifetime, 所有权, 借用, 生命周期, async, cargo
tags:
- Rust
- 编程规范
- 所有权
...
Rust 工具包免费版是面向个人开发者的 Rust 最佳实践助手。覆盖 Rust 开发中最常见的所有权、借用、生命周期陷阱,提供即查即用的修复建议与代码规范,帮助开发者写出安全可靠的系统级代码。
...
`Result<T, E>` 与 `Option<T>` 的正确使用,`?` 操作符的最佳实践。
...
- **分类**: MD(纯 Markdown 指令)
- **说明**: 通过自然语言指令驱动 Agent 提供 Rust 开发最佳实践建议
