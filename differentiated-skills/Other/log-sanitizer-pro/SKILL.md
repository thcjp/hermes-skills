...
edition: pro
description: |-
  日志脱敏工具专业版面向团队与企业级日志安全场景，在免费版基础上扩展全功能脱敏能力。解决团队日志治理的"规模与合规"痛点：海量日志文件需要定时自动扫描、不同业务线需要差异化脱敏规则、合规审计要求可追溯的脱敏报告、敏感信息泄露需要实时告警、跨平台日志流水线需要统一编排。Use when 需要系统监控、日志分析、运维告警、部署管理时使用。不适用于物理硬件维修。
tags:
- 日志安全
- 企业合规
- 数据脱敏
...
**处理**: 按照skill规范执行批量定时监控操作,遵循单一意图原则。
**输出**: 返回批量定时监控的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自定义复杂规则引擎操作,遵循单一意图原则。
**输出**: 返回自定义复杂规则引擎的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行MCP工具流水线集成操作,遵循单一意图原则。
**输出**: 返回MCP工具流水线集成的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行合规报告导出操作,遵循单一意图原则。
**输出**: 返回合规报告导出的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行威胁情报更新操作,遵循单一意图原则。
**输出**: 返回威胁情报更新的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行多维度深度检测操作,遵循单一意图原则。
**输出**: 返回多维度深度检测的执行结果,包含操作状态和输出数据。
