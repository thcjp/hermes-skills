...
edition: free
description: |-
  文档工具箱免费版是一款面向开发者的DOCX文档处理Skill，封装python-docx与LibreOffice渲染能力，提供从读取、创建到编辑的完整基础工作流。

  核心能力：
  - 读取与审查DOCX内容（表格、段落、样式）
  - 创建结构化DOCX文档（标题、列表、表格、图片）
  - 基础格式化（字体、字号、颜色、对齐）
  - 视觉校验：DOCX转PDF转PNG，逐页检查排版
  - 文本提取作为视觉校验不可用时的降级方案
  ...
  差异化：相比直接使用python-docx，本Skill强调"视觉校验优先"的工作流，每次修改后自动渲染检查，确保交付物真正可用而非"代码能跑"。免费版覆盖日常文档处理需求，专业版提供模板管理与批量能力。

  触发关键词：DOCX、Word文档、python-docx、文档生成、文档编辑、排版校验、LibreOffice
tags:
- 文档处理
- DOCX
- 办公自动化
