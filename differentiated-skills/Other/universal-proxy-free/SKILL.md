...
edition: free
description: |-
  通用代理工具是一套面向AI Agent的隐私通信代理方案，通过建立分布式加密电路转发流量，让Agent在访问网络服务时隐藏源IP地址，保护调用方身份隐私，适用于需要匿名性或地理可达性的Agent场景。

  核心能力：启动本地SOCKS5代理服务（默认端口9050）；建立多层加密电路转发流量；支持HTTP/HTTPS/SOCKS5多种协议；首次连接30秒内完成电路建立；代理服务可常驻后台，跨请求复用。

  适用场景：Agent调用地理位置受限的API、Agent身份隐私保护、绕过区域网络封锁、爬虫与数据采集的IP轮换、内部Agent网络隔离测试。

  差异化：相比传统VPN的全局路由，本工具采用按需SOCKS5代理模式——仅代理指定流量，对Agent其他行为无影响。多层加密电路设计让中间节点也无法还原源IP与目标地址。

  触发关键词：代理服务、IP隐藏、SOCKS5代理、加密电路、匿名通信、流量转发、隐私保护
tags:
- 网络代理
- 隐私保护
- SOCKS5
