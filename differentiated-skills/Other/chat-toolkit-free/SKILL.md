...
edition: free
description: |-
  通过显式用户反馈学习沟通偏好的轻量工具，自动适配语气、格式与表达风格，将个人化沟通模式沉淀为可复用资产。
  ...
  - 团队内部多人共享偏好基线，统一对外沟通风格

  差异化：仅采纳显式反馈信号，杜绝"沉默即同意"的误判风险；偏好存储透明可审计，用户随时可查看与撤销。

  触发关键词：沟通偏好、风格学习、语气适配、显式反馈、个人化助手
tags:
- 沟通偏好
- 个人化
- 风格适配
...
| 信号类型 | 是否采纳 | 示例 |
|---------|---------|------|
| 用户明确表达偏好 | ✅ 采纳 | "我喜欢X"、"不要做Y" |
| 用户沉默未表态 | ❌ 不采纳 | 沉默≠同意 |
| Agent观察到的模式 | ❌ 不采纳 | 仅观察不足以推断偏好 |
| 用户间接暗示 | ❌ 不采纳 | 必须明确表达 |
...
| 阶段 | 存储位置 | 触发条件 | 下一步 |
|------|---------|---------|--------|
| 实验中 | experiments.md | 观察到1-2次 | 累计观察 |
| 确认中 | （询问用户） | 达到3次触发 | 询问用户是否采纳 |
| 已采纳 | memory.md | 用户明确同意 | 应用到所有回应 |
| 已拒绝 | rejected.md | 用户明确否决 | 不再重复建议 |

### 规则3：紧凑存储格式

memory.md中每条偏好占一行，便于审查与迁移：
...
- 最新显式声明优先于历史偏好
- 偏好含义模糊时，主动询问用户澄清
- 用户明确否决后，相应偏好从memory.md移至rejected.md
- 已拒绝偏好在合理周期内不再重复建议（默认30天）
...
用户请求时，展示memory.md完整内容：
> "当前已确认的5条偏好如下：[列出memory.md内容]"

用户说"忘记X"时，从所有相关文件中清除该偏好。
