...
edition: free
description: |-
  网页内容获取工具免费版，面向个人开发者的轻量级网页内容抓取工具。

  核心能力:
  - 三种替代服务的降级获取策略
  - 自动按优先级尝试（jina/markdown.new/defuddle）
  - Markdown 格式输出，便于后续处理
  - 命令行脚本与 API 双模式调用

  适用场景:
  - 常规 web_fetch 被过滤时的替代方案
  - Cloudflare 保护网站的内容获取
  - 网页内容的 Markdown 转换

  差异化: 免费版聚焦核心内容获取与降级策略，去除所有外部平台与作者引用，强化中文本地化与触发关键词，适合个人用户零成本上手。

  触发关键词: 网页抓取, 内容获取, Markdown转换, jina, markdown.new, defuddle, 反爬虫, Cloudflare
tags:
- 网页抓取
- 内容获取
- Markdown转换
