...
**处理**: 解析能力1：多电路并发管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力1：多电路并发管理的响应数据,包含状态码、结果和日志.
### 能力2：请求级负载均衡
在多电路间智能分配请求：
...
**处理**: 解析能力2：请求级负载均衡的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力2：请求级负载均衡的响应数据,包含状态码、结果和日志.
### 能力3：按地区路由
支持全球50+地区出口节点选择：
...
**处理**: 解析能力3：按地区路由的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力3：按地区路由的响应数据,包含状态码、结果和日志.
...
**处理**: 解析能力4：企业级审计日志的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力4：企业级审计日志的响应数据,包含状态码、结果和日志.
### 能力5：自建代理节点
...
**处理**: 解析能力6：高可用部署的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力6：高可用部署的响应数据,包含状态码、结果和日志.
### 能力7：流量监控与告警
实时监控代理流量与电路健康：

**输入**: 用户提供能力7：流量监控与告警所需的指令和必要参数.
**处理**: 解析能力7：流量监控与告警的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回能力7：流量监控与告警的响应数据,包含状态码、结果和日志.
...
```json
{
  "success": true,
  "data": {
    "result": "通用代理工具专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "universal proxy pro"
    }
  },
