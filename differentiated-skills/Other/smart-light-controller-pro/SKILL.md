...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能智能灯控方、支持多灯同步、灯光秀、场景预设与定时计、智能灯控专业版是、一款面向局域网智、能灯泡的全功能控、制方案、Link、Kasa、协议设备、覆盖单灯控制、多灯同步、灯光秀编排、场景预设、定时计划与日出日、落联动等完整能力、核心能力、多灯批量同步控制、支持分组与级联编、灯光秀序列引擎、支持渐变、节拍同步等过渡效、场景预设保存与一、键切换、JSON、配置导入导出、定时计划任务与日、出日落联动、自动适配地理位置、设备健康监控与异、常告警等.
## 使用场景
...
```json
{
  "success": true,
  "data": {
    "result": "智能灯控(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "smart light controller pro"
    }
  },
