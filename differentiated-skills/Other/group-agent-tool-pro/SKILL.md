...
| 权限验证失败 | SSO时钟漂移 | 同步NTP服务 | P1 |
| 群机器人无响应 | 触发词错误 | 检查 `triggers` 配置 | P2 |
| 端到端加密失败 | 密钥版本不一致 | 同步密钥管理服务 | P1 |
| IM同步延迟 | 飞书API限流 | 检查速率限制 | P2 |
| 数据分析报告缺失 | 数据源连接超时 | 检查`PostgreSQL`连接 | P1 |
| 多实例状态不一致 | 共享存储故障 | 检查`PostgreSQL` HA | P0 |
