...
license: Proprietary
edition: pro
description: "自发文件整理器专业版面向高效文件治理场景，在免费版基础上扩展全功能自发化能力。解决文件整理的\"规模与智能\"痛点：大量重复文件占用空间需要清理、按扩展名分类不够准确需要内容感知、多个文件夹需要成批定时整理、团队需要共享分类规则、文件变更需要实时监控自发整理。Use. 适用于需要auto file organizer相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 文件整理
  - 智能分类
  - 重复清理
