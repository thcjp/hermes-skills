...
edition: free
description: |-
  DNS查询免费版是一款面向运维与开发者的轻量级DNS诊断Skill，封装标准dig命令并提供结构化输出。

  核心能力：
  - 解析A、AAAA、CNAME、MX、TXT、NS等常见记录类型
  - 支持反向DNS查询（PTR记录）
  - 指定上游DNS服务器（如1.1.1.1、8.8.8.8）进行对比
  - 输出精简格式与完整格式双模式
  - 自动识别CNAME链并追踪到最终A记录
  ...
  - 验证DNS配置变更是否生效
  - 开发调试中确认服务端点可达性
  - 邮件配置排查（MX记录与SPF/DKIM）

  差异化：相比直接使用dig，本Skill提供智能化的结果解读与故障定位建议。免费版覆盖单域名日常查询需求，专业版提供批量查询与监控告警能力。

  触发关键词：DNS、域名解析、dig、A记录、MX记录、CNAME、反向查询、NS记录
tags:
- DNS查询
- 网络诊断
- 运维工具
