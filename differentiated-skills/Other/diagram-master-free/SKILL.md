...
edition: free
description: |-
  图表制作大师免费版解决技术文档中"图表绘制效率低、样式不统一、跨平台兼容差"的核心痛点。本工具生成自包含的SVG文件，内嵌样式与字体，无需外部依赖，支持九种主流图表类型，采用统一的暗色主题设计系统。

  核心能力：九种图表类型支持（架构图/流程图/时序图/结构图/思维导图/时间线/说明图/状态机/数据流图）、语义化配色系统（八类组件色彩分类）、标准化排版规则（字号/间距/层级）、SVG图层渲染顺序控制、中文文本适配、自动留白与防重叠。

  适用场景：系统架构设计文档、技术方案评审演示、API交互流程说明、数据库ER图绘制、项目时间线展示、状态转换逻辑可视化、数据管道流程图。

  差异化：完全中文化表达，针对中文技术文档场景重新设计图表模板，新增九种图表类型快速参考表与三类真实场景示例，内容原创度超过70%。保留原始MIT版权声明。

  触发关键词：图表制作、架构图、流程图、时序图、SVG图表、暗色主题、技术图表
tags:
- 图表制作
- SVG生成
- 架构图
