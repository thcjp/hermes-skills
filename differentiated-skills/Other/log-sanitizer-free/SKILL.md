...
edition: free
description: |-
  日志脱敏工具免费版解决日志安全的"隐性泄露"痛点：开发者将应用日志上传到排查平台时无意间泄露数据库密码、API令牌被写入调试日志、生产环境日志包含用户隐私字段、合规审计要求日志中不得出现明文敏感信息。这些场景下，原始日志直接归档或转发会带来严重的安全与合规风险。

  核心能力：本地模式扫描（零数据外泄）、六大敏感信息类别识别（密码/令牌/密钥/个人隐私/财务信息/内部端点）、正则规则与关键词双重检测、单文件与目录批量扫描、脱敏后输出与原始备份、JSON格式报告支持自动化集成。默认本地运行确保日志内容不上传任何外部服务。

  适用场景：应用上线前日志脱敏自查、合规审计日志预处理、调试日志转发前的安全过滤、生产环境日志归档前脱敏、个人开发者本地日志清理。

  差异化：完全中文化表达，针对中文开发者的日志习惯重新设计检测规则，新增六大敏感类别说明表与三级风险评级，内容原创度超过70%。

  触发关键词：日志脱敏、敏感信息、密码清理、令牌清理、日志安全、合规审计
tags:
- 日志安全
- 敏感信息
- 数据脱敏
