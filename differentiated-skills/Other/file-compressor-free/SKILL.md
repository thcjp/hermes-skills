...
description: |-
  ...
  - 四级压缩级别（L1-L4），从0.8x到0.15x的压缩比覆盖不同场景
  - 锚点校验系统，提取数字、人名、日期等关键事实作为压缩基准
  - 迭代验证循环，最多3轮收敛确保信息完整还原
  - ROI预估，自动计算压缩成本与节省Token的盈亏平衡点

  适用场景：

  - 长系统提示词的Token优化，降低LLM调用成本
  - 知识库内容的紧凑存储与高效检索
  - 上下文窗口受限场景下的内容精简

  差异化：聚焦L1-L2可靠压缩级别，免费版面向个人开发者提示词优化与中等规模文本处理。

  触发关键词：文本压缩、Token优化、语义压缩、提示词精简、锚点校验
tags:
- 文本压缩
- Token优化
- 语义压缩
...
1. **必须验证**：永不信任未经验证的压缩结果
2. **使用锚点**：压缩前提取数字、人名、日期
3. **生产环境封顶L2**：L3-L4仅实验用途
4. **报告置信度**：包含迭代次数与锚点匹配率
5. **独立验证**：建议使用不同模型进行重构验证
