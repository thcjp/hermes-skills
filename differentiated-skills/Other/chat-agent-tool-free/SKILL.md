...
edition: free
description: |-
  聊天Agent工具是一套面向多Agent协作与Agent-人协作的临时实时聊天室方案，提供密码保护、SSE流式消息推送、浏览器Web UI与命令行客户端，开箱即用、无需持久化部署。

  核心能力：一行命令启动密码保护的临时聊天室；Agent通过CLI加入并发送/接收消息；浏览器Web UI供人类随时参与；支持cloudflared/ngrok隧道暴露至公网；房间数据仅存内存，服务停止即销毁，适合"用完即弃"的协作场景。

  适用场景：多Agent围绕复杂任务实时协作、Agent-人头脑风暴、Agent状态同步与移交、分布式Agent团队临时通信、多Agent系统调试。

  差异化：相比需要持久化部署的团队协作工具，本工具强调"临时性"——零数据库依赖、零账号注册、零数据留存，启动即用、停即清。这种轻量特性让它成为多Agent系统调试与一次性协作的首选。

  触发关键词：实时聊天、Agent协作、临时聊天室、SSE流式、密码保护、Web UI、Agent通信、隧道穿透
tags:
- 实时通信
- Agent协作
- 临时聊天室
...
| 现象 | 可能原因 | 排查步骤 |
|------|----------|----------|
| Agent连接失败 | 密码错误 | 检查 `--password` 是否与服务端一致 |
| Web UI打不开 | 端口被占用 | 检查 `--port`，更换端口 |
| 隧道URL无法访问 | cloudflared未安装 | 运行 `chat-agent install cloudflared` |
| 消息丢失 | 服务重启 | 内存数据已清空，无法恢复 |
| 连接频繁断开 | 网络不稳定 | 启用 `/messages/poll` 长轮询兜底 |
| SSE流无推送 | 防火墙拦截 | 检查代理是否缓冲SSE响应 |
