...
edition: "free"
description: |-
  聊天Agent工具是一套面向多Agent协作与Agent-人协作的临时实时聊天室方案，提供密码保护、SSE流式消息推送、浏览器Web UI与命令行客户端，开箱即用、无需持久化部署。核心能力：一行命令启动密码保护的临时聊天室；Agent通过CLI加入并发送/接收消息；浏览器Web UI供人类随时参与；支持cloudflared/ngrok隧道暴露至公网；房间数据仅存内存，服务停止即销毁，适合"用完即弃"的协作场景
tags:
  - 实时通信
  - Agent协作
  - 临时聊天室
...
**处理**: 按照skill规范执行能力1：一行命令启动聊天室操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力2：Agent CLI接入操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力3：SSE流式消息推送操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力4：浏览器Web UI操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力5：隧道穿透支持操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行能力6：内存级数据隔离操作,遵循单一意图原则。
