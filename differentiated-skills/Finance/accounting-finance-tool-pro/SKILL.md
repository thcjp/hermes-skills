...
description: |-
  ...
  核心能力:
  - 完整估值模型体系（14个技能）：DCF多阶段、行业特化估值、银行/保险/地产专用模型
  - 深度财务分析（26个技能）：杜邦五因素、现金流八维分析、资本结构、报表重组
  - 全面风险评估（18个技能）：欺诈检测、盈利质量、关联交易、审计意见分析
  - 批量处理与CSV导出：同时分析多只标的并生成对比矩阵
  - 自动化估值报告：一键生成专业级估值报告
  ...
  差异化:
  - 兼容免费版全部技能，无缝升级
  - 新增银行/保险/地产/科技公司专用估值模型
  - 支持批量标的并行分析与对比矩阵生成
  - 自动化估值报告撰写与多格式导出
  - 包含审计报告分析、会计政策深度解读等高级模块

  触发关键词: 财务分析, 估值建模, DCF, WACC, 银行估值, 保险估值, 欺诈检测, 关联交易, 审计报告, 批量分析, 估值报告, accounting, finance, valuation
tags:
- Finance
- 估值分析
- 财务建模
