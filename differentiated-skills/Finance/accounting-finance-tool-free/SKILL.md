...
edition: "free"
description: |-
  面向个人投资者与初级分析师的财务分析技能包，提供估值建模、财务比率分析、
  风险评估等基础能力。通过自然语言指令驱动Agent执行专业财务分析任务。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。适用于独立开发者、企业团队和自动化工作流场景。Use when 需要数据分析、报表生成、统计洞察、数据可视化时使用。不适用于实时流数据处理。
tags:
  - Finance
  - 估值分析
  - 财务建模
...
**处理**: 按照skill规范执行估值建模模块操作,遵循单一意图原则。
**输出**: 返回估值建模模块的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行财务比率分析模块操作,遵循单一意图原则。
**输出**: 返回财务比率分析模块的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行风险评估模块操作,遵循单一意图原则。
**输出**: 返回风险评估模块的执行结果,包含操作状态和输出数据。
