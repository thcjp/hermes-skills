...
description: |-
  ...
  核心能力:
  - 多数据源聚合与自动回退（Yahoo/Alpha Vantage/IEX/Fixer等）
  - 实时外汇汇率（不再限于每日更新）
  - 加密货币行情支持（BTC/ETH/SOL等主流币种）
  - 价格告警与定时推送
  - 批量标的导出与组合分析报告
  - 自定义数据频率与缓存策略
  ...
  - 新增加密货币实时行情
  - 支持价格告警与定时推送
  - 批量导出CSV/Excel，支持组合级分析

  触发关键词: 行情, 股价, 报价, ETF, 指数, 外汇, 汇率, 加密货币, 告警, 批量导出, 组合分析, stock, quote, forex, crypto, alert
tags:
- Finance
- 行情追踪
- 多数据源
