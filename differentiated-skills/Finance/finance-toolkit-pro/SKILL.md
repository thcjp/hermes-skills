...
**输入**: 用户提供PRO版增强功能所需的指令和必要参数。
**处理**: 按照skill规范执行PRO版增强功能操作,遵循单一意图原则。
**输出**: 返回PRO版增强功能的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：多数据源行情追踪、支持股票、含告警与批量导出、面向专业投资者与、交易团队的多数据、源行情追踪工具、加密货币的实时报、价与深度历史数据、内置多数据源自动、批量导出与组合分、析功能、Use、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
