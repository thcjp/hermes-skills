...
**处理**: 按照skill规范执行OWASP Top 10 深度安全扫描操作,遵循单一意图原则。
**输出**: 返回OWASP Top 10 深度安全扫描的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行多格式报告输出操作,遵循单一意图原则。
**输出**: 返回多格式报告输出的执行结果,包含操作状态和输出数据。
