...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、含通配符、全服务器配置、自动化监控与告警、SSL、工具箱、专业版、面向团队与企业、提供完整的、证书管理、全主流服务器配置、安全审计评分、自动化监控告警与、多域名治理能力、核心能力、全证书类型、全主流服务器、Use、when、需要安全检测、合规审计、加密防护时使用、不适用于渗透测试、未授权目标等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- `-RSA-AES256-GCM-SHA384`: 命令参数,用于指定操作选项
- `-RSA-AES128-GCM-SHA256`: 命令参数,用于指定操作选项
...
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent完成操作。PRO版面向团队与企业,提供全证书类型、全主流服务器配置、安全审计、自动化监控与多域名治理能力,完全兼容免费版证书申请与基础配置。
