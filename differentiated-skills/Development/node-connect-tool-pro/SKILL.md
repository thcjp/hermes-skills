...
summary: "全拓扑节点连接诊断,含尾网/公网/远程网关、批量配对、审计日志与自动修复。。节点连接工具(专业版)面向团队与运维,提供全拓扑节点连接诊断、批量配对、连接审计日志与快速自动修复能力。核心能力:"
license: "Proprietary"
edition: "pro"
...
节点连接工具(专业版)面向团队与运维,在兼容免费版本地与局域网诊断能力的基础上,扩展了全拓扑支持(含Tailscale尾网、公网反代、远程网关)、批量设备配对、连接审计日志与快速自动修复脚本.
当你在请求中提及 跨网络节点、远程网关、Tailscale、公网反代、批量配对、连接审计 等关键词时,该工具会自动激活,为团队提供结构化的连接诊断与运维方案.
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全拓扑节点连接诊、含尾网、审计日志与自动修、节点连接工具、专业版、面向团队与运维、提供全拓扑节点连、接诊断、连接审计日志与一、键自动修复能力、核心能力、全拓扑支持、批量设备配对与状、连接审计日志与回、快速诊断与自动修、远程网关与多网关、高级鉴权配置等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
## 触发条件
# ...
需要系统监控、日志分析、运维告警、部署管理时使用。不适用于非该工具能力范围的需求.
# ...
## 设备配对看板 (32台设备)
# ...
| 设备ID | 类型 | 状态 | 最后连接 | 操作 |
