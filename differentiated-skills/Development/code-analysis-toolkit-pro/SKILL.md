---
slug: code-analysis-toolkit-pro
name: code-analysis-toolkit-pro
version: "1.0.0"
...
**处理**: 按照skill规范执行团队复盘分析(需全员同意)操作,遵循单一意图原则。
**输出**: 返回团队复盘分析(需全员同意)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行多仓库批量扫描操作,遵循单一意图原则。
**输出**: 返回多仓库批量扫描的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行匿名化输出操作,遵循单一意图原则。
**输出**: 返回匿名化输出的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、历史分析、支持团队复盘、趋势追踪与多格式、面向团队与企业的、历史分析工具、在免费版基础上扩、展团队复盘、趋势追踪等能力、核心能力、同意管理与审计追、多仓库批量扫描与、历史趋势追踪与基、匿名化输出与隐私等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
步骤3: 执行分析(带 --multi-author-team-retro)
步骤4: 输出匿名化报告
步骤5: 团队讨论(将结果作为讨论提示)
步骤6: 形成改进行动计划
步骤7: 下次复盘时对比趋势
