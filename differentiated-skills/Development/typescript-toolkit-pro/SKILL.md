---
slug: typescript-toolkit-pro
name: typescript-toolkit-pro
version: "1.0.0"
...
`typescript-toolkit-pro` 是面向团队与企业的 TypeScript 类型安全工程平台。它在免费版核心建议的基础上,扩展了批量代码迁移、企业规范落地、CI/CD 集成与高级类型工程设计能力,帮助团队在规模化项目中维持高水准的类型安全与代码质量。

本版本完全兼容免费版输出的所有建议与代码片段,可平滑升级。所有指令通过 Markdown 驱动 Agent,无需额外安装私有脚本。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：面向团队与企业的、TypeScript、类型安全平台、含批量迁移、规范审查与高级泛、工具集专业版为团、队与企业提供端到、端类型安全工程能、涵盖批量、流水线集成、企业级规范审查与、高级泛型设计、核心能力、批量代码迁移与渐、进式类型化策略、企业级、ESLint、规则集、映射类型工程化设、流水线类型检查与、团队规范文档自动、生成与代码审查等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
1. **采用分层 `tsconfig.json`**:根配置定义通用选项,子包通过 `extends` 继承并覆盖项目特定项,降低维护成本。
2. **启用 `composite` 与 `incremental`**:在多包仓库中启用项目引用,大幅缩短类型检查耗时。
3. **设置类型覆盖率门禁**:用 `type-coverage` 工具跟踪未类型化代码比例,目标 ≥ 95%。
4. **PR 级别强制类型检查**:CI 中将 `tsc --noEmit` 设为必须通过的门禁,阻断类型错误进入主分支。
5. **慎用模板字面量递归类型**:复杂递归类型会显著增加编译耗时,必要时拆分或预生成。
6. **统一命名规范**:通过 `@typescript-eslint/naming-convention` 强制类型、变量、常量的命名风格。
7. **`exactOptionalPropertyTypes` 慎用**:启用后 `undefined` 与缺失属性不再等价,需团队充分培训。
8. **定期复盘类型覆盖率与错误趋势**:每月生成报告,识别热点模块并安排专项治理。
