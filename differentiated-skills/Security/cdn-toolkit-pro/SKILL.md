...
**处理**: 按照skill规范执行多CDN智能调度(专业版独有)操作,遵循单一意图原则。
**输出**: 返回多CDN智能调度(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行Edge Workers边缘计算(专业版独有)操作,遵循单一意图原则。
**输出**: 返回Edge Workers边缘计算(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行高级WAF与DDoS防护(专业版独有)操作,遵循单一意图原则。
**输出**: 返回高级WAF与DDoS防护(专业版独有)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
- `-Control`: 命令参数,用于指定操作选项
- `-Cache-Status`: 命令参数,用于指定操作选项
- `-AB-Variant`: 命令参数,用于指定操作选项
- `-Edge-Location`: 命令参数,用于指定操作选项
- `-X`: 命令参数,用于指定操作选项
- `-H`: 命令参数,用于指定操作选项
- `-Type`: 命令参数,用于指定操作选项
- `--CdnType`: 命令参数,用于指定操作选项

### 命令参数说明

- `--DomainName`: 命令参数,用于指定操作选项
- `--Sources`: 命令参数,用于指定操作选项
