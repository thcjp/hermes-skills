...
**处理**: 按照skill规范执行十二阶段深度评估(专业版独有)操作,遵循单一意图原则。
**输出**: 返回十二阶段深度评估(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行合规框架映射(专业版独有)操作,遵循单一意图原则。
**输出**: 返回合规框架映射(专业版独有)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行100分制多维安全评分(专业版独有)操作,遵循单一意图原则。
**输出**: 返回100分制多维安全评分(专业版独有)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行自动化漏洞SLA管理(专业版独有)操作,遵循单一意图原则。
**输出**: 返回自动化漏洞SLA管理(专业版独有)的执行结果,包含操作状态和输出数据。
