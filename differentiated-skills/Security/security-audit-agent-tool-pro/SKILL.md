...
edition: pro
description: |-
  Agent安全审计专业版,为企业安全团队提供全方位AI Agent安全审计能力。核心能力:多Agent批量审计、上下文感知提示注入检测、沙盒逃逸分析、工具参数投毒检测、供应链安全、SARIF报告与CI/CD集成。Use when 需要安全检测、合规审计、漏洞扫描、加密防护时使用。不适用于渗透测试未授权目标。
tags:
- 安全
- AI安全
- Agent审计
...
**处理**: 按照skill规范执行多Agent批量审计(专业版独有)操作,遵循单一意图原则。
**输出**: 返回多Agent批量审计(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行上下文感知提示注入检测(专业版独有)操作,遵循单一意图原则。
**输出**: 返回上下文感知提示注入检测(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行沙盒逃逸分析(专业版独有)操作,遵循单一意图原则。
**输出**: 返回沙盒逃逸分析(专业版独有)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
