...
description: |-
  ...
  触发关键词: Agent安全, 企业安全, 安全巡检, 信任管理, 安全评估, 健康检查, security guard, agent guard, trust registry, security patrol
tags:
- 安全
- Agent安全
- 企业安全
...
```bash
# 评估网络请求
node scripts/action.js --type network_request \
  --url "https://api.example.com" \
  --method POST \
  --body '{"data": "sensitive"}'

# 评估命令执行
node scripts/action.js --type exec_command \
  --command "rm -rf /tmp/cache"

# 评估Web3交易
node scripts/action.js --type web3_tx \
  --chain-id 1 \
  --from 0x1234... \
  --to 0x5678... \
...
# 查询信任记录
node scripts/trust.js lookup --source /path/to/skill

# 撤销信任
node scripts/trust.js revoke --source /path/to/skill --reason "安全违规"

# 列出所有信任记录
node scripts/trust.js list
```
...
```bash
# 新Skill安装:先扫描再注册
node scripts/scan.js /path/to/new-skill/
node scripts/trust.js attest --id new-skill --trust-level restricted

# 定期验证:每月检查完整性
node scripts/patrol.js run

# 发现问题:立即撤销
node scripts/trust.js revoke --source /path/to/skill --reason "检测到恶意行为"
```
...
# 每日巡检
node scripts/patrol.js setup --schedule "0 3 * * *"
```
