...
**处理**: 按照skill规范执行交易模拟(专业版独有)操作,遵循单一意图原则。
**输出**: 返回交易模拟(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行批量地址检查(专业版独有)操作,遵循单一意图原则。
**输出**: 返回批量地址检查(专业版独有)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行地址信誉查询(兼容免费版)操作,遵循单一意图原则。
**输出**: 返回地址信誉查询(兼容免费版)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行代币深度检测(兼容免费版增强)操作,遵循单一意图原则。
**输出**: 返回代币深度检测(兼容免费版增强)的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
