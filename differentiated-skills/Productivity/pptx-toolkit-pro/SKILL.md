...
summary: "企业级演示文稿系统,支持品牌模板管理、批量生成、协作审阅与多格式导出。核心能力: 文档处理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
...
本工具是 **文档处理** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**输入**: 用户提供品牌模板管理与多模板切换所需的指令和必要参数.
**处理**: 解析品牌模板管理与多模板切换的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回品牌模板管理与多模板切换的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 批量演示文稿生成与数据驱动幻灯
批量演示文稿生成与数据驱动幻灯片

**输入**: 用户提供批量演示文稿生成与数据驱动幻灯所需的指令和必要参数.
**处理**: 解析批量演示文稿生成与数据驱动幻灯的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量演示文稿生成与数据驱动幻灯的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 团队协作审阅与版本管理
团队协作审阅与版本管理

**输入**: 用户提供团队协作审阅与版本管理所需的指令和必要参数.
**处理**: 解析团队协作审阅与版本管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回团队协作审阅与版本管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多格式导出(PDF/图片/视频
多格式导出(PDF/图片/视频/Web)

**输入**: 用户提供多格式导出(PDF/图片/视频所需的指令和必要参数.
**处理**: 解析多格式导出(PDF/图片/视频的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多格式导出(PDF/图片/视频的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 演示文稿分析
演示文稿分析:阅读时间估算与内容密度评估

**输入**: 用户提供演示文稿分析所需的指令和必要参数.
**处理**: 解析演示文稿分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回演示文稿分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级演示文稿系、支持品牌模板管理、批量生成、协作审阅与多格式、核心能力、文档处理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | PPT 演示工具包专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "PPT 演示工具包专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pptxkit pro"
    }
  },
