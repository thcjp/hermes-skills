...
**输入**: 用户提供批量 JQL 查询与结果导出(所需的指令和必要参数。
**处理**: 按照skill规范执行批量 JQL 查询与结果导出(操作,遵循单一意图原则。
**输出**: 返回批量 JQL 查询与结果导出(的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供多项目事务批量创建与字段映射所需的指令和必要参数。
**处理**: 按照skill规范执行多项目事务批量创建与字段映射操作,遵循单一意图原则。
**输出**: 返回多项目事务批量创建与字段映射的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自定义字段与组件管理
自定义字段与组件管理

**输入**: 用户提供自定义字段与组件管理所需的指令和必要参数。
**处理**: 按照skill规范执行自定义字段与组件管理操作,遵循单一意图原则。
**输出**: 返回自定义字段与组件管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 操作审计日志与变更历史追踪
操作审计日志与变更历史追踪

**输入**: 用户提供操作审计日志与变更历史追踪所需的指令和必要参数。
**处理**: 按照skill规范执行操作审计日志与变更历史追踪操作,遵循单一意图原则。
**输出**: 返回操作审计日志与变更历史追踪的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
