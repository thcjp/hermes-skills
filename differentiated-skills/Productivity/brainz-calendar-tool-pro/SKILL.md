...
**输入**: 用户提供多日历同步与跨平台日历聚合(G所需的指令和必要参数。
**处理**: 按照skill规范执行多日历同步与跨平台日历聚合(G操作,遵循单一意图原则。
**输出**: 返回多日历同步与跨平台日历聚合(G的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供团队空闲时间分析与会议室预订所需的指令和必要参数。
**处理**: 按照skill规范执行团队空闲时间分析与会议室预订操作,遵循单一意图原则。
**输出**: 返回团队空闲时间分析与会议室预订的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 重复事件批量管理与会者通知
重复事件批量管理与会者通知

**输入**: 用户提供重复事件批量管理与会者通知所需的指令和必要参数。
**处理**: 按照skill规范执行重复事件批量管理与会者通知操作,遵循单一意图原则。
**输出**: 返回重复事件批量管理与会者通知的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日程冲突检测与智能调整建议
日程冲突检测与智能调整建议

**输入**: 用户提供日程冲突检测与智能调整建议所需的指令和必要参数。
**处理**: 按照skill规范执行日程冲突检测与智能调整建议操作,遵循单一意图原则。
**输出**: 返回日程冲突检测与智能调整建议的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
