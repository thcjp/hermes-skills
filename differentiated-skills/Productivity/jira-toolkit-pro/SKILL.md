...
**输入**: 用户提供批量 JQL 查询与结果导出所需的指令和必要参数。
**处理**: 按照skill规范执行批量 JQL 查询与结果导出操作,遵循单一意图原则。
**输出**: 返回批量 JQL 查询与结果导出的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 敏捷看板管理与 Sprint 
敏捷看板管理与 Sprint 规划

**输入**: 用户提供敏捷看板管理与 Sprint 所需的指令和必要参数。
**处理**: 按照skill规范执行敏捷看板管理与 Sprint 操作,遵循单一意图原则。
**输出**: 返回敏捷看板管理与 Sprint 的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供自定义字段与组件批量管理所需的指令和必要参数。
**处理**: 按照skill规范执行自定义字段与组件批量管理操作,遵循单一意图原则。
**输出**: 返回自定义字段与组件批量管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多项目事务关联与依赖追踪
多项目事务关联与依赖追踪

**输入**: 用户提供多项目事务关联与依赖追踪所需的指令和必要参数。
**处理**: 按照skill规范执行多项目事务关联与依赖追踪操作,遵循单一意图原则。
**输出**: 返回多项目事务关联与依赖追踪的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
1. **变更状态前先获取当前状态**
2. **MCP server 分配负责人需要 accountId 而非显示名**
3. **编辑描述前先展示原始内容**
4. **批量修改前需获得明确批准**
