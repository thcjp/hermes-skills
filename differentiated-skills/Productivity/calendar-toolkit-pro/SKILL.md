...
**输入**: 用户提供多租户日历管理与权限隔离所需的指令和必要参数。
**处理**: 按照skill规范执行多租户日历管理与权限隔离操作,遵循单一意图原则。
**输出**: 返回多租户日历管理与权限隔离的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供智能排程所需的指令和必要参数。
**处理**: 按照skill规范执行智能排程操作,遵循单一意图原则。
**输出**: 返回智能排程的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 团队日历可视化与冲突检测
团队日历可视化与冲突检测

**输入**: 用户提供团队日历可视化与冲突检测所需的指令和必要参数。
**处理**: 按照skill规范执行团队日历可视化与冲突检测操作,遵循单一意图原则。
**输出**: 返回团队日历可视化与冲突检测的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
