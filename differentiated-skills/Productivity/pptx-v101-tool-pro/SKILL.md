...
slug: pptx-v101-tool-pro
name: pptx-v101-tool-pro
version: 1.0.0
displayName: PPT 工具 v1.0 专业版
summary: "企业级演示文稿工坊,支持智能布局引擎、数据驱动幻灯片与协作审阅流水线。核心能力: 文档处理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 文档处置领域的专业化 AI 辅助工具,包含企业级高级功能兼容. 适用于需要pptx v101 tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - PowerPoint
  - PPTX
  - v1.0
...
**处理**: 解析智能布局引擎的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回智能布局引擎的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 数据驱动幻灯片
数据驱动幻灯片:从数据源自动生成图表

**处理**: 解析数据驱动幻灯片的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回数据驱动幻灯片的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 协作审阅流水线
协作审阅流水线:草稿->审阅->修改->定稿

**处理**: 解析协作审阅流水线的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回协作审阅流水线的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多版本管理与变更对比
多版本管理与变更对比

**处理**: 解析多版本管理与变更对比的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多版本管理与变更对比的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 演示文稿 SEO 与可访问性检
演示文稿 SEO 与可访问性检查

**处理**: 解析演示文稿 SEO 与可访问性检的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回演示文稿 SEO 与可访问性检的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级演示文稿工、支持智能布局引擎、数据驱动幻灯片与、核心能力、文档处理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
```json
{
  "success": true,
  "data": {
    "result": "PPT 工具 v1.0 专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pptx v101 pro"
    }
  },
