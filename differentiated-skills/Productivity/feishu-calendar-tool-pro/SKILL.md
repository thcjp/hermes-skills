...
**输入**: 用户提供共享日历创建与成员权限管理所需的指令和必要参数。
**处理**: 按照skill规范执行共享日历创建与成员权限管理操作,遵循单一意图原则。
**输出**: 返回共享日历创建与成员权限管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多人协作日历与事件共享
多人协作日历与事件共享

**输入**: 用户提供多人协作日历与事件共享所需的指令和必要参数。
**处理**: 按照skill规范执行多人协作日历与事件共享操作,遵循单一意图原则。
**输出**: 返回多人协作日历与事件共享的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 跨时区日程同步与自动转换
跨时区日程同步与自动转换

**输入**: 用户提供跨时区日程同步与自动转换所需的指令和必要参数。
**处理**: 按照skill规范执行跨时区日程同步与自动转换操作,遵循单一意图原则。
**输出**: 返回跨时区日程同步与自动转换的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供日历事件 Webhook 通知所需的指令和必要参数。
**处理**: 按照skill规范执行日历事件 Webhook 通知操作,遵循单一意图原则。
**输出**: 返回日历事件 Webhook 通知的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
