...
license: Proprietary
edition: pro
description: '核心能力: 运维监控领域的专业化 AI 辅助工具,提供企业级高级功能支持。
...
**输入**: 用户提供自定义仪表盘面板与布局配置所需的指令和必要参数。
**处理**: 按照skill规范执行自定义仪表盘面板与布局配置操作,遵循单一意图原则。
**输出**: 返回自定义仪表盘面板与布局配置的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 告警规则
告警规则:阈值触发邮件/短信/Webhook

**输入**: 用户提供告警规则所需的指令和必要参数。
**处理**: 按照skill规范执行告警规则操作,遵循单一意图原则。
**输出**: 返回告警规则的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多节点集群监控与聚合视图
多节点集群监控与聚合视图

**输入**: 用户提供多节点集群监控与聚合视图所需的指令和必要参数。
**处理**: 按照skill规范执行多节点集群监控与聚合视图操作,遵循单一意图原则。
**输出**: 返回多节点集群监控与聚合视图的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 历史数据存储与趋势分析图表
历史数据存储与趋势分析图表

**输入**: 用户提供历史数据存储与趋势分析图表所需的指令和必要参数。
**处理**: 按照skill规范执行历史数据存储与趋势分析图表操作,遵循单一意图原则。
**输出**: 返回历史数据存储与趋势分析图表的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 权限管理
权限管理:多用户角色与访问控制

**输入**: 用户提供权限管理所需的指令和必要参数。
**处理**: 按照skill规范执行权限管理操作,遵循单一意图原则。
**输出**: 返回权限管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
