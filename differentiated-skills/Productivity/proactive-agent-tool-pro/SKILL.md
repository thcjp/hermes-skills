...
license: Proprietary
edition: pro
description: "核心能力: AI Agent领域的专业化 AI 辅助工具,包含企业级高级功能兼容. 适用于需要proactive agent tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 主动型Agent
  - 记忆管理
  - 自我改进
...
本工具是 **AI Agent** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析WAL 协议的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回WAL 协议的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 工作缓冲区
工作缓冲区:上下文压缩危险区生存

**处理**: 解析工作缓冲区的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回工作缓冲区的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 压缩恢复
压缩恢复:上下文截断后自动恢复

**处理**: 解析压缩恢复的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回压缩恢复的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析ADL/VFM 自我改进护栏协的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回ADL/VFM 自我改进护栏协的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级主动型、Agent、压缩恢复与安全加、核心能力、领域的专业化、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 主动型 Agent 专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "主动型 Agent 专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "proactive agent pro"
    }
  },
