...
**处理**: 按照skill规范执行WAL 协议操作,遵循单一意图原则。
**输出**: 返回WAL 协议的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行工作缓冲区操作,遵循单一意图原则。
**输出**: 返回工作缓冲区的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行ADL/VFM 自我改进护栏协操作,遵循单一意图原则。
**输出**: 返回ADL/VFM 自我改进护栏协的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
