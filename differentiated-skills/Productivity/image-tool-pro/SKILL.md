...
version: 1.0.0
displayName: 图像处理专业版
summary: 企业级图像处理流水线,支持批量处理、CDN 集成、响应式图片生成与无障碍优化
license: Proprietary
edition: pro
description: '核心能力: 图像处理领域的专业化 AI 辅助工具,提供企业级高级功能支持。
...
**输入**: 用户提供批量图像处理与自动化流水线所需的指令和必要参数。
**处理**: 按照skill规范执行批量图像处理与自动化流水线操作,遵循单一意图原则。
**输出**: 返回批量图像处理与自动化流水线的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 响应式图片生成
响应式图片生成:多尺寸 srcset 自动输出

**输入**: 用户提供响应式图片生成所需的指令和必要参数。
**处理**: 按照skill规范执行响应式图片生成操作,遵循单一意图原则。
**输出**: 返回响应式图片生成的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### CDN 集成与图片上传管道
CDN 集成与图片上传管道

**输入**: 用户提供CDN 集成与图片上传管道所需的指令和必要参数。
**处理**: 按照skill规范执行CDN 集成与图片上传管道操作,遵循单一意图原则。
**输出**: 返回CDN 集成与图片上传管道的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 无障碍优化
无障碍优化:Alt 文本建议与对比度检查

**输入**: 用户提供无障碍优化所需的指令和必要参数。
**处理**: 按照skill规范执行无障碍优化操作,遵循单一意图原则。
**输出**: 返回无障碍优化的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 图片质量评分与自动优化建议
图片质量评分与自动优化建议

**输入**: 用户提供图片质量评分与自动优化建议所需的指令和必要参数。
**处理**: 按照skill规范执行图片质量评分与自动优化建议操作,遵循单一意图原则。
**输出**: 返回图片质量评分与自动优化建议的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
