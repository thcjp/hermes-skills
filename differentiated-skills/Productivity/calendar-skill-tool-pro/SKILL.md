...
**输入**: 用户提供多配置文件管理隔离不同账户所需的指令和必要参数。
**处理**: 按照skill规范执行多配置文件管理隔离不同账户操作,遵循单一意图原则。
**输出**: 返回多配置文件管理隔离不同账户的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 安全策略
安全策略:操作前确认与最小权限原则

**输入**: 用户提供安全策略所需的指令和必要参数。
**处理**: 按照skill规范执行安全策略操作,遵循单一意图原则。
**输出**: 返回安全策略的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 事件内容安全审计
事件内容安全审计:防注入与内容过滤

**输入**: 用户提供事件内容安全审计所需的指令和必要参数。
**处理**: 按照skill规范执行事件内容安全审计操作,遵循单一意图原则。
**输出**: 返回事件内容安全审计的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 批量事件操作与会者通知管理
批量事件操作与会者通知管理

**输入**: 用户提供批量事件操作与会者通知管理所需的指令和必要参数。
**处理**: 按照skill规范执行批量事件操作与会者通知管理操作,遵循单一意图原则。
**输出**: 返回批量事件操作与会者通知管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日历数据导出与合规报告生成
日历数据导出与合规报告生成

**输入**: 用户提供日历数据导出与合规报告生成所需的指令和必要参数。
**处理**: 按照skill规范执行日历数据导出与合规报告生成操作,遵循单一意图原则。
**输出**: 返回日历数据导出与合规报告生成的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
