...
edition: free
description: |-
  事实核查助手免费版,面向个人用户提供基础的事实核查能力。通过对接全球专业事实核查机构,对用户提供的声明进行多源交叉验证。Use when 需要AI模型调用、智能对话、Agent编排、LLM应用时使用。不适用于需要100%确定性的关键决策。适用于独立开发者、企业团队和自动化工作流场景。Use when 需要AI模型调用、智能对话、Agent编排、LLM应用时使用。不适用于需要100%确定性的关键决策。
tags:
- 研究工具
- 事实核查
- 信息鉴别
...
**处理**: 按照skill规范执行多源交叉验证操作,遵循单一意图原则。
**输出**: 返回多源交叉验证的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行区域相关性匹配操作,遵循单一意图原则。
**输出**: 返回区域相关性匹配的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行语言匹配操作,遵循单一意图原则。
**输出**: 返回语言匹配的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行结构化结果呈现操作,遵循单一意图原则。
**输出**: 返回结构化结果呈现的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行历史声明归档操作,遵循单一意图原则。
**输出**: 返回历史声明归档的执行结果,包含操作状态和输出数据。
