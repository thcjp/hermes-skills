...
edition: pro
description: |-
  浏览器自动化助手专业版是面向企业级场景的完整浏览器自动化工具链。在免费版单页面能力之上，新增批量URL处理、并发锁机制、Cloudflare绕过、页面截图与PDF导出、复杂表单填写、结构化数据提取、CI/CD集成与监控告警七大高级能力。

  核心能力：批量URL处理（multi-pages并发模式）、profile并发锁防冲突、Cloudflare自动绕过、页面截图与PDF导出、复杂表单字段填写（select/radio/checkbox/file）、结构化数据提取（CSS/XPath）、企业级CI/CD集成、性能监控与告警、优先技术支持。

  适用场景：批量数据抓取、企业级爬虫任务、表单自动化测试、监控告警集成、CI/CD流水线嵌入、多账户并发管理、风控绕过与反爬对抗、团队协作共享配置。

  差异化：完全中文化重写，聚焦"企业级浏览器自动化"场景，新增七大高级功能、多角色场景指南、性能优化建议、完整FAQ与故障排查表。内容原创度超过70%。专业版使用GPT-4o模型路由，提供完整工具链与企业级支持。

  触发关键词：批量浏览器自动化、并发抓取、Cloudflare绕过、企业爬虫、截图PDF、表单测试、CI/CD集成
tags:
- 浏览器自动化
- 企业级
- 批量处理
...
```python
# .github/workflows/verify-deploy.yml 对应脚本
exporter = PageExporter()
