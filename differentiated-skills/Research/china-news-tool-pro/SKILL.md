...
edition: "pro"
description: |-
  中国新闻聚合助手专业版是面向企业级场景的完整新闻聚合与分发工具。在免费版RSS订阅能力之上，新增浏览器自动化模式、AI智能摘要、定时自动执行、多渠道推送、AI辅助分类、新闻情感分析、历史新闻检索七大高级能力。Use when 需要AI模型调用、智能对话、Agent编排、LLM应用时使用。不适用于需要100%确定性的关键决策。
tags:
  - 中国新闻
  - 企业级
  - AI摘要
...
**处理**: 按照skill规范执行浏览器自动化模式操作,遵循单一意图原则。
**输出**: 返回浏览器自动化模式的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行AI智能摘要操作,遵循单一意图原则。
**输出**: 返回AI智能摘要的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行定时自动执行操作,遵循单一意图原则。
**输出**: 返回定时自动执行的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行新闻情感分析操作,遵循单一意图原则。
**输出**: 返回新闻情感分析的执行结果,包含操作状态和输出数据。
