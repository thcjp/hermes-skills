...
edition: pro
description: |-
  浏览器会话助手专业版是面向企业级场景的完整浏览器会话管理工具。在免费版单次抓取能力之上，新增定时任务（recurring）、会话持久化、Webhook通知、批量会话管理、企业级监控告警、Cloudflare自动绕过、会话调试增强七大高级能力。Use when 需要项目管理、任务规划、进度跟踪、团队协作时使用。不适用于实际人员绩效评估。
tags:
- 浏览器会话
- 定时任务
- 企业级
...
**处理**: 按照skill规范执行定时任务（recurring循环执行）操作,遵循单一意图原则。
**输出**: 返回定时任务（recurring循环执行）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行会话持久化（保存为可复用脚本）操作,遵循单一意图原则。
**输出**: 返回会话持久化（保存为可复用脚本）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行Webhook通知（多渠道）操作,遵循单一意图原则。
**输出**: 返回Webhook通知（多渠道）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行企业级监控告警操作,遵循单一意图原则。
**输出**: 返回企业级监控告警的执行结果,包含操作状态和输出数据。
