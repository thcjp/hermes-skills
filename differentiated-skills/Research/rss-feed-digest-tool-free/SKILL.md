...
0 8 * * * cd /path/to/skill && python3 scripts/rss_digest.py fetch --feed-file my-feeds.txt --hours 24 --output /reports/$(date +\%Y\%m\%d)-digest.md
```
...
A: 免费版提供固定的Markdown模板(标题+来源+条目列表+统计信息)。如需自定义格式,可修改`scripts/rss_digest.py`中的输出模板部分,或升级到专业版使用模板系统。
