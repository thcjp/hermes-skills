...
edition: free
description: |-
  真实浏览器控制免费版,面向个人用户提供直接控制用户真实 Chrome 浏览器的能力。
  通过用户脚本注入技术,在页面上下文中执行 JavaScript,共享所有 Cookie、会话和登录状态。

  核心能力:
  - 控制用户真实 Chrome 浏览器,保持登录状态
  - 在页面上下文执行 JavaScript,操作 DOM 元素
  - 共享 Cookie、会话、登录状态,无需重复登录
  - 页面文本提取、元素发现、截图等基础操作
  - 多标签页管理与切换
  - CSP 回退机制,应对安全策略限制
  ...
  差异化:
  - 免费版聚焦个人本地浏览器场景,共享真实登录态
  - 移除外部平台依赖,纯净本地运行
  - 增强中文触发关键词,提升中文环境识别率
  - 完全适配 SkillHub 平台规范

  触发关键词: 真实浏览器, Chrome 控制, 页面操作, 登录态共享, 用户脚本, 浏览器自动化, real browser, tampermonkey, page automation
tags:
- 研究工具
- 浏览器控制
- 个人效率
