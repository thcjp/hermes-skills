...
summary: "AI Agent子代理分派开发系统，每任务新代理+两阶段review，高质量快速迭代.。集群编码器免费版为AI Agent提供子代理驱动的开发执行系统，将"一个人从头到尾做完所有任务"升级为"
license: Proprietary
edition: free
description: 集群编码器免费版为AI Agent提供子代理驱动的开发执行系统，将"一个人从头到尾做完所有任务"升级为"每任务分派新子代理+两阶段review"的集群协作模式。基于"新鲜上下文+规范review+质量review"双阶段审查，实现高质量、快迭代、零上下文污染。Use
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | 集群编码器(免费版)处理的输入数据或指令 |
...
**处理**: 解析每任务新子代理分派的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回每任务新子代理分派的响应数据,包含状态码、结果和日志.
### 2. 两阶段review流水线
```text
实现完成
    │
...
**处理**: 解析两阶段review流水线的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回两阶段review流水线的响应数据,包含状态码、结果和日志.
...
**处理**: 解析子代理提问机制的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回子代理提问机制的响应数据,包含状态码、结果和日志.
### 4. 自我审查捕获
子代理在提交前必须自我审查：
...
**处理**: 解析自我审查捕获的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自我审查捕获的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析review循环修复的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回review循环修复的响应数据,包含状态码、结果和日志.
### 6. TodoWrite任务追踪
```markdown
...
**处理**: 解析TodoWrite任务追踪的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回TodoWrite任务追踪的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析最终全局review的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回最终全局review的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析上下文策展的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回上下文策展的响应数据,包含状态码、结果和日志.
### 9. 红旗清单（12项禁止）
详见快速开始中的`.swarm/red-flags.md`。每项违反都可能导致质量问题或流程失败.
**输入**: 用户提供红旗清单（12项禁止）所需的指令和必要参数.
**处理**: 解析红旗清单（12项禁止）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回红旗清单（12项禁止）的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
```json
{
  "success": true,
  "data": {
    "result": "集群编码器(免费版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "swarm coder"
    }
  },
