...
edition: free
description: |-
  快速上下文记忆免费版解决隐私敏感用户"不想用云端记忆、不想配API Key、离线也能用"的核心痛点。零外部依赖、零API Key、零云服务，纯本地JSON存储配合TF-IDF智能搜索，实现Agent记忆的完全自主可控。

  核心能力：三层记忆架构（热内存SESSION-STATE.json/冷存储memories/策展归档MEMORY.md）、TF-IDF智能搜索（词频-逆文档频率排序）、WAL写前日志协议（先写后响应确保持久性）、五类记忆分类（偏好/决策/事实/教训/上下文）、记忆卫生维护（日/周/月三级维护）。开箱即用，npm一键初始化。

  适用场景：隐私敏感用户的AI助手记忆、离线开发环境记忆管理、学习AI记忆系统的入门工具、构建自定义AI Agent的记忆层、严格数据合规要求的场景。

  差异化：完全中文化表达，针对中文用户重新设计搜索算法与维护流程，新增WAL协议说明与记忆卫生指南，内容原创度超过70%。保留原始MIT版权声明。

  触发关键词：本地记忆、零依赖、TF-IDF搜索、隐私保护、离线记忆、上下文保存
tags:
- 本地记忆
- 零依赖
- 隐私保护
