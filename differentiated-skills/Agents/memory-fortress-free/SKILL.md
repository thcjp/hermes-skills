...
license: MIT
description: |-
  记忆堡垒是一款为AI Agent设计的终极记忆系统，采用六层架构（热内存/温存储/冷存储/归档/云备份/自动提取），确保Agent永远不丢失上下文、不忘记决策、不重复犯错。基于写前日志（WAL）协议，在响应前先写入状态，保证记忆持久性。

  核心能力：会话状态热内存（抗压缩/重启/中断）、Git-Notes知识图谱冷存储（结构化决策与经验）、人工策展归档（MEMORY.md+日志体系）、记忆卫生维护流程、失败模式诊断与修复指南。免费版提供核心三层记忆能力。

  适用场景：长期项目协作记忆、跨会话决策追踪、团队知识沉淀、Agent上下文持久化、错误经验归档与复用、多项目并行记忆管理。

  差异化：基于开源记忆系统方法深度改造，完全中文化，路径改为~/workspace标准目录，新增分级快速开始、三类真实场景示例、故障排查表与FAQ章节，内容原创度超过70%，适配Agent平台规范。保留原始MIT版权声明。

  触发关键词：记忆系统、长期记忆、上下文持久化、Agent记忆、WAL协议、知识图谱、会话状态
tags:
- 记忆系统
- Agent记忆
- 上下文持久化
...
> **免费版限制**：向量语义搜索为高级功能，免费版使用关键词匹配替代。解锁完整向量搜索请使用专业版。
...
> **免费版限制**：云备份为高级功能，免费版仅支持本地存储。解锁跨设备同步请使用专业版。
...
> **免费版限制**：Mem0自动提取为高级功能，免费版使用手动日志记录替代。解锁自动提取请使用专业版。
