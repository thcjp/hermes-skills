...
**处理**: 按照skill规范执行原生记忆检索操作,遵循单一意图原则。
**输出**: 返回原生记忆检索的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行后台自动事实捕获操作,遵循单一意图原则。
**输出**: 返回后台自动事实捕获的执行结果,包含操作状态和输出数据。

### 3. 端到端加密与去中心化存储
记忆端到端加密，仅用户可解密。数据分布于去中心化网络节点（非任何单一公司服务器），避免单点依赖与供应商锁定。配对流程通过浏览器生成/导入 12 词恢复短语，短语从不触及聊天。

**输入**: 用户提供端到端加密与去中心化存储所需的指令和必要参数。
**处理**: 按照skill规范执行端到端加密与去中心化存储操作,遵循单一意图原则。
**输出**: 返回端到端加密与去中心化存储的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行零暴露凭据管理（硬性约束）操作,遵循单一意图原则。
**输出**: 返回零暴露凭据管理（硬性约束）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自动恢复与智能槽位管理操作,遵循单一意图原则。
**输出**: 返回自动恢复与智能槽位管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行本地缓存与离线检索回退操作,遵循单一意图原则。
**输出**: 返回本地缓存与离线检索回退的执行结果,包含操作状态和输出数据。
