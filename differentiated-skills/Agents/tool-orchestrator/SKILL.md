...
version: 1.0.1
displayName: 工具编排器
summary: "端到端加密去中心化记忆编排,统一工具接口,零暴露凭据管理,自动恢复.。端到端加密、去中心化存储的智能体记忆编排系统，直击安装复杂、凭据暴露、网关重启困难、槽位绑定冲突四大痛点。适用于跨会话偏"
license: MIT
description: "端到端加密、去中心化存储的智能体记忆编排系统，直击安装复杂、凭据暴露、网关重启困难、槽位绑定冲突四大痛点。适用于跨会话偏好持久化、隐私优先记忆、去中心化存储、记忆固定与作用域管控等场景。核心能力含原生记忆检索、后台自发事实捕获、零暴露凭据管控、自发恢复与智能槽位管控。适用关键词：工具编排、加密记忆、去中心化存储、记忆检索、凭据管控、memory-search、encrypted-memory."
tags:
  - 智能代理
  - 记忆管理
  - 数据安全
