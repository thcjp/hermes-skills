...
license: MIT
edition: free
description: "元代理调优器免费版为AI Agent包含结构化的学习闭环系统，将零散的错误、纠正、特性需求转化为可检索、可提升、可复用的知识库。每次犯错都是进化的燃料，每次纠正都是规则的种子。Use. 适用于需要meta agent optimizer相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 元代理优化
  - 学习日志
  - 错误沉淀
...
**处理**: 解析三类日志分类记录的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回三类日志分类记录的响应数据,包含状态码、结果和日志.
### 2. 标准化ID生成
格式：`TYPE-YYYYMMDD-placeholder`

| 组成 | 说明 | 示例 |
|---:|---:|---:|
| TYPE | 日志类型 | LRN / ERR / FEAT |
| YYYYMMDD | 当前日期 | 20260118 |
| placeholder | 顺序号或随机3字符 | 001 / A7B |

**示例**：`LRN-20260118-001`、`ERR-20260118-A3F`、`FEAT-20260118-002`

**处理**: 解析标准化ID生成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回标准化ID生成的响应数据,包含状态码、结果和日志.
### 3. 学习条目格式
```markdown
...
**处理**: 解析学习条目格式的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回学习条目格式的响应数据,包含状态码、结果和日志.
...
**处理**: 解析错误条目格式的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回错误条目格式的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析特性请求条目格式的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回特性请求条目格式的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析状态管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回状态管理的响应数据,包含状态码、结果和日志.
### 7. 优先级矩阵
| 优先级 | 使用场景 | 示例 |
|:------|------:|:------|
...
**处理**: 解析优先级矩阵的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回优先级矩阵的响应数据,包含状态码、结果和日志.
### 8. 区域标签体系
| 区域 | 范围 | 典型场景 |
|---:|:---|---:|
...
**处理**: 解析区域标签体系的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回区域标签体系的响应数据,包含状态码、结果和日志.
### 9. 检测触发器
自动捕获以下4类信号并记录：
...
**处理**: 解析检测触发器的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回检测触发器的响应数据,包含状态码、结果和日志.
### 10. 复发模式检测
当记录类似条目时：
...
**处理**: 解析复发模式检测的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回复发模式检测的响应数据,包含状态码、结果和日志.
...
```json
{
  "success": true,
  "data": {
    "result": "Meta Agent Optimizer处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "meta agent optimizer"
    }
  },
