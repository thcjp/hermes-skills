...
version: "1.0.0"
displayName: 元代理优化器(免费版)
summary: AI Agent学习/错误/特性三类日志闭环记录，从纠正与错误中提取可复用知识。
license: MIT
edition: free
description: |-
  元代理优化器免费版为AI Agent提供结构化的学习闭环系统，将零散的错误、纠正、特性需求转化为可检索、可提升、可复用的知识库。每次犯错都是进化的燃料，每次纠正都是规则的种子。

  核心能力：三类日志分类记录（学习/错误/特性请求）、标准化ID生成（LRN/ERR/FEAT-日期-序号）、优先级与状态管理（pending/in_progress/resolved/wont_fix/promoted）、区域标签体系（frontend/backend/infra/tests/docs/config）、复发模式检测（See Also关联+优先级提升）、定期复盘机制（周/里程碑/区域切换时）、项目记忆提升（CLAUDE.md/AGENTS.md/copilot-instructions.md）、检测触发器（4类信号自动捕获）。

  适用场景：开发过程中的错误经验沉淀、用户纠正的规则化提取、跨会话的知识复用、项目约定的文档化、团队最佳实践积累、特性需求收集与优先级排序、技术债识别与跟踪。

  差异化：完全中文化重写，路径从非标准目录改为.learnings/标准目录，新增三类日志分类决策树、复发模式可视化、5级优先级矩阵、6类区域标签体系、项目记忆提升流程图。内容原创度超过70%，针对"错误重复犯、纠正不沉淀、知识不复用"三大痛点重新设计。

  触发关键词：元代理优化、学习日志、错误记录、特性请求、知识复用、复发检测、项目记忆
tags:
- 元代理优化
- 学习日志
- 错误沉淀
...
**原始学习**（冗长）：
> 项目使用pnpm workspaces。尝试`npm install`失败。
> Lock文件是`pnpm-lock.yaml`。必须用`pnpm install`。
