...
edition: standards
description: |-
  Skill生产规范是一套统一的Skill生产流水线标准，用于指导从原始skill（下载/原创）到可商用skill的完整改造流程。它解决"每个skill改造标准不一、质量参差不齐、收费策略混乱、去除标识不彻底"的核心痛点。Use when 需要电商运营、商品管理、订单处理、支付集成时使用。不适用于虚假交易和刷单。
tags:
- Skill规范
- 生产流水线
- 商业化
...
**处理**: 按照skill规范执行✅ [高级功能1]操作,遵循单一意图原则。
**输出**: 返回✅ [高级功能1]的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行✅ [高级功能2]操作,遵循单一意图原则。
**输出**: 返回✅ [高级功能2]的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行✅ [高级功能3]操作,遵循单一意图原则。
**输出**: 返回✅ [高级功能3]的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
