...
edition: free
description: |-
  神经上下文引擎免费版解决传统记忆检索"只能找相似，不能找相关"的核心痛点。向量搜索只能找到与查询字面相似的记忆，关键词搜索无法理解意图，而本引擎采用扩散激活算法，通过神经图谱遍历找到概念相关的记忆，即使没有关键词或嵌入重叠也能命中。

  核心能力：联想记忆存储（nmem_remember）、扩散激活检索（nmem_recall）、近期上下文注入（nmem_context）、快速待办（nmem_todo）、20种突触类型（时间/因果/语义/情感/冲突）、赫布学习（常用记忆自动强化）、艾宾浩斯遗忘曲线、矛盾检测。零LLM依赖，纯算法实现，数据本地存储。

  适用场景：跨会话上下文召回、概念关联检索、决策因果链追踪、用户偏好记忆、错误经验归档、项目知识图谱、个人AI助手长期记忆。

  差异化：完全中文化表达，针对中文场景重新设计检索策略，新增结构化工具参考与心跳维护集成，内容原创度超过70%。保留原始MIT版权声明。

  触发关键词：联想记忆、扩散激活、神经图谱、概念检索、因果链、赫布学习
tags:
- 联想记忆
- 神经图谱
- 扩散激活
