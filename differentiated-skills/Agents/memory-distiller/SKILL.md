...
**处理**: 按照skill规范执行记忆蒸馏器是面向 AI Agent 日志操作,遵循单一意图原则。
**输出**: 返回记忆蒸馏器是面向 AI Agent 日志的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行它借鉴古文压缩哲学（去重复、留转折、去过操作,遵循单一意图原则。
**输出**: 返回它借鉴古文压缩哲学（去重复、留转折、去过的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行(2) 压缩溯源链操作,遵循单一意图原则。
**输出**: 返回(2) 压缩溯源链的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
