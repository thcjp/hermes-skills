...
**输出**: 返回记忆类型与重要度参考的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行TF-IDF（50%）操作,遵循单一意图原则。
**输出**: 返回TF-IDF（50%）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行近期加权（20%）操作,遵循单一意图原则。
**输出**: 返回近期加权（20%）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行重要度加权（20%）操作,遵循单一意图原则。
**输出**: 返回重要度加权（20%）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行标签匹配（10%）操作,遵循单一意图原则。
**输出**: 返回标签匹配（10%）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

**输入**: 用户提供混合检索加权公式所需的指令和必要参数。
**处理**: 按照skill规范执行混合检索加权公式操作,遵循单一意图原则。
