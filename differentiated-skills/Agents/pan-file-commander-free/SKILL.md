...
description: |-
  ...
  核心能力包括:bdpan CLI完整的文件管理(上传/下载/转存/分享/搜索/移动/复制/重命名/创建/删除);严格的安全约束与路径校验(/apps/bdpan/范围限制);分层确认规则(高/中/低风险);操作意图识别与歧义消除;支持所有兼容SKILL.md的Agent平台。

  适用场景:百度网盘文件日常管理、文件分类浏览与查找、分享链接转存、单文件上传下载、路径安全校验、团队文件共享链接生成。

  差异化亮点:相比通用网盘工具,本指挥官聚焦"命令式高效操作",新增快捷命令模板(常用操作一键化)、文件分类视图(按类型/大小/时间)、智能路径校验(自动补全与越界检测)、操作意图识别(歧义消除)、分层确认规则(高/中/低风险)。免费版提供核心文件管理能力,记忆备份恢复、大文件后台下载与批量操作为专业版功能。

  触发关键词:网盘指挥官、百度网盘、文件管理、bdpan、pan-file-commander、网盘操作
tags:
- 智能代理
- 文件管理
- 百度网盘
