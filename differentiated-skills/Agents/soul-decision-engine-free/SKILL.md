...
version: "1.0.0"
displayName: 灵魂决策引擎(免费版)
summary: AI Agent结构化决策支持系统，混合命名空间记忆，从决策复盘中持续学习偏好。
license: MIT
edition: free
description: |-
  灵魂决策引擎免费版为AI Agent提供结构化决策支持系统，将"我应该选A还是B"的纠结过程转化为可追溯、可复盘、可学习的决策记忆。基于混合命名空间（域×类型）存储决策模式，从每次决策复盘中提取用户的风险偏好与框架偏好。

  核心能力：混合命名空间记忆（域×类型双维度），四层记忆分层（HOT风险画像/WARM域模式/WARM类型模式/RECORD决策记录/COLD归档），决策框架库（含推荐匹配），决策回顾机制（4问复盘+偏见检测），置信度信号体系（高/中/低三级），主动决策检测（5类隐式决策点），风险画像学习（5类风险偏好信号），框架偏好学习（5类框架偏好信号），结果反馈学习（4类结果信号），透明引用（每次分析标注来源），优雅降级。

  适用场景：技术架构选型、产品需求优先级排序、商业策略决策、个人生活重大选择、跨会话决策风格学习、团队决策复盘、风险评估与权衡、决策历史追溯。

  差异化：完全中文化重写，路径从非标准目录改为~/soul-decision/标准目录，新增混合命名空间可视化、决策回顾4问模板、置信度三级信号体系、5类主动决策检测、风险画像学习矩阵、框架偏好学习矩阵。内容原创度超过70%，针对"决策无结构、复盘无纪律、偏好不沉淀"三大痛点重新设计。

  触发关键词：决策引擎、结构化决策、决策复盘、风险画像、框架偏好、混合命名空间、置信度信号
tags:
- 决策引擎
- 结构化决策
- 决策复盘
...
分析和框架是你的。选择永远是用户的。
呈现选项，而非指令。用"你可以考虑"而非"你应该"。

### 2. 永远标注置信度

每次决策分析必须包含置信度标签（🟢高/🟡中/🔴低）。
