...
summary: "AI Agent结构化决策支持系统，混合命名空间记忆，从决策复盘中持续学习偏好.。灵魂决策引擎免费版为AI Agent提供结构化决策支持系统，将"我应该选A还是B"的纠结过程转化为可追溯、可"
license: MIT
edition: free
description: "灵魂决策引擎免费版为AI Agent包含结构化决策兼容系统，将\"我应该选A还是B\"的纠结过程转化为可追溯、可复盘、可学习的决策记忆。基于混合命名空间（域×类型）存储决策模式，从每次决策复盘中提取用户的风险偏好与框架偏好。Use. 适用于需要soul decision engine相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 决策引擎
  - 结构化决策
  - 决策复盘
...
**处理**: 解析混合命名空间记忆（域×类型）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回混合命名空间记忆（域×类型）的响应数据,包含状态码、结果和日志.
### 2. 四层记忆分层
| 层级 | 位置 | 大小限制 | 加载策略 |
|:-----|:-----|:-----|:-----|
...
**处理**: 解析四层记忆分层的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回四层记忆分层的响应数据,包含状态码、结果和日志.
### 3. 决策框架库
| 框架 | 适用场景 | 输出格式 |
|---:|---:|---:|
...
**处理**: 解析决策框架库的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回决策框架库的响应数据,包含状态码、结果和日志.
### 4. 决策回顾（4问复盘）
决策结果可观察后，触发复盘：
...
**处理**: 解析决策回顾（4问复盘）的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回决策回顾（4问复盘）的响应数据,包含状态码、结果和日志.
### 5. 置信度信号体系
每次决策分析必须包含置信度标注：
...
**处理**: 解析置信度信号体系的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回置信度信号体系的响应数据,包含状态码、结果和日志.
### 6. 主动决策检测
不等待用户询问，主动识别5类隐式决策点：
...
| 直接建议 | "给我你的优秀建议" | 偏好专家建议模式 |
| 权衡走查 | "走查权衡" | 偏好详细权衡过程 |
| 角色代入 | "如果是你会怎么做？" | 偏好角色代入分析 |

**处理**: 解析框架偏好学习的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回框架偏好学习的响应数据,包含状态码、结果和日志.
### 9. 结果反馈学习
自动捕获4类结果反馈信号：
...
**处理**: 解析结果反馈学习的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回结果反馈学习的响应数据,包含状态码、结果和日志.
### 10. 透明引用
每次框架应用必须标注来源：
