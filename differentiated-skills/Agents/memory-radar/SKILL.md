...
**处理**: 解析七大威胁类别检测的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回七大威胁类别检测的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 2. 五级安全分级与告警
SAFE（无威胁）/LOW（轻微关注）/MEDIUM（潜在威胁，建议审查）/HIGH（很可能威胁，立即审查）/CRITICAL（活跃威胁，立即隔离），MEDIUM 及以上自动告警.
**输入**: 用户提供五级安全分级与告警所需的指令和必要参数.
**处理**: 解析五级安全分级与告警的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回五级安全分级与告警的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 3. 跨文件威胁关联分析
...
**处理**: 解析跨文件威胁关联分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回跨文件威胁关联分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 4. 误报抑制机制
...
**处理**: 解析误报抑制机制的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回误报抑制机制的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
### 5. 隔离与恢复链路
...
**处理**: 解析隔离与恢复链路的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回隔离与恢复链路的响应数据,包含状态码、结果和日志.
