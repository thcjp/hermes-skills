...
edition: free
description: |-
  全息记忆AI（免费版）为AI Agent提供端到端加密的去中心化记忆系统。针对"会话结束后记忆丢失、用户隐私无法保护、中心化存储单点风险"等真实痛点，提供加密记忆存储、原生记忆检索与手动事实记录三大核心能力。

  核心能力：端到端加密记忆存储（12词恢复短语派生密钥）、原生记忆检索（memory_search/memory_get工具集成）、手动事实记录（remember命令）、去中心化存储架构（非单一服务器）、恢复短语安全协议（绝不进入对话上下文）、凭据文件保护机制（Agent永不读取credentials.json）。

  适用场景：AI Agent长期记忆、跨会话用户偏好保留、隐私敏感场景记忆、个人助理记忆持久化、去中心化应用记忆层。

  差异化：针对"记忆丢失与隐私两难"的真实痛点重新设计，强调"恢复短语永不进入对话"的安全铁律。引入去中心化存储与端到端加密，原创度超过70%。免费版提供核心加密存储与手动记录，专业版解锁自动事实提取与跨设备同步。

  触发关键词：加密记忆、去中心化记忆、恢复短语、记忆检索、事实记录、隐私保护、跨会话记忆
tags:
- 加密记忆
- 去中心化
- 隐私保护
