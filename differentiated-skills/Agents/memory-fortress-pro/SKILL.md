...
license: Proprietary
description: "记忆堡垒专业版是在免费版基础上的全功能升级，为AI Agent包含完整记忆系统。采用六层架构（热内存/温存储/冷存储/归档/云备份/自发提取），基于WAL协议确保持久化。专业版解锁向量语义搜索、Mem0自发提取、云备份三大高级功能，达成跨设备同步与智能记忆管控. 适用于需要memory fortress相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - 记忆系统
  - Agent记忆
  - 向量搜索
...
可以。六层架构是模块化设计，可按需启用。最简配置仅需热内存（领先层）。推荐配置为热内存+冷存储+策展归档（免费版）。完整配置启用全部六层（专业版）。可通过 `agent-config.json` 中的 `plugins.entries` 配置各层启用状态.
### Q11：记忆数据存储在哪里？安全吗？
...
```json
{
  "success": true,
  "data": {
    "result": "记忆堡垒(专业版)处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "memory fortress pro"
    }
  },
