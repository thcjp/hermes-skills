...
edition: free
description: |-
  Token经济学家（免费版）为AI Agent提供智能Token成本优化方案。针对"长对话Token暴涨、重复查询浪费、代码上下文堆积"等真实痛点，提供上下文压缩、语义缓存与自适应优化三大核心能力。

  核心能力：智能上下文压缩（按重要性分级，保留近期消息与关键决策）、语义缓存（L1精确匹配100%节省、L2语义相似80%节省）、自适应优化（按Token压力分级压缩）、质量守卫（代码块/错误/重要消息永不压缩）、自然语言指令与斜杠命令双模式。

  适用场景：长对话Token优化、技术讨论上下文压缩、重复查询缓存、代码审查场景保护、API成本控制。

  差异化：针对"Token消耗失控"的真实痛点重新设计，强调"质量守卫"原则（代码与关键决策永不压缩）。引入三级压缩策略与质量回滚机制，原创度超过70%。免费版提供核心压缩与单级缓存，专业版解锁多级缓存、成本预估与预算控制。

  触发关键词：Token优化、上下文压缩、语义缓存、成本控制、自适应压缩、质量守卫
tags:
- Token优化
- 上下文压缩
- 语义缓存
