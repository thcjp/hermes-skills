...
edition: free
description: |-
  进化循环免费版为AI Agent提供一套基于PDCA（计划-执行-检查-改进）闭环的自改进引擎，将零散的"用户纠正-自我反思"过程结构化为可追溯、可演化的记忆系统。

  核心能力：三层记忆分层（HOT热内存≤100行 / WARM温存储项目域 / COLD冷归档），自动升降级机制（3次命中升级HOT，30天未用降级WARM，90天归档COLD），纠正信号识别（8类显式纠正短语），自我反思触发器（多步任务后/反馈后/Bug修复后），命名空间隔离（项目/域/全局三级继承），冲突解决（最具体优先+最近优先+歧义询问），透明引用（每次行动标注来源），优雅降级（上下文超限时仅加载HOT并告知用户）。

  适用场景：长期项目协作中的偏好沉淀、跨会话避免重复犯错、独立开发者工作流优化、技术决策记忆、代码规范自动学习、写作风格持续校准、个人助理习惯养成。

  差异化：完全中文化重写，路径从非标准目录改为~/evolve-loop标准目录，新增PDCA闭环可视化、量化改进指标看板、8类纠正信号模板、3级命名空间继承图、冲突解决决策树、优雅降级清单。内容原创度超过70%，针对"AI遗忘偏好、重复犯错、跨会话失忆"三大真实痛点重新设计。

  触发关键词：自改进、自我反思、PDCA、进化循环、记忆分层、纠正学习、偏好沉淀、跨会话记忆
tags:
- 自改进
- Agent记忆
- 反思学习
