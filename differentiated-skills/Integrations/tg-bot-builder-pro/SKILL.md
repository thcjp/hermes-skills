...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能、机器人构建平台、支持支付集成、批量推送、状态机与高级群管、面向企业团队与专、业开发者的、机器人全功能构建、全生命周期管理与、高级业务集成、核心能力、在免费版基础上新、批量消息推送引擎、多轮对话状态机、高级群组权限矩阵、测试支持与性能监、控仪表盘、Use、when、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
|:---------|:---------|:---------|:-------|------|
| Webhook无响应 | 证书过期/防火墙阻断 | 检查证书有效期，调用getWebhookInfo查看错误 | P0 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 大量429错误 | 速率控制未生效 | 检查rate_limit配置，确认队列缓冲正常 | P0 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 支付回调丢失 | pre_checkout超时 | 确认handler响应时间<10秒，执行ping命令测试网络连通性,检查防火墙和代理设置延迟 | P1 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 状态机卡死 | 超时未触发 | 检查Redis连接，确认定时任务正常运行 | P1 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 群组消息遗漏 | Bot被踢出群组 | 调用getChat验证成员状态，重新邀请 | P2 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 模板渲染失败 | 变量缺失 | 检查模板变量注入逻辑，添加默认值 | P2 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
