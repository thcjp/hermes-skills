...
edition: free
description: |-
  面向AI Agent场景重新设计的轻量级SQLite本地数据库管理工具。通过单一文件、零运维的方式提供结构化数据存储能力，配套Python封装API、性能调优参数与常用Schema模板，可快速接入会话记忆、缓存层、日志归档等典型工作流。

  核心能力：
  - 单文件零运维部署，典型内存占用2-5MB，开箱即用
  - 内置WAL并发模式、连接池、批量写入、自动备份等关键能力
  - 提供会话日志、缓存表、Agent备忘等预置Schema模板
  - 支持内存模式、磁盘模式灵活切换，适配临时数据与持久化场景

  适用场景：
  - AI Agent的短期记忆与会话上下文持久化
  - 个人开发者与一人公司的本地数据落盘
  - 自动化工作流中的中间结果存储与缓存

  差异化：原创中文使用指南，按角色拆分场景示例，覆盖常见锁表、慢查询、体积膨胀等故障排查，配套内存优化与批量处理最佳实践。

  触发关键词：sqlite、轻量数据库、本地存储、嵌入式数据库、AI Agent存储
tags:
- 集成工具
- 本地存储
- 数据库
