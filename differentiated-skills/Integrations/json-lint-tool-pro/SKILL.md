...
**处理**: 按照skill规范执行并行扫描引擎操作,遵循单一意图原则。
**输出**: 返回并行扫描引擎的执行结果,包含操作状态和输出数据。
...
**输出**: 返回JSON Schema验证的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自动修复建议操作,遵循单一意图原则。
**输出**: 返回自动修复建议的执行结果,包含操作状态和输出数据。
