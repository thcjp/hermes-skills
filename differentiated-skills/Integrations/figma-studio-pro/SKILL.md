...
**输入**: 用户提供能力1：批量并行导出所需的指令和必要参数。
**处理**: 按照skill规范执行能力1：批量并行导出操作,遵循单一意图原则。
**输出**: 返回能力1：批量并行导出的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力2：Webhook全生命周期管理所需的指令和必要参数。
**处理**: 按照skill规范执行能力2：Webhook全生命周期管理操作,遵循单一意图原则。
**输出**: 返回能力2：Webhook全生命周期管理的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力3：设计变更监控所需的指令和必要参数。
**处理**: 按照skill规范执行能力3：设计变更监控操作,遵循单一意图原则。
**输出**: 返回能力3：设计变更监控的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力4：团队资源治理所需的指令和必要参数。
**处理**: 按照skill规范执行能力4：团队资源治理操作,遵循单一意图原则。
**输出**: 返回能力4：团队资源治理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供能力5：设计令牌转Tailwind所需的指令和必要参数。
**处理**: 按照skill规范执行能力5：设计令牌转Tailwind操作,遵循单一意图原则。
**输出**: 返回能力5：设计令牌转Tailwind的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供能力6：设计版本对比所需的指令和必要参数。
**处理**: 按照skill规范执行能力6：设计版本对比操作,遵循单一意图原则。
**输出**: 返回能力6：设计版本对比的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、设计协作工具、支持批量导出、设计变更监控与团、工作室专业版是一、款面向设计团队与、企业级设计工程化、场景的全功能、协作工具、在免费版的文件读、评论管理、图片导出、设计令牌提取基础、新增批量导出、与设计版本对比六、大高级能力、覆盖从单文件读取、到企业级设计系统、治理的全场景需求等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
