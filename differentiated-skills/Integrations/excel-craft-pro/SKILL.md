...
edition: pro
description: |-
  Excel 工艺专业版在免费版基础上扩展批量生成、多文件合并拆分、条件格式与数据验证、高级图表、模板系统与数据源对接能力。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。适用于独立开发者、企业团队和自动化工作流场景。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。
tags:
- 集成工具
- 表格生成
- 企业级
...
**输入**: 用户提供多 Sheet 工作簿生成所需的指令和必要参数。
**处理**: 按照skill规范执行多 Sheet 工作簿生成操作,遵循单一意图原则。
**输出**: 返回多 Sheet 工作簿生成的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 基础格式化（字体 / 填充 /
基础格式化（字体 / 填充 / 边框 / 对齐）

**输入**: 用户提供基础格式化（字体 / 填充 /所需的指令和必要参数。
**处理**: 按照skill规范执行基础格式化（字体 / 填充 /操作,遵循单一意图原则。
**输出**: 返回基础格式化（字体 / 填充 /的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 公式支持
公式支持

**输入**: 用户提供公式支持所需的指令和必要参数。
**处理**: 按照skill规范执行公式支持操作,遵循单一意图原则。
**输出**: 返回公式支持的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 基础图表（柱状 / 折线 / 
基础图表（柱状 / 折线 / 饼图）

**输入**: 用户提供基础图表（柱状 / 折线 / 所需的指令和必要参数。
**处理**: 按照skill规范执行基础图表（柱状 / 折线 / 操作,遵循单一意图原则。
**输出**: 返回基础图表（柱状 / 折线 / 的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自动列宽与合并单元格
自动列宽与合并单元格

**输入**: 用户提供自动列宽与合并单元格所需的指令和必要参数。
**处理**: 按照skill规范执行自动列宽与合并单元格操作,遵循单一意图原则。
**输出**: 返回自动列宽与合并单元格的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、Excel、生成引擎、含批量处理、模板系统与数据源、工艺专业版在免费、版基础上扩展批量、多文件合并拆分、条件格式与数据验、对接能力、Use、when、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
