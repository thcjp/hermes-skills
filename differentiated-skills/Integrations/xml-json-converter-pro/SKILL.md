...
**处理**: 按照skill规范执行能力2：XSD Schema校验操作,遵循单一意图原则。
**输出**: 返回能力2：XSD Schema校验的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行能力3：XPath字段映射DSL操作,遵循单一意图原则。
**输出**: 返回能力3：XPath字段映射DSL的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力4：流式转换所需的指令和必要参数。
**处理**: 按照skill规范执行能力4：流式转换操作,遵循单一意图原则。
...
**输入**: 用户提供能力5：SOAP协议封装所需的指令和必要参数。
**处理**: 按照skill规范执行能力5：SOAP协议封装操作,遵循单一意图原则。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、互转工具、支持批量转换、映射与、专业版是一款面向、集成团队与企业级、数据交换场景的全、功能结构化数据格、式互转工具、在免费版的双向转、属性处理、命名空间基础上、新增批量转换、协议封装与数据库、直写六大高级能力、覆盖从单文件到企、业级数据交换流水、线的全场景需求、Use、when、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
