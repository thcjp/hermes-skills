...
> 详细内容已移至 `references/detail.md` - ### 标准搭建（<120秒）：跑第一个回归测试集
### 完整搭建（<300秒）：启用Mock与压测
...
**处理**: 按照skill规范执行功能1：回归测试集（批量测试）操作,遵循单一意图原则。
**输出**: 返回功能1：回归测试集（批量测试）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能2：本地Mock服务器所需的指令和必要参数。
**处理**: 按照skill规范执行功能2：本地Mock服务器操作,遵循单一意图原则。
**输出**: 返回功能2：本地Mock服务器的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能3：性能压测所需的指令和必要参数。
**处理**: 按照skill规范执行功能3：性能压测操作,遵循单一意图原则。
**输出**: 返回功能3：性能压测的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能4：OpenAPI契约校验所需的指令和必要参数。
**处理**: 按照skill规范执行功能4：OpenAPI契约校验操作,遵循单一意图原则。
**输出**: 返回功能4：OpenAPI契约校验的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、测试调试全套件、含批量回归、契约校验与团队协、工具箱专业版是面、向研发团队的全功、测试调试套件、在免费版的请求模、认证范式、错误诊断基础上、解锁批量回归测试、按服务细分的完整、错误码字典、团队协作空间六大、高级能力、覆盖从联调到上线、再到持续回归的完、整生命周期、when、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
**输入**: 用户提供功能6：团队协作空间所需的指令和必要参数。
**处理**: 按照skill规范执行功能6：团队协作空间操作,遵循单一意图原则。
**输出**: 返回功能6：团队协作空间的执行结果,包含操作状态和输出数据。
### 错误场景2
