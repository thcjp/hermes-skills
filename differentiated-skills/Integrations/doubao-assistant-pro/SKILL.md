...
**输入**: 用户提供模块三：知识库检索增强 RAG（专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行模块三：知识库检索增强 RAG（专业版独有）操作,遵循单一意图原则。
**输出**: 返回模块三：知识库检索增强 RAG（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行模块四：批量并发管理（专业版独有）操作,遵循单一意图原则。
**输出**: 返回模块四：批量并发管理（专业版独有）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行模块五：系统提示词模板库（专业版独有）操作,遵循单一意图原则。
**输出**: 返回模块五：系统提示词模板库（专业版独有）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供模块六：用量与会话分析（专业版独有）所需的指令和必要参数。
**处理**: 按照skill规范执行模块六：用量与会话分析（专业版独有）操作,遵循单一意图原则。
**输出**: 返回模块六：用量与会话分析（专业版独有）的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能豆包大模型、集成平台、支持流式响应、知识库与批量处理、豆包助手专业版是、面向团队与生产环、境的全功能豆包大、模型集成平台、在免费版基础上新、增流式响应、用量与会话分析六、大高级模块、核心能力、工具集成模板、知识库检索增强方、批量请求调度与并、发控制、可复用提示词模板、会话级用量统计与、分析面板等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
