...
**输入**: 用户提供1 数据读取全封装(免费版基础 + 企业级扩展)所需的指令和必要参数。
**处理**: 按照skill规范执行1 数据读取全封装(免费版基础 + 企业级扩展)操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行3 假设检验与 A/B 测试操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行4 大数据批处理操作,遵循单一意图原则。
...
**输出**: 返回5 自定义报告模板的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供6 定时任务与邮件分发所需的指令和必要参数。
**处理**: 按照skill规范执行6 定时任务与邮件分发操作,遵循单一意图原则。
**输出**: 返回6 定时任务与邮件分发的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能中文数据分、析平台、支持时序分解、大数据集批处理与、中文数据分析专业、版面向专业数据分、工程师与数据团队、负责人、提供完整的数据分、析生产线能力、核心能力、涵盖免费版全部能、无条数与功能限制、效应量与置信区间、大数据集批处理、千万级数据增量计、模板引擎、定时分析任务与邮、自动化周报、与企业数据库深度等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
