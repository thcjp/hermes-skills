...
description: |-
  ...
  核心能力：Go/Java/Python/Node.js注解自动扫描提取接口；JSON/YAML/HTML/PDF/Swagger UI单页五种格式导出；文档版本管理与字段级diff；文档→Mock服务器一键启动；团队PR评审、评论、@提及；GraphQL SDL与type/query/mutation生成；自定义模板引擎；中英双语对照输出。

  适用场景：企业级API文档治理、多语言代码仓库接口提取、文档与代码同步维护、前后端文档评审、多版本接口并存管理、对外开放API的门户文档、国际化团队双语文档。

  差异化：完全中文化重写，去除原始项目标识与外部仓库引用，针对企业级文档治理场景重新设计。相比免费版的"个人起草工具"定位，专业版重构为"团队级API文档平台"，新增八大独有功能、四类角色场景指南、性能优化策略、多平台集成示例与版本迁移指南。内容原创度超过70%。

  触发关键词：API文档治理、代码扫描、多格式导出、文档版本管理、Mock联动、团队评审、GraphQL Schema、文档门户
tags:
- API文档
- OpenAPI
- 代码扫描
...
# 扫描Python仓库（基于FastAPI/Flask装饰器）
api-doc scan --lang python --path ./app --output ./openapi.yaml

# 扫描Node.js仓库（基于Express/Koa路由）
...
# Mock自动根据Spec生成响应
# 前端访问 http://localhost:8080/api/v1/users 即得Mock数据
...
```bash
# Git pre-commit hook：提交代码时自动更新文档
#!/bin/sh
api-doc scan --lang python --path ./app --output ./openapi.yaml
git add openapi.yaml

# Git pre-push hook：推送前校验文档与代码一致
