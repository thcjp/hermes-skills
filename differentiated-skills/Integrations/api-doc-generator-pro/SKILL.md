...
**输入**: 用户提供功能1：代码仓库自动扫描所需的指令和必要参数。
**处理**: 按照skill规范执行功能1：代码仓库自动扫描操作,遵循单一意图原则。
**输出**: 返回功能1：代码仓库自动扫描的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能2：多格式导出所需的指令和必要参数。
**处理**: 按照skill规范执行功能2：多格式导出操作,遵循单一意图原则。
**输出**: 返回功能2：多格式导出的执行结果,包含操作状态和输出数据。
...
DIFF: v1.2.0 → v1.3.0
=====================
补充内容   POST /api/v1/orders/search - 订单搜索接口
...
**输入**: 用户提供功能3：文档版本管理与diff所需的指令和必要参数。
**处理**: 按照skill规范执行功能3：文档版本管理与diff操作,遵循单一意图原则。
...
**输入**: 用户提供功能4：Mock服务器联动所需的指令和必要参数。
**处理**: 按照skill规范执行功能4：Mock服务器联动操作,遵循单一意图原则。
**输出**: 返回功能4：Mock服务器联动的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行功能5：团队评审与协作操作,遵循单一意图原则。
**输出**: 返回功能5：团队评审与协作的执行结果,包含操作状态和输出数据。
...
**输出**: 返回功能8：多语言文档的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、文档平台、含代码扫描、联动与团队评审、文档生成器专业版、是面向研发团队的、全功能、在免费版的自然语、Markdown、双产出基础上、解锁代码仓库自动、团队评审协作、多语言文档八大高、级能力、覆盖从代码到文档、再到评审的完整闭等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
