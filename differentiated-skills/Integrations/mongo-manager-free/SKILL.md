...
edition: free
description: |-
  面向MongoDB开发者的数据库设计与优化实战指南。通过原创中文文档覆盖Schema建模哲学、索引策略、聚合管道、一致性模式、性能调优等核心主题，配套常见陷阱清单与故障排查表，帮助开发者避开MongoDB使用中的高频坑点。

  核心能力：
  - Schema设计模式：嵌入vs引用的决策框架
  - 索引策略：ESR规则、复合索引、TTL索引
  - 聚合管道：分阶段构建与性能优化
  - 一致性配置：读写关注、读偏好、因果一致性
  - 性能诊断：explain执行计划解读

  适用场景：
  - MongoDB新项目Schema设计与评审
  - 慢查询排查与索引优化
  - 聚合管道性能调优

  差异化：原创中文指南，按痛点拆分主题，覆盖文档大小陷阱、数组陷阱、$lookup陷阱等高频问题，配套ESR规则与explain解读实战。

  触发关键词：mongodb、schema设计、索引、聚合管道、性能优化
tags:
- 集成工具
- 数据库
- NoSQL
