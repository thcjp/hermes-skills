...
**输入**: 用户提供按任务选择 pandas / 所需的指令和必要参数。
**处理**: 按照skill规范执行按任务选择 pandas / 操作,遵循单一意图原则。
**输出**: 返回按任务选择 pandas / 的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 保留公式、样式、合并单元格、定
保留公式、样式、合并单元格、定义名称

**输入**: 用户提供保留公式、样式、合并单元格、定所需的指令和必要参数。
**处理**: 按照skill规范执行保留公式、样式、合并单元格、定操作,遵循单一意图原则。
**输出**: 返回保留公式、样式、合并单元格、定的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日期序列号与 1900/190
日期序列号与 1900/1904 系统处理

**输入**: 用户提供日期序列号与 1900/190所需的指令和必要参数。
**处理**: 按照skill规范执行日期序列号与 1900/190操作,遵循单一意图原则。
**输出**: 返回日期序列号与 1900/190的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 类型保护（长数字、电话、邮编）
类型保护（长数字、电话、邮编）

**输入**: 用户提供类型保护（长数字、电话、邮编）所需的指令和必要参数。
**处理**: 按照skill规范执行类型保护（长数字、电话、邮编）操作,遵循单一意图原则。
**输出**: 返回类型保护（长数字、电话、邮编）的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 工作簿结构保护与错误校验
工作簿结构保护与错误校验

**输入**: 用户提供工作簿结构保护与错误校验所需的指令和必要参数。
**处理**: 按照skill规范执行工作簿结构保护与错误校验操作,遵循单一意图原则。
**输出**: 返回工作簿结构保护与错误校验的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、XLSX、读写引擎、含批量处理、大文件流式、跨平台兼容与数据、处理专业版在免费、版基础上扩展批量、大文件流式读写、工作簿差异对比、公式审计与依赖追、数据源对接与跨平、台深度兼容能力、Use、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
