...
edition: free
description: |-
  Pipedrive工具免费版是一款面向销售数据管理的命令行辅助Skill，让AI Agent能够通过API查询Pipedrive中的商机、联系人、组织和活动数据，实现销售信息的快速检索和展示。

  核心能力：商机列表与搜索、联系人查询、组织查询、活动列表、管道与阶段查看、当前用户信息。覆盖销售数据只读查询的高频需求。

  适用场景：销售数据快速查询、客户信息检索、商机状态查看、活动安排确认、管道概览。

  差异化：通过API代理管理OAuth认证，无需手动处理Token刷新。免费版聚焦只读查询，满足销售人员日常信息获取需求。

  触发关键词：pipedrive、CRM、商机管理、销售数据、客户管理、deals
tags:
- 销售管理
- CRM集成
- 数据查询
