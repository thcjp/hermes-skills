...
edition: free
description: |-
  Figma工作室免费版是一款面向前端工程师与独立设计师的轻量级Figma设计文件读取与协作工具。围绕"文件读取—评论管理—图片导出—设计令牌提取"四件事，提供可复制即用的Python/Node.js脚本模板、Figma REST API调用规范、OAuth2认证流程指南，以及一份覆盖8类常用Figma资源（文件、节点、组件、样式、评论、图片、设计令牌、团队项目）的API速查表。

  核心能力：通过Figma REST API读取设计文件JSON与指定节点；获取与发布文件评论；按节点导出PNG/SVG/JPG/PDF图片；提取设计令牌（颜色、文字、效果、网格）为结构化数据；OAuth2认证流程标准化封装。

  适用场景：设计稿转前端代码、设计文件批量导出图片、设计令牌提取为CSS变量、团队协作评论管理、独立开发者设计资源自动化、设计版本对比。

  差异化：完全中文化重写，去除原始项目标识与外部仓库引用，将分散的API参考整合为按资源类型分类的调用模板与速查表，新增OAuth2认证流程指南与设计令牌提取规范。免费版聚焦"能跑通"的只读访问核心诉求，专业版在此基础上扩展批量导出、Webhook管理、设计变更监控与团队资源治理。内容原创度超过70%。

  触发关键词：figma、设计文件、设计令牌、图片导出、评论管理、oauth认证、设计转代码
tags:
- 设计协作
- 资源导出
- 集成工具
...
**安全红线**：Token永远放在环境变量或凭证文件中，禁止硬编码在脚本里；禁止把Token输出到日志或echo；示例中一律用 `<YOUR_TOKEN>` 占位。
