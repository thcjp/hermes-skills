...
edition: pro
description: |-
  数据分析专家专业版面向需要把复杂数据分析任务"委派"给 AI Agent 完成的专业用户与团队,提供完整的多任务编排、大数据处理与企业级集成能力。
  ...
  差异化:相比免费版,专业版补齐"大数据、多任务、缓存、自动化"四大短板,形成完整的委派式分析生产线。

  触发关键词:多任务编排、流式处理、结果缓存、定时报告、`PostgreSQL`、企业集成、委派分析
tags:
  - 数据分析
  - 多任务编排
  - 大数据处理
...
```text
请编排多任务分析:
- 任务1: 从 PostgreSQL 拉取销售数据(数据库)
- 任务2: 拉取商品主数据(数据库,与1并行)
- 任务3: 关联销售与商品(依赖1+2)
- 任务4: 按品类汇总(依赖3)
- 任务5: 生成报告(依赖4)
- 检查点: 每个任务完成后落盘
...
- 频率: 每日 09:00 自动拉取昨日数据
- 分析: 销售汇总 + Top 10 商品 + 异动归因
- 模板: 电商行业模板 + 公司品牌
- 分发: 邮件 + 企业 IM Webhook
- 缓存: 7 天内相同请求命中缓存
```
...
| clickhouse-driver | Python 库 | 可选 | `pip install clickhouse-driver` |
| redis | Python 库 | 可选 | `pip install redis`(缓存) |
...
- **缓存服务**: 通过 `REDIS_URL` 环境变量传入
- **邮件 SMTP**: 通过 `SMTP_HOST`/`SMTP_USER`/`SMTP_PASSWORD` 等环境变量传入
- **Webhook**: 通过 `IM_WEBHOOK_URL` 环境变量传入
...
- **说明**: 基于自然语言驱动的 AI Skill,集成大数据处理、多任务编排、缓存与自动化分发的完整生产线
