...
**输入**: 用户提供OTS外部时间戳同步所需的指令和必要参数。
**处理**: 按照skill规范执行OTS外部时间戳同步操作,遵循单一意图原则。
**输出**: 返回OTS外部时间戳同步的执行结果,包含操作状态和输出数据。
...
**输出**: 返回并行索引与批量验证的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供篡改检测告警所需的指令和必要参数。
**处理**: 按照skill规范执行篡改检测告警操作,遵循单一意图原则。
**输出**: 返回篡改检测告警的执行结果,包含操作状态和输出数据。
...
**输出**: 返回多传输协议适配器的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行审计报表与可视化操作,遵循单一意图原则。
**输出**: 返回审计报表与可视化的执行结果,包含操作状态和输出数据。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级数据溯源平、多传输协议适配与、总线工具包专业版、是一款面向团队与、企业的数据溯源平、在免费版三列协议、基础上、并行索引构建、与审计报表导出等、高级能力、核心能力、提升时间戳可信度、并行索引构建与批、支持百万级记录快、速检索、自动通知、审计报表导出、支持按时间与名称等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
