...
edition: free
description: |-
  PG任务队列免费版是一套基于 `PostgreSQL` 的轻量任务队列知识库，帮助独立开发者在不引入 Redis 或 RabbitMQ 的前提下，用一张数据库表实现可靠的异步任务调度。

  核心能力：提供基于 SKIP LOCKED 的批量领取设计、优先级调度字段、进度跟踪结构、失败重试与超时回收方案、单 Worker 串行到多 Worker 并行的演进路径。所有模式均以 SQL 片段与最小可运行示例呈现，便于直接复制落地。

  适用场景：中小型业务的异步任务调度、定时批处理任务编排、长耗时任务的进度可视化、服务重启后的任务自动恢复。

  差异化：以中文场景化表达重构队列设计知识体系，将零散的最佳实践整理为"场景-表结构-领取逻辑-异常处理"四段式结构，每个模式均附带可执行的验证方法与监控建议。

  触发关键词：任务队列、`PostgreSQL`队列、SKIP LOCKED、优先级调度、批量领取、进度跟踪、重试回收
tags:
- 数据库
- 集成工具
- 任务队列
...
| 症状 | 可能原因 | 排查方法 | 对策 |
|:-----|:---------|:---------|:-----|
| 任务一直 pending | 无 Worker 在运行 | 查看 Worker 日志与进程 | 重启 Worker 或扩容 |
| 任务重复执行 | 未使用 SKIP LOCKED | 检查领取 SQL | 改用 SKIP LOCKED |
| 任务卡在 running | Worker 崩溃未标记失败 | 查看 claimed_at 是否超时 | 执行超时回收 |
| 队列表膨胀 | 大量已完成任务未清理 | 查看表大小与行数 | 定期归档历史任务 |
| 领取延迟高 | 缺失部分索引 | 查看 EXPLAIN 计划 | 创建 idx_jobs_claimable |
