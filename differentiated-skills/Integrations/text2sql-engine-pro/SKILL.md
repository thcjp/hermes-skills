...
edition: pro
description: |-
  面向数据工程师与DBA的全功能自然语言转SQL引擎，支持复杂多表关联、窗口函数、查询性能优化与执行计划分析。

  核心能力：在免费版基础上新增多表嵌套JOIN、子查询/CTE/窗口函数、查询解释注释、替代方案对比、执行计划分析、索引优化建议、SQL安全审计等高级特性。

  适用场景：复杂报表生成、数据仓库分析、ETL管道构建、查询性能调优、SQL代码审查、数据迁移等企业级数据工程场景。

  差异化：相比免费版新增7项独有能力，提供完整的多角色场景指南（数据分析师/DBA/后端开发/数据工程师），内置性能基准与安全审计规则。

  触发关键词：自然语言、转SQL、text2sql、查询优化、执行计划、窗口函数、CTE、性能调优
tags:
- 数据查询
- SQL优化
- 数据工程
