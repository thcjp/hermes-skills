...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能、支持复杂多表查询、性能优化、查询解释与替代方、案生成、面向数据工程师与、DBA、的全功能自然语言、支持复杂多表关联、查询性能优化与执、核心能力、在免费版基础上新、增多表嵌套、查询解释注释、替代方案对比、安全审计等高级特、Use、when、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
|:---------|:---------|:---------|:-------|------|
| 生成的SQL执行报错 | Schema不完整或方言不匹配 | 核对Schema字段，确认数据库版本 | P0 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 查询结果不正确 | 意图解析偏差 | 细化需求描述，检查WHERE/HAVING条件 | P0 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 查询超时 | 缺少索引或全表扫描 | 查看执行计划，按建议添加索引 | P1 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 方言转换失败 | 不支持的特有语法 | 查看转换日志，手动调整不兼容部分 | P1 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 安全审计误报 | 规则过于严格 | 查看审计规则配置，调整白名单 | P2 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 替代方案性能差异大 | 数据分布不均匀 | 使用真实数据测试，参考执行计划成本 | P2 | 对照依赖说明章节逐项验证配置项,确认环境变量已正确设置后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
