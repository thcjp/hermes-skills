...
**处理**: 按照skill规范执行多数据库引擎适配操作,遵循单一意图原则。
**输出**: 返回多数据库引擎适配的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供性能诊断引擎所需的指令和必要参数。
**处理**: 按照skill规范执行性能诊断引擎操作,遵循单一意图原则。
**输出**: 返回性能诊断引擎的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供Schema版本管理所需的指令和必要参数。
**处理**: 按照skill规范执行Schema版本管理操作,遵循单一意图原则。
**输出**: 返回Schema版本管理的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供自定义抽取器所需的指令和必要参数。
**处理**: 按照skill规范执行自定义抽取器操作,遵循单一意图原则。
**输出**: 返回自定义抽取器的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供团队协作模式所需的指令和必要参数。
**处理**: 按照skill规范执行团队协作模式操作,遵循单一意图原则。
**输出**: 返回团队协作模式的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能数据库建模、多引擎、含性能诊断、团队协作与自定义、设计器、专业版、是企业级数据库建、模与演进管理工具、在免费版软、方法论基础上、扩展多数据库引擎、抽取器等高级能力、核心能力、三大引擎的统一建、模与差异化适配、内置性能诊断引擎、自动识别慢查询、版本管理与迁移脚、本生成、支持灰度发布与回、自定义抽取器模板、驱动的结构化数据、变更审批、权限分级等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent完成操作,部分高级功能需要schema-designer CLI
