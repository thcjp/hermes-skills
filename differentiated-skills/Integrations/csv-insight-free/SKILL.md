...
edition: free
description: |-
  CSV Insight 是面向数据分析师与开发者的轻量级 CSV 分析工具，通过命令行脚本完成统计、筛选、异常检测与分组聚合，零外部依赖。

  核心能力：自动列类型检测（数值/日期/文本）、灵活筛选（比较运算符）、统计摘要（均值/中位数/标准差/分位数）、Z-Score 异常检测、分组聚合、结果导出。

  适用场景：快速探查陌生 CSV 数据、订单/日志数据筛选、异常值定位、按维度分组汇总、低资源环境数据分析。

  差异化：相比 pandas 等重型方案，本 Skill 仅依赖 Python 标准库（csv 模块），在 2GB 内存服务器上即可运行；针对"快速看一眼数据"的场景优化，单条命令输出关键统计，无需编写脚本。

  触发关键词：csv分析、统计、筛选、异常检测、分组聚合、数据探查、零依赖
tags:
- 集成工具
- 数据分析
- 开发者工具
