...
edition: pro
description: |-
  CSV转JSON专业版是一款面向数据团队与企业级ETL场景的全功能表格数据格式转换工具。在免费版的表头推断、类型识别、编码探测基础上，新增批量转换、流式转换、JSON Schema校验、自定义字段映射、增量同步与数据库直写六大高级能力，覆盖从单文件到TB级数据流水线的全场景需求。

  核心能力：多文件并行流式转换支持GB级数据不OOM；JSON Schema结构校验确保转换结果符合下游契约；CSV列名到JSON键名的复杂映射规则（含重命名、拆分、合并、计算字段）；基于状态文件的增量同步只处理新增行；转换后直写 `PostgreSQL`/MySQL/SQLite/ClickHouse四种数据库；断点续传与幂等重试保障可靠性。

  适用场景：企业级ETL流水线的CSV接入节点、数据仓库ODS层落地、多源CSV文件合并入湖、API数据契约校验、跨系统数据同步、报表系统数据导出预处理。

  差异化：完全中文化重写，去除原始项目标识与外部仓库引用，将分散的命令行参考整合为按场景分类的企业级流水线模板与速查表，新增Schema校验流程、字段映射DSL与增量同步状态机。专业版相比免费版新增6项独有功能，内容原创度超过70%。

  触发关键词：csv转json、批量转换、流式转换、schema校验、字段映射、增量同步、数据库直写、ETL流水线
tags:
- 数据转换
- ETL流水线
- 企业工具
