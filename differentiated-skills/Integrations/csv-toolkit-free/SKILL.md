...
edition: free
description: |-
  CSV Toolkit 是面向开发者的 CSV 文件解析与生成参考指南，帮助处理跨工具交换时的引号、分隔符、编码与 Excel 兼容性问题。

  核心能力：RFC 4180 引号规则、多分隔符检测（逗号/分号/制表符/管道）、UTF-8 BOM 处理策略、Excel 公式注入防护、数字与日期格式标准化、常见解析失败排查。

  适用场景：跨系统数据交换、Excel 导入导出、遗留系统 CSV 集成、多语言数据迁移、数据清洗预处理。

  差异化：相比零散的网络资料，本 Skill 系统化梳理 CSV 处理的全部分支情况，包括引号转义、分隔符嗅探、编码检测、Excel 怪癖等，并给出每种情况的可执行对策；针对"空字段 vs 空字符串"、"前导零丢失"、"公式注入"等高频陷阱给出明确防护建议。

  触发关键词：csv、解析、生成、RFC 4180、分隔符、编码、Excel、引号、BOM
tags:
- 集成工具
- 数据处理
- 开发者工具
