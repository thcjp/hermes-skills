...
**处理**: 按照skill规范执行功能1：连接编排（工作流引擎）操作,遵循单一意图原则。
**输出**: 返回功能1：连接编排（工作流引擎）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能2：数据同步管道所需的指令和必要参数。
**处理**: 按照skill规范执行功能2：数据同步管道操作,遵循单一意图原则。
...
**输入**: 用户提供功能3：OAuth2 Token自动刷新所需的指令和必要参数。
**处理**: 按照skill规范执行功能3：OAuth2 Token自动刷新操作,遵循单一意图原则。
**输出**: 返回功能3：OAuth2 Token自动刷新的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能4：Webhook管理操作,遵循单一意图原则。
**输出**: 返回功能4：Webhook管理的执行结果,包含操作状态和输出数据。
...
**输出**: 返回功能6：连接器市场的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能7：多租户凭证隔离操作,遵循单一意图原则。
**输出**: 返回功能7：多租户凭证隔离的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**输入**: 用户提供功能8：批量调用与结果聚合所需的指令和必要参数。
**处理**: 按照skill规范执行功能8：批量调用与结果聚合操作,遵循单一意图原则。
**输出**: 返回功能8：批量调用与结果聚合的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、集成平台、含连接编排、刷新与监控告警、连接中心专业版是、面向企业的全功能、在免费版的连接器、凭证安全存储、统一调用模板、错误重试策略基础、解锁连接编排、多租户隔离、批量调用八大高级、覆盖从单服务调用、到多服务编排到数、据同步到事件驱动、的完整集成生命周等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
