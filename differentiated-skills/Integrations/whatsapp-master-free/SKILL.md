...
edition: free
description: |-
  WhatsApp 大师（免费版）面向个人用户与独立开发者，封装 WhatsApp 的基础消息能力：文本发送、媒体发送、贴纸、语音笔记与 GIF。通过原生通道集成直接调用 WhatsApp Web 协议，无需外部 Docker 或 CLI 包装。

  核心能力：
  - 5 类消息发送：文本、图片/视频/文档、贴纸、语音笔记、GIF
  - JID 格式说明（个人与群组）
  - 速率限制与反垃圾策略
  - ffmpeg 媒体转换模板（语音/贴纸/GIF）
  ...
  差异化：以"消息类型 × 参数表 × 示例"三段式组织，每类消息均附可直接复制的命令与媒体格式要求，原创内容占比超过 70%。免费版覆盖 5 类基础消息与速率限制，专业版额外提供交互（反应/回复/编辑/撤回）、群组管理、多智能体讨论等高级能力。

  触发关键词：whatsapp、消息发送、媒体、贴纸、语音、GIF、JID
tags:
- 即时通讯
- WhatsApp
- 消息自动化
