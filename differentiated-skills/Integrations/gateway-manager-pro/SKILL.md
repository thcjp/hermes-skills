...
**处理**: 按照skill规范执行功能1：多租户差异化限流操作,遵循单一意图原则。
**输出**: 返回功能1：多租户差异化限流的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能2：熔断与降级所需的指令和必要参数。
**处理**: 按照skill规范执行功能2：熔断与降级操作,遵循单一意图原则。
**输出**: 返回功能2：熔断与降级的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能3：灰度发布与AB测试所需的指令和必要参数。
**处理**: 按照skill规范执行功能3：灰度发布与AB测试操作,遵循单一意图原则。
**输出**: 返回功能3：灰度发布与AB测试的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供功能4：动态配置下发所需的指令和必要参数。
**处理**: 按照skill规范执行功能4：动态配置下发操作,遵循单一意图原则。
**输出**: 返回功能4：动态配置下发的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能5：分布式限流操作,遵循单一意图原则。
**输出**: 返回功能5：分布式限流的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能7：插件体系操作,遵循单一意图原则。
**输出**: 返回功能7：插件体系的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行功能8：配置dry-run操作,遵循单一意图原则。
**输出**: 返回功能8：配置dry-run的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、网关平台、含多租户、可观测性与插件体、网关管理器专业版、是面向企业的全功、在免费版的声明式、统一认证、基础限流、监控指标基础上、解锁多租户差异化、八大高级能力、覆盖从路由到流量、治理到可观测性的、完整网关生命周期、Use、when、需要项目管理、任务规划、进度跟踪、团队协作时使用、不适用于实际人员、绩效评估等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
