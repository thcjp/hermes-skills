...
edition: pro
description: |-
  全功能 JSON 处理，含高级序列化、解析安全、Unicode 边界、自动化校验。

  核心能力：
  - 全部基础最佳实践 + 高级序列化（Map/Set/BigInt/循环引用）
  - 深度解析安全（reviver、BOM、原型污染防护）
  - Unicode 边界处理（代理对、控制字符、BOM）
  - 自动化校验流水线（批量校验与报告）
  - 自定义 replacer 与 reviver 模板
  - 敏感数据剥离与安全序列化
  ...
  差异化：相比免费版新增高级序列化、深度解析安全、Unicode 边界三大能力，覆盖企业级与安全敏感场景，配套自动化校验流水线与自定义模板。

  触发关键词：json、序列化、循环引用、原型污染、unicode、批量校验、脱敏
tags:
- 集成工具
- 数据规范
- 企业效率
