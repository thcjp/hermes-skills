...
edition: free
description: |-
  API测试运行器免费版是一款面向独立开发者的服务状态探针工具，通过本地HTTP服务将多个端点的健康状态聚合为标准化JSON输出，便于接入仪表盘或监控告警系统。核心能力：
  - 零依赖Node。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。
tags:
- 集成工具
- 状态监控
- API测试
