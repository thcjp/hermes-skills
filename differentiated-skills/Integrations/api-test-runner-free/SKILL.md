...
**输入**: 用户提供多类型探测所需的指令和必要参数。
**处理**: 按照skill规范执行多类型探测操作,遵循单一意图原则。
**输出**: 返回多类型探测的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供配置驱动所需的指令和必要参数。
**处理**: 按照skill规范执行配置驱动操作,遵循单一意图原则。
**输出**: 返回配置驱动的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供后台刷新机制所需的指令和必要参数。
**处理**: 按照skill规范执行后台刷新机制操作,遵循单一意图原则。
**输出**: 返回后台刷新机制的执行结果,包含操作状态和输出数据。

### 标准化输出
- `/status`：返回完整状态JSON，包含所有探测项的结果
- `/health`：返回简单的`{"status":"ok"}`，用于服务自身存活检查

**输入**: 用户提供标准化输出所需的指令和必要参数。
**处理**: 按照skill规范执行标准化输出操作,遵循单一意图原则。
**输出**: 返回标准化输出的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：轻量级、HTTP、服务状态探针、将任意端点的健康、状态暴露为标准化、JSON、支持基础、探活与命令行检查、API、测试运行器免费版、是一款面向独立开、发者的服务状态探、针工具、通过本地、服务将多个端点的、健康状态聚合为标、便于接入仪表盘或、监控告警系统、核心能力、零依赖、Node、Use、when、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求、适用于独立开发者、企业团队和自动化、工作流场景等。
