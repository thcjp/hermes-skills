...
edition: free
description: |-
  通话桥接免费版是一款面向独立开发者的AI电话代理工具，通过语音AI代理拨打美国电话号码，完成通话后返回转写文本、通话结果与录音链接。代理负责拨号、对话、处理电话菜单与等待，并将结果结构化返回。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。适用于独立开发者、企业团队和自动化工作流场景。
tags:
- 集成工具
- 语音通信
- AI代理
...
**处理**: 按照skill规范执行单次外呼操作,遵循单一意图原则。
**输出**: 返回单次外呼的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行通话指令构建操作,遵循单一意图原则。
**输出**: 返回通话指令构建的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行状态轮询操作,遵循单一意图原则。
**输出**: 返回状态轮询的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行API Key管理操作,遵循单一意图原则。
**输出**: 返回API Key管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行持久化状态操作,遵循单一意图原则。
**输出**: 返回持久化状态的执行结果,包含操作状态和输出数据。
