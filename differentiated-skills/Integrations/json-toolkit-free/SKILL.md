...
summary: JSON 数据结构处理最佳实践，含校验、命名、空值、日期、数字规范。
license: MIT
edition: free
description: |-
  JSON 数据结构处理最佳实践，含校验、命名、空值、日期、数字规范。
  ...
  触发关键词：json、schema、校验、序列化、api 响应、数据结构
tags:
- 集成工具
- 数据规范
- 开发者效率
...
本 Skill 为 Agent 提供 JSON 数据结构处理的最佳实践指南。核心理念是"防御式设计"：在处理不可信输入前先校验，在定义 API 响应时先立契约，避免"假设结构正确"的陷阱。免费版覆盖个人开发者高频场景：Schema 校验、命名一致性、空值处理、日期时间、数字 ID、结构最佳实践。
...
### API Key 配置
- **本 Skill 基于指令驱动**：无需额外 API Key，纯 Markdown 最佳实践指南

### 可用性分类
- **分类**：MD（纯 Markdown 指令，无需 exec 命令行执行能力）
- **说明**：基于 Markdown 的 AI Skill，通过自然语言指令驱动 Agent 遵循 JSON 处理最佳实践
