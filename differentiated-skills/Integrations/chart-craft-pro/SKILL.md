---
slug: chart-craft-pro
name: chart-craft-pro
version: "1.0.0"
displayName: 图表工坊专业版
...
企业级图表生成平台，支持12种图表类型、自定义模板、批量生成与多格式导出，覆盖从快速出图到出版级精度的全场景需求。

## 概述

当图表需求从个人快速验证扩展到企业报表生产时，四种基础图表的局限性显现：堆叠柱状图无法表达分层结构、热力图无法展示二维密度、矢量格式无法满足印刷精度、批量出图只能手动逐张执行。专业版针对这些痛点，提供12种图表类型、模板系统与批量生成能力。

专业版向后兼容免费版的所有命令与配置，可直接升级替换。
...
**处理**: 按照skill规范执行自定义模板与主题操作,遵循单一意图原则。
**输出**: 返回自定义模板与主题的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供批量生成与自动化所需的指令和必要参数。
**处理**: 按照skill规范执行批量生成与自动化操作,遵循单一意图原则。
**输出**: 返回批量生成与自动化的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行多格式导出操作,遵循单一意图原则。
**输出**: 返回多格式导出的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行智能推荐引擎升级操作,遵循单一意图原则。
**输出**: 返回智能推荐引擎升级的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供使用统计分析所需的指令和必要参数。
**处理**: 按照skill规范执行使用统计分析操作,遵循单一意图原则。
**输出**: 返回使用统计分析的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级图表生成平、种高级图表类型、多格式导出与使用、图表工坊专业版是、一款面向团队与企、业的图表生成平台、在免费版四种基础、图表基础上、新增堆叠柱状图、热力图等、并提供自定义模板、统计分析等高级能、核心能力、一键应用企业品牌、四种格式、图表历史分析与使等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
本专业版相比免费版新增以下能力：
- 8种高级图表类型：堆叠柱状、面积、雷达、热力图、箱线图、气泡、瀑布、组合
- 自定义模板与主题：6套预置主题+企业品牌模式
- 批量生成与自动化：CSV数据源批量出图，支持断点续传
- 多格式导出：新增SVG、PDF、JSON三种格式
- 智能推荐升级：多维数据组合图表建议
- 使用统计分析：图表生成次数与模板使用频率

## 定价

| 版本 | 价格 | 功能 | 适用场景 |
|------|------|------|----------|
| 免费体验版 | 0元 | 4种基础图表+智能推荐 | 个人试用 |
| 收费专业版 | 19.9元/月 | 12种图表+模板+批量+多格式 | 团队/企业 |
