...
edition: free
description: |-
  面向AI应用开发者的向量数据库管理工具，覆盖向量索引创建、嵌入向量管理、相似度检索与基础性能调优。

  核心能力：创建与删除向量索引、批量导入嵌入向量、执行KNN/ANN相似度检索、管理集合与分区、查询基础统计信息。

  适用场景：语义搜索引擎、推荐系统、文档问答系统、图像检索等AI应用的向量数据管理场景。

  差异化：重新设计中文交互流程，新增多引擎适配层（支持 `PostgreSQL` pgvector/Milvus/ChromaDB），提供索引选择决策树，去除外部依赖。

  触发关键词：向量数据库、vector、嵌入向量、相似度检索、向量索引、embedding
tags:
- 数据库管理
- 向量检索
- AI基础设施
