...
edition: free
description: |-
  JSON解析引擎免费版提供文件解析、字符串解析、嵌套展平、路径提取四项核心能力，帮助开发者将复杂JSON转化为可消费的结构化数据。

  核心能力：支持文件与字符串两种输入源、嵌套JSON展平为单层字典、点分路径提取嵌套元素、必填字段校验与完整度评分、记录计数与错误报告。

  适用场景：API响应解析、配置文件读取、数据清洗预处理、日志结构化、教学示例演示。

  差异化：相比通用JSON.parse，本工具内置嵌套展平算法（保留数组索引）、点分路径提取（支持数组下标）、必填字段校验（含完整度百分比），输出结构化的解析结果对象，便于下游消费；中文化指令与示例贴合国内开发者。

  触发关键词：JSON解析、嵌套展平、路径提取、字段校验、flatten、parse
tags:
- 集成工具
- JSON
- 解析
