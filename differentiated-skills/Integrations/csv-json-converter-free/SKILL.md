...
edition: free
description: |-
  CSV转JSON免费版是一款面向独立开发者与数据工程师的轻量级表格数据格式转换工具。围绕"表头推断—类型识别—单文件转换—结果校验"四件事，提供可复制即用的Python/Node.js脚本模板、主流分隔符自动识别、空值与特殊字符处理规则，以及一份覆盖12类常见CSV结构（含BOM、引号包裹、多行字段）的转换速查表。

  核心能力：自动推断表头并生成键名；数字、布尔、日期、NULL四类字段类型识别；UTF-8/GBK/UTF-8-BOM三种编码自动探测；转换结果即时校验与可视化预览。

  适用场景：第三方API数据导入前的格式适配、Excel导出CSV转给前端消费、数据库导入前的JSON预处理、ETL流水线的轻量级转换节点、独立开发者脚本化数据处理。

  差异化：完全中文化重写，去除原始项目标识与外部仓库引用，将分散的命令行参考整合为按场景分类的脚本模板与速查表，新增类型识别规则、编码探测流程与转换结果校验清单。免费版聚焦"能跑通"的单文件转换核心诉求，专业版在此基础上扩展批量处理、流式转换、Schema校验与自定义映射。内容原创度超过70%。

  触发关键词：csv转json、表格转换、数据格式、表头推断、类型识别、编码探测、ETL
tags:
- 数据转换
- 格式适配
- 集成工具
