...
**输入**: 用户提供能力1：批量并行校验所需的指令和必要参数。
**处理**: 按照skill规范执行能力1：批量并行校验操作,遵循单一意图原则。
**输出**: 返回能力1：批量并行校验的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力3：自动修复所需的指令和必要参数。
**处理**: 按照skill规范执行能力3：自动修复操作,遵循单一意图原则。
...
**输入**: 用户提供能力4：流式校验所需的指令和必要参数。
**处理**: 按照skill规范执行能力4：流式校验操作,遵循单一意图原则。
...
**输入**: 用户提供能力6：CI/CD集成所需的指令和必要参数。
**处理**: 按照skill规范执行能力6：CI/CD集成操作,遵循单一意图原则。
**输出**: 返回能力6：CI/CD集成的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、校验工具、支持批量校验、自动修复与、校验器专业版是一、款面向数据团队与、企业级数据质量保、障场景的全功能、在免费版的语法检、错误定位、兼容解析基础上、新增批量校验、持续监控与、集成六大高级能力、覆盖从单文件到企、业级数据质量流水、线的全场景需求、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
