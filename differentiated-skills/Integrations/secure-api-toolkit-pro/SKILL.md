...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级凭据代理方、团队金库、自定义提供商与合、工具箱、为团队与企业提供、完整的凭据代理治、理方案、从架构层杜绝、Agent、接触真实密钥、核心能力、团队凭据金库与共、全量审计日志与不、自定义私有、提供商注册、凭据隔离与提供商等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
try {
  const res = await secureFetch('https://api.user/repos', {
    headers: { Authorization: 'Bearer {{OAUTH2_ACCESS_TOKEN}}' },
