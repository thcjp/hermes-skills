...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Agent、的轻量、全功能专业版、含连接池监控、自动备份、分析集成与高并发、与专业开发者的轻、数据库全功能专业、在免费版基础上新、增连接池监控、自动定时备份、分析引擎集成、高并发写入优化、增量备份与断点恢、复等高级能力、配套面向运维、数据工程师、架构师的多角色场、景指南、Use、when、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
