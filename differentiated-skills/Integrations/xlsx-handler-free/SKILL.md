...
edition: free
description: |-
  XLSX 处理免费版面向需要程序化读写 Excel 工作簿的开发者与运维人员，提供"按任务选工具"的工作流——区分 pandas 与 openpyxl 适用场景，避免公式丢失、日期损坏、合并单元格错乱等常见陷阱。所有操作在交付前进行校验，确保零公式错误。

  核心能力：

  - 按任务选择 pandas / openpyxl 路径
  - 保留公式、样式、合并单元格、定义名称
  - 处理日期序列号与 1900 / 1904 系统
  - 类型保护（长数字、电话、邮编存为文本）
  - 工作簿结构保护（不破坏既有模板）
  - 交付前校验（零 #REF! / #DIV/0! / #VALUE!）
  ...
  差异化：相比通用 Excel 库教程，本 Skill 强调"保护优先"——先理解既有工作簿结构再修改，避免常见陷阱（公式被静默覆盖、日期被解析错误、长 ID 被截断）。免费版覆盖单文件编辑与检查场景。

  触发关键词：XLSX 读写、openpyxl、pandas、公式保留、日期处理、工作簿结构、合并单元格
tags:
- 集成工具
- 表格处理
- 开发者工具
