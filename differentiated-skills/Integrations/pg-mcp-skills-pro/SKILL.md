...
edition: pro
description: |-
  PG-MCP助手专业版是一套面向中大型团队与企业级场景的 `PostgreSQL` 数据库管理解决方案，在免费版基础上扩展出自动化性能调优、智能索引推荐、数据库迁移与版本升级、多实例统一管理、高可用与故障转移监控等能力。

  核心能力：提供基于执行计划的自动索引推荐、冗余索引清理流程、大版本升级迁移方案、多实例配置中心化管 理、主备切换与故障转移监控、慢查询自动归档与分析报告。所有方案均附带配置示例、风险评估与回滚预案。

  适用场景：生产数据库的深度性能优化、跨版本升级迁移、多团队共享数据库平台、金融级高可用部署、合规审计与性能报告生成。

  差异化：以企业级场景化表达重构数据库管理知识体系，将零散的运维经验整理为"场景-诊断-方案-验证-监控"五段式结构，每个方案均附带风险评估与回滚预案。相比免费版新增 6 项独有高级能力。

  触发关键词：`PostgreSQL`、MCP工具、自动化调优、索引推荐、版本升级、多实例管理、高可用、故障转移
tags:
- 数据库
- 集成工具
- MCP工具
