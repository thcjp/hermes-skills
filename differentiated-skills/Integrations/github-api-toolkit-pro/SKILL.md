...
edition: pro
description: |-
  GitHub API工具包(专业版)是企业级GitHub API集成工具,在免费版REST基础能力上,扩展GraphQL查询、批量操作、Webhook管理、GitHub Actions API、组织与团队管理等高级能力。
  ...
  - 高级搜索: 复杂查询语法、跨仓库聚合、结果导出

  适用场景:
  - 企业级GitHub集成与自动化
  - 大规模仓库群的批量管理
  - DevOps平台与GitHub的深度集成
  - 组织级权限与合规治理
  - 数据分析与BI报表的数据源

  差异化: 相比免费版,新增GraphQL与批量能力,适合需要深度集成的企业级场景。

  触发关键词: GitHub API、GraphQL、批量操作、Webhook、Actions API、组织管理、企业集成、高级搜索
tags:
- GitHub
- API
- 企业集成
...
# 搜索代码(跨仓库)
curl -H "Authorization: Bearer $GITHUB_TOKEN" \
  "https://api.github.com/search/code?q=TODO+language:python+org:my-org"
...
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent执行任务,核心功能通过gh-api-toolkit CLI或直接API调用
