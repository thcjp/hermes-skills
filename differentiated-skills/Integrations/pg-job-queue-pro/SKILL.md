...
**输入**: 用户提供能力二：多队列水平分片所需的指令和必要参数。
**处理**: 按照skill规范执行能力二：多队列水平分片操作,遵循单一意图原则。
**输出**: 返回能力二：多队列水平分片的执行结果,包含操作状态和输出数据。

### 能力三：死信队列与人工补偿

重试 `max_attempts` 次后仍失败的任务进入死信队列，由人工审核后决定重试、丢弃或修复。本助手提供死信队列表设计、告警通知方案、补偿流程模板。

**输入**: 用户提供能力三：死信队列与人工补偿所需的指令和必要参数。
**处理**: 按照skill规范执行能力三：死信队列与人工补偿操作,遵循单一意图原则。
**输出**: 返回能力三：死信队列与人工补偿的执行结果,包含操作状态和输出数据。

### 能力四：高可用与故障转移

队列服务自身也需要高可用。本助手提供主备 Worker 部署方案、数据库主备切换流程、Worker 健康检查与自动重启机制。

**输入**: 用户提供能力四：高可用与故障转移所需的指令和必要参数。
**处理**: 按照skill规范执行能力四：高可用与故障转移操作,遵循单一意图原则。
**输出**: 返回能力四：高可用与故障转移的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力五：灰度发布与版本回滚所需的指令和必要参数。
**处理**: 按照skill规范执行能力五：灰度发布与版本回滚操作,遵循单一意图原则。
**输出**: 返回能力五：灰度发布与版本回滚的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供能力六：高级监控告警体系所需的指令和必要参数。
**处理**: 按照skill规范执行能力六：高级监控告警体系操作,遵循单一意图原则。
**输出**: 返回能力六：高级监控告警体系的执行结果,包含操作状态和输出数据。

- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：PostgreSQL、的企业级任务队列、分片扩展、死信队列与高可用、任务队列专业版是、一套面向中大型团、队与企业级场景的、任务队列解决方案、在免费版基础上扩、展出任务依赖图编、多队列分片、高可用部署、灰度发布与高级监、控等能力、核心能力、任务编排模型、水平分片方案、主备切换与故障转、移设计、滚策略、指标体系与告警规等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
