...
edition: free
description: |-
  面向MongoDB Atlas运维者的Admin API浏览与查询工具。通过命令行检索50+API分类、查看端点定义、获取Schema详情，免凭证即可作为只读文档浏览器使用，配置凭证后可执行实时API调用。

  核心能力：
  - API目录浏览：按关键字检索50+API分类
  - 端点详情查看：method、path、params、requestBody完整定义
  - Schema定义获取：复杂类型的数据模型查询
  - 只读API调用：列出集群、查询备份、查看监控等无副作用操作
  - 安全协议：状态变更操作必须dry-run + 用户确认
  ...
  差异化：原创中文使用指南，按角色拆分场景，覆盖安全协议、dry-run流程、只读与状态变更操作区分，配套常见调用问题排查。

  触发关键词：atlas、mongodb atlas、admin api、集群管理、api浏览
tags:
- 集成工具
- MongoDB
- 云数据库
