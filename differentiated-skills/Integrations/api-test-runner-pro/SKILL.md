...
edition: pro
description: |-
  API测试运行器专业版是一款面向团队与企业的服务状态聚合平台，在免费版HTTP探活与命令行检查基础上，新增文件存在性检查、Docker容器健康监控、开发服务器自动发现、邮件未读数统计、定时任务状态读取与系统资源指标采集等高级能力。

  核心能力：
  - 六种探测类型：HTTP、命令行、文件存在性、Docker容器、开发服务器、系统资源
  - Docker容器健康检查，通过Portainer API获取容器状态
  - 开发服务器自动发现，基于进程名grep识别运行中的服务
  - 邮件未读数统计，支持himalaya等多种邮件客户端
  - 定时任务状态读取，直接解析cron任务配置文件
  - 系统资源指标采集（CPU、内存、磁盘），从/proc读取
  - systemd服务化部署模板，支持开机自启与崩溃恢复
  - 多实例聚合与仪表盘集成，适配团队级监控需求

  适用场景：
  - 团队需要统一监控多个服务器与容器化服务
  - 运维需要将服务状态集成到现有仪表盘系统
  - 开发团队需要自动发现并监控本地开发服务器

  差异化：相比免费版新增6种探测类型与仪表盘集成能力，提供完整故障排查表与多角色场景指南。

  触发关键词：服务监控、Docker健康、状态聚合、仪表盘集成、多机监控
tags:
- 集成工具
- 状态监控
- 企业监控
