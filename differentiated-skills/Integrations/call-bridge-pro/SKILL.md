...
**处理**: 按照skill规范执行并行外呼与通话活动操作,遵循单一意图原则。
**输出**: 返回并行外呼与通话活动的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供实时转接（live handoff）所需的指令和必要参数。
**处理**: 按照skill规范执行实时转接（live handoff）操作,遵循单一意图原则。
**输出**: 返回实时转接（live handoff）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供呼入号码配置所需的指令和必要参数。
**处理**: 按照skill规范执行呼入号码配置操作,遵循单一意图原则。
**输出**: 返回呼入号码配置的执行结果,包含操作状态和输出数据。
...
**输出**: 返回个性化语音配置的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、电话代理平台、支持并行外呼、通话活动管理与个、通话桥接专业版是、一款面向团队与企、在免费版单次外呼、基础上、新增并行外呼、通话活动管理、个性化语音配置与、完整错误处理等高、级能力、核心能力、同时拨打、通电话进行信息比、设置来电应答规则、与转接策略、跨通话保持状态等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
**处理**: 按照skill规范执行外呼前侦察操作,遵循单一意图原则。
