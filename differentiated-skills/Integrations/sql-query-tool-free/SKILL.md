...
edition: free
description: |-
  面向独立开发者与AI Agent的SQL查询执行工具免费版。聚焦命令行场景下的关系型数据库查询、参数化执行、执行计划分析与跨数据库可移植性，提供经过实战检验的查询模式、索引陷阱清单与EXPLAIN解读方法，帮助用户在不依赖重量级ORM的前提下高效完成数据访问任务。

  核心能力：
  - 一键连接SQLite、`PostgreSQL`、MySQL、SQL Server四大主流数据库
  - 内置参数化查询、事务包裹、EXPLAIN执行计划分析等安全查询模式
  - 提供NULL陷阱、索引失效、JOIN误用等高频踩坑清单
  - 支持LIMIT、UPSERT、布尔类型等跨数据库语法差异速查

  适用场景：
  - 独立开发者本地数据探查与临时查询
  - AI Agent在执行工作流中对关系型数据的读写
  - 自动化脚本中的结构化数据访问与导出

  差异化：原创中文使用指南，按角色拆分场景示例，配套跨数据库可移植性对照表与执行计划红旗清单，原创度>70%。

  触发关键词：sql查询、数据库查询、执行计划、参数化、EXPLAIN、跨数据库
tags:
- 集成工具
- 数据库
- SQL查询
