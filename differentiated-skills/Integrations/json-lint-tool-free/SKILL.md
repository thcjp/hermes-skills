...
edition: free
description: |-
  JSON校验工具免费版提供工作区级别的JSON语法批量校验能力，帮助开发者快速发现配置文件、数据文件中的语法错误。

  核心能力：递归扫描指定目录的所有.json文件、逐文件语法校验、错误定位（文件路径+错误位置+错误信息）、结构化报告输出、通过率统计。

  适用场景：项目提交前校验、配置文件巡检、依赖文件检查、数据文件验收、教学作业批改。

  差异化：相比单文件校验，本工具支持目录级递归扫描，一次校验整个项目的所有JSON文件；输出结构化报告（含时间戳、文件计数、通过率），便于集成到CI/CD或生成检查报告；中文化错误信息贴合国内开发者。

  触发关键词：JSON校验、JSON语法检查、lint、workspace扫描、配置文件检查
tags:
- 集成工具
- JSON
- 校验
