...
edition: free
description: |-
  数据格式转换免费版面向需要在不同数据格式间快速转换的开发者与数据工作者,提供 CSV、JSON、XML、YAML、TOML 等主流格式的互转能力。
  ...
  适用场景:
  - 配置文件迁移(如 YAML → TOML)
  - API 数据格式适配(如 JSON → CSV)
  - 数据交换格式互转(如 XML → JSON)
  - 数据库导出文件加工

  差异化:强调"单文件快速转换"与"嵌套结构保留",降低格式转换的常见痛点(数据丢失、类型错乱、编码乱码)。

  触发关键词:格式转换、格式互转、csv to json、json to yaml、xml to json、toml
tags:
  - 格式转换
  - 数据交换
  - 配置文件
