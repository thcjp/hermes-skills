...
edition: free
description: |-
  Jira Flow Skill 是面向个人开发者与小型团队的 Jira 任务管理辅助工具，通过命令行脚本驱动 Jira Cloud REST API 完成日常事务流转。

  核心能力：任务检索与详情查看、状态流转与分配、工时记录与统计、评论与创建任务，覆盖一名开发者日常所需的 Jira 操作。

  适用场景：个人 Sprint 任务跟进、工时周报自动汇总、缺陷快速登记、跨项目任务检索、状态批量校验。

  差异化：相比通用 Jira 客户端，本 Skill 聚焦"命令行即工作流"的体验，所有操作可通过单条命令完成；内置状态流转校验机制，避免无效流转；工时统计直接输出 JSON，便于二次加工。

  触发关键词：jira、任务流转、工时记录、sprint、issue、worklog、状态变更
tags:
- 集成工具
- 任务管理
- 效率提升
