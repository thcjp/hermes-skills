...
edition: free
description: |-
  Python数据分析免费版是一套面向独立开发者与初级数据分析师的 Python 数据处理知识库，帮助用户在日常工作中快速完成数据清洗、统计分析与可视化建议。

  核心能力：提供缺失值处理、异常值识别、数据类型转换等清洗模式；描述性统计、相关性分析、分组聚合等分析方法；折线图、柱状图、散点图、热力图等可视化建议。所有模式均以可运行的 pandas 代码片段呈现。

  适用场景：业务报表的快速生成、销售数据的趋势分析、用户行为数据的探索性分析、科研数据的预处理与统计。

  差异化：以中文场景化表达重构数据分析知识体系，将零散的处理技巧整理为"数据理解-清洗-分析-可视化-结论"五段式流程，每个模式均附带典型业务场景与代码示例。

  触发关键词：Python数据分析、数据清洗、统计分析、可视化、pandas、业务报表、探索性分析
tags:
- 数据分析
- 集成工具
- Python
...
| 症状 | 可能原因 | 排查方法 | 对策 |
|:-----|:---------|:---------|:-----|
| 读取 CSV 报错 | 编码问题 | 尝试 encoding='gbk' | 指定正确编码 |
| 数值列被识别为字符串 | 千分位逗号或特殊符号 | 查看 dtype | 用 astype 转换 |
| 分组聚合结果异常 | 分组键存在空值 | 检查分组列缺失 | dropna 或填充 |
| 图表中文乱码 | 字体不支持中文 | 设置中文字体 | plt.rcParams 设置 |
| 内存不足 | 数据量过大 | 查看 df.memory_usage | 分块读取或降精度 |
