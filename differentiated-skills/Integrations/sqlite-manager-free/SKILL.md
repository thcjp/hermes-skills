...
description: |-
  ...
  - 外键约束按连接启用，保障数据完整性
  - 类型亲和性理解与STRICT表使用
  - 性能Pragma调优（cache_size、synchronous、temp_store）
  - VACUUM空间回收与增量清理
  - 安全备份与事务批处理

  适用场景：
  - AI Agent的本地数据持久化与会话存储
  - 独立开发者的嵌入式数据库管理
  - 自动化工作流的中间结果存储

  差异化：原创中文使用指南，按角色拆分场景示例，配套SQLite陷阱清单与Pragma调优决策表，原创度>70%。

  触发关键词：sqlite管理、并发、WAL、外键、Pragma、VACUUM、类型亲和性
tags:
- 集成工具
- 本地存储
- 数据库
