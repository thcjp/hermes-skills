...
edition: free
description: |-
  微云工具箱（免费版）面向个人用户与独立开发者，封装微云网盘的日常文件操作：目录浏览、文件下载、文件上传。通过 MCP工具协议与微云网盘交互，所有文件哈希计算在本地完成，避免敏感数据外泄。

  核心能力：
  - 目录列表查询与分页导航
  - 文件 HTTPS 批量下载（含 cookie 携带说明）
  - 文件上传两阶段协议（预上传 + 分片上传），支持秒传
  - 一键上传脚本，自动处理参数计算与重试
  - 内置 7 类高频错误码与处理建议
  ...
  差异化：以"工作流驱动"组织内容，每条工作流附带完整调用链与错误恢复策略，原创内容占比超过 70%。免费版覆盖 3 类核心操作与基础错误码，专业版额外提供分享外链、批量删除、重命名、移动、分类查询等 9 类操作。

  触发关键词：微云、网盘、上传、下载、目录列表、文件管理
tags:
- 云存储
- 微云
- 文件管理
