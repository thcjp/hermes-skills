---
slug: format-converter-pro
name: format-converter-pro
version: "1.0.0"
...
数据格式转换专业版在免费版的快速转换能力之上,补齐"批量处理、流式转换、字段映射、自动化调度"四大短板,形成完整的数据格式转换生产线。面向专业开发者、数据工程师与运维团队,适合需要在大规模数据与多源系统间进行格式转换的团队场景。

专业版支持目录级批量转换、GB 级文件流式处理、自定义字段映射、转换模板复用、定时任务调度,以及与企业数据库的双向导入导出。
...
**处理**: 按照skill规范执行2 批量转换操作,遵循单一意图原则。
...
**输入**: 用户提供3 流式转换所需的指令和必要参数。
**处理**: 按照skill规范执行3 流式转换操作,遵循单一意图原则。
**输出**: 返回3 流式转换的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供4 自定义字段映射所需的指令和必要参数。
**处理**: 按照skill规范执行4 自定义字段映射操作,遵循单一意图原则。
**输出**: 返回4 自定义字段映射的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供5 转换模板保存与复用所需的指令和必要参数。
**处理**: 按照skill规范执行5 转换模板保存与复用操作,遵循单一意图原则。
**输出**: 返回5 转换模板保存与复用的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供6 定时转换任务所需的指令和必要参数。
**处理**: 按照skill规范执行6 定时转换任务操作,遵循单一意图原则。
**输出**: 返回6 定时转换任务的执行结果,包含操作状态和输出数据。
...
**输出**: 返回7 企业数据库导入导出的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供8 验证与对账所需的指令和必要参数。
**处理**: 按照skill规范执行8 验证与对账操作,遵循单一意图原则。
**输出**: 返回8 验证与对账的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能数据格式转、换平台、支持批量、自定义映射、定时任务与企业数、数据格式转换专业、版面向需要在大规、模数据与多源系统、间进行格式转换的、专业开发者与数据、工程师、提供完整的批量、自动化转换能力、核心能力、涵盖免费版全部能、无文件大小与数量、目录级批量处理、支持通配符与递归、大文件分块读取与、常见场景一键复用、自动监听与转换等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
11. 数据源: 凭证从环境变量读取
12. 表: sales(2024 年全量)
13. 输出: sales_2024.parquet
14. 分区: 按月分区(partition=YYYY-MM)
15. 压缩: snappy
```
...
16. 重命名: user_name → username
17. 过滤: 排除 internal_id、debug_flag
18. 合并: first_name + last_name → full_name
19. 类型: age 转 int,price 转 float
```
...
20. 监听 ./input/ 目录
21. 触发: 新增 *.json 文件
22. 转换: JSON → CSV(应用模板 yaml_config)
23. 输出: ./output/{原文件名}.csv
24. 校验: 行数一致性
25. 失败: 重试 3 次,仍失败则告警
```
...
stream:
  ...
  output_file: large.csv
  chunk_size: 100000
  format:
    from: json
    to: csv
  nested: flatten    # 嵌套字段以点号展开
  memory_limit: 4GB
  resume: true       # 支持断点续转
```
...
- **校验一致性**:转换后必须校验行数与字段
- **流式优先**:大文件优先流式,避免内存溢出
- **模板复用**:常见场景保存为模板,提升复用效率
- **字段映射**:重命名/过滤/合并等操作集中配置
- **并行控制**:批量转换并行度建议 4-8,避免 IO 瓶颈
- **失败隔离**:单文件失败不影响整批,记录日志后继续
- **审计留痕**:任务执行历史完整保留,便于追溯
- **凭证安全**:数据库密码等一律走环境变量
...
- 批量转换:目录级处理、通配符、递归、并行执行、失败隔离
- 流式转换:分块读取、流式输出、断点续传、内存监控
- 自定义字段映射:重命名、过滤、合并、拆分、类型转换
- 转换模板保存与复用:参数化模板、版本管理、团队共享
- 定时转换任务:cron 调度、文件监听、任务依赖、失败重试
- 企业数据库导入导出:`PostgreSQL`/MySQL/ClickHouse
- 验证与对账:行数校验、字段校验、哈希校验、差异报告
- 转换历史与审计日志:完整留痕
...
| 版本 | 价格 | 功能 | 适用场景 |
|------|------|------|----------|
| 免费体验版 | ¥0 | 单文件 + 主流格式 + 类型智能 | 个人试用 |
| 收费专业版 | ¥99/月 | 全功能 + 批量 + 流式 + 自动化 + 企业集成 | 团队/企业 |
