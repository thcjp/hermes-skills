...
license: MIT
edition: free
description: "API网关管控器免费版解决中小团队\"网关配置散乱、限流靠猜、认证各自达成\"的痛点。包含声明式路由配置、统一认证接入、基础速率限制、实时监控看板四大能力，兼容以YAML或JSON声明式定义网关规则，自发产出主流网关（Kong/APISIX/Nginx/Envoy）的配置文件。Use. 适用于需要gateway manager相关能力的开发场景,提供结构化的工作流程和配置指引."
tags:
  - API网关
  - 路由配置
  - 限流
