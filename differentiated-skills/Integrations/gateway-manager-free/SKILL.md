...
edition: free
description: |-
  API网关管理器免费版解决中小团队"网关配置散乱、限流靠猜、认证各自实现"的痛点。提供声明式路由配置、统一认证接入、基础速率限制、实时监控看板四大能力，支持以YAML或JSON声明式定义网关规则，自动生成主流网关（Kong/APISIX/Nginx/Envoy）的配置文件。内置网关选型决策矩阵与常见路由模式速查。

  核心能力：声明式路由（路径/方法/Header多维度匹配）；统一认证代理（API Key/JWT/OAuth2三种模式）；基础限流（固定窗口与滑动窗口）；监控指标采集（QPS/延迟/错误率）；多网关配置生成（Kong/APISIX/Nginx/Envoy）。

  适用场景：中小团队API统一入口、微服务网关起步配置、老项目Nginx反代规范化、限流策略快速验证、网关选型对比。

  差异化：完全中文化重写，去除原始项目标识与外部服务URL引用，把单一厂商CLI文档重构为面向通用网关场景的配置管理器。新增网关选型决策矩阵、声明式路由DSL、多网关配置生成与常见路由模式速查。免费版聚焦"能管住路由与限流"，专业版扩展多租户、熔断、灰度、可观测性与插件体系。内容原创度超过70%。

  触发关键词：API网关、路由配置、限流、统一认证、Kong、APISIX、Nginx、Envoy、反向代理
tags:
- API网关
- 路由配置
- 限流
