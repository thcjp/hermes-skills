...
edition: pro
description: |-
  面向数据工程团队与安全测试人员的全功能去中心化SOCKS5代理平台，支持多代理并发管理、自动轮换、请求熔断与性能监控。

  核心能力：在免费版基础上新增多代理会话池、自动轮换策略、请求重试与熔断机制、代理性能监控仪表盘、x402按需支付流程与批量请求引擎。

  适用场景：大规模数据采集、分布式压力测试、多地区合规审计、广告验证、价格监控、SEO排名追踪等企业级网络工程场景。

  差异化：相比免费版新增6项独有能力，提供完整的多角色场景指南（数据工程师/安全测试/运维/产品），内置性能基准与成本优化策略。

  触发关键词：代理、proxy、socks5、自动轮换、熔断、并发代理、性能监控、x402支付
tags:
- 网络工具
- 代理服务
- 企业级网络
...
| 问题现象 | 可能原因 | 排查步骤 | 优先级 |
|:---------|:---------|:---------|:-------|
| 所有代理请求超时 | 代理网络服务异常 | 检查健康状态接口，等待恢复 | P0 |
| 大量403错误 | 代理IP被目标封锁 | 切换住宅节点，降低请求频率 | P0 |
| 额度消耗异常快 | 代理未及时释放 | 检查代理池回收逻辑，设置自动过期 | P1 |
| 熔断器频繁触发 | 代理质量不稳定 | 调整熔断阈值，启用代理质量评分 | P1 |
| 轮换失败 | 新代理生成失败 | 检查API密钥有效性，查看额度余额 | P1 |
| 监控数据缺失 | 指标采集失败 | 检查MetricsCollector连接，验证Redis | P2 |
