...
edition: free
description: |-
  Notion API工具箱(免费版)是面向个人开发者与知识工作者的轻量化Notion集成Skill,通过托管OAuth与REST API的组合,帮助用户在数分钟内接入Notion工作空间。
  ...
  差异化:相比直接调用Notion API,本Skill通过托管OAuth屏蔽鉴权复杂度,使开发者专注于业务逻辑。免费版聚焦查询与基础读写,适合个人与小型团队试用。

  触发关键词:Notion、API、OAuth、页面查询、数据库检索、块管理、工作空间
tags:
- 集成工具
- Notion
- 生产力
...
| 429 Rate limited | 触发频率限制 | 等待1秒重试,或升级专业版 |
| 写操作失败 | 用户未确认 | Agent明确询问用户后重试 |
