...
edition: free
description: |-
  Azure网关CLI免费版是一款面向独立开发者的本地代理工具，用于解决OpenAI兼容客户端与Azure OpenAI服务之间的协议适配问题。Azure OpenAI使用专属的URL路径与查询参数格式，与标准OpenAI API存在差异，直接对接往往导致请求失败或参数丢失。

  核心能力：
  - 本地Node.js代理服务，零外部依赖，开箱即用
  - 自动将标准OpenAI格式的请求转换为Azure OpenAI所需的部署级URL
  - 支持环境变量配置端点、部署名、API版本
  - 内置健康检查端点，便于集成到监控体系
  - 支持systemd服务化部署，实现开机自启与异常恢复

  适用场景：
  - 个人开发者希望在本地Agent平台中复用Azure OpenAI额度
  - 自动化脚本需要通过统一接口调用不同模型供应商
  - 学习与原型验证阶段需要快速搭建测试代理

  差异化：完全重构协议适配逻辑，采用中文化文档与场景化指南，新增故障排查矩阵与配置校验机制，适配多平台Agent环境。

  触发关键词：Azure代理、OpenAI转发、本地网关、API适配、模型路由
tags:
- 集成工具
- API网关
- 本地代理
