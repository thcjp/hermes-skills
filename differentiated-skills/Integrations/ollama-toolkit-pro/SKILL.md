...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能本地、Ollama、模型管理工具、支持多轮对话、批量推理与、工具箱专业版是面、向团队和高级用户、的完整本地、模型管理方案、在免费版基础上解、锁多轮对话会话管、自定义模型创建、批量文件推理处理、性能资源监控和模、型对比评估等全部、高级能力、核心能力、多轮上下文对话、自定义模型构建与、批量文件推理与结、服务集成、内存实时监控、多模型横向对比评、提示词模板管理、流式输出处理等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
