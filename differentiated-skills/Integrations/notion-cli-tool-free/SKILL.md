...
edition: free
description: |-
  Notion命令行(免费版)是面向个人开发者与知识工作者的轻量化Notion操作Skill,通过命令行工具的组合,帮助用户从终端高效完成Notion工作空间的日常操作。
  ...
  差异化:相比直接调用Notion API,本Skill通过别名机制屏蔽UUID复杂度,使操作更自然。免费版聚焦单工作空间与基础操作,适合个人与小型团队试用。

  触发关键词:Notion、命令行、CLI、数据库查询、页面管理、别名、终端操作
tags:
- 集成工具
- Notion
- 命令行
