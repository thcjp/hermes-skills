...
edition: free
description: |-
  API文档生成器免费版解决"接口文档写不动、写了不更新、新人看不懂"三大痛点。从代码注释、接口签名或自然语言描述出发，自动生成符合OpenAPI 3.0规范的YAML/JSON文档，并同步产出一份人类可读的Markdown接口手册。内置RESTful设计规范校验、命名建议与状态码模板。

  核心能力：自然语言→OpenAPI 3.0 YAML转换；Markdown接口手册生成（含请求/响应/错误码/变更记录）；RESTful命名规范校验与修正建议；HTTP状态码与业务错误码模板库；接口概览表自动汇总。

  适用场景：新项目接口文档起步、老项目接口反推文档、个人项目快速出文档、联调期接口说明速写、接口评审材料准备。

  差异化：完全中文化重写，去除原始项目标识与外部仓库引用，把模板填充式工具升级为"自然语言→规范文档"的智能生成器。新增RESTful规范校验、状态码模板库与接口概览自动汇总。免费版聚焦"能出文档"，专业版扩展多格式导出、版本管理、Mock联动与团队评审。内容原创度超过70%。

  触发关键词：API文档生成、OpenAPI、Swagger、接口文档、RESTful文档、接口手册、文档自动化
tags:
- API文档
- OpenAPI
- 接口规范
...
|------|--------|------|----------|
| 2xx | 200 | 成功 | GET查询成功 |
| 2xx | 201 | 创建成功 | POST创建资源成功 |
| 2xx | 204 | 无内容 | DELETE删除成功 |
| 4xx | 400 | 参数错误 | 请求体格式错/缺必填 |
| 4xx | 401 | 未授权 | Token缺失/过期 |
| 4xx | 403 | 禁止访问 | 权限不足 |
| 4xx | 404 | 不存在 | 资源ID无效 |
| 4xx | 409 | 冲突 | 资源已存在/状态冲突 |
| 4xx | 422 | 业务校验失败 | 字段格式对但业务不通过 |
| 4xx | 429 | 限流 | 触发速率限制 |
| 5xx | 500 | 服务器错误 | 上游异常 |
| 5xx | 503 | 服务不可用 | 维护中/降级 |
