...
edition: free
description: |-
  CSV Processor 是面向数据工程师的 CSV 清洗与预处理工具，自动检测文件编码与分隔符，提供读取、清洗、合并、拆分与类型转换能力。

  核心能力：编码自动检测（chardet）、分隔符自动嗅探（逗号/分号/制表符/管道）、列名规范化、空行空列清理、智能类型转换、多文件合并、按列值拆分、导出带 BOM 的 UTF-8 CSV。

  适用场景：建筑软件导出数据清洗、ERP 系统数据预处理、多源 CSV 合并、按维度拆分大文件、编码乱码修复、列名规范化。

  差异化：相比通用 CSV 库，本 Skill 针对行业软件导出场景优化，内置编码检测与分隔符嗅探的自动组合策略；列名规范化规则可配置；支持按列值拆分大文件为多个子文件，便于分批处理。

  触发关键词：csv清洗、编码检测、分隔符、列名规范、类型转换、csv合并、csv拆分、数据预处理
tags:
- 集成工具
- 数据处理
- 数据工程
