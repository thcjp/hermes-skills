...
**处理**: 按照skill规范执行2 大规模批量操作操作,遵循单一意图原则。
...
**输出**: 返回3 分区表与表空间的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供4 存储过程与触发器所需的指令和必要参数。
**处理**: 按照skill规范执行4 存储过程与触发器操作,遵循单一意图原则。
...
**输入**: 用户提供5 查询性能基线所需的指令和必要参数。
**处理**: 按照skill规范执行5 查询性能基线操作,遵循单一意图原则。
...
**输入**: 用户提供8 Schema 迁移版本管理所需的指令和必要参数。
**处理**: 按照skill规范执行8 Schema 迁移版本管理操作,遵循单一意图原则。
...
**输入**: 用户提供9 跨数据库同步所需的指令和必要参数。
**处理**: 按照skill规范执行9 跨数据库同步操作,遵循单一意图原则。
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：全功能数据库管理、支持大规模批量操、定时备份与多实例、数据库管理台专业、版面向专业、DBA、后端架构师与运维、提供完整的数据库、性能优化与运维自、动化能力、核心能力、涵盖免费版全部能、无单表大小与操作、规模限制、事务优化与检查点、列表分区、空间管理、自动化业务逻辑封、性能回归检测等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
