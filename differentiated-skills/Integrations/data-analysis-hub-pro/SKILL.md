...
edition: pro
description: |-
  数据分析中枢专业版面向数据分析师、增长负责人、产品负责人与 BI 团队,提供完整的方法论纪律、批量处理能力与团队协同机制。
  ...
  - 定时分析任务、阈值告警与多渠道通知(邮件/IM/Webhook)
  - 自定义报告模板与品牌化输出(HTML/PDF/PPT)
  ...
  差异化:相比免费版,专业版补齐"批处理、协同、自动化"三大能力短板,形成完整的数据分析生产线。

  触发关键词:批量分析、多重比较校正、贝叶斯估计、定时报告、协同审阅、异动告警、决策分析
tags:
  - 数据分析
  - 批量处理
  - 协同审阅
...
```text
请对这份销售数据做归因分析:
- 数据源: postgres 连接(凭证从环境变量读取)
- 表: sales_fact(2024-01-01 至 2024-12-31)
- 关注问题: 华东销售额下降 15% 的根因
- 输出: HTML 报告 + 结论卡片
```
...
```text
请编排批量分析任务:
- 任务1: 对 10 个 A/B 实验做结论判定(并行)
- 任务2: 对结论做 BH 多重比较校正(依赖任务1)
- 任务3: 生成汇总 PDF 报告(依赖任务2)
- 检查点: 每个任务完成后落盘
- 失败策略: 单任务失败重试 3 次,仍失败则跳过并告警
```
