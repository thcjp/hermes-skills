...
**处理**: 按照skill规范执行批量Logo生成操作,遵循单一意图原则。
**输出**: 返回批量Logo生成的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自动矢量化操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行品牌变体管理操作,遵循单一意图原则。
**输出**: 返回品牌变体管理的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
