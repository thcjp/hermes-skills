...
**处理**: 解析优先级分层规则体系的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回优先级分层规则体系的响应数据,包含状态码、结果和日志.
### 2. 基础设计域搜索

## 输入格式
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | UI/UX ProMax V2免费版处理的输入数据或指令 |
...
**处理**: 解析设计系统生成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回设计系统生成的响应数据,包含状态码、结果和日志.
### 4. 默认技术栈指引
