...
**输入**: 用户提供函数调用（Function Tools）所需的指令和必要参数。
**处理**: 按照skill规范执行函数调用（Function Tools）操作,遵循单一意图原则。
**输出**: 返回函数调用（Function Tools）的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供电话音频格式支持所需的指令和必要参数。
**处理**: 按照skill规范执行电话音频格式支持操作,遵循单一意图原则。
...
**输入**: 用户提供中断处理与手动轮次所需的指令和必要参数。
**处理**: 按照skill规范执行中断处理与手动轮次操作,遵循单一意图原则。
