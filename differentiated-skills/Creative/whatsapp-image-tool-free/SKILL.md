...
edition: free
description: |-
  WhatsApp 图片发送免费版，为个人用户提供轻量化的多媒体消息发送能力。
  ...
  - 四步工作流清晰（下载 → 复制 → 发送 → 清理）
  - 支持基础图片格式与说明文字
  - PRO 版本提供批量发送、多媒体支持、定时发送、群组消息等高级能力

  触发关键词: WhatsApp 发图, 图片发送, 多媒体消息, 文件分享, 发送图片, whatsapp image, send image
tags:
- Creative
- 消息发送
- WhatsApp
...
| 参数 | 类型 | 是否必需 | 说明 |
|:-----|:-----|:---------|:-----|
| --channel | 字符串 | 必需 | 固定为 `whatsapp` |
| --target | 字符串 | 必需 | 接收方手机号 |
| --filePath | 字符串 | 必需 | 工作区文件路径 |
| --message | 字符串 | 可选 | 附加说明文字 |
...
WhatsApp 要求文件必须在工作区目录中：

- 默认工作区：`~/.skill-platform/workspace/`
- 发送前必须将文件复制到此目录
- 发送后建议清理工作区文件
