...
**处理**: 按照skill规范执行完整企业级组件库操作,遵循单一意图原则。
**输出**: 返回完整企业级组件库的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行设计令牌多平台导出操作,遵循单一意图原则。
**输出**: 返回设计令牌多平台导出的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行设计系统文档自动生成操作,遵循单一意图原则。
**输出**: 返回设计系统文档自动生成的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
| 能力维度 | 免费版 | PRO版 |
|---------|--------|-------|
| 设计令牌 | 核心令牌 | 完整系统+多平台导出 |
| 组件库 | 3个基础组件 | 30+企业级组件 |
| 主题系统 | 不支持 | 浅色/深色/品牌定制 |
| 文档生成 | 不支持 | 自动生成完整文档 |
| 设计审计 | 不支持 | 自动审计+报告 |
| 令牌导出 | CSS变量 | CSS/Tailwind/Figma/SCSS/JSON |
| 动效系统 | 基础配置 | 高级动效预设+stagger |
| 多品牌 | 不支持 | 多品牌+子品牌管理 |
| CI/CD | 不支持 | 流水线集成 |
| 团队协作 | 单人 | 多人+版本管理 |
...
component_standards = {
    "structure": {
        "required_files": ["index.tsx", "styles.ts", "test.tsx", "stories.tsx"],
        "naming": "PascalCase",
        "directory": "by_category/by_component"
    },
    "accessibility": {
        "wcag": "2.1_AA",
        "keyboard_nav": True,
