...
**输入**: 用户提供批量上传与队列管理所需的指令和必要参数。
**处理**: 按照skill规范执行批量上传与队列管理操作,遵循单一意图原则。
**输出**: 返回批量上传与队列管理的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供分片上传大文件所需的指令和必要参数。
**处理**: 按照skill规范执行分片上传大文件操作,遵循单一意图原则。
**输出**: 返回分片上传大文件的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级音频上传工、支持批量上传、多质量预设、分片上传与元数据、音频流上传专业版、面向企业团队与专、业创作者的高级音、频上传工具、核心能力、批量音频上传、支持队列管理与断、完全自定义编码配、比特率、采样率、编解码器、多质量预设输出、满足不同播放场景、双流媒体格式支持、适配多终端播放、丰富的元数据管理、自定义键值对、分片上传支持大文等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
