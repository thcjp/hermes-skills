...
**输入**: 用户提供多模型切换所需的指令和必要参数。
**处理**: 按照skill规范执行多模型切换操作,遵循单一意图原则。
**输出**: 返回多模型切换的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行完整发布流程操作,遵循单一意图原则。
...
**输入**: 用户提供智能配图管理所需的指令和必要参数。
**处理**: 按照skill规范执行智能配图管理操作,遵循单一意图原则。
...
**输入**: 用户提供多草稿管理所需的指令和必要参数。
**处理**: 按照skill规范执行多草稿管理操作,遵循单一意图原则。
**输出**: 返回多草稿管理的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级公众号写作、支持多模型切换、业务资料库、发布流程、配图管理与团队协、公众号写作专业版、面向专业内容团队、与企业运营的高级、公众号写作引擎、核心能力、多模型切换、DeepSeek、GPT、Claude、等模型、按场景灵活选择、业务资料库集成、自动关联产品、服务资料、实现增量改写、完整发布流程、从选题到发布一条、支持草稿箱与直接、智能配图管理、自动生成配图占位、支持用户供图与、多草稿管理、支持续写、多版本对比、历史草稿追踪、自定义预设、结构预设、文末区块、禁用词等深度定制、内容审查集成、自动剥离引用标注等。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
