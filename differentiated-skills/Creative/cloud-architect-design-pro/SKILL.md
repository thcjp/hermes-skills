---
slug: cloud-architect-design-pro
name: cloud-architect-design-pro
version: "1.0.0"
displayName: 云架构师专业版
...
云架构师专业版是企业级云架构设计平台，在免费版基础上提供多云架构策略、云迁移规划、FinOps成本优化、合规架构设计、灾难恢复方案等专业能力。适用于企业级云架构规划、混合云设计、云迁移项目等高阶场景。
...
| 单云架构设计 | 支持 | 支持 |
| 多云架构 | 不支持 | 跨云容灾+负载分流 |
| 成本估算 | 基础估算 | FinOps深度优化 |
| 迁移规划 | 不支持 | 完整迁移路线图 |
| 合规设计 | 基础安全 | 等保/GDPR/HIPAA |
| 灾备方案 | 不支持 | RPO/RTO+多区域 |
| 架构治理 | 不支持 | ADR+治理框架 |
| 微服务设计 | 不支持 | 服务拆分+网格 |
| 性能工程 | 不支持 | 容量规划+基准 |
| 安全纵深 | 基础安全 | 零信任+加密体系 |
...
**处理**: 按照skill规范执行多云架构策略操作,遵循单一意图原则。
...
**输入**: 用户提供云迁移规划所需的指令和必要参数。
**处理**: 按照skill规范执行云迁移规划操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行FinOps成本优化操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行合规架构设计操作,遵循单一意图原则。
**输出**: 返回合规架构设计的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级云架构设计、支持多云策略、迁移规划、FinOps、合规设计与灾备方、云架构师专业版、面向企业团队与架、构师的高级云架构、设计平台、核心能力、多云架构策略、AWS、Azure、GCP、跨云架构设计与容、灾方案、云迁移规划、本地到云、云到云的完整迁移、路线图、成本优化、深度成本分析与节、省计划制定、合规架构设计、满足等保、GDPR、HIPAA、等合规要求、灾难恢复方案、RPO、RTO、规划与多区域灾备、架构评审与治理、架构决策记录、ADR、与治理框架、微服务架构设计、服务拆分、API、服务网格方案、性能工程、容量规划、性能基准与扩展策等。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
1. **多云策略**：避免过度多云，2-3个云平台即可覆盖大多数需求
2. **迁移节奏**：先简单后复杂，先非核心后核心，降低风险
3. **FinOps文化**：建立成本意识文化，全员参与成本优化
4. **合规前置**：架构设计阶段就考虑合规要求，避免后期改造
5. **灾备演练**：定期执行灾备切换演练，验证方案有效性
6. **架构治理**：使用ADR记录架构决策，建立架构评审流程
7. **渐进现代化**：迁移后逐步优化，不必一步到位
