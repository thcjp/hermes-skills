...
| 错误 | 正确做法 |
|------|----------|
| 文字触碰屏幕边缘 | 移动端最小px-4内边距 |
| emoji用作图标 | 使用SVG图标 |
| 纯黑背景 | 使用分层暗色(bg-900 > bg-800) |
| 无悬停状态 | 添加颜色/阴影过渡 |
| 圆角不一致 | 统一rounded-lg |
| 移动端字号溢出 | 测试390px宽度 |
| 缺少viewport meta | 添加meta viewport标签 |
| 移动端无汉堡菜单 | 包含移动导航菜单 |
...
本Skill基于Markdown指令和本地脚本运行,无需额外API Key。页面生成由Agent内置LLM驱动,截图和图片转换为本地工具执行。CDN前端库通过公网加载,无需配置。

### 可用性分类

- **分类**: MD+EXEC(纯Markdown指令,部分功能需要exec命令行执行能力)
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent执行任务。页面生成后需要exec工具执行截图脚本和图片转换脚本。本地服务需通过Bash启动,截图功能依赖Chrome/Chromium浏览器。
