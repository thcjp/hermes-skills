...
edition: pro
description: |-
  图表生成工具专业版，面向团队的全格式图表生成与管理平台。

  核心能力：
  - 三大格式全覆盖：Draw.io、Mermaid、Excalidraw
  - 八种图表类型：网络拓扑、系统架构、流程图、泳道图、UML、ER图、思维导图、白板草图
  - JSON 规范驱动生成，支持复杂嵌套结构
  - 批量图表生成与模板化工作流
  - 质量门禁自动校验
  - 自定义输出路径与配置管理
  ...
  差异化：专业版在免费版基础上扩展至全格式、全类型，支持 MCP工具集成、批量生成与企业级配置管理。与免费版完全兼容。

  触发关键词: diagram, drawio, excalidraw, 网络拓扑, 系统架构, 泳道图, UML, ER图, 思维导图, 白板, 批量生成
tags:
- Creative
- Diagram
- Enterprise
