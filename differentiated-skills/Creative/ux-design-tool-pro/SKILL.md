...
**处理**: 按照skill规范执行深度流程分析与漏斗优化操作,遵循单一意图原则。
**输出**: 返回深度流程分析与漏斗优化的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行用户研究方法论操作,遵循单一意图原则。
**输出**: 返回用户研究方法论的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行状态机设计模式操作,遵循单一意图原则。
**输出**: 返回状态机设计模式的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行认知负荷量化分析操作,遵循单一意图原则。
**输出**: 返回认知负荷量化分析的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 按照skill规范执行企业级UX审计框架操作,遵循单一意图原则。
**输出**: 返回企业级UX审计框架的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：团队的专业体验设、计引擎、面向企业、团队和专业设计师、的完整用户体验设、涵盖深度流程分析、和全面边界状态设、Use、when、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
