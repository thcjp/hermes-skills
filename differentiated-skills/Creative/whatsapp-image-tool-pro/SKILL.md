...
**处理**: 按照skill规范执行群组消息与广播操作,遵循单一意图原则。
**输出**: 返回群组消息与广播的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行发送频率控制操作,遵循单一意图原则。
