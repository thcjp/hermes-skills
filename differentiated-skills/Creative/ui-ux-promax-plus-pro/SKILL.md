...
description: |-
  ...
  差异化:专业版在免费版基础上扩展至完整资源库,新增图表推荐、UX模式库、
  组件规范和设计令牌系统。完全兼容免费版设计决策框架,可无缝升级。

  触发关键词: 设计资源库, 图表类型推荐, UX交互模式, 组件设计规范, 设计令牌系统, 配色选择矩阵, 字体配对, 数据可视化, 组件库规范, 企业设计系统, 设计决策矩阵
tags:
- 设计
- UI
- UX
...
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent执行任务。Tailwind CSS配置、图表库安装和设计令牌文件创建需要exec工具执行npm命令或编辑文件。
