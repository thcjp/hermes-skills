...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级、设计工具包、支持设计系统、可访问性与组件库、适配团队协作与大、型项目、面向团队与企业用、核心能力、涵盖免费版全部能、Design、System、构建与维护、Tokens、深度规范与自动化、组件库抽象与复用、多主题与品牌切换、Motion、设计考虑、协作规范、设计文档与等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
...
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent完成操作。专业版支持设计系统建设、设计令牌管理与可访问性规范,适合企业级产品设计与团队协作。
