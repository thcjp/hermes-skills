...
**处理**: 按照skill规范执行余额查询操作,遵循单一意图原则。
**输出**: 返回余额查询的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行提供商管理操作,遵循单一意图原则。
**输出**: 返回提供商管理的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 使用量和费用、提供定时提醒、余额预警和跨会话、使用量和费用监控、实时跟踪、消耗和余额、支持定时提醒、余额预警、工具集成和跨会话、触发预警、适用于独立开发者、企业团队和自动化、工作流场景、不适用于无、计费的场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Built-in Tool Integration (P1) | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `快速开始`, `生成报告`, `所有数据本地存储`, `本地运行`, `智能建议`, `register`, `ollama`, `使用和费用`, `在以下情况使用此`, `触发时输出`, `google`, `保持开启确保质量`, `使用场景`, `providers`, `主动通知`
...
```bash
node scripts/session-tracker.js record \
  --input 1200 \
  --output 800 \
