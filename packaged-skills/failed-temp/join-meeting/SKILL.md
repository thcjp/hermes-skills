...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Media | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `agentcall`, `tts.audio`, `page`, `permissions`, `prerequisites`, `max_duration`, `skipping`, `authorization`, `deduplicates`, `tts.interrupted`, `override`, `retrieval`, `tts.started/done`, `bridge.py`, `详细内容已移至`
...
| 转写服务中断 | 语音识别API超时 | 执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令连接；使用本地转写作为备选 |
