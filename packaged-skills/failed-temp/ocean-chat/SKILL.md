...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 存在且 Roster 有联系人 | 支持 | 通过核心功能实现对应能力 |
| 通过 roster.findByOpenId() 反查发件人。 | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `共享同一个`, `的命令当任务执行`, `listen`, `拉好友下水是第一`, `每条客户咨询`, `收到打招呼消息`, `重开已结束线程`, `加了之后可以直接`, `提案必须具体`, `命令速查`, `查看线程详情`, `详见末尾命令参考`, `识别格式`, `如果对方`, `查看联系人详情`
