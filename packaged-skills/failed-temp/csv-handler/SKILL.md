...
**处理**: 按照skill规范执行编码自动检测操作,遵循单一意图原则。### 分隔符自动识别
...
**输出**: 返回分隔符自动识别的执行结果,包含操作状态和输出数据。### CSV文件画像分析
...
**处理**: 按照skill规范执行多文件合并操作,遵循单一意图原则。
**输出**: 返回多文件合并的执行结果,包含操作状态和输出数据。### 按列拆分
...
**处理**: 按照skill规范执行按列拆分操作,遵循单一意图原则。### 智能类型转换
...
**输出**: 返回智能类型转换的执行结果,包含操作状态和输出数据。### 进度计划CSV专用解析
...
**输出**: 返回进度计划CSV专用解析的执行结果,包含操作状态和输出数据。### 成本CSV专用解析
...
**输出**: 返回成本CSV专用解析的执行结果,包含操作状态和输出数据。### 多编码导出
`export_csv()` 默认使用 `utf-8-sig` 编码导出（带BOM，确保Excel正确显示中文），支持自定义分隔符，`index=False` 不写入行索引。

**输入**: 用户提供多编码导出所需的指令和必要参数。
**处理**: 按照skill规范执行多编码导出操作,遵循单一意图原则。

### 能力覆盖范围

本skill还覆盖以下能力场景: 自动检测编码与分、支持进度计划与成、本数据专用解析、文件处理专家、覆盖建筑、财务等场景下的、全生命周期管理、核心能力包括、数据清洗、空行空列删除、或纵向、按列值拆分、为多个子文件、日期时间自动推断、日期列自动转换、金额列去、符号并转数值。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
