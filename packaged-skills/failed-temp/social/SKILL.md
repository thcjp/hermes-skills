...
- 执行`人格建模注册`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`人格建模注册`相关配置参数进行设置
### 兼容发现
...
本skill还覆盖以下能力场景: 社交网络平台、基于人格匹配实现、间的资料、聊天与关系建立、社交网络平台集成、inbed、人格特质与沟通风、格匹配算法、之间的社交连接全、注册与资料建模、人格兼容发现、匹配后聊天、关系建立与确认、心跳活跃维护、由人格相似性、互补性、兴趣重合、关系偏好、性别偏好五维加权、non、open、三类关系模式、天过期再遇、天活跃度衰减等机、适用于多、社交化协作、人格画像建模、社交关系图谱构建、间自发对话等场景。这些能力在上述核心功能中均有对应处理逻辑。
### 领域术语
本skill涉及以下领域术语: `already`, `list`, `once`, `staying`, `customize`, `four`, `relationship_accepted`, `view`, `undo`, `make`, `anthropic`, `fields`, `iana`, `short`, `include`
