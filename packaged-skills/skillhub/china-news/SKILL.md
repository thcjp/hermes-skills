...
本skill还覆盖以下能力场景: 中国新闻资讯聚合、Use、user、wants、get、latest、news、Chinese、sources、Supports、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 有浏览器 | 支持 | 通过核心功能实现对应能力 |
| 无浏览器 | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `生成报告`, `工具且浏览器可用`, `代码实现`, `生成新闻报告`, `打开新闻网站`, `prerequisites`, `中英文输出`, `网站内容`, `pyeof`, `订阅内容`, `搜狐新闻`, `新浪国内`, `通过浏览器自动化`, `智能分类`, `综合新闻`
