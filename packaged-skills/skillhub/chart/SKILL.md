...
**处理**: 按照skill规范执行多类型组合工作流操作,遵循单一意图原则。

### 能力覆盖范围

本skill还覆盖以下能力场景: 本地优先的图表生、成引擎、四种类型、无需联网、输出可复用于报告、与幻灯片、本地优先、local、first、的图表生成引擎、将数字转换为清晰、的可视化输出、matplotlib、支持柱状图、折线图、散点图四种核心类、所有数据与产物仅、落地本地、不依赖任何第三方、API、或云同步、核心能力、四种图表类型、智能推荐、根据数据特征选最、佳类型、生成图片、历史管理、查看历史图表记录、一键初始化本地存、幻灯片与快速决策。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
