...
summary_zh: "企业级 Discord 聊天管理,支持高级搜索、批量消息分发与频道全生命周期管理。。面向运营团队与企业场景的 Discord 聊天全功能管理工具。核心能力: - 多条件组合高级搜索(作者/时"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| Discord聊天管理专业版聊天管理 | 不支持 | 支持 |
| Discord聊天管理专业版与频道全生命周期管理 | 不支持 | 支持 |
| 多渠道消息批量发送 | 不支持 | 支持 |
| 消息模板与变量注入 | 不支持 | 支持 |
| 送达状态实时回调 | 不支持 | 支持 |
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| content | string | 否 | discord-chat-manager处理的内容输入 |,  |
| content | string | 否 | discord-chat-manager处理的内容输入 |, 可选值: json/text/markdown |
