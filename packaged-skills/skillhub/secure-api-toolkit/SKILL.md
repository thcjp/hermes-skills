...
- 执行`团队凭据金库`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`团队凭据金库`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级凭据代理方、团队金库、自定义提供商与合、工具箱、为团队与企业提供、完整的凭据代理治、理方案、从架构层杜绝、Agent、接触真实密钥、核心能力、团队凭据金库与共、全量审计日志与不、自定义私有、提供商注册、凭据隔离与提供商。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
- 不适用: 需要人工判断的复杂决策场景
