...
- 执行`同步方式`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`同步方式`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级知识库爬取、与检索平台、支持多工作空间、分布式调度、增量索引、全文检索与自定义、适合团队与企业规、模化使用、智能爬虫、是面向团队与企业、的全功能知识库爬、在免费版基础上新、增多工作空间管理、全文检索引擎、自定义导出等高级、核心能力。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-research`: 命令参数,用于指定操作选项
2. `-papers`: 命令参数,用于指定操作选项
3. `--highlight`: 命令参数,用于指定操作选项
4. `--mode`: 命令参数,用于指定操作选项
5. `--cache-hit-rate`: 命令参数,用于指定操作选项
...
- `--index-size`: 命令参数,用于指定操作选项
- `--workspaces`: 命令参数,用于指定操作选项
- `--expand-synonyms`: 命令参数,用于指定操作选项
- `--port`: 命令参数,用于指定操作选项

### 命令参数说明

- `--fuzziness`: 命令参数,用于指定操作选项
- `--format`: 命令参数,用于指定操作选项
- `--query-latency-p99`: 命令参数,用于指定操作选项
- `-app`: 命令参数,用于指定操作选项
