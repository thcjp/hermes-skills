...
- 执行`进阶类型`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`进阶类型`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 团队级、Mermaid、图表套件、含全类型、批量生成与、图表工具、专业版、面向团队与企业、提供全类型、图表生成、复杂多节点图、自定义主题样式、从文档批量生成、语法校验与文档自、动嵌入能力、核心能力、全图表类型、需求图等进阶类型、复杂多节点图与子、样式与品牌色、批量生成多张图表、文档自动嵌入与版。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-database`: 命令参数,用于指定操作选项
2. `--format`: 命令参数,用于指定操作选项
3. `--theme`: 命令参数,用于指定操作选项
4. `-latest`: 命令参数,用于指定操作选项
5. `-flow`: 命令参数,用于指定操作选项

### 命令参数说明

- `-check`: 命令参数,用于指定操作选项
- `--input`: 命令参数,用于指定操作选项
- `-machine`: 命令参数,用于指定操作选项
- `-standard`: 命令参数,用于指定操作选项
- `-serif`: 命令参数,用于指定操作选项

### 命令参数说明

- `-generate`: 命令参数,用于指定操作选项
