...
- 执行`资源组`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`资源组`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 通过本地、Azure、CLI、查询与管理、默认只读、写操作需确认、Infra、技能通过本地、帮助用户查询、诊断和管理、云资源、默认所有操作为只、读查询、任何写操作或破坏、性变更、IAM、计费配置等、必须先展示完整命、令与影响范围、经用户显式确认后、方可执行。这些能力在上述核心功能中均有对应处理逻辑。
### 领域术语

本skill涉及以下领域术语: `不适用于超出本技`, `依赖云服务`, `相关功能时使用`, `safety`, `costs`, `需要网络连接`, `边界输入处理`, `示例输入`, `格式返回`, `降级策略`, `prefer`, `guide`, `tenant`, `otherwise`, `inventory`
