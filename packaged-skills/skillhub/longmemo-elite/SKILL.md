...
执行L1热内存操作,处理用户输入并返回结果。

**输入**: 用户提供L1热内存所需的参数和指令。

**输出**: 返回L1热内存的处理结果。

- 执行`L1热内存`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`L1热内存`相关配置参数进行设置
### L2温向量

执行L2温向量操作,处理用户输入并返回结果。

**输入**: 用户提供L2温向量所需的参数和指令。

**输出**: 返回L2温向量的处理结果。

- 执行`L2温向量`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`L2温向量`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: Agent、精英级长记忆、的精英级长期记忆、解决跨会话遗忘、检索不准、成本失控三大核心、写前日志协议采用、融合排序解决单一、成本预算控制通过、图谱增量更新四项、配置日、月预算上限与自动、记忆卫生自动化执、行去重、删除六项维护、适用于跨会话项目、多代理协作、长期偏好维护等生、记忆管理场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
