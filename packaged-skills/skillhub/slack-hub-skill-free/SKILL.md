...
summary: Slack基础消息发送与公共频道列表，Bot Token直连Web API
license: MIT
description: |-
...
- **网络**: 需可访问 `https://slack.com/api/*`

### 依赖项
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| Slack Bot Token | 环境变量 | 必需 | https://api.slack.com/apps 创建App，配置Bot Token Scopes后获取 |
| SLACK_BOT_TOKEN | 环境变量 | 必需 | 格式 `xoxb-...`，写入 `.env` 或shell环境变量 |
| curl 或等价 HTTP 客户端 | 命令行工具 | 必需 | 系统自带或包管理器安装 |

### 可用性分类
- **分类**: MD+EXEC（Markdown 指令驱动，需 exec 执行 curl 命令）
- **说明**: 基于自然语言指令驱动 Agent 调用 Slack Web API，透传结构化 JSON
...
> **升级提示**：线程回复、工作区内容搜索、用户DM发送、频道名自动解析、速率限制感知重试等高级功能仅在 slack-hub-skill 付费版中提供。
### 指令解析与执行
