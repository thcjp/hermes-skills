...
**处理**: 按照skill规范执行团队仪表盘操作,遵循单一意图原则。### Webhook管理
```bash
# 列出仓库Webhook
...
**输出**: 返回Webhook管理的执行结果,包含操作状态和输出数据。### 安全审计
```bash
# 权限审查
...
本skill还覆盖以下能力场景: 全功能、管理工具、含批量操作、团队仪表盘与、适合团队企业级协、管理器、专业版、是企业级、协作管理工具、在免费版基础查询、能力上、扩展批量操作、管理等高级能力、核心能力、复杂关联查询、跨仓库聚合、定时检查、自动分配、状态流转、通知告警、成员贡献统计、事件订阅、自动触发、回调配置。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--reason`: 命令参数,用于指定操作选项
- `-failure-alert`: 命令参数,用于指定操作选项
- `--file`: 命令参数,用于指定操作选项
