...
- 执行`知识体系`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`知识体系`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 专业金融教育与理、财规划系统、支持个性化方案、认证培训与企业内、面向专业用户与机、构的全栈金融素养、教育系统、支持个性化理财方、案生成、金融从业认证培训、企业内训定制与学、习进度追踪、Use、需要项目管理、任务规划、进度跟踪、团队协作时使用、不适用于实际人员、绩效评估、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--retire`: 命令参数,用于指定操作选项
2. `--progress`: 命令参数,用于指定操作选项
3. `--retirement-age`: 命令参数,用于指定操作选项
4. `--report`: 命令参数,用于指定操作选项
5. `--mode`: 命令参数,用于指定操作选项
...
- `--scenario`: 命令参数,用于指定操作选项
- `--company`: 命令参数,用于指定操作选项
- `--risk-tolerance`: 命令参数,用于指定操作选项

### 命令参数说明

- `--course`: 命令参数,用于指定操作选项
- `--exam`: 命令参数,用于指定操作选项
- `--level`: 命令参数,用于指定操作选项
- `--user`: 命令参数,用于指定操作选项
- `--income`: 命令参数,用于指定操作选项

### 命令参数说明

- `--focus`: 命令参数,用于指定操作选项
- `--mortgage`: 命令参数,用于指定操作选项
- `--employees`: 命令参数,用于指定操作选项
