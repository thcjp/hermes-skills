...
`visual-file-sorter`按照以下流程处理用户请求:
...
本skill涉及以下领域术语: `退出预览界面`, `desktop`, `在后台分析该文件`, `主要内容`, `双击打开`, `将截图或文本内容`, `归档桌面上的杂乱`, `核心目标`, `保留原文件名`, `自动清理用户杂乱`, `餐饮发票`, `移动的权限`, `画面内容描述`, `查阅内容`, `按以下规则生成新`
