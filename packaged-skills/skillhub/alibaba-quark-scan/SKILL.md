...
**处理**: 按照skill规范执行考试增强（exam-enhance）操作,遵循单一意图原则。
**输出**: 返回考试增强（exam-enhance）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回画质增强（image-hd-enhance）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行证件票据增强（certificate-enhance）操作,遵循单一意图原则。
**输出**: 返回证件票据增强（certificate-enhance）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回图像去手写（remove-handwriting）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行图像去水印（remove-watermark）操作,遵循单一意图原则。
**输出**: 返回图像去水印（remove-watermark）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行图像去阴影（remove-shadow）操作,遵循单一意图原则。
**输出**: 返回图像去阴影（remove-shadow）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行图像去屏纹（remove-screen-pattern）操作,遵循单一意图原则。
**输出**: 返回图像去屏纹（remove-screen-pattern）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回文档去底色（remove-background-color）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行图像裁剪矫正（image-crop-rectify）操作,遵循单一意图原则。
**输出**: 返回图像裁剪矫正（image-crop-rectify）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回素描速写（sketch-drawing）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回提取线稿（extract-lineart）的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行扫描合同（scan-contract）操作,遵循单一意图原则。
**输出**: 返回扫描合同（scan-contract）的执行结果,包含操作状态和输出数据。
...
**输出**: 返回扫描文件兜底（scan-document）的执行结果,包含操作状态和输出数据。
...
| API服务端返回非00000错误码 | 服务端处理异常 | 原样透出错误信息，不执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
| 网络连接扫描服务超时 | 网络不可达 | 执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
