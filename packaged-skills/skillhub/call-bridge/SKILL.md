...
本skill还覆盖以下能力场景: 企业级、电话代理平台、支持并行外呼、通话活动管理与个、通话桥接专业版是、一款面向团队与企、在免费版单次外呼、基础上、新增并行外呼、通话活动管理、个性化语音配置与、完整错误处理等高、级能力、核心能力、同时拨打、通电话进行信息比、设置来电应答规则、与转接策略、跨通话保持状态。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-X`: 命令参数,用于指定操作选项
- `-Type`: 命令参数,用于指定操作选项
- `-comparison-2024-03`: 命令参数,用于指定操作选项
