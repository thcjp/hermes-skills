...
**处理**: 按照skill规范执行智能拆解算法（专业版）操作,遵循单一意图原则。
**输出**: 返回智能拆解算法（专业版）的执行结果,包含操作状态和输出数据。

- 执行`智能拆解算法（专业版）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`智能拆解算法（专业版）`相关配置参数进行设置
### 2. 多模型并行编排（专业版）
```bash
...
**处理**: 按照skill规范执行性能分析（专业版）操作,遵循单一意图原则。
**输出**: 返回性能分析（专业版）的执行结果,包含操作状态和输出数据。

- 执行`性能分析（专业版）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`性能分析（专业版）`相关配置参数进行设置
### 5. 模板库（专业版）
```bash
...
本skill还覆盖以下能力场景: 全功能任务拆解引、含智能算法、性能分析与模板库、工作流分解器专业、版是在免费版基础、上的全功能升级、为复杂项目团队提、供企业级任务拆解、与子任务分配能力、除基础拆解外、解锁智能拆解算法、版本管理八大高级、Use、需要项目管理、任务规划、进度跟踪、团队协作时使用、不适用于实际人员、绩效评估。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--primary`: 命令参数,用于指定操作选项
2. `--secondary`: 命令参数,用于指定操作选项
3. `--reasoning`: 命令参数,用于指定操作选项
4. `-splitter-pro`: 命令参数,用于指定操作选项
5. `--multi-model`: 命令参数,用于指定操作选项
...
- `--task`: 命令参数,用于指定操作选项
- `-opus`: 命令参数,用于指定操作选项
- `--from-task`: 命令参数,用于指定操作选项
- `--skills`: 命令参数,用于指定操作选项

### 命令参数说明

- `--task-id`: 命令参数,用于指定操作选项
- `-approved`: 命令参数,用于指定操作选项
- `--task-type`: 命令参数,用于指定操作选项
- `--creative`: 命令参数,用于指定操作选项
...
- `--max-workers`: 命令参数,用于指定操作选项
- `--visualize`: 命令参数,用于指定操作选项
- `-sonnet`: 命令参数,用于指定操作选项

### 命令参数说明

- `--strategy`: 命令参数,用于指定操作选项
- `--coding`: 命令参数,用于指定操作选项
- `--condition`: 命令参数,用于指定操作选项
- `--rule-id`: 命令参数,用于指定操作选项
