...
执行查询结果缓存操作,处理用户输入并返回结果。

**输入**: 用户提供查询结果缓存所需的参数和指令。

**输出**: 返回查询结果缓存的处理结果。

- 执行`查询结果缓存`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`查询结果缓存`相关配置参数进行设置
### 慢查询采集

执行慢查询采集操作,处理用户输入并返回结果。

**输入**: 用户提供慢查询采集所需的参数和指令。

**输出**: 返回慢查询采集的处理结果。

- 执行`慢查询采集`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`慢查询采集`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 面向团队与企业的、查询专业版、含慢查询采集、性能基准测试与优、先工单支持、面向团队、企业与专业运维的、查询执行工具专业、在免费版基础上新、增查询结果缓存与、命中率监控、慢查询自动采集与、跨数据库、自动转换、查询性能基准测试、连接池调优与读写、分离路由等高级能、配套面向运维、数据工程师、DBA、的多角色场景指南、Use、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
