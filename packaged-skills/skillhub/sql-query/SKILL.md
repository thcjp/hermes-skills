...
针对查询结果缓存,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供查询结果缓存相关的配置参数、输入数据和处理选项.
**输出**: 返回查询结果缓存的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`查询结果缓存`的配置文档进行参数调优
### 慢查询采集

针对慢查询采集,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供慢查询采集相关的配置参数、输入数据和处理选项.
**输出**: 返回慢查询采集的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`慢查询采集`的配置文档进行参数调优
#
## 快速开始
