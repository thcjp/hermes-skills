...
执行索引创建与管理操作,处理用户输入并返回结果。

**输入**: 用户提供索引创建与管理所需的参数和指令。

**输出**: 返回索引创建与管理的处理结果。

- 执行`索引创建与管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`索引创建与管理`相关配置参数进行设置
### 嵌入向量导入

执行嵌入向量导入操作,处理用户输入并返回结果。

**输入**: 用户提供嵌入向量导入所需的参数和指令。

**输出**: 返回嵌入向量导入的处理结果。

- 执行`嵌入向量导入`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`嵌入向量导入`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能向量数据库、支持混合检索、自动调优与性能监、工程团队的全功能、向量数据库管理平、索引自动调优与实、时性能监控、核心能力、在免费版基础上新、增混合检索引擎、多副本分片管理、索引参数自动调优、向量数据版本管理、实时监控仪表盘、增量更新管道与跨、引擎迁移工具、Use、需要数据库操作、SQL、数据存储管理时使、不适用于数据库架、构设计决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-vit-large`: 命令参数,用于指定操作选项
- `-large-zh`: 命令参数,用于指定操作选项
- `-reranker-large`: 命令参数,用于指定操作选项
...
| 节点频繁掉线 | 网络或资源问题 | 执行ping命令测试网络连通性,检查防火墙和代理设置连通性，查看资源监控 | P1 |
| 迁移校验失败 | 数据丢失或格式错误 | 查看差异日志，分批执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 | P2 |
