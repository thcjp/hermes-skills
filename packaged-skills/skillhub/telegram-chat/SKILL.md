...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| 电报聊天工具专业版gram多Bot管理 | 不支持 | 支持 |
| 电报聊天工具专业版档审计与群组批量管理 | 不支持 | 支持 |
| 多渠道消息批量发送 | 不支持 | 支持 |
| 消息模板与变量注入 | 不支持 | 支持 |
| 送达状态实时回调 | 不支持 | 支持 |
...
**处理**: 解析一、多 Bot 统一管理(专业版独有)的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回一、多 Bot 统一管理(专业版独有)的处理结果,包含执行状态码、结果数据和执行日志。### 二、主动消息推送(专业版独有)
- 无需被艾特即可主动发送消息
- 支持指定群组/用户定向推送
...
**处理**: 解析三、消息归档与审计(专业版独有)的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 四、群组批量管理(专业版独有)
- 批量将 Bot 加入多个群组
- 群组成员列表同步与导出
- 群组权限统一配置
...
**处理**: 解析四、群组批量管理(专业版独有)的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。### 五、跨群组广播(专业版独有)
- 一条消息同时推送到多个群组
- 支持按标签筛选目标群组
- 广播任务调度与进度追踪
- 失败自动重试
...
**处理**: 解析五、跨群组广播(专业版独有)的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回五、跨群组广播(专业版独有)的处理结果,包含执行状态码、结果数据和执行日志。### 六、Webhook 集成与自动化(专业版独有)
...
**输出**: 返回七、消息模板与定时发送(专业版独有)的处理结果,包含执行状态码、结果数据和执行日志.
#
## 快速开始
