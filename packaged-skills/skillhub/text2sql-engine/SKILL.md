...
**输出**: 返回Schema自动识别的处理结果。

- 执行`Schema自动识别`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Schema自动识别`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、支持复杂多表查询、性能优化、查询解释与替代方、案生成、面向数据工程师与、DBA、的全功能自然语言、支持复杂多表关联、查询性能优化与执、核心能力、在免费版基础上新、增多表嵌套、查询解释注释、替代方案对比、安全审计等高级特、Use、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
