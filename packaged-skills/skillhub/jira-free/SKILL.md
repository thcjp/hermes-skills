...
- **网络**: 需可访问 Jira 实例（Cloud 或 Server/Data Center）

### 依赖项
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| Agent 平台 | 运行环境 | 必需 | 任意支持 SKILL.md 的 AI Agent |
| Jira 实例 | 服务 | 必需 | Atlassian Cloud 或自建 Jira Server/DC |
| Jira API Token | 认证凭证 | 必需 | Atlassian 账户设置中生成 API Token |
| curl | 命令行工具 | 必需 | 系统自带或安装，用于执行 REST API 调用 |

### 可用性分类
- **分类**: MD+EXEC（Markdown 指令驱动，需 exec 执行 curl 命令调用 Jira API）
- **说明**: 基于自然语言指令驱动 Agent 执行 Jira 问题管理、Sprint 规划与查询
