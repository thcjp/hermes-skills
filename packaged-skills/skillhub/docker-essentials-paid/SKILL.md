---
slug: docker-essentials-paid
name: docker-essentials-paid
version: "1.0.0"
...
docker images --format "table 相关信息\t相关信息\t相关信息" | sort -k3 -h

docker images --format "相关信息 相关信息:相关信息" | sort -rh | head -10

docker inspect --format='相关信息相关信息（根据实际场景填充）' myapp:1.0
```

**输入**: 用户提供镜像深度优化与分析所需的指令和必要参数。
**输出**: 返回镜像深度优化与分析的执行结果,包含操作状态和输出数据。

- 执行`批量容器运维`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量容器运维`相关配置参数进行设置
### 2. 容器安全漏洞扫描
扫描镜像中的已知安全漏洞。

> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供容器安全漏洞扫描所需的指令和必要参数。
**处理**: 按照skill规范执行容器安全漏洞扫描操作,遵循单一意图原则。
**输出**: 返回容器安全漏洞扫描的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供Docker Swarm 多节点编排所需的指令和必要参数。
**处理**: 按照skill规范执行Docker Swarm 多节点编排操作,遵循单一意图原则。
...
echo -e "\n=== 批量更新镜像 ==="
for image in $(docker images --format "相关信息:相关信息" | grep -v "<none>"); do
    echo "更新: $image"
...
**输出**: 返回批量容器运维的执行结果,包含操作状态和输出数据。

- 执行`批量容器运维`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `批量容器运维` 选项
- 处理流程: 接收输入 -> 执行批量容器运维 -> 返回结果
- 输入: 用户提供批量容器运维所需的参数和指令
- 输出: 返回批量容器运维的执行结果,包含操作状态和输出数据

### 5. 资源监控与告警
> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供资源监控与告警所需的指令和必要参数。
**处理**: 按照skill规范执行资源监控与告警操作,遵循单一意图原则。
**输出**: 返回资源监控与告警的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、支持镜像深度优化、安全扫描、批量运维与、面向企业运维团队、的高级、管理工具、流水线集成、核心能力、镜像深度优化与体、积分析、批量容器运维管理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
echo -e "\n服务任务分布:"
docker service ps myapp_web --format "table docker-essentials-paid\t相关信息\t相关信息"
...
docker_build_deploy:
  ...
  script:
    ...
    - |
      SIZE=$(docker inspect --format='相关信息' "$IMAGE_TAG")
...
1. `-gt`: 命令参数,用于指定操作选项
2. `--no-deps`: 命令参数,用于指定操作选项
3. `--availability`: 命令参数,用于指定操作选项
4. `-mirrors`: 命令参数,用于指定操作选项
5. `--with-registry-auth`: 命令参数,用于指定操作选项

### 命令参数说明

- `-rh`: 命令参数,用于指定操作选项
- `--advertise-addr`: 命令参数,用于指定操作选项
- `-dind`: 命令参数,用于指定操作选项
- `--update-failure-action`: 命令参数,用于指定操作选项
- `--build`: 命令参数,用于指定操作选项

### 命令参数说明

- `--no-trunc`: 命令参数,用于指定操作选项
- `-stack`: 命令参数,用于指定操作选项
- `-token`: 命令参数,用于指定操作选项
- `-k3`: 命令参数,用于指定操作选项
- `--severity`: 命令参数,用于指定操作选项

### 命令参数说明

- `--format`: 命令参数,用于指定操作选项
- `--image`: 命令参数,用于指定操作选项
- `--update-parallelism`: 命令参数,用于指定操作选项
- `-registries`: 命令参数,用于指定操作选项
- `--filter`: 命令参数,用于指定操作选项

### 命令参数说明

- `--replicas`: 命令参数,用于指定操作选项
- `--exit-code`: 命令参数,用于指定操作选项
- `--update-delay`: 命令参数,用于指定操作选项
