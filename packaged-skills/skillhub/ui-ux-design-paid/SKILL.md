...
本skill还覆盖以下能力场景: 企业级、设计体系、趋势与高级产品深、面向企业设计团队、和专业前端的完整、组件库深度集成、微交互动画模式、以及对高级产品的、深度设计分析、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--app`: 命令参数,用于指定操作选项
2. `--typescript`: 命令参数,用于指定操作选项
3. `--primary`: 命令参数,用于指定操作选项
4. `--secondary`: 命令参数,用于指定操作选项
5. `-pointer`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `-hidden`: 命令参数,用于指定操作选项
- `--primary-foreground`: 命令参数,用于指定操作选项
- `-next-app`: 命令参数,用于指定操作选项
- `--eslint`: 命令参数,用于指定操作选项
- `--color-brand-primary`: 命令参数,用于指定操作选项
...
- `--muted`: 命令参数,用于指定操作选项
- `-offset`: 命令参数,用于指定操作选项
- `--background`: 命令参数,用于指定操作选项
- `--accent`: 命令参数,用于指定操作选项

### 命令参数说明

- `--border`: 命令参数,用于指定操作选项
- `-D`: 命令参数,用于指定操作选项
