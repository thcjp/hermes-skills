...
执行全量风格规则操作,处理用户输入并返回结果。

**输入**: 用户提供全量风格规则所需的参数和指令。

**输出**: 返回全量风格规则的处理结果。

- 执行`全量风格规则`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`全量风格规则`相关配置参数进行设置
### 异步并发模型

执行异步并发模型操作,处理用户输入并返回结果。

**输入**: 用户提供异步并发模型所需的参数和指令。

**输出**: 返回异步并发模型的处理结果。

- 执行`异步并发模型`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`异步并发模型`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 面向团队与企业的、JavaScript、规范套件、含性能优化、安全审查与、规范工具、专业版、面向团队与企业、代码风格规范、性能优化策略、安全审查清单与、自动化集成方案、核心能力、完整异步并发模型、与错误链路优秀实、团队级、Lint、规则定制与预提交、钩子集成、性能优化与安全审、查清单、深度兼容与混用规。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-unused-vars`: 命令参数,用于指定操作选项
2. `-plugin-enterprise`: 命令参数,用于指定操作选项
3. `-latest`: 命令参数,用于指定操作选项
4. `-implied-eval`: 命令参数,用于指定操作选项
5. `-arrow-callback`: 命令参数,用于指定操作选项

### 命令参数说明

- `-body-style`: 命令参数,用于指定操作选项
- `-magic-number`: 命令参数,用于指定操作选项
- `-D`: 命令参数,用于指定操作选项
- `-object-injection`: 命令参数,用于指定操作选项
...
- `-D`: 命令参数,用于指定操作选项
- `--write`: 命令参数,用于指定操作选项
- `-commit`: 命令参数,用于指定操作选项
- `-console`: 命令参数,用于指定操作选项
- `--init`: 命令参数,用于指定操作选项
    ...
    return {
      Literal(node) {
        if (typeof node.value === 'number' && !ignored.includes(node.value)) {
          context.report({
            node,
            message: '魔法数字 （根据实际场景填充） 应提取为命名常量',
