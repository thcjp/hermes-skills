...
- 执行`Mobile-First`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`Visual Hierarchy`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Visual Hierarchy`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 免费版、设计指南、涵盖基础设计原则、配色与排版系统、设计指南免费版、提供基础设计原则、与快速参考、排版尺度与基础无、障碍要求、适用于个人项目的、界面设计指导。这些能力在上述核心功能中均有对应处理逻辑。
