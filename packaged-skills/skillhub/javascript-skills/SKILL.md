...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Iterators and Generators | 支持 | 通过核心功能实现对应能力 |
| Group shorthand properties at the beginning of the object declaration. | 支持 | 通过核心功能实现对应能力 |
| Control Statements | 支持 | 通过核心功能实现对应能力 |
| Wrap immediately invoked function expressions (IIFE) in parentheses. | 支持 | 通过核心功能实现对应能力 |
| Events | 支持 | 通过核心功能实现对应能力 |
| Hoisting | 支持 | 通过核心功能实现对应能力 |
| Prefix comments with FIXME: or TODO: to annotate problems or suggest actions. | 支持 | 通过核心功能实现对应能力 |
| Blocks | 支持 | 通过核心功能实现对应能力 |
| Whitespace | 支持 | 通过核心功能实现对应能力 |
| Always include parentheses around arguments for clarity and consistency. | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `math`, `findindex`, `number`, `once`, `promise.allsettled`, `place`, `conventions`, `ternaries`, `isfinite`, `try...catch`, `comparison`, `arrow`, `coalescing`, `destructuring`, `nullish`
