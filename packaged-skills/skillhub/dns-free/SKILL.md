...
执行TTL迁移提示,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供TTL迁移提示相关的配置参数、输入数据和处理选项。

**输出**: 返回TTL迁移提示的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`TTL迁移提示`相关配置参数进行设置
### SPF基础检查

执行SPF基础检查,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供SPF基础检查相关的配置参数、输入数据和处理选项。

**输出**: 返回SPF基础检查的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`SPF基础检查`相关配置参数进行设置
### DMARC基础配置

执行DMARC基础配置,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供DMARC基础配置相关的配置参数、输入数据和处理选项。

**输出**: 返回DMARC基础配置的处理结果。- 验证执行结果，确认输出符合预期格式
