...
- 执行`规范规则`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`规范规则`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、IRIS、ObjectScript、代码审查方案、含批量处理、自定义规则与报告、面向企业级、开发团队的、代码审查与治理工、核心能力、批量代码审查与格、自定义规范规则与、团队级配置、结构化审查报告导、格式规范与性能检、Trap、深度分析与安全审、代码复杂度与重复。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
- 不适用: 需要人工判断的复杂决策场景
...
2. `-align`: 命令参数,用于指定操作选项
3. `-family`: 命令参数,用于指定操作选项
4. `-iris-standards`: 命令参数,用于指定操作选项
5. `-weight`: 命令参数,用于指定操作选项
