...
执行基础 IDE 操作操作,处理用户输入并返回结果。

**输入**: 用户提供基础 IDE 操作所需的参数和指令。

**输出**: 返回基础 IDE 操作的处理结果。

- 执行`基础 IDE 操作`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`基础 IDE 操作`相关配置参数进行设置
### 终端执行

执行终端执行操作,处理用户输入并返回结果。

**输入**: 用户提供终端执行所需的参数和指令。

**输出**: 返回终端执行的处理结果。

- 执行`终端执行`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`终端执行`相关配置参数进行设置
### Agent 委托

执行Agent 委托操作,处理用户输入并返回结果。

**输入**: 用户提供Agent 委托所需的参数和指令。

**输出**: 返回Agent 委托的处理结果。

- 执行`Agent 委托`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Agent 委托`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 面向团队的企业级、远程操作平台、含调试、委托与团队协作能、VSCode、节点工具专业版为、团队与企业提供端、远程操作能力、涵盖调试、工作区管理与团队、核心能力、调试能力、测试能力、将复杂任务委托给、工作区信息管理、多节点团队协作、安全策略与审计日。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
