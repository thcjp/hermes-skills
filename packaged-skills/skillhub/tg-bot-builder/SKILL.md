...
**输出**: 返回Reply/Inline键盘的处理结果。

- 执行`Reply/Inline键盘`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Reply/Inline键盘`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、机器人构建平台、支持支付集成、批量推送、状态机与高级群管、面向企业团队与专、业开发者的、机器人全功能构建、全生命周期管理与、高级业务集成、核心能力、在免费版基础上新、批量消息推送引擎、多轮对话状态机、高级群组权限矩阵、测试支持与性能监、控仪表盘、Use、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| 支付回调丢失 | pre_checkout超时 | 确认handler响应时间<10秒，执行ping命令测试网络连通性,检查防火墙和代理设置延迟 | P1 |
