...
本skill还覆盖以下能力场景: 设计多云架构、规划迁移策略、优化云成本、的架构设计与治理、资深云架构师技能、三大云平台的企业、级架构设计、核心能力包括多云、与混合云架构设计、迁移策略制定、Well、Architected、Framework、提供从现状评估到、运营优化的全流程、架构指导。这些能力在上述核心功能中均有对应处理逻辑。
### 输出格式
...
| Discovery | 支持 | 通过核心功能实现对应能力 |
| Migration | 支持 | 通过核心功能实现对应能力 |
| Operate | 支持 | 通过核心功能实现对应能力 |
| Design | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `references/multi-cloud.md`, `setting`, `依赖云服务`, `designing`, `需要网络连接`, `model`, `deployment`, `kubernetes`, `apply`, `guide`, `需要代码生成`, `optimization`, `assess`, `definition`, `ignore`
