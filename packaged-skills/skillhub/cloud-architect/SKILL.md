...
**处理**: 按照skill规范执行多云架构设计操作,遵循单一意图原则。
**输出**: 返回多云架构设计的执行结果,包含操作状态和输出数据。### 云迁移策略(6Rs 框架)
...
**处理**: 按照skill规范执行云迁移策略(6Rs 框架)操作,遵循单一意图原则。
**输出**: 返回云迁移策略(6Rs 框架)的执行结果,包含操作状态和输出数据。### 成本优化(FinOps)
...
**输出**: 返回成本优化(FinOps)的执行结果,包含操作状态和输出数据。### 高可用与灾备
- 多区域与多可用区部署拓扑设计
...
**处理**: 按照skill规范执行高可用与灾备操作,遵循单一意图原则。
**输出**: 返回高可用与灾备的执行结果,包含操作状态和输出数据。### 安全与合规
- 零信任架构与最小权限 IAM 策略设计
...
**处理**: 按照skill规范执行安全与合规操作,遵循单一意图原则。
**输出**: 返回安全与合规的执行结果,包含操作状态和输出数据。### 基础设施即代码
...
**处理**: 按照skill规范执行基础设施即代码操作,遵循单一意图原则。
**输出**: 返回基础设施即代码的执行结果,包含操作状态和输出数据。### Landing Zone 与治理
- 多账号订阅结构与组织单元设计
...
**处理**: 按照skill规范执行Landing Zone 与治理操作,遵循单一意图原则。
**输出**: 返回Landing Zone 与治理的执行结果,包含操作状态和输出数据。
#
## 使用流程
...
处理:立即触发回滚到本地环境;使用云厂商压测工具(Locust、k6)验证目标实例性能;执行ping命令测试网络连通性,检查防火墙和代理设置配置(MTU、ENA);确认数据库连接池与索引是否适配新环境;重新评估规格后再割接。
