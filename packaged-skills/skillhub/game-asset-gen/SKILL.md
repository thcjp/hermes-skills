...
- 执行`D资产`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`D资产`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全栈游戏资产生成、批量生成与多艺术、游戏资产生成工具、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-bridge`: 命令参数,用于指定操作选项
- `-knives`: 命令参数,用于指定操作选项
- `-outline`: 命令参数,用于指定操作选项
- `-style-recommendations`: 命令参数,用于指定操作选项
- `-barrel`: 命令参数,用于指定操作选项
...
- `-low-poly-village`: 命令参数,用于指定操作选项
...
- `-loop`: 命令参数,用于指定操作选项
- `-matte`: 命令参数,用于指定操作选项
- `-of-fire`: 命令参数,用于指定操作选项

### 命令参数说明

- `-Painted`: 命令参数,用于指定操作选项
- `-hammer`: 命令参数,用于指定操作选项
- `-levels`: 命令参数,用于指定操作选项
