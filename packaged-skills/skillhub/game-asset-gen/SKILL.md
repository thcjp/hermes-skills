...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| 游戏资产生成-专业版全栈游戏资产生成 | 不支持 | 支持 |
| 游戏资产生成-专业版批量生成 | 不支持 | 支持 |
| 复杂工作流可视化编排 | 不支持 | 支持 |
| 条件分支与异常重试 | 不支持 | 支持 |
| 定时触发与事件驱动 | 不支持 | 支持 |
...
针对2D资产,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供2D资产相关的配置参数、输入数据和处理选项.
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| content | string | 否 | game-asset-gen处理的内容输入 |,  |
| content | string | 否 | game-asset-gen处理的内容输入 |, 可选值: json/text/markdown |
