...
执行单仓库聚合分析操作,处理用户输入并返回结果。

**输入**: 用户提供单仓库聚合分析所需的参数和指令。

**输出**: 返回单仓库聚合分析的处理结果。

- 执行`单仓库聚合分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`单仓库聚合分析`相关配置参数进行设置
### 多仓库批量分析

执行多仓库批量分析操作,处理用户输入并返回结果。

**输入**: 用户提供多仓库批量分析所需的参数和指令。

**输出**: 返回多仓库批量分析的处理结果。

- 执行`多仓库批量分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多仓库批量分析`相关配置参数进行设置
### 自定义指标

执行自定义指标操作,处理用户输入并返回结果。

**输入**: 用户提供自定义指标所需的参数和指令。

**输出**: 返回自定义指标的处理结果。

- 执行`自定义指标`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`自定义指标`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 面向团队的企业级、Git、仓库协作分析平台、含批量分析、集成与企业报告、仓库协作分析工具、专业版为团队与企、业提供端到端、仓库协作分析能力、涵盖多仓库批量分、集成与企业级聚合、核心能力、多仓库批量分析与、自定义指标与团队、级聚合、流水线集成、周期性报告、历史趋势对比与基、线管理、团队流程讨论启动。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--is-inside-work-tree`: 命令参数,用于指定操作选项
- `-github-action`: 命令参数,用于指定操作选项
- `--pdf-engine`: 命令参数,用于指定操作选项
- `-depth`: 命令参数,用于指定操作选项
