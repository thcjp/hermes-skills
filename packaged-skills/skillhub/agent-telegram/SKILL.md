...
执行8 类角色账号映射操作,处理用户输入并返回结果。

**输入**: 用户提供8 类角色账号映射所需的参数和指令。

**输出**: 返回8 类角色账号映射的处理结果。

- 执行`类角色账号映射`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`类角色账号映射`相关配置参数进行设置
### 统一消息格式
...
- 执行`统一消息格式`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`统一消息格式`相关配置参数进行设置
### 四类汇报时机

执行四类汇报时机操作,处理用户输入并返回结果。

**输入**: 用户提供四类汇报时机所需的参数和指令。

**输出**: 返回四类汇报时机的处理结果。

- 执行`四类汇报时机`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`四类汇报时机`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 通信规范、类角色消息发送与、角色的、标识与消息发送格、向用户发送、消息时必须遵循此、统一使用、工具配合、确保消息正确路由、到用户账号、覆盖任务开始、子任务完成、任务全部完成四类、提供标准化消息模、板与多角色协作流、适用于多、协作开发、自动化工作流通知、团队任务进度同步、问题上报决策等场。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
