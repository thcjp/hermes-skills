...
**处理**: 按照skill规范执行工作区频道深度管理操作,遵循单一意图原则。

- 执行`工作区频道深度管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`搜索结果导出`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`搜索结果导出`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、集成工具、支持批量消息发送、消息模板与工作区、Hub、专业版、面向团队和企业的、全功能、核心能力、智能限流处理与重、试机制、消息模板库与变量、搜索结果导出与分。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
