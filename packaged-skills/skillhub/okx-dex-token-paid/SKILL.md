...
- 执行`支持链数`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`支持链数`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 代币分析与交易工、支持多链深度分析、批量查询与限价交、面向专业、DeFi、交易者与研究员的、深度数据分析、批量代币查询、链上行为追踪、流动性监控告警与、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
2. `--alert`: 命令参数,用于指定操作选项
3. `--top`: 命令参数,用于指定操作选项
4. `--output`: 命令参数,用于指定操作选项
5. `--price`: 命令参数,用于指定操作选项

### 命令参数说明

- `--amount`: 命令参数,用于指定操作选项
- `--mev-protection`: 命令参数,用于指定操作选项
- `--side`: 命令参数,用于指定操作选项
