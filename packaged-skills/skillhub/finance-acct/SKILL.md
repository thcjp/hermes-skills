...
- 执行`功能模块`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`功能模块`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级财务会计系、支持多账套、批量发票、集成与审计追踪、面向中大型企业财、务部门的全栈会计、处理系统、支持多公司多账套、全税种自动计算、批量电子发票生成、直连对账、系统集成、与完整审计追踪、Use、接口对接、Webhook、系统连接时使用、不适用于逆向工程、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--verify`: 命令参数,用于指定操作选项
2. `-sheet`: 命令参数,用于指定操作选项
3. `--template`: 命令参数,用于指定操作选项
4. `--include-unmatched`: 命令参数,用于指定操作选项
5. `-Dec`: 命令参数,用于指定操作选项
...
- `--parent`: 命令参数,用于指定操作选项
- `-Mar`: 命令参数,用于指定操作选项
- `--auto-reconcile`: 命令参数,用于指定操作选项
- `-Dec`: 命令参数,用于指定操作选项
- `--eliminate`: 命令参数,用于指定操作选项
