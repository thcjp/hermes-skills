...
**输出**: 返回安全态势评估的执行结果,包含操作状态和输出数据。

- 执行`安全态势评估`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`安全态势评估`相关配置参数进行设置
### 2. STRIDE+威胁建模
...
**输出**: 返回基础设施安全加固的执行结果,包含操作状态和输出数据。

- 执行`基础设施安全加固`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`基础设施安全加固`相关配置参数进行设置
### 5. 漏洞管理程序
...
**处理**: 按照skill规范执行漏洞管理程序操作,遵循单一意图原则。
**输出**: 返回漏洞管理程序的执行结果,包含操作状态和输出数据。

### 6. 事件响应
按严重性分级响应：SEV-1（活跃入侵/数据泄露，15分钟响应）、SEV-2（漏洞被利用，1小时）、SEV-3（可疑活动，4小时）、SEV-4（低风险，下一工作日）。

**输入**: 用户提供事件响应所需的指令和必要参数。
**处理**: 按照skill规范执行事件响应操作,遵循单一意图原则。
**输出**: 返回事件响应的执行结果,包含操作状态和输出数据。

- 执行`事件响应`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`事件响应`相关配置参数进行设置
### 7. 安全头部与浏览器安全
强制HTTP头配置：
...
**处理**: 按照skill规范执行安全头部与浏览器安全操作,遵循单一意图原则。
**输出**: 返回安全头部与浏览器安全的执行结果,包含操作状态和输出数据。

### 8. 认证授权深度审计
NIST 800-63B密码策略对齐、JWT安全清单（算法白名单、过期验证、签名校验）、OAuth 2.0/OIDC检查（强制PKCE、state参数验证、nonce防重放、最小权限scope）。

- 执行`认证授权深度审计`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`认证授权深度审计`相关配置参数进行设置
### 9. 安全程序设计
从零构建安全程序的分阶段路线图、安全度量仪表盘、渗透测试方法论与报告模板。

**输入**: 用户提供安全程序设计所需的指令和必要参数。
**处理**: 按照skill规范执行安全程序设计操作,遵循单一意图原则。
**输出**: 返回安全程序设计的执行结果,包含操作状态和输出数据。

- 执行`安全程序设计`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`安全程序设计`相关配置参数进行设置
### 输出格式
...
本skill还覆盖以下能力场景: 全面安全评估、漏洞管理与应急响、应体系、企业级网络安全评、估与加固体系、覆盖安全态势评估、基础设施加固、漏洞生命周期管理、与安全程序设计、核心能力、阶段安全评估方法、态势评估到评分体、威胁建模与风险优、先级矩阵、应用安全审计清单、驱动的漏洞管理、事件响应分级与通、信模板、密码策略与、认证审计、供应链安全审查。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Reconnaissance | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `permissions`, `ignoring`, `treating`, `lifecycle`, `focus`, `urls`, `burp`, `register`, `phases`, `natural`, `checklist`, `cryptographic`, `program`, `github`, `definition`
...
- `-Frame-Options未设置`: 命令参数,用于指定操作选项
