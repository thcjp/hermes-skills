...
- 执行`自动修复建议`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`自动修复建议`相关配置参数进行设置
### 2. 自动修复建议
```python
...
**处理**: 按照skill规范执行批量仓库诊断操作,遵循单一意图原则。
**输出**: 返回批量仓库诊断的执行结果,包含操作状态和输出数据。

### 4. 自动化故障恢复
> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供自动化故障恢复所需的指令和必要参数。
**处理**: 按照skill规范执行自动化故障恢复操作,遵循单一意图原则。
**输出**: 返回自动化故障恢复的执行结果,包含操作状态和输出数据。

### 5. 团队知识库
> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供团队知识库所需的指令和必要参数。
**处理**: 按照skill规范执行团队知识库操作,遵循单一意图原则。
**输出**: 返回团队知识库的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、辅助工具、支持智能冲突分析、批量诊断与团队知、提升协作效率、面向企业研发团队、的高级、提供智能冲突分析、团队知识库与故障、自动恢复、核心能力、智能冲突分析与自、批量仓库健康诊断、知识库与、FAQ、操作审计与回溯、团队工作流向导。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
2. `--diff-filter`: 命令参数,用于指定操作选项
3. `-C`: 命令参数,用于指定操作选项
4. `--global`: 命令参数,用于指定操作选项
5. `--name-only`: 命令参数,用于指定操作选项
