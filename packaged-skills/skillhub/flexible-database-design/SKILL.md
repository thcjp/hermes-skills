...
`flexible-database-design`按照以下流程处理用户请求:
...
本skill还覆盖以下能力场景: Guide、agents、users、implement、集成工具领域的专、辅助工具、基于高人气开源、深度优化升级、移除风险代码、增强安全性和稳定、适用场景、第三方、API、平台对接、数据同步、独立开发者与一人、公司效率提升。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 英文为主 | 支持 | 通过核心功能实现对应能力 |
| 建表成功，无报错 | 支持 | 通过核心功能实现对应能力 |
| 中文为主 | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `不同来源不一样`, `在用户项目中创建`, `文号等唯一标识查`, `存相对路径`, `场景速查`, `验证清单`, `能归档一条测试数`, `政策类建议`, `中文检索建议实现`, `随心所欲`, `真正构建`, `需要全文搜索吗`, `扫描件无法提取时`, `flexible_db.py`, `按需建索引`
