...
执行v1 稳定 CLI操作,处理用户输入并返回结果。

**输入**: 用户提供v1 稳定 CLI所需的参数和指令。

**输出**: 返回v1 稳定 CLI的处理结果。

- 执行`v1 稳定 CLI`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`v1 稳定 CLI`相关配置参数进行设置
### 输出格式
...
本skill还覆盖以下能力场景: Whisper、稳定版企业级转录、支持批量处理、性能调优与服务化、稳定版本的企业级、语音转文字工具、核心能力、涵盖免费版全部能、多格式输出、目录递归与任务队、多版本预加载与热、性能调优、半精度推理、批处理、注入领域术语、服务化部署、支持远程调用、置信度分析与校对、进度追踪与日志审。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
### 可用性分类
- **分类**: MD+EXEC()
- **说明**: 基于Markdown的AI Skill,。专业版基于 v1 稳定接口,支持批量处理、GPU 加速与服务化部署,适合长期维护的企业级转录场景。
