...
summary_zh: "SMTP邮件发送自动化,支持多供应商、附件、HTML模板与TLS加密。基于SMTP协议的邮件发送自动化Skill,支持Gmail、Outlook、QQ邮箱等主流服务商. 提供纯文本与HTML"
license: "MIT"
description: |-
...
针对多供应商适配,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供多供应商适配相关的配置参数、输入数据和处理选项.
**输出**: 返回多供应商适配的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`多供应商适配`的配置文档进行参数调优
### 双内容模式

针对双内容模式,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供双内容模式相关的配置参数、输入数据和处理选项.
**输出**: 返回双内容模式的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`双内容模式`的配置文档进行参数调优
### 多附件并行投递

针对多附件并行投递,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供多附件并行投递相关的配置参数、输入数据和处理选项.
