...
**处理**: 按照skill规范执行多媒体类型支持操作,遵循单一意图原则。
**输出**: 返回多媒体类型支持的执行结果,包含操作状态和输出数据。
...
- 执行`批量发送`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `批量发送` 选项
- 处理流程: 接收输入 -> 执行批量发送 -> 返回结果
- 输入: 用户提供批量发送所需的参数和指令
- 输出: 返回批量发送的执行结果,包含操作状态和输出数据
...
**处理**: 按照skill规范执行消息模板管理操作,遵循单一意图原则。

- 执行`消息模板管理`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `消息模板管理` 选项
...
**输出**: 返回发送频率控制的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、WhatsApp、多媒体消息平台、支持批量发送、视频音频文档、群组消息与模板管、多媒体消息发送专、Use、需要视频处理、音频编辑、媒体转换、配音生成时使用、不适用于版权受保、护的媒体内容处理、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--delay-min`: 命令参数,用于指定操作选项
- `--template`: 命令参数,用于指定操作选项
- `--variables-file`: 命令参数,用于指定操作选项
- `--datetime`: 命令参数,用于指定操作选项
...
- `--delay-max`: 命令参数,用于指定操作选项
- `-platform`: 命令参数,用于指定操作选项
- `--channel`: 命令参数,用于指定操作选项
- `--config`: 命令参数,用于指定操作选项
- `--rate-limit`: 命令参数,用于指定操作选项

### 命令参数说明

- `--report`: 命令参数,用于指定操作选项
- `-send-report`: 命令参数,用于指定操作选项
- `--filePath`: 命令参数,用于指定操作选项
- `-report`: 命令参数,用于指定操作选项
- `-poster`: 命令参数,用于指定操作选项

### 命令参数说明

- `--target`: 命令参数,用于指定操作选项
- `--daily-limit`: 命令参数,用于指定操作选项
- `--filePath`: 命令参数,用于指定操作选项
