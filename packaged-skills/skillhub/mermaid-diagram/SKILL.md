...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Keep node names short (≤10 chars) to avoid layout issues | 支持 | 通过核心功能实现对应能力 |
| Wrong arrow syntax (--> vs --- vs ->>) | 支持 | 通过核心功能实现对应能力 |
| Unclosed brackets | 支持 | 通过核心功能实现对应能力 |
| Missing quotes around labels with spaces/Chinese | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `timeline`, `项目时间线`, `mindmap`, `产品架构`, `产品经理常用图模`, `查询数据`, `数据库结构`, `带完整示例代码`, `用户状态`, `statediagram-v2`, `erdiagram`, `发起请求`, `系统结构`, `系统交互`, `sequencediagram`
