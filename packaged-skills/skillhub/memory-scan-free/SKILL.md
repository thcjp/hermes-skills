...
本skill还覆盖以下能力场景: 基础版、Agent、记忆文件安全扫描、检测恶意内容与凭、记忆文件的基础安、全扫描工具、帮助发现记忆文件、中的恶意指令、注入和凭证泄漏等、常见威胁、提供七大检测类别、基础隔离功能三大、核心能力、适用于、记忆日常安全审计、凭证泄漏排查、引入外部数据后安、全检查等场景、本地模式运行。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
