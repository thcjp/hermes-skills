...
- 执行`端点参考`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`端点参考`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 个第三方服务的、REST、含认证模式、分页与、参考文档库、个第三方服务的认、证模式、分页模式与、按类别组织、每类含索引表与逐、服务详解、提供多账户凭证命、名规范、错误处理模式、幂等键使用等工程、仅作文档参考、不代用户执行请求。这些能力在上述核心功能中均有对应处理逻辑。
### 输出格式
