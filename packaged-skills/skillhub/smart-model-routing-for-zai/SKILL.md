...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Analysis & Planning | 支持 | 通过核心功能实现对应能力 |
| Research synthesis | 支持 | 通过核心功能实现对应能力 |
| Comparisons and evaluations | 支持 | 通过核心功能实现对应能力 |
| Multi-step reasoning | 支持 | 通过核心功能实现对应能力 |
| Planning — roadmaps, task breakdowns | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `three`, `judgment`, `plus`, `based`, `implementation`, `strong`, `nuanced`, `technical`, `relative`, `factual`, `major`, `subagents`, `work`, `strategic`, `performance`
