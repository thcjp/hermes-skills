...
解析用户指令,执行Qwen视频智能分析的核心操作。使用Qwen多模态模型分析视频内容,支持本地文件和远程URL,可自定义提示词与抽帧频率 基于 Qwen 3.5 Plus 多模态模型对视频进行智能分析。支持本地视频文件和远程 URL 两种输入方式,
可自定义分析提示词与抽帧频率(FPS),灵活控制分析精度与 API 调用成本。
核心能力涵盖场景描述

**输出**: 返回指令解析与执行的执行结果,包含操作状态和输出数据。
...
本skill还覆盖以下能力场景: 动作识别、物体检测、视频摘要生成、内容审核与问答式、适用于内容创作辅、视频内容索引、媒体资产管理、教学视频分析、监控录像审查等场、Key、skill、platform、json、skills、dashscope、apiKey、字段读取。这些能力在上述核心功能中均有对应处理逻辑。
## 认证
...
- `-demo`: 命令参数,用于指定操作选项
- `-upload`: 命令参数,用于指定操作选项
- `-dashscope-api-key`: 命令参数,用于指定操作选项
