...
解析用户指令,执行Qwen视频分析LITE的核心操作。Qwen多模态模型视频分析基础版,支持本地视频文件场景描述与内容理解 Qwen 视频智能分析基础客户端(免费版)。基于 Qwen 3.5 Plus 多模态模型对本地视频文件进行内容分析,
支持场景描述与基础内容理解。使用默认抽帧频率与默认提示词,适合快速概览视频内容。
API Key 从 ~/.sk
...
处理输入数据,执行Qwen视频分析LITE的转换操作并输出结果。
...
本skill还覆盖以下能力场景: skill、platform、json、skills、dashscope、apiKey、字段读取、适用于个人创作者、快速理解视频内容、学习试用多模态分、析能力。这些能力在上述核心功能中均有对应处理逻辑。
## 认证
