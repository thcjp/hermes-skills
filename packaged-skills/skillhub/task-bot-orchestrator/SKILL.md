...
**处理**: 按照skill规范执行三、数据库集成（专业版独有）操作,遵循单一意图原则。
**输出**: 返回三、数据库集成（专业版独有）的执行结果,包含操作状态和输出数据。### 四、错误恢复（专业版独有）
| 功能 | 说明 |
|------|------|
...
**输出**: 返回四、错误恢复（专业版独有）的执行结果,包含操作状态和输出数据。### 五、多渠道通知（专业版独有）
| 渠道 | 适用场景 | 特点 |
|------|----------|------|
| 邮件 | 正式通知 | 支持附件、HTML模板 |
| 企业微信 | 团队协作 | 群消息、@提醒 |
...
**处理**: 按照skill规范执行五、多渠道通知（专业版独有）操作,遵循单一意图原则。
**输出**: 返回五、多渠道通知（专业版独有）的执行结果,包含操作状态和输出数据。### 六、监控仪表盘（专业版独有）
| 功能 | 说明 |
|------|------|
| 实时状态 | 查看当前运行中的任务与状态 |
...
**处理**: 按照skill规范执行七、团队协作（专业版独有）操作,遵循单一意图原则。
**输出**: 返回七、团队协作（专业版独有）的执行结果,包含操作状态和输出数据。### 八、免费版全部功能
...
本skill还覆盖以下能力场景: 全功能任务编排、含条件触发、数据库集成与监控、种角色场景、任务编排机器人、是在免费版基础上、的全功能升级、Agent、提供完整的任务编、排与自动化能力、在数据处理、定时调度、通知推送核心能力、解锁条件触发、团队协作七大高级、需要消息发送、邮件短信、通信集成时使用、不适用于垃圾信息。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| 数据库连接失败 | 连接配置错误/网络问题 | 验证连接字符串；执行ping命令测试网络连通性,检查防火墙和代理设置；检查防火墙 | 高 |
...
| 分布式Worker失联 | 网络问题/节点宕机 | 执行ping命令测试网络连通性,检查防火墙和代理设置；重启Worker；启用故障转移 | 高 |
