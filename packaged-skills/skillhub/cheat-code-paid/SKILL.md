...
**输出**: 返回智能预取的执行结果,包含操作状态和输出数据。

- 执行`智能预取`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `智能预取` 选项
...
**处理**: 按照skill规范执行团队共享知识库操作,遵循单一意图原则。

- 执行`团队共享知识库`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `团队共享知识库` 选项
...
本skill还覆盖以下能力场景: 企业级外部知识检、支持批量查询、知识缓存与团队共、面向团队与企业的、高级能力扩展工具、在免费版基础上扩、展批量查询、知识缓存等能力、核心能力、批量知识查询与并、自定义数据源接入、内部文档库、本地知识缓存与智、团队共享知识库与、查询历史、查询质量评分与结、果去重。这些能力在上述核心功能中均有对应处理逻辑。

**输入**: 用户提供查询质量评分所需的指令和必要参数。
...
- `-wiki-token`: 命令参数,用于指定操作选项
- `-docs`: 命令参数,用于指定操作选项
- `-git-token`: 命令参数,用于指定操作选项
- `-access-token`: 命令参数,用于指定操作选项
