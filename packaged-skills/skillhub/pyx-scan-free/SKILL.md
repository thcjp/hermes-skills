...
本skill还覆盖以下能力场景: 技能安全检查工具、对技能进行安全评、级和风险报告、免费版、评估维度、包括恶意指令、数据泄露、权限滥用、供应链风险等、返回信任评分、和置信度百分比、WebFetch、curl、两种调用方式、适用于、独立开发者、企业团队和自动化、工作流场景、不适用于非、技能的安全扫描。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
