...
- 执行`每晚 22:00 扫描明日日历`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 点扫描明日、基础飞书提醒、支持手动运行与、面向个人开发者的、日历提醒、免费版、将上午日程与下午、日程分别按、两种基础策略推送、owa、提供的、calendar、读取日历数据、CLI、适用于个人每日日、程前置提醒的基础、高级特性、跨时区处理、团队群共享、安静时段、多渠道通知、请升级付费版。这些能力在上述核心功能中均有对应处理逻辑。
## 依赖说明
...
1. `--session`: 命令参数,用于指定操作选项
2. `--system-event`: 命令参数,用于指定操作选项
