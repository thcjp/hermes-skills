...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Usage | 支持 | 通过核心功能实现对应能力 |
| Environment Variables | 支持 | 通过核心功能实现对应能力 |
| Parameters | 支持 | 通过核心功能实现对应能力 |
| Example | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语

本skill涉及以下领域术语: `id_rsa`, `google_api_key`, `optionally`, `model`, `repository`, `generates`, `anthropic_api_key`, `fetch`, `verify`, `google`, `network`, `moltblock.json`, `consider`, `anthropic`, `disclaimer`
