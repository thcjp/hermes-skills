...
- 技能文件结构: SKILL.md(主文件,≤100行)、REFERENCE.md(详细参考)、EXAMPLES.md(完整示例)
- front matter必填字段8个: slug、name、version、displayName(≤20字符)、summary(≤100字符)、license、description(≤1024字符)、tools(YAML数组格式)
- slug必须等于name必须等于文件夹名
- 引用文件最多1层深度,不支持深层嵌套
- `tools` 字段必须为YAML数组格式(如 `- read`,非 `- - read`)
- 脚本文件放入 `scripts/` 目录,不计入SKILL.md的行数限制
- 不适用于非AI技能的文档创建
