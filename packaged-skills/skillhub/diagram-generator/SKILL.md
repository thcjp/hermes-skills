...
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| mcp-diagram-generator MCP 服务器 | MCP 服务器 | 必需 | `npx -y mcp-diagram-generator` 远程拉取 |
| Node.js 运行时 | 命令行工具 | 必需 | 系统包管理器安装 |
| LLM API | API | 必需 | 由 Agent 内置 LLM 提供 |

### 可用性分类
- **分类**: MD+EXEC(Markdown 指令驱动,部分功能需 exec 验证 MCP 配置)
- **说明**: 基于自然语言指令驱动 Agent 调用 MCP 服务器生成图表
...
### 采集意图或读取已有文件
- 新建: 收集图类型/格式/方向/场景/文件名
- 编辑: 读取目标 `.drawio`/`.mmd`/`.excalidraw` 文件,解析现有结构
