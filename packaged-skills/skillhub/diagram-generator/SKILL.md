...
**输出**: 返回主工作流的执行结果,包含操作状态和输出数据。

- 执行`主工作流`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`主工作流`相关配置参数进行设置
### 采集意图
...
本skill还覆盖以下能力场景: 工具生成与编辑、支持网络拓扑、图表生成客户端、服务器将自然语言、意图转换为结构化、生成与编辑、三种格式的图表、系统架构、白板手绘等六类图、提供默认输出路径、自定义文件名、交互式采集、质量门校验、配置初始化等能力、管理输出目录、规范生成文件、适用于架构文档、流程规范、白板协作等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 必需 MCP 工具
...
3. `-overview`: 命令参数,用于指定操作选项
4. `-svc`: 命令参数,用于指定操作选项
5. `-cashier`: 命令参数,用于指定操作选项
6. `-tier`: 命令参数,用于指定操作选项
7. `-finance`: 命令参数,用于指定操作选项

### 命令参数说明

- `-employee`: 命令参数,用于指定操作选项
- `-gw`: 命令参数,用于指定操作选项
- `-manager`: 命令参数,用于指定操作选项
