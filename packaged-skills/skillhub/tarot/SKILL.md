...
summary_zh: "反思式塔罗抽取，用于情感支持和自我觉察，非临床非预测性。。tarot 是一个反思式塔罗抽取技能，用于情感支持和意义构建。核心理念是"塔罗是镜子而非预言"， 采用 `presence-firs"
license: "MIT"
description: |-
...
- 验证返回数据的完整性和格式正确性
- 参考`三牌阵抽取（`Three-card spread`: Situation / Tension / Next Step）`的配置文档进行参数调优
### 3. 22 张大阿卡纳牌解读库
