...
**处理**: 按照skill规范执行线程回复操作,遵循单一意图原则。### 历史读取
```bash
...
**输出**: 返回表情回应的执行结果,包含操作状态和输出数据。### 消息写作风格
...
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| Discord Bot Token | 凭证 | 必需 | 在 Discord Developer Portal 创建应用并获取 bot token,配置到 gateway config |
| LLM API | API | 必需 | 由 Agent 内置 LLM 提供 |

### 可用性分类
- **分类**:MD+EXEC(基于 Markdown 指令,消息操作需 Agent 具备命令行执行能力)
- **说明**:通过自然语言指令驱动 Agent 调用 message 工具完成 Discord 频道基础交互

---

> 想要全文搜索、消息编辑与删除、频道列表与详情查询、消息特效(balloons/invisible-ink)、按作者/时间分页检索等能力?升级到付费版 **discord-chat**,获取 Discord 频道全功能交互。
