...
- 执行`PNG`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`PNG`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级图表生成平、种高级图表类型、多格式导出与使用、图表工坊专业版是、一款面向团队与企、业的图表生成平台、在免费版四种基础、图表基础上、新增堆叠柱状图、热力图等、并提供自定义模板、统计分析等高级能、核心能力、一键应用企业品牌、四种格式、图表历史分析与使。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
