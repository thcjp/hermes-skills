...
- 需要安装浏览器扩展并保持Chrome运行
- 指示灯为红色时无法操作,需重新连接(点击扩展图标重新连接)
- `tmwd_text` 的 `max_chars` 默认5000字符,超出需分多次提取
- CSP严格的网站无法使用 `tmwd_exec`,需改用 `tmwd_elements`
- 跨域iframe内容不可访问(浏览器同源策略限制)
- 每次操作仅针对一个标签页
- 截图通过 `tmwd_exec` 执行 `screenshot()` 返回Base64数据,需手动解码保存
- 不适用于无头浏览器或远程浏览器场景
