...
本skill还覆盖以下能力场景: 控制用户、Chrome、浏览器进行页面读、表单填充和数据提、免费版、通过浏览器扩展、直接操作用户已登、录的浏览器会话、无需重新认证、支持页面文本提取、JavaScript、截图和标签页管理、适用于独立开发者、企业团队、和自动化工作流场、不适用于无头浏览、器或远程浏览器场。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
