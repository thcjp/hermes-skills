...
执行问题查询操作,处理用户输入并返回结果。

**输入**: 用户提供问题查询所需的参数和指令。

**输出**: 返回问题查询的处理结果。

- 执行`问题查询`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`问题查询`相关配置参数进行设置
### 批量操作
...
执行Webhook集成操作,处理用户输入并返回结果。

**输入**: 用户提供Webhook集成所需的参数和指令。

**输出**: 返回Webhook集成的处理结果。

- 执行`Webhook集成`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Webhook集成`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、Linear、管理工具、支持批量操作、高级分析与自定义、工具箱、是面向团队与项目、管理者的全功能、交互工具、在免费版基础上新、增批量操作、查询模板等高级能、核心能力、完整的问题查询、项目管理与团队协、作能力、批量操作引擎、支持批量创建、事件订阅与自动化。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| 现象 | 可能原因 | 解决方案 | 优先级 |
|------|----------|----------|--------|
| 批量导入部分失败 | 个别数据格式错误 | 查看失败日志，修正后单独执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 | 中 |
