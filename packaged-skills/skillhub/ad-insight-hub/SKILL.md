...
- 执行`参数自然语言翻译与端点编排`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`参数自然语言翻译与端点编排`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: AdMapix、广告情报、API、参数翻译、可信度标注四层能、面向广告投放与市、场分析场景的结构、化广告情报数据中、之上叠加参数自然、端点依赖编排、结果缓存复用、估算数据可信度、分级标注四层核心、支持广告创意搜索、参数翻译与端点编、排五大能力域、适用于买量团队竞、品创意监控、出海选品调研、广告素材趋势分析、开发者画像与、跨地区投放策略制。这些能力在上述核心功能中均有对应处理逻辑。
## 参数映射表
