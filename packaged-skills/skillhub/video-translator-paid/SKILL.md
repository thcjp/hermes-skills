...
**输出**: 返回多语言翻译支持的执行结果,包含操作状态和输出数据。

- 执行`任务优先级队列`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 企业级视频翻译与、配音平台、种语言、语音克隆与优先队、适合跨国内容本地、视频翻译专业版、Use、需要视频处理、音频编辑、媒体转换、配音生成时使用、不适用于版权受保、护的媒体内容处理、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--voice-clone`: 命令参数,用于指定操作选项
- `-F`: 命令参数,用于指定操作选项
- `-api-global`: 命令参数,用于指定操作选项
