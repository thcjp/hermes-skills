...
本skill还覆盖以下能力场景: 长篇网文拆文、Use、需要生成营销文案、写作内容、标题优化、内容创作时使用、不适用于纯技术文、档撰写、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 一个都对不上 → 记录「无匹配选题，未回填」，不改文件。 | 支持 | 通过核心功能实现对应能力 |
| 已有分析利用 | 支持 | 通过核心功能实现对应能力 |
| 写停靠状态 | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语

本skill涉及以下领域术语: `套路框架和可复现`, `质量不受影响`, `本对标作品`, `整书级写作技法视`, `轻量提及`, `循环单元`, `它是派生临时文件`, `输出落盘后跑`, `黑暗伦理是常规虚`, `用什么语言回复`, `质量阈值体系`, `如果没有明确目标`, `段原文锚点范例片`, `别名归类`, `与已有设定合并`
