...
本skill还覆盖以下能力场景: Use、模型调用、智能对话、Agent、LLM、应用时使用、不适用于需要、确定性的关键决策。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| site:demagog.org.pl szczepionki autyzm | 支持 | 通过核心功能实现对应能力 |
| site:factcheck.org Andrew Wakefield MMR | 支持 | 通过核心功能实现对应能力 |
| Main topics/subjects | 支持 | 通过核心功能实现对应能力 |
| Important keywords | 支持 | 通过核心功能实现对应能力 |
| Subject matter specialists | 支持 | 通过核心功能实现对应能力 |
| site:fullfact.org vaccines autism | 支持 | 通过核心功能实现对应能力 |
| site:healthfeedback.org vaccine safety | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `determine`, `tiktok`, `criteria`, `wikipedia`, `approach`, `synthesize`, `person`, `checklist`, `compare`, `spain`, `详细内容已移至`, `make`, `regional`, `portuguese`, `provided`
