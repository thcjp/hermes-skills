...
tools:
  ...
  - write
homepage: ""
category: "Development"---# Jira工具箱(专业版)
...
针对托管 OAuth 认证,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供托管 OAuth 认证相关的配置参数、输入数据和处理选项.
**输出**: 返回托管 OAuth 认证的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`托管 OAuth 认证`的配置文档进行参数调优
### JQL 搜索

针对JQL 搜索,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供JQL 搜索相关的配置参数、输入数据和处理选项.
**输出**: 返回JQL 搜索的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`JQL 搜索`的配置文档进行参数调优
### 查看议题详情

针对查看议题详情,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供查看议题详情相关的配置参数、输入数据和处理选项.
**输出**: 返回查看议题详情的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`查看议题详情`的配置文档进行参数调优
#
## 快速开始
...
中间产物模板参考: `assets/jira-api-toolkit_template`
...
| 429 Rate limited | 超过 10 请求/秒 | 降速、分批、 | P2 |
