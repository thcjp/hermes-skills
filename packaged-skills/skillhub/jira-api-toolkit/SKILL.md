...
执行托管 OAuth 认证,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供托管 OAuth 认证相关的配置参数、输入数据和处理选项。

**输出**: 返回托管 OAuth 认证的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`托管 OAuth 认证`相关配置参数进行设置
### JQL 搜索

执行JQL 搜索,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供JQL 搜索相关的配置参数、输入数据和处理选项。

**输出**: 返回JQL 搜索的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`JQL 搜索`相关配置参数进行设置
### 查看议题详情

执行查看议题详情,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供查看议题详情相关的配置参数、输入数据和处理选项。

**输出**: 返回查看议题详情的处理结果。- 验证执行结果，确认输出符合预期格式
