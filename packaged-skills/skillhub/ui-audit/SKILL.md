...
可扩展性和反馈机制。参照 `13-checklist-innovation.md` 评估 5 级原创性光谱（Level 0-4）。

- 执行`新界面设计检查清单（6 步流程）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
`31-patterns-navigation.md`（优先导航/离屏导航/粘性导航/底部导航）。

- 执行`UI 模式库匹配（12 类模式）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
标注 pass/warn/fail/na 状态。报告包含 macro_bets 对齐分析和 priority_fixes 优先修复列表。

- 执行`结构化审计报告生成（JSON 格式）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回Macro Bet 对齐分析的执行结果,包含操作状态和输出数据。

- 执行`Macro Bet 对齐分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 自动化、审计工具、原则评估界面的视、觉层级、样式和无障碍性、audit、是一个自动化、审计技能、Warp、Speed、Decisioning、框架评估界面设计、大支柱、Scaffolding、Crafting、级创新光谱和、输出包含视觉层级、无障碍性等必检项、以及导航、可用性、入门引导等上下文、适用于设计师、前端工程师和产品、团队的设计评审场。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| NARROW OPTIONS | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `references/10-checklist-new-interfaces.md`, `evaluate`, `ctas`, `tooltips`, `tommy`, `structuring`, `relative`, `references/21-patterns-progressive-disclosure.md`, `choosing`, `references/11-checklist-fidelity.md`, `screen`, `philosophy`, `respects`, `stronger`, `reports`
