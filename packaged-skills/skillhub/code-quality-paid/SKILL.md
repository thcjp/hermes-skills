...
**处理**: 按照skill规范执行自定义规则引擎操作,遵循单一意图原则。

- 执行`自定义规则引擎`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `自定义规则引擎` 选项
...
5. `--severity`: 命令参数,用于指定操作选项
6. `-gt`: 命令参数,用于指定操作选项
7. `--fail-on`: 命令参数,用于指定操作选项
8. `--format`: 命令参数,用于指定操作选项
9. `--output`: 命令参数,用于指定操作选项

### 命令参数说明

- `--project`: 命令参数,用于指定操作选项
- `-backend`: 命令参数,用于指定操作选项
