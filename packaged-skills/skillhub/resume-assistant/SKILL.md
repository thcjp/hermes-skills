...
- **导出工具**: Pandoc（Word导出）、LaTeX发行版（LaTeX导出）、浏览器（PDF导出）

### 依赖项
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| LLM API | API | 必需 | 由Agent内置LLM提供 |
| Pandoc | CLI工具 | 可选 | https://pandoc.org/installing.html |
| LaTeX发行版 | 编译器 | 可选 | https://www.latex-project.org/get/ |
| 浏览器 | 应用 | 可选 | 操作系统自带（PDF导出） |

### 可用性分类
- **分类**: MD+EXEC（纯Markdown指令，导出功能需要exec命令行执行能力）
- **说明**: 基于Markdown的AI Skill，通过自然语言或斜杠命令驱动Agent执行简历优化任务
