...
| 能力 | 免费版 | 付费版 |
|:-----|:-------|:-------|
| **TTL迁移策略**: 迁移前48小时将TTL降至300秒,确保旧缓存过期后再切换;迁移稳定24小时后恢复至3600-86400秒 | 支持 | 支持 |
| **邮件认证三件套**: SPF(单TXT记录)、DKIM(签名)、DMARC(策略聚合)必须同时配置,缺一不可;SPF单独配置不足以保证投递率 | 不支持 | 支持 |
| **CAA证书锁定**: 限制可签发证书的CA,配合iodef实现安全事件上报;未配置CAA时任何CA均可签发证书 | 不支持 | 支持 |
| **www规范化**: apex与www必须同时配置或互转,HTTPS重定向需双域名证书;未配置www会导致链接失效 | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
...
执行TTL迁移策略操作,处理用户输入并返回结果。

**输入**: 用户提供TTL迁移策略所需的参数和指令。

**输出**: 返回TTL迁移策略的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`TTL迁移策略`相关配置参数进行设置
### 邮件认证三件套

执行邮件认证三件套操作,处理用户输入并返回结果。

**输入**: 用户提供邮件认证三件套所需的参数和指令。

**输出**: 返回邮件认证三件套的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`邮件认证三件套`相关配置参数进行设置
### CAA证书锁定

执行CAA证书锁定操作,处理用户输入并返回结果。

**输入**: 用户提供CAA证书锁定所需的参数和指令。

**输出**: 返回CAA证书锁定的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`CAA证书锁定`相关配置参数进行设置
#
## 使用流程
