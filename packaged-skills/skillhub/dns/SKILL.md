...
本skill还覆盖以下能力场景: 记录配置、证书锁定与、代理排障、领域专业配置与排、覆盖记录生命周期、邮件认证链、证书授权限制与、CDN。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
