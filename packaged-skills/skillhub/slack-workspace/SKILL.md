...
本skill还覆盖以下能力场景: 全量工作区管理、ClawLink、OAuth、面向团队协作与工、作区治理的、全量管理技能、托管连接、团队管理、通话管理十二大能、Web、API、支持定时消息、频道创建、文件上传、用户组管理、审计日志等高级操、内置读写分级安全、读操作直接执行、写操作需用户确认、后调用、适用于项目频道搭、事件响应协作、文件分发、团队协调等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 工具调用规范
...
3. `--query`: 命令参数,用于指定操作选项
4. `-east-1`: 命令参数,用于指定操作选项
5. `--params`: 命令参数,用于指定操作选项
