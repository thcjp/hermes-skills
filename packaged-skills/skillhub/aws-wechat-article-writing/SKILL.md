...
本skill还覆盖以下能力场景: 从提纲或话题生成、完整初稿、支持改写续写润色、与多模型切换、面向微信公众号长、写作引擎、提纲或口述话题生、成完整初稿、覆盖改写、开头结尾优化等环、写作约束来自、与本篇、等文风字段、可调用、DeepSeek、GPT、Claude、兼容端点、或在模型未配置时、自动降级为当前对、话模型按相同约束、集成业务资料库引。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
