...
**处理**: 按照skill规范执行文件名生成操作,遵循单一意图原则。
**输出**: 返回文件名生成的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行截断标记操作,遵循单一意图原则。
**输出**: 返回截断标记的执行结果,包含操作状态和输出数据。

- 执行`截断标记`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`截断标记`相关配置参数进行设置
### 6. Git推送
从博客目录自动检测git状态并推送：
...
**处理**: 按照skill规范执行Git推送操作,遵循单一意图原则。
**输出**: 返回Git推送的执行结果,包含操作状态和输出数据。

- 执行`Git推送`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `git推送` 选项
...
**处理**: 按照skill规范执行部署链接返回操作,遵循单一意图原则。

- 执行`部署链接返回`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`部署链接返回`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: Markdown、文章发布到静态站、点生成器博客并推、送到代码仓库、自动分析内容提取、创建标签分类映射、添加截断标记、推送并返回部署链、映射机制、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
