...
| 场景 | 输入 | 输出 |
|:-----|:-----|:-----|
| 连任意外部应用并对其 | 目标数据与配置参数 | 处理结果与执行状态 |
| self操作执行 | self相关参数与配置 | 执行结果与返回数据 |
| self状态查询 | 查询条件与过滤选项 | 当前状态与详细信息 |
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| content | string | 否 | self-integration处理的内容输入 |,  |
...
```json
{
  ...
  "data": {
    "final_result": {
      "integration_result": "integration_result_value",
      "integration_metadata": "integration_metadata_value",
      "integration_status": "integration_status_value"
    },
    "execution_log": [
      {
        "step": 1,
...
中间产物模板参考: `assets/self-integration_template`
