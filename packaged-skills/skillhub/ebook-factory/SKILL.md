...
| section生成失败 | LLM调用失败或超时 | 执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令3次,仍失败跳过该节并标记SECTION_FAILED | SECTION_FAILED |
...
| 断点续写冲突 | 同一book_id正在生成中 | 返回错误,提示稍后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 | BOOK_LOCKED |
