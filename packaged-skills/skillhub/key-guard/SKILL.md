...
| 错误现象 | 可能原因 | 诊断步骤 | 解决方案 |
|:------|:------:|:------:|:------:|
| 无法启动Key Guard | 运行环境不满足要求 | 检查依赖项和运行环境 | 确保所有依赖项已安装，运行环境符合要求 |
| API Key无法识别 | API Key配置错误 | 检查API Key配置 | 确保API Key配置正确，无拼写错误 |
| 请求处理失败 | 请求内容格式不正确 | 检查请求内容格式 | 确保请求内容格式符合要求，参考输入格式章节 |
| 日志记录异常 | 日志文件权限问题 | 检查日志文件权限 | 确保日志文件可写，无权限限制 |
| 安全工具集成失败 | 集成配置错误 | 检查集成配置 | 确保集成配置正确，无错误信息 |
