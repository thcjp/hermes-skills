...
| 隔离操作失败 | 文件权限不足或被其他进程占用 | 检查文件写入权限，关闭占用该文件的进程后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 |
...
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:---|:---|:---|:---|
| Agent平台（支持SKILL.md的AI Agent） | 运行环境 | 必需 | 安装Claude Code / Cursor / Codex / Gemini CLI等 |
| Python 3.8+ | 运行时 | 必需 | 从python.org安装 |
| memory-scan.py | 核心扫描脚本 | 必需 | 随技能提供 |
| quarantine.py | 隔离脚本 | 必需 | 随技能提供 |
| schedule-scan.sh | 定时任务脚本 | 可选 | 随技能提供 |
| detection-prompt.md | LLM检测提示模板 | 可选 | 随技能提供（远程模式使用） |
| OpenAI API Key | API Key | 可选 | 用于远程LLM分析（`--allow-remote`模式） |

**API Key配置：**
- 本地模式：无需任何API Key，零网络请求
- 远程LLM模式（可选）：配置 `OPENAI_API_KEY` 环境变量，优先使用gpt-4o-mini
- 告警渠道（可选）：配置告警渠道环境变量，用于定时任务发送告警通知

**运行环境：** Windows / macOS / Linux均可运行，需Python 3.8+环境。
