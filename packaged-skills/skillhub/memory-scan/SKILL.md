...
本skill还覆盖以下能力场景: 记忆文件与工作区、检测恶意内容、凭证泄漏与注入攻、配置的安全扫描工、检测恶意指令、权限提升七大威胁、提供五级安全分级、本地模式与可选远、定时监控集成、心跳任务集成五大、核心能力、记忆日常安全审计、凭证泄漏排查、引入外部数据后安、协作前信任验证等、本地模式零网络请、可选远程、分析仅传输脱敏内。这些能力在上述核心功能中均有对应处理逻辑。
### 领域术语
本skill涉及以下领域术语: `already`, `redacts`, `不适用于纯技术文`, `scans`, `边界输入处理`, `provider`, `severity`, `complementary`, `消息和数据字段`, `minor`, `确保流程不中断`, `openclaw`, `audit`, `privilege`, `包含状态码`
