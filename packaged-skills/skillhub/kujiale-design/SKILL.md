...
执行户型搜索/上传操作,处理用户输入并返回结果。

**输入**: 用户提供户型搜索/上传所需的参数和指令。

**输出**: 返回户型搜索/上传的处理结果。

- 执行`户型搜索/上传`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`户型搜索/上传`相关配置参数进行设置
### 硬装风格选择

执行硬装风格选择操作,处理用户输入并返回结果。

**输入**: 用户提供硬装风格选择所需的参数和指令。

**输出**: 返回硬装风格选择的处理结果。

- 执行`硬装风格选择`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`硬装风格选择`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级室内设计工、支持多方案对比、自定义风格与团队、适配商业设计生产、酷家乐设计专业版、面向企业团队与专、业室内设计师的高、级智能设计工具、核心能力、多方案并行对比、支持同户型多风格、快速生成、批量渲染高清效果、图与全景图、自定义硬装风格库、与软装搭配、团队协作与方案版、优先渲染队列与企、业级技术支持。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--comment`: 命令参数,用于指定操作选项
- `--multi-style`: 命令参数,用于指定操作选项
...
- `-render`: 命令参数,用于指定操作选项
- `-team-a`: 命令参数,用于指定操作选项
- `-repo`: 命令参数,用于指定操作选项
