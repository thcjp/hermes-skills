...
**处理**: 按照skill规范执行多数据库引擎适配操作,遵循单一意图原则。### 性能诊断引擎
...
本skill还覆盖以下能力场景: 全功能数据库建模、多引擎、含性能诊断、团队协作与自定义、设计器、专业版、是企业级数据库建、模与演进管理工具、在免费版软、方法论基础上、扩展多数据库引擎、抽取器等高级能力、核心能力、三大引擎的统一建、模与差异化适配、内置性能诊断引擎、自动识别慢查询、版本管理与迁移脚、本生成、支持灰度发布与回、自定义抽取器模板、驱动的结构化数据、变更审批、权限分级。这些能力在上述核心功能中均有对应处理逻辑。
