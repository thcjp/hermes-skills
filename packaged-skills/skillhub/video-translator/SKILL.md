...
本skill还覆盖以下能力场景: 需要文本翻译、多语言转换、本地化处理时使用、不适用于专业医学、法律翻译认证、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 必须二选一： | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `或给了可访问的视`, `english`, `若用户明确指定目`, `字幕参数规则`, `必须遵守`, `en/zh/ko/ja/fr/ru/es/de`, `字段名固定`, `show`, `是否显示字幕`, `authorization`, `继续轮询`, `返回结果处理`, `视频配音`, `源语言支持`, `必须提取并传`
