...
本skill还覆盖以下能力场景: 全功能新闻聚合系、多格式导出与品牌、支撑企业级资讯场、每日新闻摘要专业、版是面向团队与企、业的全功能新闻聚、合与可视化系统、在免费版基础上解、品牌水印定制、登录态抓取八大高、级能力、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-gradient`: 命令参数,用于指定操作选项
2. `-right`: 命令参数,用于指定操作选项
3. `-serif`: 命令参数,用于指定操作选项
