...
summary_zh: "通过 Telegram Bot API 发送消息、管理聊天、处理文件与自动化机器人工作流。。Telegram Bot API 集成 Skill。支持发送文本、图片、文档、位置、投票等多类型消"
license: "MIT"
description: |-
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | Telegram 消息机器人处理的输入数据或指令 |
...
针对多类型消息发送,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供多类型消息发送相关的配置参数、输入数据和处理选项.
**输出**: 返回多类型消息发送的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`多类型消息发送`的配置文档进行参数调优
### 消息生命周期管理

针对消息生命周期,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供消息生命周期管理相关的配置参数、输入数据和处理选项.
**输出**: 返回消息生命周期管理的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`消息生命周期管理`的配置文档进行参数调优
### 聊天信息检索

针对聊天信息检索,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供聊天信息检索相关的配置参数、输入数据和处理选项.
