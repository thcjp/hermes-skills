...
**处理**: 按照skill规范执行Risk Detection(风险检测)操作,遵循单一意图原则。
**输出**: 返回Risk Detection(风险检测)的执行结果,包含操作状态和输出数据。
...
**输出**: 返回Structure Improvement(结构改进)的执行结果,包含操作状态和输出数据。
...
**输出**: 返回Guardrails(护栏机制)的执行结果,包含操作状态和输出数据。
