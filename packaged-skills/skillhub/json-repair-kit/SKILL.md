...
- 执行`Trailing Commas 修复`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`Hex/Octal Numbers 修复`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 解析修复格式错误、支持尾逗号、单引号等修复、修复工具通过、解析将格式错误的、如含尾逗号、未加引号键、对象并重新序列化、为有效、五种修复模式、内置备份与验证安、全机制。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
