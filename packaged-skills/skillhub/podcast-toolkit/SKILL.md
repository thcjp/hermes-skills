...
执行节目定位操作,处理用户输入并返回结果。

**输入**: 用户提供节目定位所需的参数和指令。

**输出**: 返回节目定位的处理结果。

- 执行`节目定位`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`节目定位`相关配置参数进行设置
### 选题规划

执行选题规划操作,处理用户输入并返回结果。

**输入**: 用户提供选题规划所需的参数和指令。

**输出**: 返回选题规划的处理结果。

- 执行`选题规划`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`选题规划`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级播客创作与、运营工具包、支持多节目管理、变现策略与、适配团队协作、面向团队与专业创、作者的播客创作与、核心能力、涵盖免费版全部能、检查清单、多节目矩阵管理、增长分析与数据驱、变现策略与商业化、辅助内容生成、切片文案、团队协作与角色分、跨平台发布与、嘉宾关系管理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
