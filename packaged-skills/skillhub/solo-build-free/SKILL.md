...
- 需要LLM支持,无LLM环境无法执行
- 依赖 `docs/plan/{trackId}/plan.md` 与 `spec.md` 存在,无计划无法执行
- MCP工具不可用时降级为Glob+Grep+Read,效率降低
- 视觉验证依赖浏览器/模拟器,无环境时跳过
- 多栈质量工具:Python(ruff/mypy/pytest)、JS/TS(pnpm lint/typecheck/test)、iOS(swiftlint/swift-format)、Android(detekt/ktlintCheck)
