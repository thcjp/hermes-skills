...
**输出**: 返回PRO版功能增强对比的执行结果,包含操作状态和输出数据。### 高级预测模型
| 模型 | 适用场景 | 准确度 |
| --- | --- | --- |
| 线性回归 | 线性趋势数据 | 中 |
| ARIMA | 时间序列预测 | 高 |
...
执行数据提取操作,处理用户输入并返回结果。

**输入**: 用户提供数据提取所需的参数和指令。

**输出**: 返回数据提取的处理结果。

- 执行`数据提取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`数据提取`相关配置参数进行设置
### 报告格式

执行报告格式操作,处理用户输入并返回结果。

**输入**: 用户提供报告格式所需的参数和指令。

**输出**: 返回报告格式的处理结果。

- 执行`报告格式`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`报告格式`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级财报分析系、支持批量处理、多格式导出、行业基准与深度预、面向专业分析师与、机构的财报分析系、支持批量文件处理、Markdown、行业基准对比、高级预测模型与自、定义报告模板、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
PRO版支持基于Jinja2的自定义模板。在templates目录创建HTML模板文件，使用变量占位符（如（根据实际场景填充）、（根据实际场景填充）），系统自动填充数据。
