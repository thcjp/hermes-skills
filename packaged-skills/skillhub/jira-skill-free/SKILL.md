...
**输出**: 返回jira.sh search "关键词" max的处理结果。

- 执行`jira.sh search "关键词" max`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`jira.sh search "关键词" max`相关配置参数进行设置

### 能力覆盖范围

本skill还覆盖以下能力场景: Cloud、REST、查看事务详情、状态流转与浏览器、适合只读浏览场景、集成助手免费版提、事务的只读浏览能、支持模糊搜索、事务详情查看、浏览器链接生成与、可用流转列表查看、事务详情与浏览器、自己的未关闭事务、升级付费版专享、工时记录、多维工时统计、适用场景、只读浏览、快速定位事务、查看自己的待办。这些能力在上述核心功能中均有对应处理逻辑。
## 核心能力（免费版）
