...
**输出**: 返回排版系统（Typography Scale）的执行结果,包含操作状态和输出数据。### Shadcn/ui + Tailwind CSS技术栈
...
**处理**: 按照skill规范执行Shadcn/ui + Tailwind CSS技术栈操作,遵循单一意图原则。### 响应式设计（Responsive Design）
Mobile-First响应式布局模式：
...
**处理**: 按照skill规范执行响应式设计（Responsive Design）操作,遵循单一意图原则。
**输出**: 返回响应式设计（Responsive Design）的执行结果,包含操作状态和输出数据。### 微交互设计（Micro-Interactions）
提升用户体验的微交互模式：
...
**处理**: 按照skill规范执行微交互设计（Micro-Interactions）操作,遵循单一意图原则。
**输出**: 返回微交互设计（Micro-Interactions）的执行结果,包含操作状态和输出数据。### 无障碍设计（Accessibility - WCAG 2.2）
WCAG 2.2无障碍标准合规：
...
**处理**: 按照skill规范执行无障碍设计（Accessibility - WCAG 2.2）操作,遵循单一意图原则。
**输出**: 返回无障碍设计（Accessibility - WCAG 2.2）的执行结果,包含操作状态和输出数据。### UI设计五法则（The 5 Laws of Beautiful UI）
...
**处理**: 按照skill规范执行UI设计五法则（The 5 Laws of Beautiful UI）操作,遵循单一意图原则。### 快速参考与构建前检查清单（Quick Reference & Pre-Build Checklist）
编码前需确认的检查清单：
...
**输出**: 返回快速参考与构建前检查清单（Quick Reference & Pre-Build Checklist）的执行结果,包含操作状态和输出数据。
### Size（大小）

执行Size（大小）,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供Size（大小）相关的配置参数、输入数据和处理选项。

**输出**: 返回Size（大小）的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`Size（大小）`相关配置参数进行设置
### Color（颜色）

执行Color（颜色）,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供Color（颜色）相关的配置参数、输入数据和处理选项。

**输出**: 返回Color（颜色）的处理结果。- 验证执行结果，确认输出符合预期格式
