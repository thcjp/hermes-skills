...
执行文件大小上限操作,处理用户输入并返回结果。

**输入**: 用户提供文件大小上限所需的参数和指令。

**输出**: 返回文件大小上限的处理结果。

- 执行`文件大小上限`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`文件大小上限`相关配置参数进行设置
### 批量转换

执行批量转换操作,处理用户输入并返回结果。

**输入**: 用户提供批量转换所需的参数和指令。

**输出**: 返回批量转换的处理结果。

- 执行`批量转换`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量转换`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: Markdown、全格式专业导出工、含批量并行、加密水印、模板云同步与、面向内容团队与企、在免费版基础上新、增批量并行转换、自定义样式表、水印加密、模板云端同步、版本对比与差异导、出等高级能力、配套面向技术写作、产品经理的多角色、场景指南、Use、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-serif`: 命令参数,用于指定操作选项
- `--encrypt-password`: 命令参数,用于指定操作选项
- `-bottom`: 命令参数,用于指定操作选项
- `-F`: 命令参数,用于指定操作选项
...
- `-X`: 命令参数,用于指定操作选项
- `-F`: 命令参数,用于指定操作选项
- `--watermark-opacity`: 命令参数,用于指定操作选项
- `-collapse`: 命令参数,用于指定操作选项
- `-H`: 命令参数,用于指定操作选项
