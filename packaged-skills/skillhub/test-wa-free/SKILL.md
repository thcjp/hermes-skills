...
执行文本消息发送操作,处理用户输入并返回结果。

**输入**: 用户提供文本消息发送所需的参数和指令。

**输出**: 返回文本消息发送的处理结果。

- 执行`文本消息发送`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`文本消息发送`相关配置参数进行设置
### 聊天列表查询
...
本skill还覆盖以下能力场景: 向他人发送、文本消息与查询聊、消息集成、Skill、免费版、实现向他人发送文、支持二维码登录认、证与基础消息发送、适用于向第三方发、送文本提醒、查找聊天等基础场。这些能力在上述核心功能中均有对应处理逻辑。
## 工具命令
