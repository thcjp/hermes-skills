...
summary_zh: "创建有效AI Skill的完整指南，覆盖frontmatter、SKILL.md结构与Progressive Disclosure设计原则"
license: "MIT"
description: |-
...
3. **Initialize the skill**：运行`（请参考skill目录中的脚本文件） <skill-name> --path <output-directory>`生成模板
4. **Edit the skill**：实现resources并编写SKILL.md，使用imperative/infinitive form
5. **Package the skill**：运行`（请参考skill目录中的脚本文件） <path/to/skill-folder>`验证并打包为.skill文件
