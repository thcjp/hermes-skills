...
执行单账号文本发送,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供单账号文本发送相关的配置参数、输入数据和处理选项。

**输出**: 返回单账号文本发送的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`单账号文本发送`相关配置参数进行设置
### 多角色账号映射

执行多角色账号映射,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供多角色账号映射相关的配置参数、输入数据和处理选项。

**输出**: 返回多角色账号映射的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`多角色账号映射`相关配置参数进行设置
### 批量消息调度

执行批量消息调度,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供批量消息调度相关的配置参数、输入数据和处理选项。

**输出**: 返回批量消息调度的处理结果。- 验证执行结果，确认输出符合预期格式
