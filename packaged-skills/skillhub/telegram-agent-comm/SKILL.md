...
针对单账号文本发送,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供单账号文本发送相关的配置参数、输入数据和处理选项.
**输出**: 返回单账号文本发送的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`单账号文本发送`的配置文档进行参数调优
### 多角色账号映射

针对多角色账号映射,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供多角色账号映射相关的配置参数、输入数据和处理选项.
**输出**: 返回多角色账号映射的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`多角色账号映射`的配置文档进行参数调优
### 批量消息调度

针对批量消息,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供批量消息调度相关的配置参数、输入数据和处理选项.
**输出**: 返回批量消息调度的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`批量消息调度`的配置文档进行参数调优
#
## 适用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | telegram-agent-comm处理的内容输入 |, 默认: 全部维度 |
