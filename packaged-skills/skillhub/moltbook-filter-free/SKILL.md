...
本skill还覆盖以下能力场景: 客户端过滤社区平、台代币铸造垃圾、去除率、支持自定义模式与、黑名单、免费版、社区平台客户端垃、圾内容过滤器、识别并移除数字资、产协议铸造机器人、批量垃圾帖、覆盖内容模式检测、过滤与共享黑名单、垃圾去除率、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
