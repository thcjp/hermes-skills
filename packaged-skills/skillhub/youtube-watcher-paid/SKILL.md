...
本skill还覆盖以下能力场景: 企业级、YouTube、内容分析平台、支持批量字幕提取、多语言对比与关键、适合内容研究团队、内容分析专业版、Use、需要文本翻译、多语言转换、本地化处理时使用、不适用于专业医学、法律翻译认证、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--cookies`: 命令参数,用于指定操作选项
- `--include-timestamps`: 命令参数,用于指定操作选项
- `--check-now`: 命令参数,用于指定操作选项
- `--incremental`: 命令参数,用于指定操作选项
- `--url`: 命令参数,用于指定操作选项

### 命令参数说明

- `--daemon`: 命令参数,用于指定操作选项
- `--query`: 命令参数,用于指定操作选项
- `--resume`: 命令参数,用于指定操作选项
- `-results`: 命令参数,用于指定操作选项
