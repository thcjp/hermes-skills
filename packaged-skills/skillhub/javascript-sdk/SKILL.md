...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| SvelteKit | 支持 | 通过核心功能实现对应能力 |
| Automatic Upload | 支持 | 通过核心功能实现对应能力 |
| Supported Frameworks | 支持 | 通过核心功能实现对应能力 |
| Manual Upload | 支持 | 通过核心功能实现对应能力 |
| Browser File Upload | 支持 | 通过核心功能实现对应能力 |
| Hono | 支持 | 通过核心功能实现对应能力 |
| Express | 支持 | 通过核心功能实现对应能力 |
| Remix | 支持 | 通过核心功能实现对应能力 |
| Express Proxy | 支持 | 通过核心功能实现对应能力 |
| Streaming Progress | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `workflows`, `greets`, `responses`, `attachments`, `ocean`, `overview`, `research`, `image`, `message`, `approval`, `allow`, `react`, `handling`, `hidden`, `files`
