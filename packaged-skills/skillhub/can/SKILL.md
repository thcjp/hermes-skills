...
**输出**: 返回内容完整性验证的执行结果,包含操作状态和输出数据。

- 执行`内容完整性验证`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回计算查找（非浏览查找）的执行结果,包含操作状态和输出数据。

- 执行`计算查找（非浏览查找）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回追加仅审计追踪的执行结果,包含操作状态和输出数据。

- 执行`追加仅审计追踪`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`追加仅审计追踪`相关配置参数进行设置
### 5. 篡改证明
你在特定时间用哈希标记了内容。如果有人后来修改内容，哈希不匹配日志记录。你知道了，他们无法隐藏。

**输出**: 返回篡改证明的执行结果,包含操作状态和输出数据。

- 执行`篡改证明`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`篡改证明`相关配置参数进行设置
### 6. 传输无关标记
CAN标记流经任何管道的内容：
...
本skill还覆盖以下能力场景: 时钟地址命名协议、通过时间戳、内容哈希和可读名、称为数据流建立可、验证索引、Clock、Address、Naming、协议为、Agent、数据流提供可验证、的内容寻址能力、通过三列结构、记录事件、实现内容完整性验、快速查找和、支持任意传输管道、本地追加日志、无需共识、或全局状态、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
...
4. `-task-result`: 命令参数,用于指定操作选项
5. `-output-2026-01-15`: 命令参数,用于指定操作选项
6. `-find-fast`: 命令参数,用于指定操作选项
7. `-f1`: 命令参数,用于指定操作选项
8. `-H`: 命令参数,用于指定操作选项
