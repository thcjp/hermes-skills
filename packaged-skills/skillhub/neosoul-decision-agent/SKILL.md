...
| 本地文件系统 | 存储 | 必需 | 用于存储 `~/decision-making/` 目录下的决策记忆文件 |
| Proactivity技能（可选） | 技能集成 | 可选 | 增强对待处理决策的跟进能力 |

**API Key配置：** 本技能无需任何API Key，无需凭证，无需网络连接。所有功能在本地完成，决策记忆存储在本地文件系统中。

**运行环境：** Windows / macOS / Linux均可运行，需Agent平台支持SKILL.md指令解析。
