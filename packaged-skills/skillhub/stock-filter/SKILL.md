...
- 执行`市场覆盖`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`市场覆盖`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 多市场股票筛选系、支持多策略组合、历史回测与批量导、面向专业投资者与、机构的股票筛选系、港股多市场筛选、多策略组合、自定义选股公式、历史回测验证与批、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
