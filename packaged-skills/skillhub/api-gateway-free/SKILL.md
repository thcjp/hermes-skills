...
- 执行`只读 GET 操作`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`只读 GET 操作`相关配置参数进行设置
### 技术细节
...
本skill还覆盖以下能力场景: 通过托管网关连接、等服务的只读路由、含基础连接管理与、托管式、网关路由服务免费、通过统一的、路由地址连接、等第三方服务、支持只读、操作与基础连接管、事件重放、写操作审批流程、高危操作审查等高、级功能需升级付费。这些能力在上述核心功能中均有对应处理逻辑。
### 输出格式
