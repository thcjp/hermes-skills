...
执行交互式变基操作,处理用户输入并返回结果。

**输入**: 用户提供交互式变基所需的参数和指令。

**输出**: 返回交互式变基的处理结果。

- 执行`交互式变基`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`交互式变基`相关配置参数进行设置
### 二分查找

执行二分查找操作,处理用户输入并返回结果。

**输入**: 用户提供二分查找所需的参数和指令。

**输出**: 返回二分查找的处理结果。

- 执行`二分查找`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`二分查找`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、Git、进阶方案、支持子树子模块、大型单体仓库与冲、突自动记忆、面向企业级研发团、进阶操作专业工具、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
3. `--prefix`: 命令参数,用于指定操作选项
4. `-lib`: 命令参数,用于指定操作选项
5. `-lib-standalone`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `--no-checkout`: 命令参数,用于指定操作选项
- `--follow`: 命令参数,用于指定操作选项
- `-feature`: 命令参数,用于指定操作选项
- `-C`: 命令参数,用于指定操作选项
- `--global`: 命令参数,用于指定操作选项

### 命令参数说明

- `--squash`: 命令参数,用于指定操作选项
- `--unset`: 命令参数,用于指定操作选项
...
- `--unshallow`: 命令参数,用于指定操作选项
- `--ours`: 命令参数,用于指定操作选项
- `-L`: 命令参数,用于指定操作选项
- `--recurse-submodules`: 命令参数,用于指定操作选项
- `-G`: 命令参数,用于指定操作选项
