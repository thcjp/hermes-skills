...
**处理**: 按照skill规范执行格式与类型对比操作,遵循单一意图原则。
**输出**: 返回格式与类型对比的执行结果,包含操作状态和输出数据。### 核心能力
```text
格式支持:
...
- 执行`流程图`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`流程图`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全格式图表生成引、种图表类型与批量、图表生成工具专业、面向团队的全格式、图表生成与管理平、三大格式全覆盖、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-network-topology`: 命令参数,用于指定操作选项
- `-diagram-generator__init_config`: 命令参数,用于指定操作选项
...
- `-diagram-generator__generate_diagram`: 命令参数,用于指定操作选项
- `-product`: 命令参数,用于指定操作选项
- `-sketch`: 命令参数,用于指定操作选项
- `-gate`: 命令参数,用于指定操作选项
- `-diagram-generator__get_config`: 命令参数,用于指定操作选项

### 命令参数说明

- `-qa`: 命令参数,用于指定操作选项
- `-dmz`: 命令参数,用于指定操作选项
- `-swimlane`: 命令参数,用于指定操作选项
- `-register-flow`: 命令参数,用于指定操作选项
- `-app`: 命令参数,用于指定操作选项

### 命令参数说明

- `-flowchart`: 命令参数,用于指定操作选项
- `-svc`: 命令参数,用于指定操作选项
- `-model`: 命令参数,用于指定操作选项
