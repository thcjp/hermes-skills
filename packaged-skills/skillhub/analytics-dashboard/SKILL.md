...
- 执行`Widget类型`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Widget类型`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能数据可视化、报表导出与团队协、支撑企业级监控、数据分析面板专业、版是面向团队与企、业的全功能数据可、视化与监控系统、在免费版基础上解、锁自定义、实时数据推送八大、高级能力、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--format`: 命令参数,用于指定操作选项
2. `--output`: 命令参数,用于指定操作选项
