...
summary_zh: "企业级JSON解析引擎，支持流式解析、批量处理、DataFrame转换、自定义展平策略与增量解析。。JSON解析引擎专业版面向企业级场景，在免费版基础上扩展流式解析、批量目录处理、DataF"
license: "MIT"
edition: "pro"
description: |-
...
**输出**: 返回批量目录处理的处理结果,包含执行状态码、结果数据和执行日志。### DataFrame表格转换
...
- 流式解析对非标准JSON格式（如含注释的JSON5、带尾随逗号的JSONC）支持有限，需预处理转换
- DataFrame转换时，深层嵌套JSON的展平策略可能导致数组字段展开为多行，改变原始数据结构语义
- 增量解析与断点续传依赖外部状态文件（如checkpoint），状态文件损坏或丢失时需从头重新解析
