...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Rejected | 支持 | 通过核心功能实现对应能力 |
| Confirmed | 支持 | 通过核心功能实现对应能力 |
| NEVER infer from observation alone | 支持 | 通过核心功能实现对应能力 |
| Conflict Resolution | 支持 | 通过核心功能实现对应能力 |
| Confirmation criteria | 支持 | 通过核心功能实现对应能力 |
| Three-Strike Confirmation | 支持 | 通过核心功能实现对应能力 |
| Hates "Great question!" openers | 支持 | 通过核心功能实现对应能力 |
| Stage | 支持 | 通过核心功能实现对应能力 |
| Transparency | 支持 | 通过核心功能实现对应能力 |
| Technical jargon OK | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `concise`, `compact`, `scope`, `prefers`, `testing`, `action`, `after`, `observed`, `confirming`, `most`, `stores`, `forget`, `cite`, `silence`, `dimensions.md`
