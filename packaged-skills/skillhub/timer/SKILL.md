...
本skill还覆盖以下能力场景: 在后台运行定时器、与闹钟、知提醒用户、支持多种时间格式、与多任务并行管理、在后台设置定时器、和闹钟、当后台定时器完成、会收到系统通知、必须将提醒内容传、达给用户、时及复合时间格式、可同时运行多个独、立定时器、每个定时器以独立、后台进程运行并拥、有唯一、适用于烹饪计时、番茄工作法、会议提醒、任务切换、休息提醒等需要准、确倒计时与到期通、知的场景、可对运行中的定时、器执行列出、终止等管理操作。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
