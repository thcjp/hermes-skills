...
本skill还覆盖以下能力场景: 扫描港股、股的公司公告、券商研报与社交媒、去噪后按、输出情绪温度计与、舆情监控与情绪分、析技能、股等公司公告、去噪后进行情绪打、支持多渠道信息采、事件类型量化与操、作建议生成、适用于投资决策辅、舆情预警与市场情、绪跟踪、Python、脚本执行、通过自然语言指令、Agent、完成任务。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
...
- **网络访问失败**: 微博、雪球等国内信源需网络可达; Google News与Twitter/X在受限网络下可能超时,可缩小信源范围或更换网络后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令。
...
- **输出编码乱码**: Windows终端默认编码可能非UTF-8,导致中文显示异常; 设置 `PYTHONIOENCODING=utf-8` 后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令。
