...
Recovery: 提示用户更换支付方式，或使用Idempotency-Key执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令
Doc: 搜索 "Stripe card_declined codes"
```

**输出**: 返回错误处理的执行结果,包含操作状态和输出数据。

- 执行`错误处理`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `错误处理` 选项
...
本skill还覆盖以下能力场景: 企业级、测试调试全套件、含批量回归、契约校验与团队协、工具箱专业版是面、向研发团队的全功、测试调试套件、在免费版的请求模、认证范式、错误诊断基础上、解锁批量回归测试、按服务细分的完整、错误码字典、团队协作空间六大、高级能力、覆盖从联调到上线、再到持续回归的完、整生命周期、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--concurrency`: 命令参数,用于指定操作选项
2. `--endpoint`: 命令参数,用于指定操作选项
3. `--report`: 命令参数,用于指定操作选项
4. `--method`: 命令参数,用于指定操作选项
5. `-Key执行ping命令测试网络连通性`: 命令参数,用于指定操作选项
...
- `-Key执行ping命令测试网络连通性`: 命令参数,用于指定操作选项
- `-response`: 命令参数,用于指定操作选项
- `-dict`: 命令参数,用于指定操作选项
- `--rampup`: 命令参数,用于指定操作选项
- `--response`: 命令参数,用于指定操作选项

### 命令参数说明

- `--service`: 命令参数,用于指定操作选项
- `--on-failure`: 命令参数,用于指定操作选项
- `-Type`: 命令参数,用于指定操作选项
