...
summary_zh: "用bird读X/Twitter时间线,增量去重生中文分类简报与JSON。基于 bird 命令行工具读取 X(Twitter)的 For You 与 Following 两条时间线, 对推文进"
license: "MIT"
description: |-
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | X 时间线摘要处理的输入数据或指令 |
...
**输入**: 用户提供去重所需的指令和必要参数.
**处理**: 解析去重的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回去重的处理结果,包含执行状态码、结果数据和执行日志。### 启发式过滤
...
**处理**: 解析启发式过滤的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。### 排序与修剪
按时间与质量信号排序,裁剪到 maxItemsPerDigest 条.
**输入**: 用户提供排序与修剪所需的指令和必要参数.
**输出**: 返回排序与修剪的处理结果,包含执行状态码、结果数据和执行日志。### 中文分类简报
...
**处理**: 解析中文分类简报的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
#
## 快速开始
