...
本skill还覆盖以下能力场景: 将冗长日志压缩为、倍压缩比、借鉴古文压缩哲学、实现零关键事件损、日志压缩器是面向、日志的智能压缩工、解决日志日均膨胀、词导致、爆炸的问题、核心能力包括、心跳集成自动维护、压缩比达、倍且零关键事件损、支持中英文混合日、志处理、适用于、每日日志归档、长会话上下文压缩、项目复盘提炼、记忆维护心跳任务、等场景、纯本地、脚本运行、无外部依赖、API、Key。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Growth: 进化, evolution, improvement… | 支持 | 通过核心功能实现对应能力 |
| Principle | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `three`, `behavior`, `chinese`, `before`, `cases`, `english`, `scenario`, `auto`, `curated`, `智能对话`, `append`, `standup`, `scans`, `logs`, `extraction`
...
- `-MM-DD`: 命令参数,用于指定操作选项
- `-memory-maintenance`: 命令参数,用于指定操作选项
