...
通过 GitHub API 查询仓库元数据：`curl -s "https://api.github.com/repos/OWNER/REPO" | jq '{stars: .stargazers_count, forks: .forks_count, updated: .updated_at}'`。
获取仓库文件列表：`curl -s "https://api.github.com/repos/OWNER/REPO/contents/skills/SKILL_NAME" | jq '.[].name'`。
...
```bash
# 1. 查询仓库元数据
curl -s "https://api.github.com/repos/example/skill-repo" | jq '{stars: .stargazers_count, forks: .forks_count, updated: .updated_at}'
# 输出：
# {
#   "stars": 342,
#   "forks": 18,
...
# 2. 获取技能文件列表
curl -s "https://api.github.com/repos/example/skill-repo/contents/skills/my-skill" | jq '.[].name'
# 输出：
# "SKILL.md"
# "scripts/helper.sh"
# "scripts/install.sh"
