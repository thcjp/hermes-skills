...
- 执行`完整企业级组件库`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`完整企业级组件库`相关配置参数进行设置
### 多主题系统
> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供多主题系统所需的指令和必要参数。
**处理**: 按照skill规范执行多主题系统操作,遵循单一意图原则。
**输出**: 返回多主题系统的执行结果,包含操作状态和输出数据。### 设计令牌多平台导出
```python
export_config = {
...
**处理**: 按照skill规范执行设计系统文档自动生成操作,遵循单一意图原则。### 视觉一致性审计
> 详细代码示例已移至 `references/detail.md`

**输入**: 用户提供视觉一致性审计所需的指令和必要参数。
**处理**: 按照skill规范执行视觉一致性审计操作,遵循单一意图原则。
**输出**: 返回视觉一致性审计的执行结果,包含操作状态和输出数据。### Framer Motion高级动效
```javascript
// 高级动效系统
const motionSystem = {
  presets: {
...
**处理**: 按照skill规范执行Framer Motion高级动效操作,遵循单一意图原则。
**输出**: 返回Framer Motion高级动效的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级极简设计系、统解决方案、支持完整组件库、多主题切换、设计审计与、适合团队与商业项、极简设计系统专业、版为企业与专业团、队提供系统化的极、简现代主义设计系、在免费版核心设计、令牌之上、增加完整组件库、集成能力、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-system`: 命令参数,用于指定操作选项
2. `-config`: 命令参数,用于指定操作选项
3. `--report`: 命令参数,用于指定操作选项
4. `-tokens`: 命令参数,用于指定操作选项
5. `-menu`: 命令参数,用于指定操作选项

### 命令参数说明

- `--themes`: 命令参数,用于指定操作选项
- `-latest`: 命令参数,用于指定操作选项
- `--with-stories`: 命令参数,用于指定操作选项
- `--with-docs`: 命令参数,用于指定操作选项
- `--scan`: 命令参数,用于指定操作选项

### 命令参数说明

- `-presets`: 命令参数,用于指定操作选项
- `--with-tests`: 命令参数,用于指定操作选项
- `--source`: 命令参数,用于指定操作选项
- `--formats`: 命令参数,用于指定操作选项
- `--base-theme`: 命令参数,用于指定操作选项
