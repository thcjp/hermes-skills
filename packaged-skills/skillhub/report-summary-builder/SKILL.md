...
`report-summary-builder`按照以下流程处理用户请求:
...
本skill涉及以下领域术语: `零散动作合并表达`, `今日完成`, `周报规则`, `而不仅是事项罗列`, `应突出阻塞`, `语言专业简洁`, `基于真实日报内容`, `整理时应优先识别`, `周报模板`, `异常原因为`, `输入模式`, `闭环导向`, `月报规则`, `工作类别`, `已形成结果的事项`
