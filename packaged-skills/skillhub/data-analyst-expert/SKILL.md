...
summary_zh: "全功能委派式分析平台,支持大数据流式处理、多任务编排、结果缓存与企业数据库集成。。数据分析专家专业版面向需要把复杂数据分析任务"委派"给 AI Agent 完成的专业用户与团队,提供完整的多"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| 数据分析专家(专业版)全功能委派式分析 | 不支持 | 支持 |
| 数据分析专家(专业版)支持大数据流式处理 | 不支持 | 支持 |
| 数据分析专家(专业版)多任务编排 | 不支持 | 支持 |
| 大数据集流式处理 | 不支持 | 支持 |
| 多数据源关联查询 | 不支持 | 支持 |
...
**处理**: 解析2 大数据流式处理的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 2.3 多任务编排
- **依赖图定义**:显式声明任务间依赖关系
- **并行执行**:无依赖任务自动并行
...
**处理**: 解析4 结果缓存与复用的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 2.5 定时任务与自动化分发
...
**处理**: 解析6 自定义输出模板的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 2.7 企业数据库集成
...
**处理**: 解析7 企业数据库集成的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回7 企业数据库集成的处理结果,包含执行状态码、结果数据和执行日志.
### 2.8 模板变量高级用法
```text
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | data-analyst-expert处理的内容输入 |,  |
| content | string | 否 | data-analyst-expert处理的内容输入 |, 可选值: json/text/markdown |
