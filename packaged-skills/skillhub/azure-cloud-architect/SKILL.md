...
- **jq**: 可选，用于 JSON 数据处理（从 stedolan.github.io/jq 安装）
...
| RBAC 权限 | 授权 | 必需 | 只读至少 Reader 角色，写操作需 Contributor |
| Agent LLM | API | 必需 | 由 Agent 内置 LLM 提供 |
| jq | 工具 | 可选 | 从 stedolan.github.io/jq 安装 |

### 凭据安全规范
绝不要在聊天或日志中输出密钥、客户端密钥、令牌。认证方式支持 `az login`（交互）或 `az login --service-principal`（服务主体）。如输出中意外包含密钥值，立即停止并提示用户轮换。
