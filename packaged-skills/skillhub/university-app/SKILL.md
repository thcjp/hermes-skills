...
- 执行`命理体系`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`命理体系`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 专业命理分析系统、支持八字、大运流年与批量排、面向命理研究者的、支持八字命理、大运流年推演与批、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--day`: 命令参数,用于指定操作选项
2. `--person1`: 命令参数,用于指定操作选项
3. `--format`: 命令参数,用于指定操作选项
4. `--parallel`: 命令参数,用于指定操作选项
5. `--hour`: 命令参数,用于指定操作选项

### 命令参数说明

- `--year`: 命令参数,用于指定操作选项
- `--gender`: 命令参数,用于指定操作选项
- `--person2`: 命令参数,用于指定操作选项
