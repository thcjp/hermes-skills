...
- 执行`触发器管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`触发器管理`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 通过托管、网关连接、等外部服务、含连接管理、事件重放与安全审、托管式、网关路由服务、通过统一的、路由地址连接、HubSpot、Salesforce、Airtable、Notion、等第三方服务、事件监听、目标配置、安全审批流程、只读优先、写操作需确认、两种调用方式、速率限制、适用于第三方、平台对接、自动化工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 输出格式
...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Watch Events | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `permissions`, `urls`, `refer`, `verify`, `invalid`, `github`, `详细内容已移至`, `retrieve`, `replay`, `community`, `handling`, `平台规范`, `treat`, `sending`, `upload`
...
- `-L`: 命令参数,用于指定操作选项
- `--limit`: 命令参数,用于指定操作选项
- `--event-type`: 命令参数,用于指定操作选项
- `--parameter`: 命令参数,用于指定操作选项
- `--json`: 命令参数,用于指定操作选项
...
- `-Type`: 命令参数,用于指定操作选项
- `-L`: 命令参数,用于指定操作选项
- `--destination`: 命令参数,用于指定操作选项
- `-X`: 命令参数,用于指定操作选项
- `--connection-id`: 命令参数,用于指定操作选项
...
| 速率超限 | 429 | 超过 10 请求/秒/账户 | 指数退避执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令（1s/2s/4s）,降低请求频率 |
