...
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| `MATON_API_KEY` | 环境变量 | 必需 | 网关控制台创建,仅作认证头使用 |
| `maton` CLI | 命令行工具 | 推荐 | `npm install -g @maton/cli` 或 `brew install maton-ai/cli/maton` |
| curl 或等价 HTTP 客户端 | 命令行工具 | 必需 | 系统自带或包管理器安装 |

### 可用性分类
- **分类**: MD+EXEC（Markdown 指令驱动,需 exec 执行 CLI 与 curl 命令）
- **说明**: 基于自然语言指令驱动 Agent 通过网关路由连接第三方服务
