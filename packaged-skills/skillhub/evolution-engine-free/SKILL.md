...
- 执行`纠错学习机制`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`纠错学习机制`相关配置参数进行设置
### 反污染防线

执行反污染防线操作,处理用户输入并返回结果。

**输入**: 用户提供反污染防线所需的参数和指令。

**输出**: 返回反污染防线的处理结果。

- 执行`反污染防线`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`反污染防线`相关配置参数进行设置
### 基础分层记忆

执行基础分层记忆操作,处理用户输入并返回结果。

**输入**: 用户提供基础分层记忆所需的参数和指令。

**输出**: 返回基础分层记忆的处理结果。

- 执行`基础分层记忆`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: Agent、自我进化、纠错学习加反污染、避免重复犯错与误、的基础自我进化引、提供纠错学习与反、污染防线两大核心、录到纠错日志、反污染防线确保永、不从沉默推断偏好、单次纠错不晋升永、基础分层记忆架构、热层加纠错层、适用于、编程助手避免重复、个人项目经验沉淀、等轻量场景、本免费版覆盖单、纠错学习需求、不含进化指标度量、命名空间隔离等高、级能力。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
