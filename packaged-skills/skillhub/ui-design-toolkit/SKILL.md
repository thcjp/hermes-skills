...
执行视觉层次操作,处理用户输入并返回结果。

**输入**: 用户提供视觉层次所需的参数和指令。

**输出**: 返回视觉层次的处理结果。

- 执行`视觉层次`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`视觉层次`相关配置参数进行设置
### 排版规范

执行排版规范操作,处理用户输入并返回结果。

**输入**: 用户提供排版规范所需的参数和指令。

**输出**: 返回排版规范的处理结果。

- 执行`排版规范`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`排版规范`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、设计工具包、支持设计系统、可访问性与组件库、适配团队协作与大、型项目、面向团队与企业用、核心能力、涵盖免费版全部能、Design、System、构建与维护、Tokens、深度规范与自动化、组件库抽象与复用、多主题与品牌切换、Motion、设计考虑、协作规范、设计文档与。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-live`: 命令参数,用于指定操作选项
2. `-D`: 命令参数,用于指定操作选项
3. `-design-system`: 命令参数,用于指定操作选项
4. `-library`: 命令参数,用于指定操作选项
5. `--color-brand-600`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `--color-brand-50`: 命令参数,用于指定操作选项
- `-D`: 命令参数,用于指定操作选项
- `--save-dev`: 命令参数,用于指定操作选项
- `-theme`: 命令参数,用于指定操作选项
- `--color-brand-500`: 命令参数,用于指定操作选项

### 命令参数说明

- `-D`: 命令参数,用于指定操作选项
- `-expect`: 命令参数,用于指定操作选项
- `--color-brand-700`: 命令参数,用于指定操作选项
