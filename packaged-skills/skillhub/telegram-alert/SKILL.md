...
- 执行`目标群组`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`目标群组`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 多通道交易告警系、支持多群组、复杂触发、富媒体消息与团队、面向专业交易团队、的多通道告警通知、多群组推送、富媒体消息、定时播报与团队协、作功能、Use、需要消息发送、通知推送、邮件短信、通信集成时使用、不适用于垃圾信息、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
