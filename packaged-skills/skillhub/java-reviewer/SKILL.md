...
- 执行`审查规则`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`审查规则`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、Java、代码审查方案、支持批量审查、报告与、面向企业级、开发团队的代码审、查治理工具、Use、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
4. `-family`: 命令参数,用于指定操作选项
5. `-start`: 命令参数,用于指定操作选项
...
- `-issues`: 命令参数,用于指定操作选项
- `-serif`: 命令参数,用于指定操作选项
- `-review-report`: 命令参数,用于指定操作选项
- `-prev`: 命令参数,用于指定操作选项
