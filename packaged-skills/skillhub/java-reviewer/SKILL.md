...
summary_zh: "企业级 Java 代码审查方案，支持批量审查、自定义规则、HTML 报告与 CI 集成。。面向企业级 Java 开发团队的代码审查治理工具。Use when 需要代码生成、编程辅助、调试测试"
license: "MIT"
edition: "pro"
description: |-
...
针对审查范围,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供审查范围相关的配置参数、输入数据和处理选项.
**输出**: 返回审查范围的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`审查范围`的配置文档进行参数调优
### 审查规则

针对审查规则,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
...
### API Key 配置
- 本工具为纯 Markdown 指令驱动，基础LLM由Agent平台提供
