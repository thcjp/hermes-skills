...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 必须兼顾手机可读性 | 支持 | 通过核心功能实现对应能力 |
| 行列标签要稳定，不要一会大写一会小写 | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `统一大厅`, `定游戏时`, `当前棋盘`, `开个扫雷`, `minesweeper`, `按标准模板渲染棋`, `number`, `显示游戏大厅`, `优先代码块`, `建立局面状态`, `口吻有趣`, `four`, `不能偷跳规则`, `通用命令`, `可以明确说明`
