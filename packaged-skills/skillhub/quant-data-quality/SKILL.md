...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 因子滞后天数？ | 支持 | 通过核心功能实现对应能力 |
| 因子更新频率？ | 支持 | 通过核心功能实现对应能力 |
| 滞后天数？ | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `适用项目`, `是否有极端值`, `因子数据时效性`, `信号数量是否合理`, `价格数据缺失`, `信号数据完整性`, `解决方案`, `信号过期`, `不同数据源的价格`, `修复计算逻辑`, `数据质量报告模板`, `问题清单`, `数据一致性`, `手动触发更新`, `严重程度`
