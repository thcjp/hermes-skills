...
- 执行`能力6：设计版本对比`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`能力6：设计版本对比`相关配置参数进行设置

- 执行`能力5：设计令牌转Tailwind`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`能力5：设计令牌转Tailwind`相关配置参数进行设置

- 执行`能力4：团队资源治理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`能力4：团队资源治理`相关配置参数进行设置

- 执行`能力3：设计变更监控`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`能力3：设计变更监控`相关配置参数进行设置
### 能力2：Webhook全生命周期管理
```python
...
本skill还覆盖以下能力场景: 企业级、设计协作工具、支持批量导出、设计变更监控与团、工作室专业版是一、款面向设计团队与、企业级设计工程化、场景的全功能、协作工具、在免费版的文件读、评论管理、图片导出、设计令牌提取基础、新增批量导出、与设计版本对比六、大高级能力、覆盖从单文件读取、到企业级设计系统、治理的全场景需求。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
