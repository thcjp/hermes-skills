...
- 执行`批量分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量分析`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 合并性预测、含全维度分析、门禁与自定义规则、合并检查工具、专业版、面向团队与维护者、全维度深度分析、历史趋势追踪、门禁集成与自定义、拒绝向量规则、核心能力、分析与团队级看板、维度深度分析、作者与仓库历史合、并趋势追踪、流水线门禁集成、自定义拒绝向量与、团队规则、风险预警与周报生。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
