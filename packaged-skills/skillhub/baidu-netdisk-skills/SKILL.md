...
**输出**: 返回转存的执行结果,包含操作状态和输出数据。

- 执行`转存`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `转存` 选项
...
**输出**: 返回分享的执行结果,包含操作状态和输出数据。

- 执行`分享`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `分享` 选项
...
**输出**: 返回搜索的执行结果,包含操作状态和输出数据。

- 执行`搜索`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `搜索` 选项
...
本skill还覆盖以下能力场景: 百度网盘文件管理、apps、支持上传下载转存、分享搜索与记忆备、命令行工具对百度、网盘进行文件操作、所有操作限制在、目录范围内、支持完整的文件生、命周期管理与、记忆备份恢复能力。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
