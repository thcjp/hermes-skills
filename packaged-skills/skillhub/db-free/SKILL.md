...
**处理**: 按照skill规范执行基础事务陷阱操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行基础查询优化操作,遵循单一意图原则。
### 基础数据完整性
...
本skill还覆盖以下能力场景: 识别并规避基础数、据库连接、查询与数据完整性、数据库设计与运维、免费版、提供基础的数据库、陷阱识别与规避能、核心能力包括、空闲连接清理、显式提交、外键索引、时区存储、金额精度、高级功能、扩展性规划。这些能力在上述核心功能中均有对应处理逻辑。
### 输出格式
