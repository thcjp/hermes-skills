...
本skill还覆盖以下能力场景: 企业级、互转工具、支持批量转换、映射与、专业版是一款面向、集成团队与企业级、数据交换场景的全、功能结构化数据格、式互转工具、在免费版的双向转、属性处理、命名空间基础上、新增批量转换、协议封装与数据库、直写六大高级能力、覆盖从单文件到企、业级数据交换流水、线的全场景需求、Use、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
