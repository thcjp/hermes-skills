...
活跃工作记忆，抗压缩与重启。参数：当前任务、关键上下文、待办动作、近期决策。输出：会话级状态文件，Agent 每次响应前先读取。遵循 WAL 协议：用户输入触发写入，先写状态再回复。文件结构含 Current Task、Key Context、Pending Actions、Recent Decisions 四个区块。
...
人类可读的长期记忆。参数：workspace 目录结构，MEMORY.md（摘要文件）+ memory/YYYY-MM-DD.md（每日日志）。输出：精炼后的长期记忆与每日活动记录。会话结束时将重要内容从 SESSION-STATE.md 迁移至 MEMORY.md，创建或更新每日日志。

- 执行`精选归档（MEMORY.md + daily/）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`精选归档（MEMORY.md + daily/）`相关配置参数进行设置
### 3. WAL 协议保障

写前日志机制确保数据持久。参数：触发条件（用户表达偏好/做出决策/给出期限/纠正代理）。输出：先写入 SESSION-STATE.md 再回复用户。若先回复再保存，崩溃或压缩会导致上下文丢失；WAL 确保写入优先，保障持久性。

- 执行`WAL 协议保障`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回记忆检索与关联的执行结果,包含操作状态和输出数据。

- 执行`记忆检索与关联`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 记忆系统、热内存加文件归档、抗压缩不丢上下文、的基础长期记忆系、提供热内存与文件、归档两层记忆能力、协议确保先写状态、提供人类可读的长、期记忆与每日日志、适用于、编程助手、智能对话等需要跨、会话记忆的基础场、本免费版覆盖单设、备记忆持久化需求、不遗忘关键上下文、与用户偏好、不适用于需要向量、跨设备同步或自动、事实抽取的高级场。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
