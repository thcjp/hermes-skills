...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Separate layers = stale package cache weeks later | 支持 | 通过核心功能实现对应能力 |
| Essential commands | 支持 | 通过核心功能实现对应能力 |
| Today's latest differs from tomorrow's — breaks immutable builds | 支持 | 通过核心功能实现对应能力 |
| OOM killer strikes without warning otherwise | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `published`, `infrastructure.md`, `leaks`, `localhost`, `inspect`, `anonymous`, `runtime`, `commands.md`, `combine`, `versions`, `exit`, `images.md`, `networks`, `configure`, `exitcode`
