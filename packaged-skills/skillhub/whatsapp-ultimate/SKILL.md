...
```bash
python3 scripts/message action=poll --to "group-id@g.us" --question "下周会议时间" --options "周一" "周二" "周三"
```
...
```bash
python3 scripts/multi-agent-discussion --group "project-group@g.us" --agents 3 --staleness-threshold 0.85
```

**输入**: 用户提供多Agent讨论所需的指令和必要参数。
**处理**: 按照skill规范执行多Agent讨论操作,遵循单一意图原则。
**输出**: 返回多Agent讨论的执行结果,包含操作状态和输出数据。
...
本skill还覆盖以下能力场景: 通讯应用消息发送、群组管理和多、讨论工具、支持文本、贴纸等多种消息类、支持投票、群组创建、历史搜索和多、通过协议库桥接通、讯应用、提供完整的消息交、互能力、适用于独立、开发者、企业团队和自动化、工作流场景、不适用于无通讯应、用账号的场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Conversation Lifecycle | 支持 | 通过核心功能实现对应能力 |
| Add Reaction | 支持 | 通过核心功能实现对应能力 |
| Get Invite Link | 支持 | 通过核心功能实现对应能力 |
| Random jitter prevents synchronization | 支持 | 通过核心功能实现对应能力 |
| Rate Limits | 支持 | 通过核心功能实现对应能力 |
| Budget-Aware Scheduling | 支持 | 通过核心功能实现对应能力 |
| Agreement loop detection — catches "I agree" / "Good point" / "Exactly" loops | 支持 | 通过核心功能实现对应能力 |
| Promote to Admin | 支持 | 通过核心功能实现对应能力 |
| Revoke Invite Link | 支持 | 通过核心功能实现对应能力 |
| Congestion Control (Exponential Courtesy Protocol) | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `quote`, `closure`, `history`, `receive`, `register`, `four`, `edit`, `comparison`, `raw_json`, `objective`, `make`, `leave`, `overview`, `fork`, `polls`
...
- `-sticker456`: 命令参数,用于指定操作选项
- `--emoji`: 命令参数,用于指定操作选项
- `-abc123def456`: 命令参数,用于指定操作选项
- `--question`: 命令参数,用于指定操作选项
- `-agent-discussion`: 命令参数,用于指定操作选项

### 命令参数说明

- `--agents`: 命令参数,用于指定操作选项
- `-xyz789`: 命令参数,用于指定操作选项
...
- `-voice789`: 命令参数,用于指定操作选项
- `--options`: 命令参数,用于指定操作选项
...
```bash
python3 scripts/message action=poll \
...
```bash
python3 scripts/message action=search \
...
| 消息发送超时 | 网络延迟超过60000ms窗口 | 执行ping命令测试网络连通性,检查防火墙和代理设置连接，执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令发送 |
