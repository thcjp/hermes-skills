...
### 可用性分类
- **分类**:MD+EXEC(基于 Markdown 指令,消息操作需 Agent 具备文件读取与执行能力)
- **说明**:通过自然语言指令驱动 Agent 调用 discord 工具的基础 JSON action

---

> 想要投票、线程管理、消息搜索、表情包/贴纸上传、成员角色查询、审核操作(禁言/踢出/封禁)、定时事件等全套能力?升级到付费版 **discord**,获取 Discord 全功能控制。
