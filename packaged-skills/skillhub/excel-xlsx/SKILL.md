...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Existing templates override generic styling advice. | 支持 | 通过核心功能实现对应能力 |
| Match styles for newly filled cells instead of quietly introducing a new visual system. | 支持 | 通过核心功能实现对应能力 |
| Only the top-left cell of a merged range stores the value. | 支持 | 通过核心功能实现对应能力 |
| Recalculate and review before delivery | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `mixed`, `filter`, `openpyxl`, `higher`, `sheets`, `libreoffice`, `cached`, `data-analysis`, `xlookup`, `verify`, `conditional`, `column`, `google`, `copying`, `protect`
