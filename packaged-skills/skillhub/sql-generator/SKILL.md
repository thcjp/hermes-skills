...
本skill还覆盖以下能力场景: 生成器、Use、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| cheatsheet — cheatsheet | 支持 | 通过核心功能实现对应能力 |
| migrate — migrate | 支持 | 通过核心功能实现对应能力 |
| create — create | 支持 | 通过核心功能实现对应能力 |
| optimize — optimize | 支持 | 通过核心功能实现对应能力 |
| mock — mock | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
### 领域术语
本skill涉及以下领域术语: `可用命令`, `基本聚合`, `mysql`, `powered`, `insert`, `commands`, `requests`, `bytesagain`, `join`, `order`, `专业建议`
