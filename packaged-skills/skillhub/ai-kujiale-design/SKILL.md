...
**输入**: 用户提供结果排序所需的参数和指令。
**处理**: 按照skill规范执行结果排序操作。

- 执行`结果排序`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`结果排序`相关配置参数进行设置
### 设计流程
...
本skill还覆盖以下能力场景: 酷家乐室内智能设、分步对话完成户型、风格选择、布局生成与渲染出、基于酷家乐、Kujiale、开放能力的室内智、能设计技能、通过分步式对话完、户型确认、布局生成、渲染出图全流程、核心能力、支持小区名搜索与、户型图上传两种路、自动识别并生成可、设计户型、风格匹配、基于偏好标签推荐、硬装风格、支持封面图预览与、单选确认、智能布局、自动布置家具、生成各房间布局方、案并支持确认、渲染出图、生成效果图与全景、按客餐厅、其他优先级输出、适用场景、业主装修前的方案、快速预览与风格试、室内设计师户型提、案与客户沟通辅助、房产经纪人对房源、进行户型展示与效、果包装、装修公司标准化方、案产出与批量出图。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
