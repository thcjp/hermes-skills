...
- 执行`数据追踪与对比分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`数据追踪与对比分析`相关配置参数进行设置

- 执行`批量下载与队列管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量下载与队列管理`相关配置参数进行设置

- 执行`视频发布管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 企业级、站运营工具、支持高清下载、批量操作、数据追踪与凭证安、面向专业、主与企业运营的全、核心能力、高清视频下载、超清画质下载、草稿管理、视频编辑、批量下载与处理、数据追踪监控、定时追踪视频播放、点赞等指标变化、视频对比分析。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
