...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Pagination | 支持 | 通过核心功能实现对应能力 |
| HTTPS enforced | 支持 | 通过核心功能实现对应能力 |
| Allowed methods and headers explicitly listed | 支持 | 通过核心功能实现对应能力 |
| CORS | 支持 | 通过核心功能实现对应能力 |
| SQL injection prevention | 支持 | 通过核心功能实现对应能力 |
| Authentication & Authorization | 支持 | 通过核心功能实现对应能力 |
| Monitoring | 支持 | 通过核心功能实现对应能力 |
| Security | 支持 | 通过核心功能实现对应能力 |
| All collection endpoints paginated | 支持 | 通过核心功能实现对应能力 |
| Filtering & Sorting | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `etags`, `many`, `route`, `urls`, `criteria`, `orchestration`, `strongly`, `websockets`, `performance`, `checklist`, `tokens`, `preflight`, `datadog`, `minimal`, `rbac`
