...
summary_zh: "全功能Linear管理工具，支持任务全生命周期、批量操作、GraphQL API与Git集成。Linear同步工具专业版是面向研发团队的完整项目管理命令行方案，在免费版基础上解锁任务全生命周"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| Linear同步(专业版)功能Linear管理 | 不支持 | 支持 |
| 代码静态分析与质量评分 | 不支持 | 支持 |
| 依赖漏洞检测与升级建议 | 不支持 | 支持 |
| 批量代码审查与报告生成 | 不支持 | 支持 |
| CI/CD流水线集成 | 不支持 | 支持 |
...
针对任务列表,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供任务列表相关的配置参数、输入数据和处理选项.
**输出**: 返回任务列表的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`任务列表`的配置文档进行参数调优
### 任务详情

针对任务详情,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
