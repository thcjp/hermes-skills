...
**处理**: 解析会议感知（Meeting Awareness）的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。
### 系统状态管理（System）
管理会议机器人的系统状态：
...
**输出**: 返回系统状态管理（System）的处理结果,包含执行状态码、结果数据和执行日志。
### 生命周期管理（Lifecycle）
...
**输出**: 返回语音状态（Voice State）的处理结果,包含执行状态码、结果数据和执行日志。
### TTS事件处理（TTS Events）
管理文本转语音（TTS）的播放生命周期：
...
**处理**: 解析TTS事件处理（TTS Events）的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。
**输出**: 返回TTS事件处理（TTS Events）的处理结果,包含执行状态码、结果数据和执行日志。### 会议转写与记录（Transcription）
实时转写会议内容并生成结构化记录：
...
**处理**: 解析会议转写与记录（Transcription）的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。
