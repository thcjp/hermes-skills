...
**输出**: 返回多文件管理的执行结果,包含操作状态和输出数据。### 发送队列
- **异步处理**: 非阻塞发送，提升吞吐量
- **优先级队列**: 支持文件发送优先级
- **队列监控**: 实时查看队列状态
- **暂停/恢复**: 队列暂停与恢复控制
...
**输出**: 返回发送队列的执行结果,包含操作状态和输出数据。### 重试与恢复
- **自动重试**: 可配置重试次数与间隔
- **断点续传**: 大文件分片上传支持断点续传
- **失败日志**: 完整记录失败原因
- **手动重发**: 一键重发失败文件
...
**输出**: 返回重试与恢复的执行结果,包含操作状态和输出数据。### 审计与合规
...
本skill还覆盖以下能力场景: 企业级文件批量分、发与稳定投递、支持审计与重试机、飞书文件发送专业、版面向企业用户与、高效能个人用户、Use、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--resume`: 命令参数,用于指定操作选项
2. `--concurrency`: 命令参数,用于指定操作选项
3. `--file-generator`: 命令参数,用于指定操作选项
4. `--rate-limit`: 命令参数,用于指定操作选项
5. `--target`: 命令参数,用于指定操作选项
