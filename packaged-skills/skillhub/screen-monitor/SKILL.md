...
本skill还覆盖以下能力场景: Use、模型调用、智能对话、Agent、LLM、应用时使用、不适用于需要、确定性的关键决策、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Setup | 支持 | 通过核心功能实现对应能力 |
| Load the unpacked extension from clawdbot browser extension path. | 支持 | 通过核心功能实现对应能力 |
| Click the Clawdbot icon in your Chrome toolbar to Attach. | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `backend`, `technical`, `interact`, `generates`, `best`, `frame`, `captures`, `references/backend-endpoint.js`, `screen_share_link`, `control`, `webrtc`, `web/screen-share.html`, `screen_analyze`, `share`, `deep`
