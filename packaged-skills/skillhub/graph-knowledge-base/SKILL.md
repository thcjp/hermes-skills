...
- 执行`知识分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`知识分析`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能知识图谱工、支持图谱关系查询、跨实体关联与多格、知识图谱库、是面向团队与知识、工作者的全功能知、识图谱管理工具、在免费版基础上新、增图谱关系查询、式导出等高级能力、核心能力、完整的实体管理、事实添加与版本替、代能力、发现跨实体隐含关、事实聚类与主题识、构建知识网络视图。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
