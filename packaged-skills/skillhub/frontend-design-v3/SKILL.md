...
本skill还覆盖以下能力场景: 生产级前端设计引、支持多框架输出、高级动效编排与可、访问性合规、前端设计工具、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-performance`: 命令参数,用于指定操作选项
- `-label`: 命令参数,用于指定操作选项
- `-width`: 命令参数,用于指定操作选项
- `-magazine`: 命令参数,用于指定操作选项
- `--framework`: 命令参数,用于指定操作选项

### 命令参数说明

- `-sm`: 命令参数,用于指定操作选项
- `--lighthouse`: 命令参数,用于指定操作选项
- `-a11y`: 命令参数,用于指定操作选项
- `--aesthetic`: 命令参数,用于指定操作选项
- `-delay`: 命令参数,用于指定操作选项
