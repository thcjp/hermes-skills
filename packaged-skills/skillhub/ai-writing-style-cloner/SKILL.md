...
本skill还覆盖以下能力场景: 篇文章克隆任意作、者文风、种公式批量产出同、风格爆款、写作分身工厂、上传任意作者的、篇文章、维反向提取风格指、生成可永久复用的、写作分身、让任何人用这个分、身写出的文章读起、来与原作者一模一、核心功能、PAS、金字塔原理等、持久化一次蒸馏永、智能公式推荐根据、风格自动匹配。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| LLM调用失败 | LLM API Key无效或服务不可用 | 返回错误+LLM错误详情,建议执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令或检查API Key | LLM_FAILED |
