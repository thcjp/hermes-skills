...
license: "Proprietary"
description: |-
  每天早上8点自动搜集并发布国际时事、经济形势、科技发展新闻的skill。基于历史模式和近期国际动向（如特朗普即将访华等），提供专业的新闻简报。Use when 需要AI模型调用、智能对话、Agent编排、LLM应用时使用。不适用于需要100%确定性的关键决策.
tags:
  - Research
  - 新闻
  - 信息
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | daily-news-brief处理的内容输入 |,  |
| content | string | 否 | daily-news-brief处理的内容输入 |, 可选值: json/text/markdown |
