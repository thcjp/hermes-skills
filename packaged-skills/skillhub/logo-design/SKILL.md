...
- 执行`自动矢量化`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`自动矢量化`相关配置参数进行设置

- 执行`批量Logo生成`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量Logo生成`相关配置参数进行设置
### 自动矢量化
...
**处理**: 按照skill规范执行品牌变体管理操作,遵循单一意图原则。### 设计质量审计
```python
# 自动设计质量检查
quality_audit = {
    "checks": [
...
本skill还覆盖以下能力场景: 企业级、设计系统、支持批量生成、适合团队与商业项、设计工具专业版为、企业与设计团队提、供系统化的、设计解决方案、在免费版基础生成、能力之上、增加批量生成、多格式导出、设计审计与、集成能力、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
3. `--vectorize`: 命令参数,用于指定操作选项
4. `-horizontal`: 命令参数,用于指定操作选项
5. `-dark`: 命令参数,用于指定操作选项

### 命令参数说明

- `-primary`: 命令参数,用于指定操作选项
- `-report`: 命令参数,用于指定操作选项
- `--checks`: 命令参数,用于指定操作选项
- `-mono`: 命令参数,用于指定操作选项
- `--variants`: 命令参数,用于指定操作选项

### 命令参数说明

- `-proposals`: 命令参数,用于指定操作选项
- `-serif`: 命令参数,用于指定操作选项
- `--client`: 命令参数,用于指定操作选项
- `--styles`: 命令参数,用于指定操作选项
- `--quality-check`: 命令参数,用于指定操作选项

### 命令参数说明

- `--directions`: 命令参数,用于指定操作选项
- `--auto-validate`: 命令参数,用于指定操作选项
- `-artifact`: 命令参数,用于指定操作选项
- `-guide`: 命令参数,用于指定操作选项
- `--generate-presentation`: 命令参数,用于指定操作选项

### 命令参数说明

- `-latest`: 命令参数,用于指定操作选项
- `--auto-vectorize`: 命令参数,用于指定操作选项
- `-primary-dark`: 命令参数,用于指定操作选项
- `--source`: 命令参数,用于指定操作选项
- `--parallel`: 命令参数,用于指定操作选项

### 命令参数说明

- `--variations-per-direction`: 命令参数,用于指定操作选项
- `-system`: 命令参数,用于指定操作选项
- `--brand-name`: 命令参数,用于指定操作选项
- `-project`: 命令参数,用于指定操作选项
- `-touch-icon`: 命令参数,用于指定操作选项

### 命令参数说明

- `-stacked`: 命令参数,用于指定操作选项
