...
执行全量文件操作操作,处理用户输入并返回结果。

**输入**: 用户提供全量文件操作所需的参数和指令。

**输出**: 返回全量文件操作的处理结果。

- 执行`全量文件操作`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`全量文件操作`相关配置参数进行设置
### 全文搜索

执行全文搜索操作,处理用户输入并返回结果。

**输入**: 用户提供全文搜索所需的参数和指令。

**输出**: 返回全文搜索的处理结果。

- 执行`全文搜索`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`全文搜索`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、命令行管理工具、支持模板、开发工具与、CLI、工具专业版是面向、团队和高级用户的、完整命令行知识管、理方案、在免费版基础上解、锁模板管理、插件与主题控制、文件版本历史、开发者调试工具、工作区布局管理和、交互模式等全部高、级能力、核心能力、模板读取与批量插、插件安装与重载、主题切换与、片段管理、同步状态与历史恢、文件版本对比与回、工作区保存加载、交互终端。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
