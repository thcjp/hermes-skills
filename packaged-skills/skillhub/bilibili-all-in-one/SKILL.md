...
- 执行`热门监控 (hot_monitor)`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`视频下载 (downloader)`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`数据追踪 (watcher)`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`数据追踪 (watcher)`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 站热门监控、播放与投稿一体化、工具集、站的六合一全功能、工具技能、集成热门监控、与视频投稿、六大模块、支持热门、分区排行实时获取、多清晰度下载与、播放量、评论长期追踪与多、视频对比、字幕下载与格式转、弹幕获取与播放列、表解析、以及视频上传、凭据支持环境变量、直接参数三种方式、持久化到、无凭据亦可使用热、标准清晰度下载、统计等公共、API。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
