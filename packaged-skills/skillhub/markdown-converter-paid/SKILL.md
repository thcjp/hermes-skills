...
- 执行`元数据提取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`元数据提取`相关配置参数进行设置

- 执行`自定义输出模板`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`自定义输出模板`相关配置参数进行设置

- 执行`批量目录扫描`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量目录扫描`相关配置参数进行设置
### 自定义输出模板
```yaml
# output-template.yml
template:
...
**输出**: 返回文档质量审计的执行结果,包含操作状态和输出数据。### 转换历史与版本管理
```python
# 转换历史管理
history_config = {
    "track_changes": True,
...
本skill还覆盖以下能力场景: 企业级文档批量转、换系统、支持目录扫描、批量处理、自定义模板、适合团队与商业项、转换器专业版为企、业与内容团队提供、系统化的文档转换、解决方案、在免费版基础转换、能力之上、增加批量处理、文档质量审计与、集成能力、Use、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-q1-report`: 命令参数,用于指定操作选项
- `-review`: 命令参数,用于指定操作选项
- `-launch`: 命令参数,用于指定操作选项
- `--preserve-structure`: 命令参数,用于指定操作选项
- `-log`: 命令参数,用于指定操作选项

### 命令参数说明

- `--deploy-preview`: 命令参数,用于指定操作选项
