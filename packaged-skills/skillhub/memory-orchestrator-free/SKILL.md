...
**处理**: 按照skill规范执行两层记忆存储操作,遵循单一意图原则。
### 2. 关键词检索
...
**处理**: 按照skill规范执行基础摘要生成操作,遵循单一意图原则。
### 4. 本地持久化
...
本skill还覆盖以下能力场景: 基础两层记忆管理、支持关键词检索与、本地持久化存储、记忆编排器免费版、提供基础记忆管理、支持短期与长期两、层记忆架构、核心能力包括、适用于简单的、Agent、记忆管理场景、当前会话上下文存、基础偏好记录、无需向量数据库、无需额外、API、开箱即用、如需四层架构、混合检索、健康度仪表盘、并发冲突解决等高、级能力、请升级到付费版。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
