...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| Markdown格式化工具专业版企业级文档批量格式化 | 不支持 | 支持 |
| 高级参数配置与自定义规则 | 不支持 | 支持 |
| 批量任务编排与队列管理 | 不支持 | 支持 |
| 结果导出与多格式转换 | 不支持 | 支持 |
| 实时状态监控与异常告警 | 不支持 | 支持 |
...
**处理**: 解析批量目录格式化的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 2. 自定义格式化模板
通过模板文件管理团队格式化规范:
...
**处理**: 解析自定义格式化模板的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回自定义格式化模板的处理结果,包含执行状态码、结果数据和执行日志.
### 3. 多格式输出
| 输出格式 | 适用场景 | 命令参数 |
|:-----|:-----|:-----|
...
**处理**: 解析多格式输出的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 4. 文档质量评分
...
**处理**: 解析Git 变更追踪与增量格式化的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回Git 变更追踪与增量格式化的处理结果,包含执行状态码、结果数据和执行日志。- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | markdown-format处理的内容输入 |, 默认: 全部维度 |
