...
- 执行`批量抓取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量抓取`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、含批量抓取、核心能力、批量抓取多语言、daily、weekly、monthly、与命中率统计、速率限制提升至、定时抓取并自动推、送到飞书、Discord、Telegram、自定义输出模板与。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--languages`: 命令参数,用于指定操作选项
2. `--limit`: 命令参数,用于指定操作选项
3. `-platform`: 命令参数,用于指定操作选项
4. `--since`: 命令参数,用于指定操作选项
