...
**处理**: 按照skill规范执行分层结构化存储操作,遵循单一意图原则。
### 2. 三层索引体系
...
本skill还覆盖以下能力场景: 基础分层持久记忆、解决跨会话遗忘与、记忆检索问题、Agent、的基础持久记忆系、在内置记忆之上构、建并行的结构化本、地存储、基础能力涵盖分层、完全位于、与内置、记忆并行运作、永不修改内置、适用于项目记忆、人脉管理、知识库构建等基础。这些能力在上述核心功能中均有对应处理逻辑。

**输出**: 返回写入前去重检查的执行结果,包含操作状态和输出数据。
