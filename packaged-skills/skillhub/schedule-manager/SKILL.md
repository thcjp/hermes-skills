...
**输出**: 返回功能二：失败自动重试与指数退避的执行结果,包含操作状态和输出数据。### 功能三：多通道失败告警
> 详细代码示例已移至 `references/detail.md`
...
**输出**: 返回功能三：多通道失败告警的执行结果,包含操作状态和输出数据。### 功能四：日历集成
```bash
...
**处理**: 按照skill规范执行功能四：日历集成操作,遵循单一意图原则。### 功能五：团队时区协调视图
```text
团队时区协调视图
┌────────────┬──────────┬──────────┬──────────┬──────────┐
...
**输出**: 返回功能五：团队时区协调视图的执行结果,包含操作状态和输出数据。### 功能六：监控仪表盘
```bash
...
**输出**: 返回功能六：监控仪表盘的执行结果,包含操作状态和输出数据。### 功能七：优先级与资源调度
```json
{
  "critical_report": {
    "task": "生成老板要看的关键报表",
