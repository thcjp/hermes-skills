...
执行托管OAuth连接操作,处理用户输入并返回结果。

**输入**: 用户提供托管OAuth连接所需的参数和指令。

**输出**: 返回托管OAuth连接的处理结果。

- 执行`托管OAuth连接`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`托管OAuth连接`相关配置参数进行设置
### 消息全操作

执行消息全操作操作,处理用户输入并返回结果。

**输入**: 用户提供消息全操作所需的参数和指令。

**输出**: 返回消息全操作的处理结果。

- 执行`消息全操作`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`消息全操作`相关配置参数进行设置
### 频道全管理

执行频道全管理操作,处理用户输入并返回结果。

**输入**: 用户提供频道全管理所需的参数和指令。

**输出**: 返回频道全管理的处理结果。

- 执行`频道全管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`频道全管理`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: Slack、全功能集成方案、含文件、批量操作与审计日、API、工具箱、为团队与企业提供、的全功能集成方案、覆盖消息、书签等全部能力、核心能力、端点覆盖、文件管理与上传、表情反应与书签管、操作审计日志、MCP、工具集成适配、Use、接口对接、Webhook、系统连接时使用、不适用于逆向工程。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
