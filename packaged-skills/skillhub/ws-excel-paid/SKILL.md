...
执行多表合并,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供多表合并相关的配置参数、输入数据和处理选项。

**输出**: 返回多表合并的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`多表合并`相关配置参数进行设置
### 透视表

执行透视表,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供透视表相关的配置参数、输入数据和处理选项。

**输出**: 返回透视表的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`透视表`相关配置参数进行设置
### 图表生成

执行图表生成,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供图表生成相关的配置参数、输入数据和处理选项。

**输出**: 返回图表生成的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`图表生成`相关配置参数进行设置
#
## 适用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| content | string | 否 | ws-excel处理的内容输入 |,  |
| content | string | 否 | ws-excel处理的内容输入 |, 可选值: json/text/markdown |
