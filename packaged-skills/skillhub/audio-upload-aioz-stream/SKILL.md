...
**输出**: 返回默认与自定义编码配置的处理结果。

- 执行`默认与自定义编码配置`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回HLS/DASH 流媒体输出的处理结果。

- 执行`HLS/DASH 流媒体输出`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: AIOZ、Stream、API、快速上传音频文件、支持默认与自定义、流媒体链接、的音频上传技能、通过三步流程、将本地音频文件上、流媒体平台、支持默认快速上传、上传完成后触发、服务端转码并返回、Key、public、secret、适用于播客、音乐分发、语音内容托管等场。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
