...
执行单文件转录操作,处理用户输入并返回结果。

**输入**: 用户提供单文件转录所需的参数和指令。

**输出**: 返回单文件转录的处理结果。

- 执行`单文件转录`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`单文件转录`相关配置参数进行设置
### 输出格式

执行输出格式操作,处理用户输入并返回结果。
...
本skill还覆盖以下能力场景: 企业级、Whisper、语音转文字工具、支持批量处理、说话人分离与、服务化、适配生产环境、面向团队与企业用、核心能力、涵盖免费版全部能、本地转录、多格式输出、目录级递归转录、支持任务队列、全面支持、diarization、多人对话识别、专业术语与品牌名、词优化、支持远程调用、模型管理、多版本切换与预加、置信度分析与人工、校对流程。这些能力在上述核心功能中均有对应处理逻辑。
### 单文件转录
...
12. `--language`: 命令参数,用于指定操作选项
13. `--port`: 命令参数,用于指定操作选项
14. `--output_format`: 命令参数,用于指定操作选项
15. `--output_dir`: 命令参数,用于指定操作选项
16. `-v3`: 命令参数,用于指定操作选项
