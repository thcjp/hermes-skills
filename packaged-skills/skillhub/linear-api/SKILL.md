...
本skill还覆盖以下能力场景: 操作工作项、标签与评论、覆盖创建查询转换、全流程、通过项目管理工具、API、操作工作项全生命、CRUD、评论与关联、工作流状态转换与、适用于独立开发者、企业团队、和自动化工作流场。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Cycles | 支持 | 通过核心功能实现对应能力 |
| Organization | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `before`, `list`, `oauth`, `permissions`, `系统连接时使用`, `post`, `不适用于逆向工程`, `uuids`, `authorization`, `getting`, `接口对接`, `verify`, `content`, `homebrew`, `correct`
...
- `-uuid-in-progress`: 命令参数,用于指定操作选项
- `-W30`: 命令参数,用于指定操作选项
- `-uuid-feature`: 命令参数,用于指定操作选项
- `-uuid-789`: 命令参数,用于指定操作选项
- `-uuid-456`: 命令参数,用于指定操作选项
