...
执行迁移管理操作,处理用户输入并返回结果。

**输入**: 用户提供迁移管理所需的参数和指令。

**输出**: 返回迁移管理的处理结果。

- 执行`迁移管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`迁移管理`相关配置参数进行设置
### 备份策略
...
本skill还覆盖以下能力场景: 面向企业的、SQL、全栈专业版、含自动化迁移、增量压缩备份、对比同步、高可用与读写分离、面向团队、企业与专业、DBA、全栈工具专业版、在免费版基础上新、增自动化迁移工具、增量与压缩备份、多数据库、对比与同步、查询性能基准测试、灾备恢复等高级能、配套面向运维、数据工程师、的多角色场景指南、Use、需要数据库操作、数据存储管理时使、不适用于数据库架、构设计决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
