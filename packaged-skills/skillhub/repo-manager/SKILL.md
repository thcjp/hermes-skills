...
本skill还覆盖以下能力场景: 全功能仓库管理工、含工作流管理、工具目录搜索与团、适合企业级仓库治、仓库管理器、专业版、是企业级、仓库管理工具、在免费版基础能力、扩展工作流管理、自定义工具集成与、团队共享配置等高、级能力、核心能力、重新运行、与构建产物、批量调用工具、含并行执行与结果、全文搜索可用工具、快速定位、统一管理团。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--label`: 命令参数,用于指定操作选项
- `-ci-tool`: 命令参数,用于指定操作选项
- `--notify-team`: 命令参数,用于指定操作选项
- `--strict-json`: 命令参数,用于指定操作选项
