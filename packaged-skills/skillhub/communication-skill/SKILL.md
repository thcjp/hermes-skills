...
**输出**: 返回四层倾听模型(LISTEN)的处理结果,包含执行状态码、结果数据和执行日志。### 五大沟通原则(CONSIDER)
...
**输出**: 返回五大沟通原则(CONSIDER)的处理结果,包含执行状态码、结果数据和执行日志。### 三种结构化表达模式(CRAFT)
...
**处理**: 解析语气校准矩阵的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。
**输出**: 返回语气校准矩阵的处理结果,包含执行状态码、结果数据和执行日志。### 六点校验清单(REFINE)
