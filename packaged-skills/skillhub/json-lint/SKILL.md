...
summary_zh: "企业级JSON校验工具，支持并行扫描、Schema验证、超集兼容、自动修复与监控告警。。JSON校验工具专业版面向企业级场景，在免费版基础上扩展并行扫描、JSON Schema验证、JSON"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| JSON校验工具专业版企业级JSON校验 | 不支持 | 支持 |
| JSON校验工具专业版支持并行扫描 | 不支持 | 支持 |
| JSON校验工具专业版自动修复与监控 | 不支持 | 支持 |
| 代码静态分析与质量评分 | 不支持 | 支持 |
| 依赖漏洞检测与升级建议 | 不支持 | 支持 |
...
**处理**: 解析JSON Schema验证的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回JSON Schema验证的处理结果,包含执行状态码、结果数据和执行日志。### 超集兼容
| 超集 | 特性 | 适用场景 |
|:-----|:-----|:-----|
...
**处理**: 解析超集兼容的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回超集兼容的处理结果,包含执行状态码、结果数据和执行日志。### 自动修复建议
- 定位错误位置（行列号+上下文）
- 生成修复方案（修改前/修改后对比）
- 修复置信度评分
- 一键应用修复（需确认）
...
**处理**: 解析自动修复建议的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。### 监控告警
- 通过率历史趋势（日/周/月）
- 通过率下降自动告警
- 新增错误文件告警
- 错误类型分布统计
...
**处理**: 解析监控告警的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回监控告警的处理结果,包含执行状态码、结果数据和执行日志.
### JSON5

针对JSON5,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供JSON5相关的配置参数、输入数据和处理选项.
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | json-lint处理的内容输入 |, 默认: 全部维度 |
...
- JSON Schema验证的准确性取决于Schema定义质量，低质量或宽松的Schema可能漏报语义层面的数据错误
- JSON5/JSONC超集兼容模式下修复的文件（含注释、尾随逗号）在严格JSON解析环境中可能被拒绝
- 并行扫描的吞吐量受CPU核心数限制，万级以上文件目录的全量扫描仍需数分钟完成
