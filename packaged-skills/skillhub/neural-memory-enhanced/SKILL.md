...
summary: "基于扩散激活的联想记忆系统，通过神经图谱实现智能持久化召回。生物学启发的联想记忆系统，采用扩散激活替代关键词检索和向量搜索。记忆以神经图谱形式组织， 神经元之间通过20种类型化突触连接，频繁"
summary_zh: "基于扩散激活的联想记忆系统，通过神经图谱实现智能持久化召回。生物学启发的联想记忆系统，采用扩散激活替代关键词检索和向量搜索。记忆以神经图谱形式组织， 神经元之间通过20种类型化突触连接，频繁"
license: "MIT"
description: |-
...
**输出**: 返回20种类型化突触关联的处理结果,包含执行状态码、结果数据和执行日志.
### 4. 记忆生命周期管理
...
- 参考`矛盾检测与冲突解决`的配置文档进行参数调优

**处理**: 解析矛盾检测与冲突解决的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 6. 赫布学习强化
...
**处理**: 解析赫布学习强化的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回赫布学习强化的处理结果,包含执行状态码、结果数据和执行日志.
...
**处理**: 解析大脑版本快照与回滚（nmem_version）的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
...
| SQLite数据库锁冲突 | 多个进程同时写入同一大脑的数据库文件 | 关闭重复的 Agent 实例，等待5秒后；若持续锁死，删除 `~/.neuralmemory/brains/<brain>.db-wal` 文件后重启 |
