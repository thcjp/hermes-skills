...
本skill还覆盖以下能力场景: Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
...
| Advanced operations | 支持 | 通过核心功能实现对应能力 |
| Open PR, get review | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `before`, `safety`, `clean`, `types`, `squash`, `docs`, `detached`, `push`, `usually`, `containerization`, `branching.md`, `gitlab`, `commands.md`, `hotfix`, `conflicts.md`
