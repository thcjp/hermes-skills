...
执行数据源用户驱动配置操作,处理用户输入并返回结果。

**输入**: 用户提供数据源用户驱动配置所需的参数和指令。

**输出**: 返回数据源用户驱动配置的处理结果。

- 执行`数据源用户驱动配置`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`数据源用户驱动配置`相关配置参数进行设置
### YAML报表配置

执行YAML报表配置操作,处理用户输入并返回结果。

**输入**: 用户提供YAML报表配置所需的参数和指令。

**输出**: 返回YAML报表配置的处理结果。

- 执行`YAML报表配置`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`YAML报表配置`相关配置参数进行设置
### Cron定时调度

执行Cron定时调度操作,处理用户输入并返回结果。

**输入**: 用户提供Cron定时调度所需的参数和指令。

**输出**: 返回Cron定时调度的处理结果。

- 执行`Cron定时调度`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Cron定时调度`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 配置自定义周期性、用户定义数据源、自动处理调度与格、式化投递、核心能力包括数据、与环境变量安全凭、证管理。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Frequency | 支持 | 通过核心功能实现对应能力 |
| On-demand | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `scheduling`, `monday`, `historical`, `please`, `list`, `delivery.md`, `scope`, `managing`, `generates`, `channels`, `variables`, `reply`, `delivers`, `environment`, `统计洞察`
...
| 数据源不可达 | API端点宕机或网络超时 | 检查API端点URL是否正确,确认网络连通性,设置执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令机制或切换备用端点 |
