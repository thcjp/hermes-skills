...
| 能力 | 免费版 | 付费版 |
|:-----|:-------|:-------|
| **数据源用户驱动配置**: 用户指定追踪的数据内容,如需外部API则由用户提供凭证,凭证以环境变量引用存储而非明文值 | 支持 | 支持 |
| **YAML报表配置**: 在 `~/report/{name}/config.md` 中以YAML格式定义报表名称、调度计划、数据源、格式与投递渠道 | 不支持 | 支持 |
| **Cron定时调度**: 支持每日 `0 9 * * *`、每周 `0 9 * * 1`、每月 `0 9 1 * *` 与按需触发四种调度频率 | 不支持 | 支持 |
| **报表管理**: 支持列表查看(读取 `~/report/memory.md`)、暂停报表(更新配置)、按需运行(立即生成) | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
...
**输出**: 返回数据源用户驱动配置的处理结果。- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回YAML报表配置的处理结果。- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回Cron定时调度的处理结果。- 验证执行结果，确认输出符合预期格式
