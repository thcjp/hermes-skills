...
执行规约查询操作,处理用户输入并返回结果。

**输入**: 用户提供规约查询所需的参数和指令。

**输出**: 返回规约查询的处理结果。

- 执行`规约查询`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`规约查询`相关配置参数进行设置
### 合规检查

执行合规检查操作,处理用户输入并返回结果。

**输入**: 用户提供合规检查所需的参数和指令。

**输出**: 返回合规检查的处理结果。

- 执行`合规检查`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`合规检查`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、Java、开发规约方案、含自定义规则、团队规范模板与、面向企业级、开发团队的开发规、约治理工具、提供团队级规范定、制能力、核心能力、大维度规约的团队、级自定义配置、规约合规性自动检、查与报告生成、新项目脚手架与规、范模板集成、架构分层规范与依、赖管理约束、设计模式与设计原、则的代码示例库、规约检查集成与质、量门禁。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
