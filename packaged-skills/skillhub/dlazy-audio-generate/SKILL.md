...
**处理**: 按照skill规范执行可用音频模型操作,遵循单一意图原则。
**输出**: 返回可用音频模型的执行结果,包含操作状态和输出数据。

- 执行`可用音频模型`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`可用音频模型`相关配置参数进行设置
### 文本转语音(TTS)
...
**输入**: 用户提供可用音频模型所需的参数和指令。
**处理**: 按照skill规范执行可用音频模型操作。

**输出**: 返回文本转语音(TTS)的执行结果,包含操作状态和输出数据。
### 模型选择策略
