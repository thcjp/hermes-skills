...
summary_zh: "批量AI图片生成引擎，支持多格式导出、消息平台集成与自动化工作流。。Grok图片生成工具专业版。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| Grok图片生成-专业版批量AI图片生成 | 不支持 | 支持 |
| Grok图片生成-专业版支持多格式导出 | 不支持 | 支持 |
| 多渠道消息批量发送 | 不支持 | 支持 |
| 消息模板与变量注入 | 不支持 | 支持 |
| 送达状态实时回调 | 不支持 | 支持 |
...
| 提示词管理 | 无 | 模板库 |
| 工作流 | 手动 | 自动化编排 |
| 后处理 | 无 | 压缩/裁剪/水印 |- 验证返回数据的完整性和格式正确性
- 参考`核心能力`的配置文档进行参数调优
### 核心能力(补充)
