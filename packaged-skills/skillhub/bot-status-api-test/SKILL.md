...
执行服务健康检查操作,处理用户输入并返回结果。

**输入**: 用户提供服务健康检查所需的参数和指令。

**输出**: 返回服务健康检查的处理结果。

- 执行`服务健康检查`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`服务健康检查`相关配置参数进行设置
### 邮件未读计数

执行邮件未读计数操作,处理用户输入并返回结果。

**输入**: 用户提供邮件未读计数所需的参数和指令。

**输出**: 返回邮件未读计数的处理结果。

- 执行`邮件未读计数`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`邮件未读计数`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 可配置、运行状态为、含服务健康检查、系统指标与、状态监控、可配置的、的运行状态暴露为、设计用于仪表盘集、监控与透明度、CLI、文件路径、列表与系统指标、零依赖、Node、内置模块、非阻塞异步架构、后台缓存刷新、TTL、配置驱动、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 架构特性
...
- `-opus-4-5`: 命令参数,用于指定操作选项
- `-frontend`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `-platformHome`: 命令参数,用于指定操作选项
