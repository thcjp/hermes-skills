...
本skill还覆盖以下能力场景: 小说一键转竖屏短、步全链路从脚本到、成片自动产出、短剧爆款生产线、丢进一章小说、步全自动管道吐出、可发布的竖屏短剧、核心功能、步管道从剧本转换、到多平台发布全链、资产持久化、经验回写自生长闭。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| 生成锁冲突 | 该书正在生成中 | 返回错误,提示稍后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令 | GENERATION_LOCKED |
