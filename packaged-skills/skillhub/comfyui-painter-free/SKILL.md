...
本skill还覆盖以下能力场景: 基础画图、支持文生图与本地、模型管理、手动调参、CivitAI、本地画图基础客户、免费版、在本地、GPU、上生成图片、支持文生图任务与、基础参数控制、状态检查与空闲自、动关闭功能、种基础模型别名覆、盖动漫与写实风格、用户需手动指定、Steps、CFG、采样器等参数、模型搜索、更新检查与自动调、参能力、适用于个人创作者、快速试图、简单画图需求场景。这些能力在上述核心功能中均有对应处理逻辑。
## 脚本路径
