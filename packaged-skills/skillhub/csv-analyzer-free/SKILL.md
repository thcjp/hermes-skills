...
**输出**: 返回为何不用pandas？的执行结果,包含操作状态和输出数据。

- 执行`为何不用pandas？`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 数据分析免费版、提供快速统计与基、零外部依赖、适合轻量数据探索、数据分析器免费版、础筛选能力、仅依赖、或重型依赖、命令返回行数、命令支持比较运算、可导出筛选结果、自动列类型检测、文本自动识别、升级付费版专享、结果导出、适用场景、轻量数据探索、基础统计、简单筛选、不适用于、的大文件、加密文件破解。这些能力在上述核心功能中均有对应处理逻辑。
## 核心能力（免费版）
