...
执行密钥生成与配置操作,处理用户输入并返回结果。

**输入**: 用户提供密钥生成与配置所需的参数和指令。

**输出**: 返回密钥生成与配置的处理结果。

- 执行`密钥生成与配置`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`密钥生成与配置`相关配置参数进行设置
### 密钥轮换自动化

执行密钥轮换自动化操作,处理用户输入并返回结果。

**输入**: 用户提供密钥轮换自动化所需的参数和指令。

**输出**: 返回密钥轮换自动化的处理结果。

- 执行`密钥轮换自动化`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`密钥轮换自动化`相关配置参数进行设置
### 团队密钥管理

执行团队密钥管理操作,处理用户输入并返回结果。

**输入**: 用户提供团队密钥管理所需的参数和指令。

**输出**: 返回团队密钥管理的处理结果。

- 执行`团队密钥管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`团队密钥管理`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 平台全功能、认证方案、含密钥轮换、团队管理、审计日志与多环境、认证工具、为团队与企业提供、的完整、认证治理方案、覆盖密钥全生命周、期管理、核心能力、密钥生成与轮换自、团队密钥分发与回、认证审计日志与异、权限精细化策略模、工具集成认证适配、批量密钥失效与替、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-Key-Id`: 命令参数,用于指定操作选项
2. `-H`: 命令参数,用于指定操作选项
3. `-auth-audit`: 命令参数,用于指定操作选项
4. `-X`: 命令参数,用于指定操作选项
5. `-key-001`: 命令参数,用于指定操作选项
