...
- 执行`智能配图管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`智能配图管理`相关配置参数进行设置

- 执行`完整发布流程`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`完整发布流程`相关配置参数进行设置

- 执行`多模型切换`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多模型切换`相关配置参数进行设置
### 2. 业务资料库集成
```bash
...
本skill还覆盖以下能力场景: 企业级公众号写作、支持多模型切换、配图管理与团队协、公众号写作专业版、面向专业内容团队、与企业运营的高级、公众号写作引擎、核心能力、GPT、Claude、等模型、按场景灵活选择、自动关联产品、服务资料、实现增量改写、从选题到发布一条、支持草稿箱与直接、自动生成配图占位、支持用户供图与、支持续写、多版本对比、历史草稿追踪、自定义预设、结构预设、文末区块、禁用词等深度定制、内容审查集成、自动剥离引用标注。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
