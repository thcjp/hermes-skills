...
summary_zh: "企业级公众号写作引擎，支持多模型切换、业务资料库、发布流程、配图管理与团队协作。。公众号写作专业版 —— 面向专业内容团队与企业运营的高级公众号写作引擎。核心能力: - 多模型切换：支持De"
license: "MIT"
edition: "pro"
description: |-
...
python3 write.py continue \
  ...
  -o draft_extended.md
```- 验证返回数据的完整性和格式正确性
- 参考`智能配图管理`的配置文档进行参数调优- 验证返回数据的完整性和格式正确性
- 参考`完整发布流程`的配置文档进行参数调优- 验证返回数据的完整性和格式正确性
- 参考`多模型切换`的配置文档进行参数调优
### 2. 业务资料库集成
```bash
# 查看业务资料库
ls .article/products/
