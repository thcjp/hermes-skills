...
本skill还覆盖以下能力场景: 企业邮件自动化与、批量发送方案、支持模板与定时调、邮件工具箱专业版、面向企业用户与高、效能个人用户、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-app-password`: 命令参数,用于指定操作选项
2. `-newsletter`: 命令参数,用于指定操作选项
3. `--rate-limit`: 命令参数,用于指定操作选项
4. `--timezone`: 命令参数,用于指定操作选项
