...
- 执行`多格式导出`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多格式导出`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、Figma、交互工具、支持批量导出、高级缓存、团队协作分析与自、设计助手、是面向团队与重度、用户的、全功能交互工具、在免费版基础上新、增批量导出、定义模板等高级能、核心能力、完整的文件结构读、取与增量缓存更新、支持自定义命名规、则与模板、团队评论聚合分析、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
