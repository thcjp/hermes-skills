...
- 需要 Python 3 运行时执行消息发送脚本
- 需要 ffmpeg 处理媒体文件(语音opus格式、贴纸webp格式、视频压缩)
- 消息格式要求: 文本(UTF-8)、图片(jpg/png/webp,16MB)、语音(opus,64k,16MB)、视频(mp4,64MB)、贴纸(webp,512x512,100KB)、文件(任意,100MB)
- JID格式必须为 `数字@s.whatsapp.net`(个人)或 `ID@g.us`(群组)
- 消息发送有速率限制,触发后需等待30秒重试
