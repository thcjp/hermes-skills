...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 4 spaces for indentation (never tabs) | 支持 | 通过核心功能实现对应能力 |
| Context managers for I/O | 支持 | 通过核心功能实现对应能力 |
| Snake_case for functions/variables, PascalCase for classes, UPPER_CASE for constants | 支持 | 通过核心功能实现对应能力 |
| Imports: stdlib → third-party → local, alphabetized within groups | 支持 | 通过核心功能实现对应能力 |
| No hardcoded secrets | 支持 | 通过核心功能实现对应能力 |
| Type hints on public functions | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `before`, `pathlib`, `pytest`, `minimum`, `py_compile`, `hello`, `committing`, `testing`, `anti`, `fetch`, `systemexit`, `better`, `syntax`, `checklist`, `pythonic`
