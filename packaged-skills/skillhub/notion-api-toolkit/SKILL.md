...
执行连接数量操作,处理用户输入并返回结果。

**输入**: 用户提供连接数量所需的参数和指令。

**输出**: 返回连接数量的处理结果。

- 执行`连接数量`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`连接数量`相关配置参数进行设置
### 写操作
...
- 执行`写操作`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`写操作`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、Notion、集成平台、支持多连接管理、批量操作、自动分页与审计日、适合团队与企业规、模化使用、工具箱、是面向团队与企业、的全功能、Skill、在免费版基础上新、增多连接管理、审计日志等高级能、核心能力。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--webhook-id`: 命令参数,用于指定操作选项
2. `--parallel`: 命令参数,用于指定操作选项
3. `--confirm`: 命令参数,用于指定操作选项
4. `--interval`: 命令参数,用于指定操作选项
5. `--events`: 命令参数,用于指定操作选项
...
- `-sync`: 命令参数,用于指定操作选项
- `--input`: 命令参数,用于指定操作选项
- `--signature`: 命令参数,用于指定操作选项
- `--parent-page`: 命令参数,用于指定操作选项
- `-create`: 命令参数,用于指定操作选项

### 命令参数说明

- `--event-id`: 命令参数,用于指定操作选项
- `--output`: 命令参数,用于指定操作选项
- `-update`: 命令参数,用于指定操作选项
- `--max-retries`: 命令参数,用于指定操作选项
- `--paginate`: 命令参数,用于指定操作选项

### 命令参数说明

- `-pages`: 命令参数,用于指定操作选项
- `--on-failure`: 命令参数,用于指定操作选项
- `--connections`: 命令参数,用于指定操作选项
...
| 自动分页卡住 | 频率限制或网络慢 | 调整`--rate-limit`,执行ping命令测试网络连通性,检查防火墙和代理设置 | 中 |
