...
本skill还覆盖以下能力场景: 企业级、WhatsApp、格式完整规范、含边界场景、高级模式、多语言指南、交互式测试、样式指南专业版、提供完整的、消息格式规范体系、面向企业和专业内、容团队、核心能力、格式语法规范、含边界场景与特殊、字符处理、技术场景、交互式格式测试与、实时预览、团队样式规范定制、与执行、格式合规审计与报、告生成、版本化规范管理与、变更追踪。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-style-guide-pro`: 命令参数,用于指定操作选项
2. `-platform`: 命令参数,用于指定操作选项
