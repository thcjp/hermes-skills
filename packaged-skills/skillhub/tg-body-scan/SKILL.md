...
执行单次视频扫描,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供单次视频扫描相关的配置参数、输入数据和处理选项。

**输出**: 返回单次视频扫描的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`单次视频扫描`相关配置参数进行设置
### 基础围度测量

执行基础围度测量,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供基础围度测量相关的配置参数、输入数据和处理选项。

**输出**: 返回基础围度测量的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`基础围度测量`相关配置参数进行设置
### 腰臀比汇总

执行腰臀比汇总,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供腰臀比汇总相关的配置参数、输入数据和处理选项。

**输出**: 返回腰臀比汇总的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`腰臀比汇总`相关配置参数进行设置
#
## 适用场景
