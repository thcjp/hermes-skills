...
执行本地/局域网诊断操作,处理用户输入并返回结果。

**输入**: 用户提供本地/局域网诊断所需的参数和指令。

**输出**: 返回本地/局域网诊断的处理结果。

- 执行`本地/局域网诊断`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`本地/局域网诊断`相关配置参数进行设置
### 尾网诊断

执行尾网诊断操作,处理用户输入并返回结果。

**输入**: 用户提供尾网诊断所需的参数和指令。

**输出**: 返回尾网诊断的处理结果。

- 执行`尾网诊断`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`尾网诊断`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全拓扑节点连接诊、含尾网、审计日志与自动修、节点连接工具、专业版、面向团队与运维、提供全拓扑节点连、接诊断、连接审计日志与一、键自动修复能力、核心能力、全拓扑支持、批量设备配对与状、连接审计日志与回、一键诊断与自动修、远程网关与多网关、高级鉴权配置。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
