...
- 执行`相关性分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`相关性分析`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、CSV、支持流式大文件、分布可视化与高级、Insight、专业版面向专业数、据分析师与数据科、在免费版基础上解、锁流式大文件处理、异常检测算法、核心能力、流式分析、多异常检测算法、多文件对比分析、配置与列类型强制。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
- 不适用: 需要人工判断的复杂决策场景
