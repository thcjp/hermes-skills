...
执行备份策略操作,处理用户输入并返回结果。

**输入**: 用户提供备份策略所需的参数和指令。

**输出**: 返回备份策略的处理结果。

- 执行`备份策略`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`备份策略`相关配置参数进行设置
### 连接池监控

执行连接池监控操作,处理用户输入并返回结果。

**输入**: 用户提供连接池监控所需的参数和指令。

**输出**: 返回连接池监控的处理结果。

- 执行`连接池监控`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`连接池监控`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 面向企业的、管理专业版、含自动备份、灾备恢复与优先支、面向团队、企业与专业开发者、全功能管理专业版、在免费版基础上新、增自动定时备份、连接池监控与告警、版本化迁移、分析引擎集成、增量备份与时间点、多租户管理与透明、数据加密等高级能、配套面向运维、数据工程师、Agent、架构师的多角色场、景指南、Use、需要系统监控、日志分析、运维告警、部署管理时使用、不适用于物理硬件。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
