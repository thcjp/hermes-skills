...
- 需要 Node.js 运行时执行 `atlas-api.mjs` 和 `atlas-call.mjs` 脚本
- 需要配置环境变量 `ATLAS_CLIENT_ID` 和 `ATLAS_CLIENT_SECRET`(公私钥认证)
- 环境变量在当前会话生效,新会话需重新设置
- API调用有速率限制,返回429时需等待60秒重试
- `--dry-run` 不会实际执行,需去掉后加 `--yes` 才执行
- 仅管理云平台配置,不执行数据库查询(查询请使用 `mongosh` 或应用程序驱动)
