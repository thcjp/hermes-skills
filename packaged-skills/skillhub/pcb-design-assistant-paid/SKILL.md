...
本skill还覆盖以下能力场景: EasyEDA、全流程、设计专业版、支持多页原理图、自定义库与、覆盖从设计到打板、设计助手、面向硬件团队与专、业电子工程师、在免费版基础上解、锁全部高级能力、设计复用模板、单对接、覆盖从需求到打板、的全流程、Use、需要设计创作、海报制作、品牌视觉时使用、不适用于、建模和动画制作。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-C`: 命令参数,用于指定操作选项
2. `-DC`: 命令参数,用于指定操作选项
3. `-C供电的STM32最小系统板`: 命令参数,用于指定操作选项
