...
summary_zh: "全功能本地Ollama AI模型管理工具，支持多轮对话、自定义模型、批量推理与API集成。Ollama工具箱专业版是面向团队和高级用户的完整本地AI模型管理方案，在免费版基础上解锁多轮对话会"
license: "MIT"
edition: "pro"
description: |-
...
针对模型,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供模型管理相关的配置参数、输入数据和处理选项.
**输出**: 返回模型管理的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`模型管理`的配置文档进行参数调优
### 单轮推理

针对单轮推理,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供单轮推理相关的配置参数、输入数据和处理选项.
...
| 参数名 | 类型 | 必填 | 说明 |
|:---:|:---:|:---:|:---:|
| content | string | 否 | ollama-toolkit处理的内容输入 |,  |
| content | string | 否 | ollama-toolkit处理的内容输入 |, 可选值: json/text/markdown |
