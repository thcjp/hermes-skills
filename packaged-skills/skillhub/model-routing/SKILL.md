...
summary_zh: "团队级模型路由套件,含成本仪表盘、批量分发、自定义规则与监控告警。。模型路由工具(专业版)面向团队与企业,在三层路由决策基础上,扩展成本分析仪表盘、批量任务分发、自定义路由规则、实时监控告警"
license: "MIT"
edition: "pro"
description: |-
...
- 路由决策依赖预设的模型能力评估表，新增模型需手动更新评估数据，否则路由精度下降
- 成本仪表盘数据存在分钟级聚合延迟，实时成本消耗可能与显示值有偏差，不适合秒级成本控制
- 自定义路由规则之间的优先级冲突需人工排查，工具不自动检测规则逻辑矛盾
