...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Apify actors | 支持 | 通过核心功能实现对应能力 |
| PH API v2 is broken | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `trustmrr`, `page`, `urls`, `verified`, `arguments`, `competitor`, `produces`, `broader`, `recommendation`, `suggested`, `排名提升`, `scraper`, `hacker`, `references/domain-check.md`, `deprecated`
