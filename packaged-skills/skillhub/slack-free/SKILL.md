...
本skill还覆盖以下能力场景: slack、工具发送与读取、支持频道通知与历、史消息查询、消息管理、免费版、工具对、工作区进行基础消、息操作、涵盖两大动作组、发送消息与读取消、工具使用已配置的、Bot、Token、进行认证、两种消息目标、适用于发送频道通、读取频道历史等基、础场景、如需消息回应、reactions、置顶管理、pins、成员信息查询、memberInfo、表情列表、emojiList、消息编辑与删除等、高级能力、请升级至、付费版。这些能力在上述核心功能中均有对应处理逻辑。
## 输入采集
