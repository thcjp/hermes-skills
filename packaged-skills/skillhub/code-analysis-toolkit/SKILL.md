...
本skill还覆盖以下能力场景: 企业级、历史分析、支持团队复盘、趋势追踪与多格式、面向团队与企业的、历史分析工具、在免费版基础上扩、展团队复盘、趋势追踪等能力、核心能力、同意管理与审计追、多仓库批量扫描与、历史趋势追踪与基、匿名化输出与隐私。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--anonymize`: 命令参数,用于指定操作选项
- `--i-have-consent`: 命令参数,用于指定操作选项
- `--scan-all`: 命令参数,用于指定操作选项
- `-trend`: 命令参数,用于指定操作选项
