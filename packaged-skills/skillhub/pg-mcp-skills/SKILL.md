...
**输出**: 返回能力一：自动化性能调优的执行结果,包含操作状态和输出数据。### 能力二：智能索引推荐
...
**输出**: 返回能力三：数据库迁移与版本升级的执行结果,包含操作状态和输出数据。
### 能力四：多实例统一管理
通过配置中心化管理多个数据库实例的连接信息、监控指标、告警规则。支持一键切换目标实例、批量执行诊断命令。

**输入**: 用户提供能力四：多实例统一管理所需的指令和必要参数。
**输出**: 返回能力四：多实例统一管理的执行结果,包含操作状态和输出数据。### 能力五：高可用与故障转移监控
提供主备复制状态监控、复制延迟告警、自动故障转移方案（基于 Patroni 或 repmgr）、脑裂检测与恢复流程。

**处理**: 按照skill规范执行能力五：高可用与故障转移监控操作,遵循单一意图原则。
**输出**: 返回能力五：高可用与故障转移监控的执行结果,包含操作状态和输出数据。### 能力六：慢查询自动归档与分析报告
自动归档超过阈值的慢查询，按周生成分析报告，包含 Top 10 慢查询、趋势变化、优化建议。

**输入**: 用户提供能力六：慢查询自动归档与分析报告所需的指令和必要参数。

- 执行`能力四：多实例统一管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`能力四：多实例统一管理`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、数据库管理方案、支持自动化调优、迁移升级、多实例管理与高可、用监控、助手专业版是一套、面向中大型团队与、企业级场景的、数据库管理解决方、在免费版基础上扩、展出自动化性能调、监控等能力、核心能力、提供基于执行计划、的自动索引推荐、冗余索引清理流程、大版本升级迁移方、多实例配置中心化、主备切换与故障转。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
