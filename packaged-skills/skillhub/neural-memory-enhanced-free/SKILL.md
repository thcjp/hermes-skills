...
本skill还覆盖以下能力场景: 基于扩散激活的联、想记忆系统、通过神经图谱实现、基础持久化召回、生物学启发的联想、记忆系统、采用扩散激活替代、关键词检索、记忆以神经图谱形、式组织、神经元之间通过类、型化突触连接、频繁共同访问的记、忆自动强化连接、陈旧记忆自然衰减、基础能力涵盖扩散、适用于跨会话上下、文召回、项目决策归档、知识关联检索等场、完全本地运行、数据存储于、SQLite、数据库、无外部网络依赖。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
