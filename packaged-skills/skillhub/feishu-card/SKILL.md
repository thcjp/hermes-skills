...
通过 `--text-file` 参数从文件读取Markdown内容发送复杂卡片。支持代码块、表格、列表等完整Markdown语法。**关键**：为防止shell转义问题（如反引号被吞），始终先将内容写入临时文件，再用 `--text-file "temp/msg.md"` 发送。适用于发送代码片段、日志和格式化报告。- 验证执行结果，确认输出符合预期格式
- 参考`Markdown复杂卡片`相关配置参数进行设置
### 3. 安全发送

通过 `node skills/feishu-card/send_safe.js` 包装器安全发送原始文本。自动处理临时文件创建和清理，避免shell转义问题。支持 `--text` 直接传入含反引号和Markdown的内容，配合 `--title` 设置卡片标题。适用于自动化流程中的安全消息发送。- 验证执行结果，确认输出符合预期格式
- 参考`安全发送`相关配置参数进行设置
### 4. 卡片标题和颜色
...
**输出**: 返回按钮组件的执行结果,包含操作状态和输出数据。- 验证执行结果，确认输出符合预期格式
- 参考`按钮组件`相关配置参数进行设置
### 6. 图片嵌入

通过 `--image-path <path>` 上传本地图片并嵌入到卡片中。支持常见图片格式（PNG/JPG/GIF等）。图片先上传到协作平台服务器获取image_key，再嵌入到卡片内容中渲染。适用于发送截图、图表和视觉内容。- 验证执行结果，确认输出符合预期格式
- 参考`图片嵌入`相关配置参数进行设置
### 7. 人格化消息

通过 `node skills/feishu-card/send_persona.js --target "ou_..." --persona "d-guide" --text "Critical error detected."` 发送带主题样式的人格化消息。自动添加匹配的头部颜色、格式前缀和风格化后缀。适用于AI助手不同角色的消息输出场景。- 验证执行结果，确认输出符合预期格式
- 参考`人格化消息`相关配置参数进行设置
### 8. 多种人格样式
