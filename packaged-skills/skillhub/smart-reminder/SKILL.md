...
summary_zh: "自然语言事件捕获与提醒助手专业版，含自定义偏移、多渠道投递、农历生日、语义搜索与完整重复模式。。智能提醒助手专业版是在免费版基础上的全功能升级，为AI Agent提供企业级自然语言事件管理能"
license: "MIT"
edition: "pro"
description: |-
...
**输出**: 返回中英文自然语言捕获的处理结果,包含执行状态码、结果数据和执行日志.
### 自定义提醒偏移
| 偏移策略 | 偏移序列 | 适用场景 |
|---:|---:|---:|
| **默认** | 24h/1h/10m | 通用事件 |
...
**输出**: 返回自定义提醒偏移的处理结果,包含执行状态码、结果数据和执行日志。### 完整重复模式
| 模式 | 字段值 | 说明 | 示例 |
|:---:|:---:|:---:|:---:|
| **无重复** | `none` | 单次事件 | 一次性会议 |
...
**处理**: 解析完整重复模式的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回完整重复模式的处理结果,包含执行状态码、结果数据和执行日志。### 农历生日支持
**农历生日存储规范**：
...
**处理**: 解析农历生日支持的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回农历生日支持的处理结果,包含执行状态码、结果数据和执行日志。### 多渠道投递
| 渠道 | 配置 | 适用场景 |
|:------|------:|:------|
...
**处理**: 解析多渠道投递的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回多渠道投递的处理结果,包含执行状态码、结果数据和执行日志。### 语义搜索
```python
...
**处理**: 解析事件生命周期管理的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `事件生命周期管理` 选项
