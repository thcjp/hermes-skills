...
- 执行`频道列表`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`频道列表`相关配置参数进行设置
### 3. 用户列表
列出工作区内全部用户，返回用户ID、姓名、邮箱、是否在线等。可用于成员查询与用户ID获取。

> **升级提示**：线程回复、定时消息、频道创建/归档/重命名、文件上传/下载、表情反应、提醒、画布、用户组管理、企业Grid审计日志等高级功能仅在 slack-workspace 付费版中提供。

- 执行`用户列表`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`用户列表`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 基础消息发送与频、ClawLink、OAuth、托管连接、面向团队协作场景、基础工作区管理技、免费版、提供频道消息发送、用户列表三大基础、支持频道、基础消息格式化、频道发现与用户查、适用于通知发布、成员查询等基础场。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
