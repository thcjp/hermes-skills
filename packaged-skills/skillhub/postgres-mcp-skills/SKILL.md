...
| 能力 | 免费版 | 付费版 |
|:-----|:-------|:-------|
| PostgreSQL 数据库管理和优化助手 | 支持 | 支持 |
| 提供完整的数据库操作能力：健康检查、索引优化、查询计划分析、模式查询、SQL 执行 | 不支持 | 支持 |
| 当用户提到 | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| content | string | 否 | postgres-mcp-skills处理的内容输入 |,  |
| content | string | 否 | postgres-mcp-skills处理的内容输入 |, 可选值: json/text/markdown |
...
2. **安全优秀**：执行写操作（UPDATE、DELETE、DROP 等）前必须向用户确认，展示将要执行的 SQL
