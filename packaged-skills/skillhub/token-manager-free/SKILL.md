...
- 需要 Node.js 运行时执行 `manager.js`、`scheduler.js`、`session-tracker.js` 脚本
- 需要配置API密钥环境变量: `MOONSHOT_API_KEY`、`OPENAI_API_KEY`、`ANTHROPIC_API_KEY`
- 计费标准为¥12/1M tokens(输入/输出),缓存Token¥6/1M tokens,不同模型价格可能不同
- 余额查询依赖API可用性,网络不可达时无法获取实时余额
- 80%上下文阈值、50k会话Token预警、¥5余额预警均为固定阈值,不可自定义
