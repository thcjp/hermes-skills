...
`free-weather-skill`按照以下流程处理用户请求:
...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Units: ?m (metric) ?u (USCS) | 支持 | 通过核心功能实现对应能力 |
| Airport codes: wttr.in/JFK | 支持 | 通过核心功能实现对应能力 |
| URL-encode spaces: wttr.in/New+York | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `compact`, `docs`, `london`, `完全适配`, `find`, `meteo`, `open`, `today`, `增强元数据和触发`, `returns`, `berlin`, `tips`, `平台规范`
