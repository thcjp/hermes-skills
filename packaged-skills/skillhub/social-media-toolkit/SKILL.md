...
|:-----|:-------|:-------|
| 基础社交 | 支持 | 支持 |
| 批量滑动/批量消息/批量关系/批量资料更新 | 不支持 | 支持 |
| 多 Agent 协调 | 不支持 | 支持 |
| 匹配率/活跃度/社交图谱/转化漏斗 | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
...
针对基础社交,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应。

**输入**: 用户提供基础社交相关的配置参数、输入数据和处理选项。

**输出**: 返回基础社交的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`基础社交`的配置文档进行参数调优
### 批量操作
...
针对多 Agent 协调,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应。

**输入**: 用户提供多 Agent 协调相关的配置参数、输入数据和处理选项。

**输出**: 返回多 Agent 协调的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`多 Agent 协调`的配置文档进行参数调优
#
## 适用场景
