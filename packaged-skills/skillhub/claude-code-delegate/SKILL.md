...
执行环境前置检查操作,处理用户输入并返回结果。

**输入**: 用户提供环境前置检查所需的参数和指令。

**输出**: 返回环境前置检查的处理结果。

- 执行`环境前置检查`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`环境前置检查`相关配置参数进行设置
### 命令模板化委派

执行命令模板化委派操作,处理用户输入并返回结果。

**输入**: 用户提供命令模板化委派所需的参数和指令。

**输出**: 返回命令模板化委派的处理结果。

- 执行`命令模板化委派`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`命令模板化委派`相关配置参数进行设置
### 异步非阻塞流程

执行异步非阻塞流程操作,处理用户输入并返回结果。

**输入**: 用户提供异步非阻塞流程所需的参数和指令。

**输出**: 返回异步非阻塞流程的处理结果。

- 执行`异步非阻塞流程`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`异步非阻塞流程`相关配置参数进行设置
### 技术细节
...
本skill还覆盖以下能力场景: 将编程任务委派给、代码助手、支持异步执行与会、话续接、支持非交互模式、异步轮询、会话续接与独立测、核心能力包括环境、安全防护策略与结。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Correct Flow | 支持 | 通过核心功能实现对应能力 |
| WRONG (causes blocking) | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `before`, `independent`, `auto`, `scenario`, `项目管理`, `trigger`, `translate`, `开发规范`, `prerequisites`, `iteration`, `parameters`, `retry`, `clear`, `summarize`, `tester`
