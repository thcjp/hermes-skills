...
本skill还覆盖以下能力场景: 企业级、支持批量分支操作、自动化审查、管理与、流水线集成、面向企业研发团队、的高级、管理工具、提供批量分支操作、历史分析与、核心能力、批量分支管理与清、自动化代码审查与、提交规范强制执行、仓库历史深度分析。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-hooks`: 命令参数,用于指定操作选项
2. `--format`: 命令参数,用于指定操作选项
3. `--show-current`: 命令参数,用于指定操作选项
4. `--output`: 命令参数,用于指定操作选项
5. `--no-verify`: 命令参数,用于指定操作选项
