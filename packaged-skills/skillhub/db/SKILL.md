...
**输出**: 返回连接管理陷阱的执行结果,包含操作状态和输出数据。### 事务陷阱
| 陷阱 | 表现 | 规避方案 |
|------|------|---------|
...
**输出**: 返回事务陷阱的执行结果,包含操作状态和输出数据。### Schema变更安全策略
...
**处理**: 按照skill规范执行备份与恢复操作,遵循单一意图原则。
**输出**: 返回备份与恢复的执行结果,包含操作状态和输出数据。### 复制陷阱
| 陷阱 | 表现 | 规避方案 |
|------|------|---------|
...
**输出**: 返回复制陷阱的执行结果,包含操作状态和输出数据。### 查询性能模式
| 问题 | 原因 | 优化方案 |
|------|------|---------|
...
**处理**: 按照skill规范执行查询性能模式操作,遵循单一意图原则。### 数据完整性
| 陷阱 | 后果 | 保障方案 |
|------|------|---------|
...
**输出**: 返回数据完整性的执行结果,包含操作状态和输出数据。### 扩展性限制
...
执行连接池耗尽操作,处理用户输入并返回结果。

**输入**: 用户提供连接池耗尽所需的参数和指令。

**输出**: 返回连接池耗尽的处理结果。

- 执行`连接池耗尽`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`连接池耗尽`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 识别并规避数据库、数据完整性与扩展、性陷阱、数据库设计与运维、帮助设计和操作数、据库时避免常见的、可靠性和数据完整、核心能力包括、连接管理陷阱识别、空闲连接内存占用、事务陷阱规避、快照阻塞、并发索引创建、列重命名与删除顺、备份与恢复最佳实、备份验证、副本一致性、复制陷阱预防、变更同步、故障转移重连、查询性能模式优化、外键索引、数据完整性保障、唯一约束竞态、时区存储、金额精度、扩展性限制规划、亿行分片、统计信息更新。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
