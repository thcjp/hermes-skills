...
本skill还覆盖以下能力场景: Twitter、增量去重后输出结、不含中文分类简报、命令行工具读取、两条时间线、对推文进行增量过、硬去重与近重复文、本合并、输出结构化、免费版不包含中文、分类简报生成、排序修剪与启发式、噪声过滤、仅提供基础的去重、数据供下游自行处。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
