...
**处理**: 解析时间线拉取的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回时间线拉取的处理结果,包含执行状态码、结果数据和执行日志。### 增量过滤
...
**输出**: 返回去重的处理结果,包含执行状态码、结果数据和执行日志.
#
## 使用流程
...
返回认证失败或空结果.
执行 bird 登录子命令重新完成 cookie 认证后检查网络连接和配置后重试.
### statePath 不可写
...
确认 bird 登录态有效,检查网络连接和配置后重试,
账号被限流时拉长 intervalHours 后检查网络连接和配置后重试.
### 近重复阈值过高
