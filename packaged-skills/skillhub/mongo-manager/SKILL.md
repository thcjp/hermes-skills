...
- 执行`高可用`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`高可用`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: MongoDB、企业级管理与优化、全功能版、含分片集群、副本集高可用、与基准测试、运维与架构师的企、业级全功能专业版、在免费版基础上新、增分片集群配置、性能基准测试套件、容量规划模型等高、级能力、配套面向、DBA、架构师、SRE、的多角色场景指南、Use、SEO、关键词分析、排名提升、搜索流量优化时使、不适用于黑帽。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
