...
**输出**: 返回一、批量多股票扫描(专业版独有)的执行结果,包含操作状态和输出数据。### 二、自定义权重配置(专业版独有)
...
**处理**: 按照skill规范执行二、自定义权重配置(专业版独有)操作,遵循单一意图原则。### 三、结构化报告导出(专业版独有)
- JSON 格式:便于程序处理与二次分析
...
**输出**: 返回三、结构化报告导出(专业版独有)的执行结果,包含操作状态和输出数据。### 四、历史趋势对比(专业版独有)
- 保存历史扫描结果
- 对比不同时间点的情绪变化
- 生成情绪趋势图表
- 识别情绪拐点
...
**处理**: 按照skill规范执行四、历史趋势对比(专业版独有)操作,遵循单一意图原则。### 五、异常情绪预警(专业版独有)
- 设置情绪分数阈值
- 情绪突变实时告警
- 支持邮件/消息通知
- 可集成到监控系统中
...
**输出**: 返回五、异常情绪预警(专业版独有)的执行结果,包含操作状态和输出数据。### 六、行业板块对比(专业版独有)
- 同行业多只股票横向对比
- 板块整体情绪汇总
- 识别板块内强弱分化
- 输出板块情绪排名
