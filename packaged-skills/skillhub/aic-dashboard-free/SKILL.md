...
本skill还覆盖以下能力场景: 轻量级只读监控面、展示入站邮件与浏、览器会话状态、本机访问、秒自动刷新、Commander、配套管理仪表盘免、轻量级只读、Web、展示最近、展示浏览器会话状、保护的本地端口提、供服务、免费版支持本机访、问与基础监控功能、等高级功能需升级。这些能力在上述核心功能中均有对应处理逻辑。
## 环境变量（免费版支持）
