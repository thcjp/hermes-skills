...
本skill还覆盖以下能力场景: 控制用户、Chrome、浏览器进行页面读、表单填充和数据提、通过浏览器扩展、直接操作用户已登、录的浏览器会话、无需重新认证、支持页面文本提取、截图和标签页管理、适用于独立开发者、企业团队、和自动化工作流场、不适用于无头浏览、器或远程浏览器场。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Check checkbox | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `switch`, `refresh`, `trigger`, `dispatchevent`, `文献分析`, `real`, `信息收集`, `verify`, `content`, `twitter`, `mouseevent`, `array`, `return`, `html`, `open`
