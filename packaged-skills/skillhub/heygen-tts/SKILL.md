...
本skill还覆盖以下能力场景: 企业级、HeyGen、词级时间戳与、服务化、适配生产环境、面向团队与企业用、文字转语音工具、核心能力、涵盖免费版全部能、Starfish、高级标记、完整语音合成标记、队列与并发处理、word、timestamps、多语言混合语音、本地化语音、音频后处理、归一化、成本监控与配额管、缓存与去重。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
### 可用性分类
- **分类**: MD+EXEC()
- **说明**: 基于Markdown的AI Skill,。专业版支持 SSML、批量生成、词级时间戳与 API 服务化,适合企业级语音内容生产流水线。
