...
**输出**: 返回系统资源指标的执行结果,包含操作状态和输出数据。### 服务化部署与仪表盘集成
- 内置systemd服务模板，一键注册为系统服务
- 提供CORS响应头，支持浏览器端仪表盘直接调用
- 兼容Grafana SimpleJSON数据源，可接入Grafana面板
### http

执行http操作,处理用户输入并返回结果。

**输入**: 用户提供http所需的参数和指令。

**输出**: 返回http的处理结果。

- 执行`http`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`http`相关配置参数进行设置
### command

执行command操作,处理用户输入并返回结果。

**输入**: 用户提供command所需的参数和指令。

**输出**: 返回command的处理结果。

- 执行`command`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`command`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级服务状态聚、合平台、定时任务多维度探、测与仪表盘集成、测试运行器专业版、是一款面向团队与、企业的服务状态聚、在免费版、探活与命令行检查、基础上、新增文件存在性检、邮件未读数统计、定时任务状态读取、与系统资源指标采、集等高级能力、核心能力、命令行、容器健康检查、获取容器状态、识别运行中的服务。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-mail`: 命令参数,用于指定操作选项
- `-scripts`: 命令参数,用于指定操作选项
- `-linger`: 命令参数,用于指定操作选项
...
- `-check`: 命令参数,用于指定操作选项
- `-main`: 命令参数,用于指定操作选项
- `-servers`: 命令参数,用于指定操作选项
- `-container`: 命令参数,用于指定操作选项
