...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Adjust tempo to match cuts if needed | 支持 | 通过核心功能实现对应能力 |
| Select best, extend or edit | 支持 | 通过核心功能实现对应能力 |
| Sound Design | 支持 | 通过核心功能实现对应能力 |
| Mix levels appropriately | 支持 | 通过核心功能实现对应能力 |
| Specify duration needed | 支持 | 通过核心功能实现对应能力 |
| Determine video length and mood | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `musicgen`, `workflows`, `flac`, `prompting`, `zimmer`, `soundraw.md`, `royalty`, `background`, `free`, `musicgen.md`, `mubert.md`, `mubert`, `aiva`, `provider`, `classical`
