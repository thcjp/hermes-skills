...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 外层（长期隧道） | 支持 | 通过核心功能实现对应能力 |
| 内层（短期隧道） | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `dimensional`, `你将扮演以下角色`, `提供的所有分析均`, `市场有风险`, `交易目的`, `自然语言分析技能`, `支撑压力`, `当信号不足`, `综合评分标准`, `日内波段`, `第一维度`, `适用市场`, `多层通道理论`, `你的分析严谨`, `轻仓试探`
