...
- 执行`分片上传大文件`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`分片上传大文件`相关配置参数进行设置

- 执行`批量上传与队列管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 企业级音频上传工、支持批量上传、多质量预设、分片上传与元数据、音频流上传专业版、面向企业团队与专、业创作者的高级音、频上传工具、核心能力、批量音频上传、支持队列管理与断、完全自定义编码配、比特率、采样率、编解码器、多质量预设输出、满足不同播放场景、双流媒体格式支持、适配多终端播放、丰富的元数据管理、自定义键值对、分片上传支持大文。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
2. `-w3stream`: 命令参数,用于指定操作选项
3. `-H`: 命令参数,用于指定操作选项
4. `-Range`: 命令参数,用于指定操作选项
5. `-X`: 命令参数,用于指定操作选项
