...
执行章节标记操作,处理用户输入并返回结果。

**输入**: 用户提供章节标记所需的参数和指令。

**输出**: 返回章节标记的处理结果。

- 执行`章节标记`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`章节标记`相关配置参数进行设置
### 高光片段

执行高光片段操作,处理用户输入并返回结果。

**输入**: 用户提供高光片段所需的参数和指令。

**输出**: 返回高光片段的处理结果。

- 执行`高光片段`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`高光片段`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级播客章节工、支持批量处理、社媒切片文案、多格式输出与、适配生产流水线、面向团队与企业用、户的播客章节标记、核心能力、涵盖免费版全部能、目录级递归、支持任务队列、社媒切片文案自动、服务化、支持远程调用、基于语义理解的精、准划分、多语言支持、中英日韩等、置信度分析与人工、校对流程、自动发布集成、支持主流播客平台。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
