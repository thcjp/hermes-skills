...
**输出**: 返回初始配置与仓库初始化的处理结果。

- 执行`初始配置与仓库初始化`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回基础工作流(暂存与提交)的处理结果。

- 执行`基础工作流(暂存与提交)`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
本skill还覆盖以下能力场景: 版本控制核心命令、与工作流、包括分支、标签与冲突处理、包括初始配置、远程操作、撤销变更、与常见工作流模式、提供冲突解决、分支恢复与安全强、制推送等错误处理。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Viewing changes | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `branching`, `workflows`, `commits`, `resolve`, `项目管理`, `开发规范`, `managing`, `history`, `logs`, `operations`, `rebasing`, `staging`, `visual`, `submodules`, `issues`
