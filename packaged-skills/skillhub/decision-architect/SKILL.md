...
执行决策框架匹配,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供决策框架匹配相关的配置参数、输入数据和处理选项。

**输出**: 返回决策框架匹配的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`决策框架匹配`相关配置参数进行设置
### 认知偏差检测

执行认知偏差检测,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供认知偏差检测相关的配置参数、输入数据和处理选项。

**输出**: 返回认知偏差检测的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`认知偏差检测`相关配置参数进行设置
### 风险画像学习

执行风险画像学习,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供风险画像学习相关的配置参数、输入数据和处理选项。

**输出**: 返回风险画像学习的处理结果。- 验证执行结果，确认输出符合预期格式
