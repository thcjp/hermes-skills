...
**输出**: 返回设计流程的执行结果,包含操作状态和输出数据。

- 执行`设计流程`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`设计流程`相关配置参数进行设置
### 阶段一: 户型搜索与确认
...
本skill还覆盖以下能力场景: 酷家乐室内设计基、文字搜索户型、风格选择、布局与渲染出图、基于酷家乐、Kujiale、开放能力的室内智、能设计基础版、通过分步式对话完、布局生成、渲染出图基础流程、核心能力、通过城市与小区名、搜索酷家乐户型库、获取标准户型图、风格匹配、基于偏好标签选择、硬装风格、智能布局、自动布置家具并生、成各房间布局、渲染出图、生成静态效果图、适用场景、业主装修前的方案、快速预览、个人户型与风格灵、感探索、学习试用与轻量设、计体验。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
...
- `--num`: 命令参数,用于指定操作选项
- `--areaId`: 命令参数,用于指定操作选项
- `--tagItemIds`: 命令参数,用于指定操作选项
- `--start`: 命令参数,用于指定操作选项
...
- `--applyDecorationStyle`: 命令参数,用于指定操作选项
...
- `--designId`: 命令参数,用于指定操作选项
- `--tagIds`: 命令参数,用于指定操作选项
- `--buildCeiling`: 命令参数,用于指定操作选项
