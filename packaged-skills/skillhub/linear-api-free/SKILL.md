...
**输出**: 返回Cycle与Project管理的执行结果,包含操作状态和输出数据。

- 执行`Cycle与Project管理`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
...
本skill还覆盖以下能力场景: 问题跟踪与项目管、与工作流、免费版、项目管理集成引擎、跟踪与工作流自动、核心能力、批量操作、Sprint、项目跟踪、状态更新、高级查询与批量操。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-uuid-2`: 命令参数,用于指定操作选项
- `-uuid-1`: 命令参数,用于指定操作选项
- `-H`: 命令参数,用于指定操作选项
- `-X`: 命令参数,用于指定操作选项
- `-Type`: 命令参数,用于指定操作选项
