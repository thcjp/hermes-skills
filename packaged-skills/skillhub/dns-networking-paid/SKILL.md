...
**处理**: 按照skill规范执行代理链完整诊断操作,遵循单一意图原则。
**输出**: 返回代理链完整诊断的执行结果,包含操作状态和输出数据。

- 执行`代理链完整诊断`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `代理链完整诊断` 选项
...
**输出**: 返回网络路径分析的执行结果,包含操作状态和输出数据。

- 执行`网络路径分析`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `网络路径分析` 选项
...
本skill还覆盖以下能力场景: 企业级网络诊断、支持批量巡检、代理链分析与持续、监控告警、面向运维团队的企、业级网络诊断工具、提供批量目标巡检、代理链完整分析、持续监控与告警通、核心能力、与端口巡检、网络拓扑路径分析、多格式诊断报告输。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
4. `-L`: 命令参数,用于指定操作选项
5. `-audit-`: 命令参数,用于指定操作选项
6. `-save`: 命令参数,用于指定操作选项
7. `-E`: 命令参数,用于指定操作选项
8. `-analysis`: 命令参数,用于指定操作选项

### 命令参数说明

- `-A`: 命令参数,用于指定操作选项
- `-L`: 命令参数,用于指定操作选项
- `--report-cycles`: 命令参数,用于指定操作选项
- `-F`: 命令参数,用于指定操作选项
- `-inspection-`: 命令参数,用于指定操作选项

### 命令参数说明

- `--line-numbers`: 命令参数,用于指定操作选项
- `-L`: 命令参数,用于指定操作选项
- `-zv`: 命令参数,用于指定操作选项
- `-z`: 命令参数,用于指定操作选项
- `-E`: 命令参数,用于指定操作选项
