...
- 执行`批量文件加密`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`批量文件加密`相关配置参数进行设置
### 2. 自动密钥轮换
> 详细代码示例已移至 `references/detail.md`
...
本skill还覆盖以下能力场景: 企业级加密管理、合规审计、批量加密与深度代、面向企业安全团队、的高级加密管理工、核心能力、自动密钥轮换与版、本管理、GDPR、PCI、DSS、批量文件加密与解、深度代码加密审计、信封加密与密钥派。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-gt`: 命令参数,用于指定操作选项
2. `-K`: 命令参数,用于指定操作选项
3. `--fail`: 命令参数,用于指定操作选项
4. `-iv`: 命令参数,用于指定操作选项
5. `--all-templates`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `--compliance`: 命令参数,用于指定操作选项
- `-K`: 命令参数,用于指定操作选项
- `-report`: 命令参数,用于指定操作选项
- `--kms-config`: 命令参数,用于指定操作选项
- `-gcm`: 命令参数,用于指定操作选项

### 命令参数说明

- `-K`: 命令参数,用于指定操作选项
- `--fail-on`: 命令参数,用于指定操作选项
- `--format`: 命令参数,用于指定操作选项
- `-compliance-report`: 命令参数,用于指定操作选项
- `--output`: 命令参数,用于指定操作选项

### 命令参数说明

- `-hex`: 命令参数,用于指定操作选项
- `-app-master-key`: 命令参数,用于指定操作选项
- `-aes-256-gcm`: 命令参数,用于指定操作选项
- `--crypto-audit`: 命令参数,用于指定操作选项
- `-K`: 命令参数,用于指定操作选项
