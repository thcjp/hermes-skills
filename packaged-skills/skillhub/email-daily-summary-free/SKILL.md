...
执行单邮箱登录操作,处理用户输入并返回结果。

**输入**: 用户提供单邮箱登录所需的参数和指令。

**输出**: 返回单邮箱登录的处理结果。

- 执行`单邮箱登录`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`单邮箱登录`相关配置参数进行设置
### 邮件列表抓取
...
执行未读统计操作,处理用户输入并返回结果。

**输入**: 用户提供未读统计所需的参数和指令。

**输出**: 返回未读统计的处理结果。

- 执行`未读统计`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`未读统计`相关配置参数进行设置
### 技术细节
...
本skill还覆盖以下能力场景: 登录单个邮箱并生、成当日邮件摘要的、基础版、CLI、自动化登录、邮箱等、Web、抓取当日收件箱列、摘要片段与时间戳、生成基础邮件日报、支持未读统计与截、适用于个人开发者、每日邮件快速梳理。这些能力在上述核心功能中均有对应处理逻辑。
## 前置依赖
...
- **处理**:在本机 Chrome 中重新登录目标邮箱后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令
