...
**处理**: 按照skill规范执行技能描述编写操作,遵循单一意图原则。
**输出**: 返回技能描述编写的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行Front Matter生成操作,遵循单一意图原则。

- 执行`Front Matter生成`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
...
**输出**: 返回质量检查的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 创建和优化、支持结构规划、脚本集成和文件拆、技能创建和优化工、支持技能结构规划、渐进式披露设计、脚本集成和文件、行时拆分到、适用于独立开发者、企业团队和自动化、工作流场景、不适用于非、技能的文档创建。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Anything missing or unclear? | 支持 | 通过核心功能实现对应能力 |
| SKILL.md with concise instructions | 支持 | 通过核心功能实现对应能力 |
| Draft the skill | 支持 | 通过核心功能实现对应能力 |
| Should any section be more/less detailed? | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `workflows`, `智能对话`, `helps`, `give`, `operation`, `good`, `main`, `utility`, `pdfs`, `concrete`, `模型调用`, `content`, `应用时使用`, `checklist`, `template`
...
- `-validator`: 命令参数,用于指定操作选项
- `--schema`: 命令参数,用于指定操作选项
- `-oP`: 命令参数,用于指定操作选项
- `--input`: 命令参数,用于指定操作选项
- `-schema`: 命令参数,用于指定操作选项
...
### 验证命令
\`\`\`bash
python3 scripts/validate.py --input data.json --schema user-schema.json
\`\`\`
...
### Q5: 脚本文件应该放在哪里？
A: 放入 `scripts/` 目录。在SKILL.md中通过路径引用（如 `python3 scripts/validate.py`）。脚本文件不计入SKILL.md的行数限制。
