...
**处理**: 按照skill规范执行无限订阅源批量管理操作,遵循单一意图原则。

- 执行`无限订阅源批量管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`无限订阅源批量管理`相关配置参数进行设置
### 2. AI深度摘要与关键词提取
除了基础摘要外，专业版支持关键词提取、情感分析、主题分类、重要性评分等深度分析。

**输入**: 用户提供AI深度摘要与关键词提取所需的指令和必要参数。
**输出**: 返回AI深度摘要与关键词提取的执行结果,包含操作状态和输出数据。

- 执行`AI深度摘要与关键词提取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`AI深度摘要与关键词提取`相关配置参数进行设置
### 3. 多渠道同时推送
支持飞书、Telegram、Email、自定义Webhook等多个渠道同时推送，按订阅源类别分配不同渠道。

**输入**: 用户提供多渠道同时推送所需的指令和必要参数。
**处理**: 按照skill规范执行多渠道同时推送操作,遵循单一意图原则。
**输出**: 返回多渠道同时推送的执行结果,包含操作状态和输出数据。

- 执行`多渠道同时推送`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多渠道同时推送`相关配置参数进行设置
### 4. 定时调度与任务队列
支持cron表达式定时调度，内置任务队列管理，支持优先级排序与失败重试。

**输入**: 用户提供定时调度与任务队列所需的指令和必要参数。
**输出**: 返回定时调度与任务队列的执行结果,包含操作状态和输出数据。

- 执行`定时调度与任务队列`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`定时调度与任务队列`相关配置参数进行设置
### 5. 内容趋势分析
提供阅读统计、热点趋势、关键词云、来源分布等可视化分析报表。

**输出**: 返回内容趋势分析的执行结果,包含操作状态和输出数据。

- 执行`内容趋势分析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`内容趋势分析`相关配置参数进行设置
### 6. 团队订阅共享
支持团队内订阅源共享、推送规则配置、成员权限管理。

**输入**: 用户提供团队订阅共享所需的指令和必要参数。
**处理**: 按照skill规范执行团队订阅共享操作,遵循单一意图原则。
**输出**: 返回团队订阅共享的执行结果,包含操作状态和输出数据。

- 执行`团队订阅共享`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`团队订阅共享`相关配置参数进行设置
### 技术细节
...
本skill还覆盖以下能力场景: 企业级、RSS、聚合平台、支持多源批量抓取、多渠道推送、定时调度与数据分、智能阅读器、面向团队和企业的、全功能信息聚合平、核心能力、无限订阅源批量抓、取与分类管理、内容趋势分析与阅、团队订阅共享与权。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `--from-free`: 命令参数,用于指定操作选项
2. `--db`: 命令参数,用于指定操作选项
3. `--to-pro`: 命令参数,用于指定操作选项
4. `-group`: 命令参数,用于指定操作选项
5. `--export-feeds`: 命令参数,用于指定操作选项

### 命令参数说明

- `--import-feeds`: 命令参数,用于指定操作选项
- `--role`: 命令参数,用于指定操作选项
- `-news`: 命令参数,用于指定操作选项
- `--team`: 命令参数,用于指定操作选项
- `--verbose`: 命令参数,用于指定操作选项

### 命令参数说明

- `--report`: 命令参数,用于指定操作选项
- `--feed-id`: 命令参数,用于指定操作选项
- `--config`: 命令参数,用于指定操作选项
- `--workers`: 命令参数,用于指定操作选项
- `--once`: 命令参数,用于指定操作选项

### 命令参数说明

- `-blog`: 命令参数,用于指定操作选项
- `-updates`: 命令参数,用于指定操作选项
- `-channel`: 命令参数,用于指定操作选项
- `--stats`: 命令参数,用于指定操作选项
- `-member`: 命令参数,用于指定操作选项

### 命令参数说明

- `--user`: 命令参数,用于指定操作选项
- `-reader-pro`: 命令参数,用于指定操作选项
