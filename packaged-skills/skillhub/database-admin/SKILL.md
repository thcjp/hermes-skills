...
**输出**: 返回数据插入的执行结果,包含操作状态和输出数据。### 查询优化
...
**输出**: 返回查询优化的执行结果,包含操作状态和输出数据。### 数据库维护
...
**输出**: 返回事务安全的执行结果,包含操作状态和输出数据。

### 能力覆盖范围

本skill还覆盖以下能力场景: 批量数据操作、类型处理与事务安、全的全面数据库管、理服务、数据库管理专家、提供全面的数据库、管理功能、包括表结构创建、数据操作、类型处理等、核心能力包括、与默认值策略、多数据类型支持、批量数据插入、大数处理、自动提交、连接池管理、数据类型兼容性验、证与主键冲突避免。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
