...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Create: Create new blank documents. | 支持 | 通过核心功能实现对应能力 |
| Blocks: List, get, update, and delete specific blocks. | 支持 | 通过核心功能实现对应能力 |

### 领域术语
本skill涉及以下领域术语: `overwrite`, `feishu_app_secret`, `prerequisites`, `append`, `introduction`, `unlimited`, `environment`, `feishu-common`, `length`, `chunk`, `feishu_doc_write`, `feishu_doc_append`, `title`, `chapter`, `config.json`
