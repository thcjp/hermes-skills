...
- 执行`编码纪律`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`编码纪律`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级项目开发标、准方案、含自动化验收、团队协作与质量度、面向企业研发团队、的项目开发标准化、专业工具、提供自动化验收与、团队协作治理方案、核心能力、自动化验收清单、团队级开发规范与、合并请求模板、缺陷修复返工率与、代码质量度量、多文件同步修改与、跨语言项目支持、代码审查自动化与、风险点识别。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
1. `-rl`: 命令参数,用于指定操作选项
2. `-path`: 命令参数,用于指定操作选项
3. `-class`: 命令参数,用于指定操作选项
4. `--cov-report`: 命令参数,用于指定操作选项
5. `-merge-check`: 命令参数,用于指定操作选项
