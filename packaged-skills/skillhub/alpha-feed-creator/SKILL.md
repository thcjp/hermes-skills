...
执行采集来源操作,处理用户输入并返回结果。

**输入**: 用户提供采集来源所需的参数和指令。

**输出**: 返回采集来源的处理结果。

- 执行`采集来源`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`采集来源`相关配置参数进行设置
### 智能排名

执行智能排名操作,处理用户输入并返回结果。

**输入**: 用户提供智能排名所需的参数和指令。

**输出**: 返回智能排名的处理结果。

- 执行`智能排名`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`智能排名`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 多源批量采集并智、支持群组推送与企、业级内容运营工作、面向内容团队与企、业级运营的多源、内容采集与智能排、名平台、核心能力、跨平台多源采集、微博等、支持批量白名单与、多关键词组、基于语义相似度与、质量评估的多维智、能排名算法、群组自动推送、定时调度、批量日报生成、企业级权限隔离与、多租户配置管理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-b-ops`: 命令参数,用于指定操作选项
- `--report-mode`: 命令参数,用于指定操作选项
- `-brand-ops`: 命令参数,用于指定操作选项
- `--report-format`: 命令参数,用于指定操作选项

### 命令参数说明

- `--push-test`: 命令参数,用于指定操作选项
- `--keywords`: 命令参数,用于指定操作选项
- `--push-channel`: 命令参数,用于指定操作选项
- `--track-accounts`: 命令参数,用于指定操作选项
- `--check-push`: 命令参数,用于指定操作选项

### 命令参数说明

- `-a-ops`: 命令参数,用于指定操作选项
- `-work`: 命令参数,用于指定操作选项
- `-feed-creator-pro`: 命令参数,用于指定操作选项
- `-content-team`: 命令参数,用于指定操作选项
- `-team`: 命令参数,用于指定操作选项

### 命令参数说明

- `--push-target`: 命令参数,用于指定操作选项
- `-quality`: 命令参数,用于指定操作选项
- `-team-daily`: 命令参数,用于指定操作选项
