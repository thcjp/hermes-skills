...
本skill还覆盖以下能力场景: 企业级、Excel、生成引擎、含批量处理、条件格式、高级图表、模板系统与数据源、工艺专业版在免费、版基础上扩展批量、多文件合并拆分、条件格式与数据验、对接能力、Use、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 已知限制
- 多 Sheet 工作簿生成
- 基础格式化（字体 / 填充 / 边框 / 对齐）
- 公式支持
...
### Q8：模板变量与公式冲突？
A：模板变量用 `（根据实际场景填充）` 语法，公式用 `=` 开头，两者不会冲突。但若公式中含 `{{` 字符，需转义。
