...
执行【强制】操作,处理用户输入并返回结果。

**输入**: 用户提供【强制】所需的参数和指令。

**输出**: 返回【强制】的处理结果。

- 执行`【强制】`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`【强制】`相关配置参数进行设置
### 【推荐】

执行【推荐】操作,处理用户输入并返回结果。

**输入**: 用户提供【推荐】所需的参数和指令。

**输出**: 返回【推荐】的处理结果。

- 执行`【推荐】`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`【推荐】`相关配置参数进行设置
### 【参考】

执行【参考】操作,处理用户输入并返回结果。

**输入**: 用户提供【参考】所需的参数和指令。

**输出**: 返回【参考】的处理结果。

- 执行`【参考】`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`【参考】`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 开发手册规约集合、大维度、编程规约、异常日志、单元测试、安全规约、MySQL、数据库、工程结构、设计规约、Use、需要代码生成、编程辅助、调试测试、开发部署时使用、不适用于无明确技、术栈的模糊需求、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
