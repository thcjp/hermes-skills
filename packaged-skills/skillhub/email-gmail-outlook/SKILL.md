...
执行多账号 profile 隔离,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供多账号 profile 隔离相关的配置参数、输入数据和处理选项。

**输出**: 返回多账号 profile 隔离的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`多账号 profile 隔离`相关配置参数进行设置
### 安全凭证存储

执行安全凭证存储,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供安全凭证存储相关的配置参数、输入数据和处理选项。

**输出**: 返回安全凭证存储的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`安全凭证存储`相关配置参数进行设置
### 邮件列表与筛选

执行邮件列表与筛选,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供邮件列表与筛选相关的配置参数、输入数据和处理选项。

**输出**: 返回邮件列表与筛选的处理结果。- 验证执行结果，确认输出符合预期格式
