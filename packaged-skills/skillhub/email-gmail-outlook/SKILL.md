...
- 执行`多账号 profile 隔离`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多账号 profile 隔离`相关配置参数进行设置
### 安全凭证存储
...
- 执行`安全凭证存储`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`安全凭证存储`相关配置参数进行设置
### 邮件列表与筛选
...
- 执行`邮件列表与筛选`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`邮件列表与筛选`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: porteden、CLI、Gmail、Outlook、Exchange、多账号邮件、email、mail、安全读写、支持多账号、覆盖邮件列表、删除全生命周期操、适用于收件箱分诊、批量模板回复、线程审阅、定时邮件发送、跨账号搜索等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
