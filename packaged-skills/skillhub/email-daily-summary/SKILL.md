...
执行多邮箱自动登录操作,处理用户输入并返回结果。

**输入**: 用户提供多邮箱自动登录所需的参数和指令。

**输出**: 返回多邮箱自动登录的处理结果。

- 执行`多邮箱自动登录`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`多邮箱自动登录`相关配置参数进行设置
### 邮件列表抓取

执行邮件列表抓取操作,处理用户输入并返回结果。

**输入**: 用户提供邮件列表抓取所需的参数和指令。

**输出**: 返回邮件列表抓取的处理结果。

- 执行`邮件列表抓取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`邮件列表抓取`相关配置参数进行设置
### 统计聚合

执行统计聚合操作,处理用户输入并返回结果。

**输入**: 用户提供统计聚合所需的参数和指令。

**输出**: 返回统计聚合的处理结果。

- 执行`统计聚合`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`统计聚合`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 自动登录邮箱并生、成每日邮件摘要与、统计日报、CLI、自动化登录、邮箱及企业微信邮、抓取收件箱列表、提取发件人、摘要片段与时间戳、生成结构化每日邮、件日报、支持未读统计、主题分类、截图归档与定时任、务调度、适用于独立开发者、每日邮件梳理、团队晨会邮件同步、企业高管、分诊等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 前置依赖
...
- **处理**:切换 `--headed` 模式手动登录一次,或在本机 Chrome 中重新登录目标邮箱后执行ping命令测试网络连通性,检查防火墙和代理设置连接后重新执行命令
