...
本skill还覆盖以下能力场景: 全功能数据格式转、换平台、支持批量、自定义映射、定时任务与企业数、数据格式转换专业、版面向需要在大规、模数据与多源系统、间进行格式转换的、专业开发者与数据、工程师、提供完整的批量、自动化转换能力、核心能力、涵盖免费版全部能、无文件大小与数量、目录级批量处理、支持通配符与递归、大文件分块读取与、常见场景一键复用、自动监听与转换。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
