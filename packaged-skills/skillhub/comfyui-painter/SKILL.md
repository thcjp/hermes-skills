...
**输出**: 返回ComfyUI 生命周期管理的执行结果,包含操作状态和输出数据。

- 执行`ComfyUI 生命周期管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**输出**: 返回图片生成的执行结果,包含操作状态和输出数据。

- 执行`图片生成`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`图片生成`相关配置参数进行设置
### 基础生成
...
**输出**: 返回CivitAI 集成的执行结果,包含操作状态和输出数据。

- 执行`CivitAI 集成`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
**处理**: 按照skill规范执行搜索模型操作,遵循单一意图原则。

- 执行`搜索模型`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `搜索模型` 选项
...
**输出**: 返回查看模型详情的执行结果,包含操作状态和输出数据。

- 执行`查看模型详情`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `查看模型详情` 选项
...
**输出**: 返回获取推荐参数的执行结果,包含操作状态和输出数据。

- 执行`获取推荐参数`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `获取推荐参数` 选项
...
本skill还覆盖以下能力场景: 画图工作流、控制文生图、图生视频、支持模型搜索、画图工作流与、模型管理集成客户、在本地、GPU、上生成图片、支持文生图与图生、视频两种任务类型、模型生态、详情查询、推荐参数提取、本地模型更新检查、与一键下载、内置自动调参引擎、样图元数据中提取、设置并写入、种模型别名覆盖动、DCG、暗黑美学、Flux、等风格、提供心跳、空闲检测、分钟无使用自动关、释放显存、适用于个人创作者、内容生产团队与自、动化画图、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 脚本路径
...
- `--filename`: 命令参数,用于指定操作选项

**结果处理**: 执行完成后,查看输出结果确认操作状态。成功时输出包含处理摘要和结果数据;失败时根据错误信息排查问题,参考错误处理章节获取恢复步骤。

### 命令参数说明

- `-SDXL-V8`: 命令参数,用于指定操作选项
- `-V4`: 命令参数,用于指定操作选项
- `-V6`: 命令参数,用于指定操作选项
- `-dev-bnb-nf4-v2`: 命令参数,用于指定操作选项
