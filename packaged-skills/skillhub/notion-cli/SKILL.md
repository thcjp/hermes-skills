...
- 执行`工作空间数量`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`工作空间数量`相关配置参数进行设置
### 文件上传

执行文件上传操作,处理用户输入并返回结果。

**输入**: 用户提供文件上传所需的参数和指令。

**输出**: 返回文件上传的处理结果。

- 执行`文件上传`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`文件上传`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、Notion、命令行工具、支持多工作空间、批量操作与审计日、适合团队与企业规、模化使用、命令行、是面向团队与企业、的全功能、Skill、在免费版基础上新、增多工作空间管理、自定义输出与审计、日志等高级能力、核心能力。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
2. `--confirm`: 命令参数,用于指定操作选项
3. `--remove-prop`: 命令参数,用于指定操作选项
4. `--add-prop`: 命令参数,用于指定操作选项
5. `--resource-id`: 命令参数,用于指定操作选项
...
- `-pqr678-`: 命令参数,用于指定操作选项
- `-delete`: 命令参数,用于指定操作选项

### 命令参数说明

- `--resource-type`: 命令参数,用于指定操作选项
- `--limit`: 命令参数,用于指定操作选项
- `--end`: 命令参数,用于指定操作选项
- `--from`: 命令参数,用于指定操作选项
- `-list`: 命令参数,用于指定操作选项
...
- `-jkl012-`: 命令参数,用于指定操作选项
- `--filter`: 命令参数,用于指定操作选项
- `--start`: 命令参数,用于指定操作选项
- `-page-id`: 命令参数,用于指定操作选项
- `--transform`: 命令参数,用于指定操作选项
