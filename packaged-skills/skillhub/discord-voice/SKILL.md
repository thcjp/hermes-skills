...
- 执行`单公会单频道约束`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
### 能力覆盖范围

本skill还覆盖以下能力场景: 语音对话助手、VAD、自动重连与流式转、录全流程、语音对话专业版、语音频道中实现实、时双向语音对话的、端到端助手、覆盖语音活动检测、语音转文字、文字转语音、打断响应、Barge、录等核心能力。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
