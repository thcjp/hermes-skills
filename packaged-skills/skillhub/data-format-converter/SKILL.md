...
本skill还覆盖以下能力场景: 格式间高效互转、支持批量处理与嵌、套结构处理、数据格式转换器、等主流数据格式间、高效转换、核心能力包括、支持嵌套结构展开、与扁平化、保留注释与锚点引、处理属性与子元素、支持表数组与嵌套、保持文件名映射、编码处理、自动识别与输出、格式美化、中文输出。这些能力在上述核心功能中均有对应处理逻辑。
## 支持格式矩阵
