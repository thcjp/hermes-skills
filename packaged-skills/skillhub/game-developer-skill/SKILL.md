...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Design architecture | 支持 | 通过核心功能实现对应能力 |
| Analyze requirements | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `comparetag`, `profile`, `references/unreal-cpp.md`, `scriptable`, `developing`, `references/performance-optimization.md`, `performance`, `references/multiplayer-networking.md`, `guide`, `glsl`, `fixedupdate`, `optimize`, `find`, `entity`, `scriptableobjects`
