...
- **浏览器**: 支持现代CSS特性的浏览器（Chrome 100+/Firefox 100+/Safari 15+）

### 依赖项
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| Agent 平台 | 运行环境 | 必需 | 任意支持 SKILL.md 的 AI Agent |
| 现代浏览器 | 运行环境 | 可选 | Chrome/Firefox/Safari 用于预览效果 |
| Tailwind CSS | 框架 | 可选 | `npm install -D tailwindcss` 按需安装 |

### 可用性分类
- **分类**: MD（Markdown 指令驱动，输出CSS代码）
- **说明**: 基于自然语言指令驱动 Agent 生成CSS样式代码与布局方案
