...
在每晚 22:00(Asia/Shanghai 时区)自动触发扫描,拉取明日 00:00 至 23:59 的全部 Outlook 日历事件,包括会议主题、开始时间、结束时间、组织者、地点、是否全天事件等字段。扫描结果会立即生成一份汇报消息推送到飞书,让用户在睡前就能预览明日全天安排。

- 执行`每日定时扫描明日日历`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`每日定时扫描明日日历`相关配置参数进行设置
### 2. 按时段差异化提醒策略
...
**处理**: 按照skill规范执行按时段差异化提醒策略操作,遵循单一意图原则。
**输出**: 返回按时段差异化提醒策略的执行结果,包含操作状态和输出数据。
...
**输出**: 返回Cron 定时任务注册的执行结果,包含操作状态和输出数据。

- 执行`Cron 定时任务注册`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Cron 定时任务注册`相关配置参数进行设置
### 5. 扫描结果汇报

扫描完成后立即发送一条汇报消息到飞书,包含明日日程总数、上午/下午日程分布、全天事件列表、需重点关注的会议(如组织者为高管或外部客户),让用户一眼掌握明日重点。

- 执行`扫描结果汇报`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`扫描结果汇报`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 点扫描明日、按上下午时段差异、化飞书提醒、自动注册定时任务、并汇报扫描结果、面向独立开发者与、企业团队的、日历智能提醒、自动扫描明日全部、按时段策略差异化、推送飞书通知、上午日程提前、小时提醒、下午日程在当天、扫描完成立即发送、owa、提供的、calendar、读取日历数据、CLI、处理时区、适用于需要每日日、程前置提醒、跨时区协作、避免错过晨会的自、动化工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
