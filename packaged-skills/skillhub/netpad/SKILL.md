...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Delete submission | 支持 | 通过核心功能实现对应能力 |
| Task | 支持 | 通过核心功能实现对应能力 |
| Get submission | 支持 | 通过核心功能实现对应能力 |
| List forms | 支持 | 通过核心功能实现对应能力 |
| Create submission | 支持 | 通过核心功能实现对应能力 |
| List projects | 支持 | 通过核心功能实现对应能力 |
| List submissions | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
### 领域术语
本skill涉及以下领域术语: `permissions`, `marketplace`, `number`, `patch`, `authorization`, `textarea`, `engineering`, `github`, `mongodb`, `michael`, `text`, `fields`, `numeric`, `rbac`, `requests`
