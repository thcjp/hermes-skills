...
| 依赖项 | 类型 | 是否必需 | 获取方式 |
|:-------|:-----|:---------|:---------|
| LLM API | API | 必需 | 由Agent内置LLM提供(建议GPT-4级别以上模型以获得更好分析质量) |
| 财务报表数据 | 数据 | 必需 | 用户提供(资产负债表、利润表、现金流量表,可从年报/季报获取) |

### 可用性分类
- **分类**: MD(纯Markdown指令,通过自然语言驱动Agent执行基础财务分析)
- **说明**: 基于Markdown的AI Skill,通过自然语言指令驱动Agent执行基础财务比率计算与分析
...
### 1. 数据准备
收集目标企业财务报表数据:
- 资产负债表(期末数,建议取2期用于计算平均值)
- 利润表(本期数据)
- 现金流量表(本期数据)
- 数据来源: 年报、季报、企业内部报表

### 2. 数据校验
- 检查三大报表基本勾稽关系(资产 = 负债 + 所有者权益)
- 确认关键科目无缺失(流动资产、流动负债、净利润、营业收入等)
- 如数据不全,标注缺失项并使用可获取数据进行替代分析

### 3. 比率计算
按分析目标选择对应比率:
- **偿债能力** → 计算流动比率、速动比率、资产负债率
- **盈利能力** → 计算毛利率、净利率、ROE
- **综合诊断** → 执行DuPont三因素分解

### 4. 趋势对比
- 将计算结果与经验阈值对比(流动比率2.0、速动比率1.0等)
- 如有多期数据,进行同比/环比趋势分析
- 与同行业可比公司进行横向对比(需用户提供对标数据)
