...
本skill还覆盖以下能力场景: Fetch、filter、summarize、RSS、Atom、feeds、into、clean、daily、weekly、digest、Supports、multipl、Use、模型调用、智能对话、Agent、LLM、应用时使用、不适用于需要、确定性的关键决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-ai-digest`: 命令参数,用于指定操作选项
- `--hours`: 命令参数,用于指定操作选项
- `--feed-file`: 命令参数,用于指定操作选项
- `-lab`: 命令参数,用于指定操作选项
