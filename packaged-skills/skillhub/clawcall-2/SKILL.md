...
description: "让AI Agent打美国电话/呼叫商家/处理等待与电话树。Use when the user wants an AI agent to place a US phone call, call a。触发关键词: clawcall, wants, agent, place。提供专业能力支持,覆盖多场景工作流,支持自动化处理。"
license: "MIT-0"
tools:
  - read
---
...
For a good setup, ask only for the assistant name/role, desired tone, hard boundaries, and default handoff/callback number when useful. See .
...
* : must read before placing, retrying, handing off, or following up on outbound calls.
* : must read before configuring, clearing, inspecting, or polling inbound reserved-number behavior.
* : must read when constructing requests, parsing responses, or relying on exact field names.
* : must read when building rich outbound, callback, handoff, campaign, or inbound instruction shapes.
* : must read before handling API errors, terminal outcomes, retries, quota, trial, or balance behavior.
* : must read before account linking, key handling, saved phone-number behavior, recordings, transcripts, or privacy answers.
* : must read before evaluating or changing skill behavior.
