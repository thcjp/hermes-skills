...
- **说明**: 基于自然语言指令驱动 Agent 调用 Gemini Flash Image API 完成图像生成与编辑
...
### 执行生成
调用 `scripts/generate_image.py` 生成图片并落盘。
...
| 429 rate_limited | `{"error":"rate_limited"}` | 短时间请求过多 | 指数退避重试(2s/4s/8s),最多 3 次 |
| 400 invalid_prompt | `{"error":"prompt_too_long"}` | 提示词超出长度限制 | 不重试,精简提示词后重新生成 |
| 400 unsupported_model | `{"error":"model_not_found"}` | 模型 ID 拼写错误 | 不重试,核对可用模型列表 |
| 400 image_input_required | `{"error":"image_input_required"}` | 图生图/风格转换未提供参考图 | 不重试,补充 --image-input 参数 |
| 5xx server_error | HTTP 500/502/503 | Gemini Image 服务端错误 | 指数退避重试,最多 2 次 |
| network_timeout | `Connection timed out` | 无法访问 IMAGE_GEN_BASE_URL | 检查网络与代理配置后重试 |
