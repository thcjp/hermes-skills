...
执行流式解析操作,处理用户输入并返回结果。

**输入**: 用户提供流式解析所需的参数和指令。

**输出**: 返回流式解析的处理结果。

- 执行`流式解析`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`流式解析`相关配置参数进行设置
### 流式生成

执行流式生成操作,处理用户输入并返回结果。

**输入**: 用户提供流式生成所需的参数和指令。

**输出**: 返回流式生成的处理结果。

- 执行`流式生成`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`流式生成`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、处理工具、支持流式解析、校验与多格式互转、Toolkit、专业版面向数据工、程师与后端开发者、在免费版基础上解、锁流式大文件处理、核心能力、流式分块解析、级文件、方言配置、校验与列类型推断、多文件合并与拆分、性能基准测试与优、增量更新与检查点。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
