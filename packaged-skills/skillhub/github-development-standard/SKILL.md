...
本skill还覆盖以下能力场景: 完整的、GitHub、项目开发标准流程、步流程、层验证、项验收清单。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| A1. 我能用一句话说清这次修复的目标 | 支持 | 通过核心功能实现对应能力 |
| D1. diff 大小与任务规模匹配 | 支持 | 通过核心功能实现对应能力 |
| A. 需求一致性（3 项） | 支持 | 通过核心功能实现对应能力 |
| D. 发布质量（4 项） | 支持 | 通过核心功能实现对应能力 |

**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `夹带私货率`, `让代码质量不再妥`, `代码改动与`, `过度修改`, `不要修改未被需求`, `注释已同步`, `修复返工率`, `夹带私货`, `数据结构变化已同`, `先搜所有使用点`, `修复说明`, `测试验证`, `语法检查通过`, `平台规范`, `核心理念`
