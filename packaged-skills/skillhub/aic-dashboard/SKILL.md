...
| `express@4.21.2` | npm 包 | 必需 | `npm install express@4.21.2` |
| `inbox.jsonl` | 数据文件 | 可选 | 由邮件收集器写入,缺失则邮件区为空 |
| `session.json` | 数据文件 | 可选 | 由浏览器认证写入,缺失则会话区显示离线 |

### 可用性分类
- **分类**: MD+EXEC（Markdown 指令驱动,需 exec 执行 Node.js 脚本）
- **说明**: 基于自然语言指令驱动 Agent 启动本地 Web 服务,展示监控数据
