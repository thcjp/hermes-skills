...
- 执行`Token 保护`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Token 保护`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 轻量级只读监控面、展示入站邮件与浏、览器会话状态、秒自动刷新、Commander、配套管理仪表盘、轻量级只读、Web、展示最近、展示浏览器会话状、保护的本地端口提、供服务、不捕获凭证、不控制浏览器、不发送消息、仅读取本地数据文、件并安全呈现、适用于独立开发者、监控邮件收集器与、浏览器认证会话的、运行状态。这些能力在上述核心功能中均有对应处理逻辑。
## 环境变量
