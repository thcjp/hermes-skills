...
| 能力 | 免费版 | 付费版 |
|:-----|:-------|:-------|
| **邮件展示**: 读取 `inbox.jsonl`,显示最近 50 封入站邮件的发件人、主题、时间、摘要 | 支持 | 支持 |
| **会话状态**: 读取 `session.json`,显示浏览器会话是否活跃、隧道地址、过期时间 | 不支持 | 支持 |
| **Token 保护**: 所有请求需携带有效 token,支持 3 种传递方式 | 不支持 | 支持 |
| **自动刷新**: 前端每 5 秒轮询后端,无需手动刷新页面 | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
