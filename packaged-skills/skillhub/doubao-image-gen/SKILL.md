...
edition: "pro"
description: |-
  豆包图片生成专业版。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。
tags:
  - Creative
  - AI绘图
  - 图片生成
...
- 执行`批量图片生成`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `批量图片生成` 选项
- 处理流程: 接收输入 -> 执行批量图片生成 -> 返回结果
- 输入: 用户提供批量图片生成所需的参数和指令
- 输出: 返回批量图片生成的执行结果,包含操作状态和输出数据
...
**处理**: 按照skill规范执行多比例同时输出操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行风格预设管理操作,遵循单一意图原则。
**输出**: 返回风格预设管理的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行提示词增强操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行参考图生成操作,遵循单一意图原则。
...
本skill还覆盖以下能力场景: 企业级、图片生成平台、支持批量生成、提示词增强与工作、豆包图片生成专业、Use、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `--enhance-prompt`: 命令参数,用于指定操作选项
- `--parallel`: 命令参数,用于指定操作选项

### 命令参数说明

- `--negative`: 命令参数,用于指定操作选项
- `--archive-dir`: 命令参数,用于指定操作选项
- `--batch`: 命令参数,用于指定操作选项
- `--retry-failed`: 命令参数,用于指定操作选项
- `--keywords`: 命令参数,用于指定操作选项

### 命令参数说明

- `-ratio`: 命令参数,用于指定操作选项
- `--ratios`: 命令参数,用于指定操作选项
...
- `--auto-archive`: 命令参数,用于指定操作选项
- `--style-preset`: 命令参数,用于指定操作选项
