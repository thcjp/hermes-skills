...
| 能力 | 免费版 | 付费版 |
|:-----|:-------|:-------|
| **自动中英翻译**：将英文新闻标题自动翻译为中文，专有名词（如 GPT-4、Claude、Anthropic）保留英文原文，确保中文用户可读 | 支持 | 支持 |
| **智能摘要生成**：为每条新闻生成 1-2 句中文摘要，凝练核心信息点，控制在 60 字以内，避免直接复制原文 | 不支持 | 支持 |
| **HTML 海报渲染**：使用全屏 HTML 模板替代 Canvas，获得更稳定的字体渲染与渐变效果 | 不支持 | 支持 |
| **三种截图方案**： | 不支持 | 支持 |
| 批量处理 | 不支持 | 支持 |
| 高级配置 | 不支持 | 支持 |
...
执行关键词热搜检索,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供关键词热搜检索相关的配置参数、输入数据和处理选项。

**输出**: 返回关键词热搜检索的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`关键词热搜检索`相关配置参数进行设置
### Top 10 精确提取

执行Top 10 精确提取,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供Top 10 精确提取相关的配置参数、输入数据和处理选项。

**输出**: 返回Top 10 精确提取的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`Top 10 精确提取`相关配置参数进行设置
### 自动中英翻译

执行自动中英翻译,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供自动中英翻译相关的配置参数、输入数据和处理选项。

**输出**: 返回自动中英翻译的处理结果。- 验证执行结果，确认输出符合预期格式
...
| 错误场景 | 原因 | 处理方式 |
|---------|------|---------|
| X.com 返回登录页而非搜索结果 | 未登录或 Cookie 过期 | 检查浏览器登录态，重新登录后；或改用未登录可访问的热门搜索接口 |
