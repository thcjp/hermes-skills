...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| input | string | 是 | X News Daily处理的输入数据或指令 |
...
针对Top 10 精确提取,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应。

**输入**: 用户提供Top 10 精确提取相关的配置参数、输入数据和处理选项。

**输出**: 返回Top 10 精确提取的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`Top 10 精确提取`的配置文档进行参数调优
### 自动中英翻译
