...
执行关键词热搜检索操作,处理用户输入并返回结果。

**输入**: 用户提供关键词热搜检索所需的参数和指令。

**输出**: 返回关键词热搜检索的处理结果。

- 执行`关键词热搜检索`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`关键词热搜检索`相关配置参数进行设置
### Top 10 精确提取

执行Top 10 精确提取操作,处理用户输入并返回结果。

**输入**: 用户提供Top 10 精确提取所需的参数和指令。

**输出**: 返回Top 10 精确提取的处理结果。

- 执行`Top 10 精确提取`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`Top 10 精确提取`相关配置参数进行设置
### 自动中英翻译

执行自动中英翻译操作,处理用户输入并返回结果。

**输入**: 用户提供自动中英翻译所需的参数和指令。

**输出**: 返回自动中英翻译的处理结果。

- 执行`自动中英翻译`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`自动中英翻译`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 关键词热门新闻、生成全屏中文海报、支持多平台分发、上指定关键词的热、门新闻、自动将英文标题翻、最终渲染为全屏、海报图片、模式配合、生成的海报可作为、文件分发到、微信等任意、适用于每日新闻简、关键词舆情监测、内容运营自动化、定时任务推送等场。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Features | 支持 | 通过核心功能实现对应能力 |
| Steps | 支持 | 通过核心功能实现对应能力 |
| Always translate to Chinese | 支持 | 通过核心功能实现对应能力 |
| Translate titles to Chinese | 支持 | 通过核心功能实现对应能力 |
...
本skill涉及以下领域术语: `triggers`, `english`, `使用对应标签发送`, `或使用默认关键词`, `海报设计`, `生成全屏精美海报`, `每条新闻添加`, `translation`, `poster`, `模板以获得更好的`, `排名提升`, `white`, `platform`, `作为文件发送到任`, `获取用户提供的关`
