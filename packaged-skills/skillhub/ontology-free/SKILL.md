...
本skill还覆盖以下能力场景: 基于类型约束的知、识图谱系统、为智能代理提供基、础结构化记忆、类型化知识图谱引、将知识表示为可验、证的实体、每个实体拥有类型、属性和关系、根据类型约束进行、基础能力涵盖类型、等核心实体类型、可通过、数据以追加式、JSONL、格式存储、适用于项目任务管、依赖关系追踪、知识库构建等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
