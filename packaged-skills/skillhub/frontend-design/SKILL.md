...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| There's one signature element that makes this memorable | 支持 | 通过核心功能实现对应能力 |
| Focus states visible for keyboard navigation | 支持 | 通过核心功能实现对应能力 |
| Reduced motion respected (prefers-reduced-motion) | 支持 | 通过核心功能实现对应能力 |
| No anti-patterns from the list above | 支持 | 通过核心功能实现对应能力 |
| Layout has a clear visual hierarchy (what's the hero?) | 支持 | 通过核心功能实现对应能力 |
| Excessive box-shadow — one subtle shadow is enough | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `published`, `before`, `placeholder`, `human`, `maximalist`, `many`, `words`, `bring`, `ascii`, `just`, `more`, `pair`, `work`, `approach`, `explain`
