...
本skill还覆盖以下能力场景: 飞书发送普通文件、与图片附件、两步上传与、图片稳定链路、飞书机器人发送文、件附件技能、覆盖普通文件、代码文件等、与图片两类链路、本地图片路径被发、成路径文本、的常见故障、提供脚本化与手动、两步两种调用方式、适配中国版飞书、适用于自动化工作、企业团队通知与开、发者文件分发场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
