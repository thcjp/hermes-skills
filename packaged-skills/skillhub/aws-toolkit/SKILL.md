...
- 执行`部署方式`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`部署方式`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 企业级、全服务管理平台、支持多区域、合规审计与成本优、面向企业运维团队、等全量、提供基础设施即代、成本优化与安全扫、描功能、Use、需要安全检测、漏洞扫描、加密防护时使用、不适用于渗透测试、未授权目标。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
