...
本skill还覆盖以下能力场景: 企业级、支持批量流式转换、字段映射与数据库、专业版是一款面向、数据团队与企业级、ETL、场景的全功能表格、数据格式转换工具、在免费版的表头推、编码探测基础上、新增批量转换、增量同步与数据库、直写六大高级能力、覆盖从单文件到、级数据流水线的全、场景需求、需要文件处理、文档转换、格式互转、内容提取时使用、不适用于加密文件。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
