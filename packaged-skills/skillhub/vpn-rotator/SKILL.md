...
执行VPN连接管理操作,处理用户输入并返回结果。

**输入**: 用户提供VPN连接管理所需的参数和指令。

**输出**: 返回VPN连接管理的处理结果。

- 执行`VPN连接管理`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`VPN连接管理`相关配置参数进行设置
### IP轮换

执行IP轮换操作,处理用户输入并返回结果。

**输入**: 用户提供IP轮换所需的参数和指令。

**输出**: 返回IP轮换的处理结果。

- 执行`IP轮换`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`IP轮换`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 全功能、轮换平台、支持自动重连、负载均衡与熔断机、面向数据工程团队、与自动化测试团队、的全功能、并发管理、智能负载均衡、自动重连与失败熔、核心能力、在免费版基础上新、增自动重连机制、失败熔断器、按地区智能路由与、性能监控仪表盘、Use、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
| 问题现象 | 可能原因 | 排查步骤 | 优先级 |
|:---------|:---------|:---------|:-------|
| 所有VPN连接失败 | 网络中断或凭证过期 | 执行ping命令测试网络连通性,检查防火墙和代理设置连通性，验证凭证 | P0 |
