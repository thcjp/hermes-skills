...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 古风水墨界面摇卦 | 支持 | 通过核心功能实现对应能力 |
| 九宫飞星（Python） | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `帮我综合看看最近`, `详细解读区`, `入家庭成员八字`, `必须先运行`, `婚姻感情`, `问题展示区`, `不得冒充高精度`, `双方互动`, `风险预警`, `面相辅助`, `今日宜忌`, `完整命盘`, `详细模板见`, `理性顾问风`, `调用脚本或手动推`
