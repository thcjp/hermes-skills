...
**处理**: 按照skill规范执行单段语音合成操作,遵循单一意图原则。
...
**处理**: 按照skill规范执行多音色切换操作,遵循单一意图原则。
**输出**: 返回多音色切换的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行长文本分段与合并操作,遵循单一意图原则。
**输出**: 返回长文本分段与合并的执行结果,包含操作状态和输出数据。

- 执行`长文本分段与合并`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
- 关键参数: `长文本分段与合并` 选项
...
**处理**: 按照skill规范执行风格控制(SSML 子集)操作,遵循单一意图原则。
**输出**: 返回风格控制(SSML 子集)的执行结果,包含操作状态和输出数据。

- 执行`风格控制(SSML 子集)`操作,处理输入数据并返回结果
- 验证执行结果,确认输出符合预期格式
- 异常时参考错误处理章节进行恢复
...
**处理**: 按照skill规范执行双格式输出操作,遵循单一意图原则。

- 执行`双格式输出`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`双格式输出`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 神经语音合成、支持多音色切换、批量分段、长文本合并与风格、零云端零密钥、神经网络引擎的本、地语音合成专业版、全部推理在本地完、零云端调用、API、零订阅费用、核心能力、长文本自动分段与、跨平台部署、macOS、Apple、Silicon、Intel、Linux、Windows、WSL、适用于语音消息投、有声内容生产、无障碍朗读、播客片段生成、多语言内容本地化、等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
