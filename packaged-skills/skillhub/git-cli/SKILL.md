...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Merge / rebase | 支持 | 通过核心功能实现对应能力 |
| Task | 支持 | 通过核心功能实现对应能力 |
| Safety first | 支持 | 通过核心功能实现对应能力 |
| Discard (destructive) | 支持 | 通过核心功能实现对应能力 |
| History | 支持 | 通过核心功能实现对应能力 |
| Amend | 支持 | 通过核心功能实现对应能力 |
| Stash | 支持 | 通过核心功能实现对应能力 |
| State & diff | 支持 | 通过核心功能实现对应能力 |
| Clone / init | 支持 | 通过核心功能实现对应能力 |
| Remotes | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `give`, `scripts/is-repo.sh`, `automation`, `errors`, `prefer`, `head`, `confirm`, `exit`, `need`, `short`, `look`, `ensure`, `scripts`, `templates`, `location`
