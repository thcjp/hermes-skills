...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 🚀 Enterprise Tier - $29/month | 支持 | 通过核心功能实现对应能力 |
| Safari | 支持 | 通过核心功能实现对应能力 |
| Privacy | 支持 | 通过核心功能实现对应能力 |
| 🆓 Free Tier | 支持 | 通过核心功能实现对应能力 |
| Accepted Payment Methods | 支持 | 通过核心功能实现对应能力 |
| The Problem | 支持 | 通过核心功能实现对应能力 |
| The Solution | 支持 | 通过核心功能实现对应能力 |
| How Payments Work | 支持 | 通过核心功能实现对应能力 |
| ⭐ Pro Tier - $9/month | 支持 | 通过核心功能实现对应能力 |
| Support | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `refund`, `不适用于纯技术文`, `billing`, `tiers`, `background`, `technical`, `happen`, `getting`, `uninstalling`, `ideas`, `brave`, `guide`, `upgrade`, `daily`, `preferences`
...
- `-embedding-3-small`: 命令参数,用于指定操作选项
- `-bindgen`: 命令参数,用于指定操作选项
