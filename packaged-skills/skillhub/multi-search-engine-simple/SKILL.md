...
`multi-search-engine-simple`按照以下流程处理用户请求:
...
本skill还覆盖以下能力场景: 国内精简版、多种免费搜索方式、在互联网上搜索最、新信息、个国内不能访问的、海外网站、研究工具领域的专、辅助工具、基于高人气开源、深度优化升级、移除风险代码、增强安全性和稳定、适用场景、数据研究、文献分析、信息收集、独立开发者与一人、公司效率提升。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Jisilu: https://www.jisilu.cn/explore/?keyword={keyword} | 支持 | 通过核心功能实现对应能力 |
| WeChat: https://wx.sogou.com/weixin?type=2&query={keyword} | 支持 | 通过核心功能实现对应能力 |
| Baidu: https://www.baidu.com/s?wd={keyword} | 支持 | 通过核心功能实现对应能力 |
| 360: https://www.so.com/s?q={keyword} | 支持 | 通过核心功能实现对应能力 |
| Sogou: https://sogou.com/web?query={keyword} | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `bing`, `wolframalpha`, `ecosia`, `个免费搜索引擎`, `完全适配`, `增强元数据和触发`, `engines`, `平台规范`, `toutiao`
