...
**输出**: 返回3 类基础角色账号映射的处理结果。

- 执行`类基础角色账号映射`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
...
- 执行`两类汇报时机`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`两类汇报时机`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 基础通信规范、类角色消息发送、通信规范免费版、三类基础、角色的、标识与消息发送格、向用户发送、消息时使用、工具配合、确保消息正确路由、到用户账号、覆盖任务开始与任、务完成两类基础汇、适用于单、消息发送、基础任务进度通知、等场景。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
