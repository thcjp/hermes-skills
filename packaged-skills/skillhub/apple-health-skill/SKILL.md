...
**处理**: 按照skill规范执行性能管理图表（PMC）操作,遵循单一意图原则。- 验证执行结果，确认输出符合预期格式
...
**处理**: 按照skill规范执行性能统计操作,遵循单一意图原则。- 验证执行结果，确认输出符合预期格式
- 参考`性能统计`相关配置参数进行设置
### 6. 运动员档案

通过 `GET /api/v1/profile` 获取运动员档案信息。返回用户的基本信息、运动偏好、历史训练摘要等数据。适用于用户画像构建和个性化建议基础数据获取。- 验证执行结果，确认输出符合预期格式
- 参考`运动员档案`相关配置参数进行设置
### 7. 聊天历史

通过 `GET /api/v1/coach/history` 获取AI教练的聊天历史记录。返回之前的对话内容，包含用户问题和AI教练回复。适用于对话上下文回顾和连续性对话场景。- 验证执行结果，确认输出符合预期格式
- 参考`聊天历史`相关配置参数进行设置
#
## 使用流程
