...
**处理**: 按照skill规范执行AI教练对话操作,遵循单一意图原则。
**输出**: 返回AI教练对话的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行性能管理图表（PMC）操作,遵循单一意图原则。

- 执行`性能管理图表（PMC）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`性能管理图表（PMC）`相关配置参数进行设置
### 5. 性能统计
通过 `GET /api/v1/performance/stats` 获取从健康数据推导的性能指标。返回FTP（功能阈值功率）、阈值配速、心率区间分布和其他运动表现指标。适用于运动能力评估和训练强度设定。

**输入**: 用户提供性能统计所需的指令和必要参数。
**处理**: 按照skill规范执行性能统计操作,遵循单一意图原则。

- 执行`性能统计`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`性能统计`相关配置参数进行设置
### 6. 运动员档案

通过 `GET /api/v1/profile` 获取运动员档案信息。返回用户的基本信息、运动偏好、历史训练摘要等数据。适用于用户画像构建和个性化建议基础数据获取。

- 执行`运动员档案`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`运动员档案`相关配置参数进行设置
### 7. 聊天历史

通过 `GET /api/v1/coach/history` 获取AI教练的聊天历史记录。返回之前的对话内容，包含用户问题和AI教练回复。适用于对话上下文回顾和连续性对话场景。

- 执行`聊天历史`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`聊天历史`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 与运动健康数据对、查询训练、活动量和、支持查询训练记录、心率趋势、活动量环、通过健康数据同步、服务获取数据、教练提供、个性化训练建议、适用于运动训练分、恢复评估和健身数、据管理场景。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
