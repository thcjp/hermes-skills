...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Hyperlinks, bookmarks, and references can break if IDs or relationships stop matching. | 支持 | 通过核心功能实现对应能力 |
| Deleted text may still exist in the XML even when it no longer appears on screen. | 支持 | 通过核心功能实现对应能力 |
| Verify round-trip compatibility before delivery | 支持 | 通过核心功能实现对应能力 |
| Page layout lives in sections | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `concise`, `unicode`, `indentation`, `docs`, `track`, `libreoffice`, `restarting`, `abstractnum`, `google`, `word/document.xml`, `prefer`, `copying`, `comments`, `empty`, `visible`
...
### Q1: 如何开始使用Word Docx 1.02？
A: 

### Q2: 遇到错误怎么办？
A: 

### Q3: Word Docx 1.02有什么限制？
A: 
