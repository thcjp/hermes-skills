...
- 执行`基础语言校准`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`基础语言校准`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 基础版反思式塔罗、支持单牌阵和基础、用于情感反思、tarot、free、是反思式塔罗抽取、技能的基础版本、用于情感支持和自、我觉察、核心理念是、塔罗是镜子、而非预言、presence、first、non、clinical、和基础语言校准、Three、三牌阵、完整语言校准规则、和边界守护安全模、适合快速情感反思、升级完整版获取三、牌阵和安全优先支。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
