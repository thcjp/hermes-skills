...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Redirect | 支持 | 通过核心功能实现对应能力 |
| Worker route (fallback; uses redirect-worker). | 支持 | 通过核心功能实现对应能力 |
| Decide routing model | 支持 | 通过核心功能实现对应能力 |
| Rulesets / Bulk Redirects (account-level; needs token perms). | 支持 | 通过核心功能实现对应能力 |
| HTTPS redirect: curl -I https://example.com (expect 301). | 支持 | 通过核心功能实现对应能力 |
| Verify | 支持 | 通过核心功能实现对应能力 |
| Nameservers | 支持 | 通过核心功能实现对应能力 |

**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `commits`, `page`, `commit`, `push`, `steipete.md`, `proxied`, `prefer`, `guardrails`, `confirm`, `cli4`, `golden`, `conventional`, `cloudflare_api_token`, `cf_api_token`, `block`
