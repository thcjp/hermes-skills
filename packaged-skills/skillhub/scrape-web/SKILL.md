...
本skill还覆盖以下能力场景: Python、Scrapling、获取网页内容、支持简单选择器。这些能力在上述核心功能中均有对应处理逻辑。
### 源能力映射
本skill覆盖源skill的以下能力点:

| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| 保存到文件 | 支持 | 通过核心功能实现对应能力 |
| 直接抓取纯文本 | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
### 领域术语

本skill涉及以下领域术语: `返回纯文本或选择`, `增强元数据和触发`, `完全适配`, `平台规范`
