...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| Slack爬虫工具Pro含API同步 | 不支持 | 支持 |
| Slack爬虫工具Pro定时调度 | 不支持 | 支持 |
| Slack爬虫工具Pro增量更新与数据导出 | 不支持 | 支持 |
| 多标签页并行抓取 | 不支持 | 支持 |
| 反爬虫策略自动绕过 | 不支持 | 支持 |
...
针对桌面端,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供桌面端同步相关的配置参数、输入数据和处理选项.
**输出**: 返回桌面端同步的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`桌面端同步`的配置文档进行参数调优
### API同步

针对API,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供API同步相关的配置参数、输入数据和处理选项.
**输出**: 返回API同步的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`API同步`的配置文档进行参数调优
### 线程回复完整化

针对线程回复完整化,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供线程回复完整化相关的配置参数、输入数据和处理选项.
**输出**: 返回线程回复完整化的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`线程回复完整化`的配置文档进行参数调优
#
## 适用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| content | string | 否 | slack-crawler处理的内容输入 |, 默认: 全部维度 |
