...
- 执行`main（主会话）`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`main（主会话）`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: Gateway、定时任务设置完全、指南专业版、话题投递、高级配置项、定时任务设置指南、专业版是在免费版、基础上的全功能升、提供完整的定时调、度配置能力、专业版解锁、思考强度配置、Telegram、高级配置项等高级、实现企业级定时任、务管理、Use、模型调用、智能对话、LLM、应用时使用、不适用于需要、确定性的关键决策。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
