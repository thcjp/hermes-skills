...
**处理**: 解析三种调度类型的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回三种调度类型的处理结果,包含执行状态码、结果数据和执行日志。### 双会话模式
...
**处理**: 解析双会话模式的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。### 模型覆盖与思考强度（专业版）
| 参数 | 可选值 | 适用场景 |
|---:|---:|---:|
...
**处理**: 解析退避策略调优（专业版）的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回退避策略调优（专业版）的处理结果,包含执行状态码、结果数据和执行日志。### 作业管理命令
**输入**: 用户提供作业管理命令所需的指令和必要参数.
**处理**: 解析作业管理命令的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回作业管理命令的处理结果,包含执行状态码、结果数据和执行日志。### Cron vs Heartbeat 决策矩阵
| 场景 | 推荐 | 理由 |
|:------|------:|:------|
...
针对main（主会话）,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供main（主会话）相关的配置参数、输入数据和处理选项.
**输出**: 返回main（主会话）的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`main（主会话）`的配置文档进行参数调优
#
## 快速开始
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|:---|---:|---:|
| content | string | 否 | cron-setup-guide处理的内容输入 |, 默认: 全部维度 |
