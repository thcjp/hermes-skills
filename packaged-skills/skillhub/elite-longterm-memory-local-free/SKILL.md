...
**输出**: 返回精选归档（MEMORY.md + daily/）的处理结果。
...
本skill还覆盖以下能力场景: 基础本地记忆系统、热内存加文件归档、纯本地运行无外部、的基础本地记忆系、提供热内存与文件、归档两层记忆能力、协议确保先写状态、提供人类可读的长、期记忆与每日日志、所有数据完全存储、在本地文件系统、Ollama、或任何外部、API、隐私至上、适用于独立开发者、与轻量使用场景、不遗忘关键上下文、与用户偏好、本免费版覆盖单设、备本地记忆持久化、不含向量语义搜索。这些能力在上述核心功能中均有对应处理逻辑。
## 使用流程
