...
summary_zh: "全功能数据库管理平台,支持大规模批量操作、分区表、性能基线、定时备份与多实例管理。。数据库管理台专业版面向专业 DBA、后端架构师与运维团队,提供完整的数据库设计、批量操作、性能优化与运维自"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| 数据库管理台(专业版)全功能数据库管理 | 不支持 | 支持 |
| 数据库管理台(专业版)定时备份与多实例管理 | 不支持 | 支持 |
| 大数据集流式处理 | 不支持 | 支持 |
| 多数据源关联查询 | 不支持 | 支持 |
| 可视化图表自动生成 | 不支持 | 支持 |
...
**处理**: 解析1 表结构设计(完整版)的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回1 表结构设计(完整版)的处理结果,包含执行状态码、结果数据和执行日志.
### 2.2 大规模批量操作
...
**输出**: 返回2 大规模批量的处理结果,包含执行状态码、结果数据和执行日志.
### 2.3 分区表与表空间
```sql
-- 按时间范围分区
CREATE TABLE events (
...
**处理**: 解析3 分区表与表空间的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
### 2.4 存储过程与触发器
...
**输出**: 返回4 存储过程与触发器的处理结果,包含执行状态码、结果数据和执行日志.
### 2.5 查询性能基线
...
**输出**: 返回5 查询性能基线的处理结果,包含执行状态码、结果数据和执行日志.
### 2.6 定时备份与恢复
| 备份类型 | 命令 | 频率 | 恢复粒度 |
|---:|---:|---:|---:|
...
**处理**: 解析6 定时备份与恢复的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回6 定时备份与恢复的处理结果,包含执行状态码、结果数据和执行日志.
### 2.7 多实例管理
- **读写分离**:写主库,读从库,自动路由
...
**处理**: 解析7 多实例管理的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回7 多实例管理的处理结果,包含执行状态码、结果数据和执行日志.
### 2.8 Schema 迁移版本管理
- **迁移脚本**:每个变更一个版本号文件
- **版本追踪**:数据库表记录当前版本
- **回滚支持**:每个迁移配套回滚脚本
...
**输出**: 返回8 Schema 迁移版本管理的处理结果,包含执行状态码、结果数据和执行日志.
### 2.9 跨数据库同步
...
**输出**: 返回9 跨数据库同步的处理结果,包含执行状态码、结果数据和执行日志.
### 2.10 安全与审计
- **权限管理**:角色级权限,最小权限原则
- **数据脱敏**:敏感字段查询时自动脱敏
- **操作审计**:DDL/DML 操作完整记录
...
| 参数名 | 类型 | 必填 | 说明 |
|:------|------:|:------|:------|
| content | string | 否 | db-admin-console处理的内容输入 |, 默认: 全部维度 |
