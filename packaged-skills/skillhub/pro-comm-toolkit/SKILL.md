...
summary_zh: "企业级职场沟通助手，支持批量邮件生成、多语言翻译、自定义模板、跨文化沟通与团队协作场景。。职场沟通工具箱（专业版）—— 面向团队和企业的全功能写作辅助工具。核心能力: - 批量邮件生成与多收"
license: "MIT"
edition: "pro"
description: |-
...
**处理**: 解析批量邮件生成与个性化的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。- 验证返回数据的完整性和格式正确性
- 参考`批量邮件生成与个性化`的配置文档进行参数调优
### 2. 多语言翻译与跨文化沟通
内置中、英、日、韩、法、德等多种语言的沟通模板，并提供跨文化沟通的注意事项指导.
**处理**: 解析多语言翻译与跨文化沟通的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回多语言翻译与跨文化沟通的处理结果,包含执行状态码、结果数据和执行日志。- 验证返回数据的完整性和格式正确性
- 参考`多语言翻译与跨文化沟通`的配置文档进行参数调优
### 3. 自定义模板库
支持创建、编辑、分类管理自定义模板，并支持团队内共享与版本控制.
**输入**: 用户提供自定义模板库所需的指令和必要参数.
**处理**: 解析自定义模板库的输入参数,执行核心处理逻辑,返回结构化结果和执行状态。- 验证返回数据的完整性和格式正确性
- 参考`自定义模板库`的配置文档进行参数调优
### 4. 危机公关与高敏感场景
针对突发事件、舆情危机、客户投诉等高敏感场景，提供专门的沟通框架与措辞指导.
**处理**: 解析危机公关与高敏感场景的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回危机公关与高敏感场景的处理结果,包含执行状态码、结果数据和执行日志。- 验证返回数据的完整性和格式正确性
- 参考`危机公关与高敏感场景`的配置文档进行参数调优
### 5. 会议全流程管理
从议程制定、会中记录到会后跟进，提供完整的会议管理工具链.
**处理**: 解析会议全流程管理的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
**输出**: 返回会议全流程管理的处理结果,包含执行状态码、结果数据和执行日志。- 验证返回数据的完整性和格式正确性
- 参考`会议全流程管理`的配置文档进行参数调优
### 6. 沟通效果分析
基于消息结构、语气、长度等维度，提供写作质量评分与优化建议.
**处理**: 解析沟通效果分析的输入参数,执行核心处理逻辑,返回结构化结果和执行状态.
...
本工具核心功能基于 Markdown 指令驱动，基础LLM由Agent平台提供 即可使用。批量发送与多语言翻译功能需要对应服务的 API Key.
### 可用性分类
