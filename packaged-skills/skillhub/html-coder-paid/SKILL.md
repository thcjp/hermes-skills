...
**输出**: 返回能力对比的执行结果,包含操作状态和输出数据。### 核心能力
```text
HTML5 高级API:
  - Canvas: 2D绘图、图表、游戏渲染
  - SVG: 矢量图形、动画、交互
...
执行语义化HTML操作,处理用户输入并返回结果。

**输入**: 用户提供语义化HTML所需的参数和指令。

**输出**: 返回语义化HTML的处理结果。

- 执行`语义化HTML`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`语义化HTML`相关配置参数进行设置
### 能力覆盖范围

本skill还覆盖以下能力场景: 开发引擎、全面合规与性能优、编码工具专业版、面向团队的企业级、开发平台、组件化开发。这些能力在上述核心功能中均有对应处理逻辑。
## 适用场景
...
- `-title`: 命令参数,用于指定操作选项
- `-labelledby`: 命令参数,用于指定操作选项
- `-scale`: 命令参数,用于指定操作选项
- `-CN`: 命令参数,用于指定操作选项
- `--output-path`: 命令参数,用于指定操作选项
