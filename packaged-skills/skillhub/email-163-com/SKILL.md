...
执行IMAP ID认证,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供IMAP ID认证相关的配置参数、输入数据和处理选项。

**输出**: 返回IMAP ID认证的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`IMAP ID认证`相关配置参数进行设置
### 发送邮件

执行发送邮件,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供发送邮件相关的配置参数、输入数据和处理选项。

**输出**: 返回发送邮件的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`发送邮件`相关配置参数进行设置
### 读取邮件

执行读取邮件,自动处理参数解析、任务调度和结果格式化,返回结构化输出。

**输入**: 用户提供读取邮件相关的配置参数、输入数据和处理选项。

**输出**: 返回读取邮件的处理结果。- 验证执行结果，确认输出符合预期格式
- 参考`读取邮件`相关配置参数进行设置
#
## 使用流程
