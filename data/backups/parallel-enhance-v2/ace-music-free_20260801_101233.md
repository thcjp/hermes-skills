...
### Q1: 为什么生成的音乐时长总是比预期短？
A: 请检查您是否正确设置了 `--duration` 参数，并确保其值在支持范围内（5-300秒）。如果参数设置正确，请检查网络连接是否稳定，或者尝试稍后再次生成。

### Q2: 为什么生成的音乐中没有歌词？
A: 请确认您是否正确使用了 `--lyrics` 参数，并确保歌词格式正确。歌词应使用 `[Verse]`、`[Chorus]` 等标签，并且每行歌词之间用换行符分隔。

### Q3: 为什么我无法生成纯音乐？
A: 请确保您使用了 `--instrumental` 参数来指定无人声音乐。如果问题依旧，请检查您的 API Key 是否有效，并确保您有权访问该功能。

### Q4: 为什么我收到 'rate_limited' 错误？
A: 这通常意味着您在短时间内发送了过多的请求。请等待一段时间后重试，或者检查您的请求频率是否符合 ACE Music 的使用限制。

### Q5: 为什么生成的音乐听起来很奇怪？
A: 请确保您的 prompt 描述清晰且具体，包括风格、情绪、乐器等信息。描述越详细，生成的音乐越符合预期。
...
### 边界条件
- 空输入：如果用户提供的输入为空或格式不正确，系统应返回错误信息，并提示用户重新输入。
- 超大数据：如果用户请求的音频时长超过300秒，系统应返回错误信息，并提示用户调整时长。
- 并发：如果系统同时处理多个请求，应确保每个请求都能得到正确处理，并且系统资源得到合理分配。

### 异常处理策略
- 空输入：抛出异常，并返回错误信息。
- 超大数据：抛出异常，并返回错误信息，提示用户调整时长。
- 并发：使用队列或锁等机制来管理并发请求，确保系统稳定运行。

### 输入参数的有效范围和限制
- `--duration`：5-300秒
- `--lyrics`：字符串，使用 `[Verse]`、`[Chorus]` 等标签分隔歌词
- `--instrumental`：布尔值，指定是否生成无人声音乐
...
- 时间：使用ACE Music Free生成音乐的时间平均减少至手动操作时间的1/6至1/3。
- 成本：使用ACE Music Free无需支付额外的人力成本，且节省了创作时间。
- 准确率：ACE Music Free生成的音乐准确率较高，能够满足90%以上的用户需求。

### 与同类工具的差异化优势对比

| 特点 | ACE Music Free | 同类工具 |
|:---:|:---:|:---:|
| 生成速度 | 快速生成，平均20分钟完成一首歌 | 生成速度较慢，可能需要数小时 |
| 定制化程度 | 支持歌词定制和纯音乐模式 | 定制化程度较低，功能相对固定 |
| 易用性 | 界面友好，操作简单 | 界面可能复杂，操作难度较高 |
| 价格 | 免费版提供基本功能 | 多数同类工具需要付费订阅 |
| 风格多样性 | 提供多种音乐风格选择 | 风格选择可能有限 |
