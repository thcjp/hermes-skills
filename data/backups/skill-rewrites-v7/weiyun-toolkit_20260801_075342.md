...
### 输入限制
- `weiyun.list` 和 `weiyun.list_by_category` 的 `limit` 参数限制为 ≤50，超过此限制可能导致查询结果不完整。
- `weiyun.download` 的 `items` 参数中的文件ID和目录ID必须正确，错误或不存在的ID将导致下载失败。
- `weiyun.rename_file` 和 `weiyun.rename_dir` 的 `new_filename` 和 `new_dir_name` 不能与现有文件或目录重名，否则将无法重命名。

### 性能边界
- 微云工具箱（专业版）的批量操作（如批量下载、批量删除）可能会受到微云服务器性能的限制，导致操作响应时间变长。
- 大量文件或目录的移动操作可能会因为网络延迟或服务器处理能力不足而失败。

### 兼容性约束
- 微云工具箱（专业版）目前仅支持微云网盘，不支持其他云存储服务。
- 部分功能可能需要微云网盘的高级版本才能使用，如版本管理、复杂工作流可视化编排等。
- 微云工具箱（专业版）不支持实时流数据处理，适用于离线数据分析、报表生成等场景。
