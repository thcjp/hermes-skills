...
### 输入限制
- **文章内容长度**：由于性能和资源限制，单个文章内容不宜过长，建议不超过50,000字符。
- **标签和分类数量**：单个文章的标签和分类数量有限制，通常不超过5个，以避免配置文件过于复杂。
- **文件名slug**：文章的文件名slug应遵循一定的命名规范，避免使用特殊字符和空格，且不得与现有文件冲突。

### 性能边界
- **处理速度**：对于大量文章的批量处理，可能需要较长时间完成，具体取决于文章数量和系统资源。
- **并发处理**：系统可能不支持同时处理多个文章的发布，以避免资源竞争和性能下降。

### 兼容性约束
- **操作系统**：目前仅支持Windows、macOS和Linux操作系统。
- **Git版本**：需要Git版本至少为1.8.5，以确保命令行工具的正常使用。
- **Markdown格式**：文章内容需遵循标准的Markdown格式，不支持复杂的Markdown扩展或自定义语法。

### 其他限制
- **API Key**：使用LLM API时，需要配置有效的API Key，且API Key的有效期和配额限制可能影响功能使用。
- **博客目录访问权限**：需要本地运行环境对博客目录有读写权限，以确保文件创建和修改。
- **SSH key或credentials**：若使用Git推送，需要配置SSH key或git credentials，以确保认证和访问权限。
