...
**输入参数**：
- `content`：字符串类型，GitHub API工具包处理的内容输入。
- `strict_level`：字符串类型，审查严格度，可选值：strict/normal/loose，默认：normal。

**输出参数**：
- `success`：布尔类型，表示操作是否成功。
- `data`：对象类型，包含操作结果和详细信息。
- `error`：字符串类型，包含错误信息。
...
该技能的差异化优势包括：
- 支持多种GitHub API，包括GraphQL、批量操作、Webhook管理、GitHub Actions API和组织与团队管理等。
- 提供了丰富的使用场景，如企业级集成、DevOps平台集成、组织权限治理和数据分析与BI。
- 支持断点续传、速率控制、回滚支持等高级功能，提高了操作的可靠性和效率。
...
该技能解决了以下特定痛点：
- 提高了GitHub API集成的效率，特别是对于批量操作和Webhook管理。
- 简化了组织权限治理和数据同步等复杂操作。
- 提供了丰富的数据分析和BI功能，帮助用户更好地理解和使用GitHub数据。
