...
为了确保消息内容的安全性，`feishu-card-free` 提供了安全发送机制。该机制通过 `send_safe.js` 包装器实现，自动处理文本中的特殊字符，防止Shell注入攻击。具体实现方式如下：

- `send_safe.js` 包装器接收原始文本内容，通过正则表达式识别并转义特殊字符。
- 转义后的文本被安全地传递给Feishu API，确保消息内容在发送过程中不会导致安全漏洞。
- 这种机制适用于自动化流程中，尤其是涉及敏感信息或用户输入的场景。

## 附加配置选项说明

除了基本的文本、标题和颜色设置外，`feishu-card-free` 还支持以下附加配置选项：

- **模式选择**：通过 `options.mode` 参数，可以选择不同的消息发送模式，如私聊、群聊等。
- **格式偏好**：通过 `options.format` 参数，可以指定消息的格式偏好，如纯文本、富文本等。

这些选项为用户提供更多定制化功能，满足不同场景下的需求。

## 实际应用案例

以下是一些实际应用案例，展示了 `feishu-card-free` 在不同场景下的使用：

- **项目管理**：用于发送任务完成通知、进度更新等。
- **运维监控**：用于发送系统告警、故障通知等。
- **客户服务**：用于发送服务通知、回复用户咨询等。

这些案例展示了 `feishu-card-free` 在实际工作中的实用性和价值。
