...
license: "MIT"
description: |-
  统一搜索 SkillHub skills 和 Smithery 协议 servers 的工具发现引擎。支持评分排序、推荐规则、来源标识。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。
tags:
  - Research
  - 工具
  - 效率
...
### 输入限制
- **关键词长度**：输入的关键词长度不应超过255个字符，以避免搜索性能问题。
- **特殊字符**：输入中不应包含特殊字符，如`<`, `>`, `|`, `&`等，这些字符可能会影响搜索结果的准确性。
- **敏感词**：输入中不应包含敏感词，以避免违反平台内容政策。

### 性能边界
- **搜索结果数量**：对于每个查询，工具发现引擎将返回最多100个搜索结果。
- **并发请求**：同一时间内的并发请求不应超过5个，以避免对服务器造成过大压力。

### 兼容性约束
- **操作系统**：工具发现引擎在Windows、macOS和Linux操作系统上均能运行。
- **浏览器兼容性**：对于Web界面，推荐使用现代浏览器如Chrome、Firefox或Safari。
- **SkillHub和Smithery协议版本**：工具发现引擎支持SkillHub和Smithery协议的最新版本。

### 数据源限制
- **SkillHub技能更新**：由于SkillHub技能库的更新频率，某些技能可能不在最新的搜索结果中显示。
- **Smithery协议数据**：Smithery协议的数据更新可能存在延迟，因此某些最新的协议可能未被包含在内。

### 自动触发限制
- **触发关键词**：自动触发功能仅对特定的关键词有效，如"找个...skill"、"有什么 协议"等。
- **技能调用**：自动触发功能不支持调用特定技能的子功能或参数。

### 安装限制
- **客户端依赖**：Smithery协议的安装需要指定客户端，如claude-code、cursor或vscode等。
- **安装权限**：安装技能或协议可能需要管理员权限。
