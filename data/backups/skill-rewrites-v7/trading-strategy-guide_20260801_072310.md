...
summary_zh: "提供交易策略分析与决策支持的AI技能。Provide AI-powered trading strategy analysis and decision support. Analyze m"
license: "MIT"
description: |-
...
### 输入数据限制
- **数据格式**: 输入数据必须为有效的OHLCV格式，包括开盘价、最高价、最低价、收盘价和成交量。
- **数据频率**: 支持日线、周线、月线等不同频率的数据，但需确保数据频率一致。
- **数据量**: 输入数据量应足够用于策略分析和回测，建议至少包含一年以上的历史数据。

### 性能边界
- **处理速度**: 对于大量数据，技能可能需要较长时间进行计算和分析。
- **并发处理**: 单个技能实例可能无法同时处理大量并发请求，建议合理分配请求。

### 兼容性约束
- **操作系统**: 支持Windows、macOS和Linux操作系统。
- **Agent平台**: 支持任何支持SKILL.md的AI Agent平台。
- **LLM API**: 必须由Agent内置LLM提供，不支持外部LLM API。

### 参数限制
- **策略类型**: 支持趋势跟踪、均值回归、动量和突破四种策略类型。
- **参数范围**: 策略参数（如均线周期、止损比例等）应在合理范围内调整，避免极端值。

### 输出限制
- **结果格式**: 输出结果为JSON格式，包含策略分析结果、执行时间和元数据。
- **结果精度**: 结果精度受限于输入数据的精度和计算方法。

### 依赖项限制
- **LLM API**: 必须使用Agent内置的LLM API，不支持自定义LLM服务。

### 实时数据限制
- **不支持实时交易**: 本技能仅提供分析和建议，不连接实盘交易系统。
- **实时数据接入**: 若需要接入实时数据，需使用支持实时数据接入的Agent平台或API。
...
- **操作系统**: 支持Windows、macOS和Linux操作系统。
- **Agent平台**: 支持任何支持SKILL.md的AI Agent平台。
- **LLM API**: 必须由Agent内置LLM提供，不支持自定义LLM服务。
