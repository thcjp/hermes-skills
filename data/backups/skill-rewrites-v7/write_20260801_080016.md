...
### 输入限制
- **输入数据长度**: 输入数据长度不应超过1000个字符，以避免处理时间和资源消耗过大。
- **文件路径长度**: 文件路径长度不应超过255个字符，以符合大多数操作系统的限制。
- **文件名长度**: 单个文件名长度不应超过255个字符，以避免与文件系统兼容性问题。

### 性能边界
- **并发处理**: 同时处理多个写作任务时，系统性能可能会受到影响，建议单个Agent同时处理的任务不超过5个。
- **处理时间**: 对于复杂的写作任务，处理时间可能会超过30秒，建议用户在等待结果时不要重复提交相同任务。

### 兼容性约束
- **操作系统**: 支持Windows（需Git Bash/WSL）、macOS和Linux（需Bash/Shell）操作系统。
- **Agent平台**: 支持SKILL.md的任意AI Agent，如Claude Code、Cursor、Codex、Gemini CLI等。
- **Sub-agent支持**: Agent需支持委托写作任务给sub-agents，否则无法完整使用所有功能。

### 资源限制
- **内存使用**: 系统内存使用不应超过2GB，以避免资源冲突和性能下降。
- **磁盘空间**: 至少需要1GB的磁盘空间用于存储写作数据和版本备份。

### 其他限制
- **版本控制**: 由于版本控制机制，直接编辑文件会被拦截，所有编辑操作必须通过`edit.sh`脚本进行。
- **清理操作**: 清理操作（`cleanup.sh`）会永久删除旧版本，操作前需确保没有误删的版本。
- **深度配置**: `depth`配置为`quick`、`standard`或`thorough`，不同配置会影响处理时间和质量审计的严格程度。
