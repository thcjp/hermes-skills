...
为了提升用户体验，建议在界面设计中增加以下功能：

- **交互式搜索提示**：在搜索框中提供即时的搜索建议，帮助用户快速定位目标事务。
- **结果分页**：对于搜索结果较多的场景，提供分页功能，避免一次性加载过多数据。
- **结果筛选**：允许用户根据状态、优先级、创建日期等条件筛选事务。
- **用户反馈机制**：提供一个反馈按钮，让用户可以报告问题或建议改进。

## 功能描述细化

- **事务搜索**：在描述中明确说明搜索结果将包含事务的key和summary，并提供示例。
- **事务详情**：详细说明事务详情包括状态、指派人、优先级、创建日期等信息。
- **可用流转列表**：明确列出当前事务可用的流转选项，例如从“待办”到“进行中”。
- **自己的未关闭事务**：详细说明如何列出用户自己的未关闭事务，包括事务key、标题和状态。

## 差异化优势突出

为了凸显与同类方案的差异化优势，建议在以下方面进行强调：

- **专注于只读浏览**：明确指出该技能专注于只读浏览，避免与其他支持写操作的Jira集成工具混淆。
- **快速定位事务**：强调通过模糊搜索和快速命令，用户可以迅速找到所需事务。
- **保持代码质量**：虽然主要功能是只读浏览，但通过结构化的工作流程和模板，间接帮助用户保持代码质量。

## 输入输出格式示例

为了帮助用户更好地理解输入输出格式，建议提供以下示例：

**输入示例**：

```json
{
  "input": "search payment failure",
  "options": {
    "maxResults": 10
  }
}
...
```json
{
  ...
  "data": {
    "result": [
      {
        "key": "ABC-123",
        "summary": "Payment failure for order XYZ",
        "status": "Open",
        "priority": "High",
        "created": "2023-01-01"
      },
...
通过这些示例，用户可以清晰地了解如何使用技能以及预期的输出格式。
