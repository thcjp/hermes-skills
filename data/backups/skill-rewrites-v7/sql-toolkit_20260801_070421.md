...
version: 1.0.1
displayName: "SQL工具箱"
summary: "关系型数据库操作工具箱，覆盖SQLite/数据库/MySQL的查询、设计、迁移与优化。SQL Toolkit 是关系型数据库命令行操作的完整工具箱，覆盖 SQLite、Post"
summary_zh: "关系型数据库操作工具箱，覆盖SQLite/数据库/MySQL的查询、设计、迁移与优化。SQL Toolkit 是关系型数据库命令行操作的完整工具箱，覆盖 SQLite、Post"
license: "MIT"
description: |-
  SQL Toolkit 是关系型数据库命令行操作的完整工具箱，覆盖 SQLite、数据库、MySQL 三大数据库.
...
关系型数据库命令行操作工具箱，覆盖 SQLite、数据库、MySQL，提供 Schema 设计、查询编写、迁移脚本、索引优化、备份恢复的完整模式.
## 输入格式
...
数据库 额外支持：UUID 主键（`uuid_generate_v4()`）、`TIMESTAMPTZ` 时间戳、ENUM 类型、`JSONB` 字段、触发器自动更新 `updated_at`、部分索引（Partial Index）、GIN 索引.
...
使用 `EXPLAIN (ANALYZE, BUFFERS)` 查看 数据库 执行计划，关注：
- `Seq Scan` 全表扫描 → 需要加索引
...
### Q1: SQLite 和 数据库 的 SQL 语法有什么主要区别？
A: SQLite 使用 `AUTOINCREMENT`、`TEXT` 类型、`datetime('now')` 函数；数据库 使用 `SERIAL`/`UUID`、`TIMESTAMPTZ`、`NOW()` 函数，还支持 `JSONB`、`ENUM`、部分索引等高级特性.
### Q2: 如何选择索引类型？
