...
summary_zh: "企业级CSV转JSON工具，支持批量流式转换、Schema校验、字段映射与数据库直写。CSV转JSON专业版是一款面向数据团队与企业级ETL场景的全功能表格数据格式转换工具。在免费版的表头"
license: "MIT"
edition: "pro"
description: |-
...
**支持的数据库**：`数据库`、MySQL、SQLite、ClickHouse。每种数据库提供独立的连接模板与批量写入策略.
...
**痛点**：业务系统A每天导出CSV，要同步到业务系统B的 `数据库`，全量重跑慢且浪费.
**使用方式**：对Agent说"基于id字段做增量同步，转换后直写 `数据库` 的 orders 表"，Agent输出增量同步脚本+数据库直写脚本，附状态文件管理与失败重试逻辑.
...
| psycopg2 | Python库 | 可选 | `pip install psycopg2-binary`（`数据库`直写） |
...
- **CSV编码探测准确率有限**：虽然集成 `chardet` 支持UTF-8/UTF-8-BOM/GBK/GB2312等常见编码探测，但对混合编码文件（同一文件中存在多种编码的行）的探测准确率仅约85%，可能导致部分中文字段乱码；UTF-16/UTF-32编码文件无法自动识别，需显式指定 `encoding='utf-16'` 参数；含BOM的UTF-8文件若未使用 `utf-8-sig` 读取，BOM字符（`\ufeff`）会被当作表头领先个字段名的一部分，导致字段映射失败。
