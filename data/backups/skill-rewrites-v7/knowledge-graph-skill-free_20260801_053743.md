...
### 输入限制
- **实体标识长度**：实体标识（`entity_id`）的长度应不超过255个字符。
- **属性键值对数量**：实体或关系的属性键值对数量不应超过100对。
- **关系类型长度**：关系类型（`relation_type`）的长度应不超过50个字符。
- **查询深度限制**：邻居查询的深度（`query_depth`）不应超过2度。

### 性能边界
- **实体数量限制**：基础版技能支持的最大实体数量为10,000个。
- **关系数量限制**：基础版技能支持的最大关系数量为100,000条。
- **查询性能**：对于基础查询，如邻居查询，响应时间应小于500毫秒。

### 兼容性约束
- **操作系统兼容性**：技能在Windows、macOS和Linux操作系统上运行。
- **Agent平台兼容性**：技能需在支持SKILL.md规范的AI Agent平台上运行。
- **LLM API兼容性**：技能依赖于Agent内置的LLM API，需确保LLM服务正常。

### 数据格式限制
- **持久化格式**：支持JSON和SQLite格式的持久化，不支持其他格式的图数据库。
- **文件路径限制**：导出和导入操作的文件路径长度不应超过255个字符。

### 功能限制
- **图谱推理**：不支持图谱推理功能，如最短路径、社区发现、中心性分析等。
- **跨图谱关联**：不支持跨图谱关联和多图谱合并功能。
- **向量检索**：不支持向量检索和语义相似度查询。
- **大规模图数据库**：不支持大规模图数据库的性能优化和接入。

### 其他限制
- **实时更新**：不支持实时流式图谱更新和增量索引。
- **图算法库**：不支持图算法库，如PageRank、Betweenness Centrality等。
