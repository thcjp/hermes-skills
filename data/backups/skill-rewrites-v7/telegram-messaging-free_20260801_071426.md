...
### 输入限制
- **消息长度**：发送的文本消息长度不能超过 Telegram 的限制，通常为 4096 个字符。
- **图片大小**：发送的图片大小不能超过 Telegram 的限制，通常为 50 MB。
- **频率限制**：发送消息的频率受到 Telegram 的限制，免费版在单个聊天中每秒最多发送 1 条消息，全局限制约为每秒 30 条。

### 性能边界
- **并发处理**：技能不支持高并发处理，单个 Agent 实例可能无法同时处理大量消息。
- **响应时间**：由于网络延迟和 Telegram API 的响应时间，技能的响应时间可能不稳定，通常在几秒到几十秒之间。

### 兼容性约束
- **Telegram 版本**：技能依赖于 Telegram Bot API，因此需要与当前支持的 Telegram 版本兼容。
- **操作系统**：技能在支持 SKILL.md 的 AI Agent 平台上运行，需要 Agent 平台与操作系统兼容。

### 其他限制
- **消息类型**：免费版不支持发送文档、位置、投票等多类型消息。
- **消息编辑和删除**：技能不支持编辑或删除已发送的消息。
- **聊天历史检索**：技能不支持检索聊天历史或查询成员角色。
- **命令列表设置**：技能不支持设置或更新机器人的命令列表。
