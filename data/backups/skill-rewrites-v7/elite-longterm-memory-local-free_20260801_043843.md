...
---
## 热内存（SESSION-STATE.md）详细说明

热内存是本地向量记忆免费版的核心组成部分，它提供了一种抗压缩和重启的活跃工作记忆。以下是对热内存的详细说明：

热内存通过WAL（Write-Ahead Logging）协议确保在用户输入触发时，首先将状态写入日志文件，然后再进行回复。这种协议确保了即使在发生崩溃的情况下，也不会丢失任何中间状态。

**热内存参数**：
- **Current Task**：当前的任务信息。
- **Key Context**：关键上下文信息，用于保持对话的连贯性。
- **Pending Actions**：待办动作列表。
- **Recent Decisions**：近期决策记录。

**热内存输出**：
- 会话级状态文件，Agent每次响应前先读取该文件。

**WAL协议**：
- 触发条件：用户表达偏好、做出决策、给出期限、纠正代理等。
- 输出：先写入SESSION-STATE.md再回复用户。

## 精选归档（MEMORY.md + daily/）详细说明

精选归档提供了人类可读的长期记忆和每日日志，它是本地向量记忆免费版中的另一重要组成部分。

**精选归档参数**：
- **MEMORY.md**：摘要文件，包含精炼后的长期记忆。
- **memory/YYYY-MM-DD.md**：每日日志文件，记录当日的活动。
- **memory/topics/**：主题文件，用于组织和管理记忆内容。

**精选归档输出**：
- 精炼后的长期记忆与每日活动记录。

**记忆迁移**：
- 会话结束时，将重要内容从SESSION-STATE.md迁移至MEMORY.md，并创建或更新每日日志。

## WAL协议保障详细说明

WAL协议（Write-Ahead Logging）是本地向量记忆免费版中用于确保数据持久性的关键机制。

**WAL协议参数**：
- 触发条件：用户表达偏好、做出决策、给出期限、纠正代理等。

**WAL协议输出**：
- 先写入SESSION-STATE.md再回复用户。

**WAL协议重要性**：
- 确保数据持久性，防止上下文丢失。
- 提供了一种机制，即使在发生崩溃的情况下，也可以恢复到一致的状态。

## 错误处理流程详细说明

错误处理是确保本地向量记忆免费版稳定运行的关键环节。以下是对错误处理流程的详细说明：
...
**处理方式**：
- 对于Agent遗忘对话中途上下文，需要在AGENTS.md中添加强制规则，并在Agent指令中包含记忆协议。
- 对于记忆文件未被加载，需要在AGENTS.md中添加强制规则，并检查文件是否存在与可读。
- 对于重复犯同类错误，需要在每次犯错后手动写入lessons.md，并在AGENTS.md中配置“犯错后强制记录”规则。

## 付费版升级内容详细说明

本地向量记忆免费版提供了基础的热内存和文件归档功能。升级到付费版可以获得以下完整能力：

- **LanceDB 向量语义搜索**：基于本地LanceDB的语义召回，无需精确关键词匹配。
- **Ollama 本地 Embedding**：通过nomic-embed-text模型本地生成768维向量。
- **完整 CLI 管理工具**：memory.js提供store、search、stats、dedup、export、backup、compact、cleanup等完整管理命令。
- **自动召回与捕获**：autoRecall会话开始自动召回相关记忆，autoCapture按类别与重要性自动存储。
- **五层完整记忆架构**：热内存、LanceDB向量、Git-Notes知识图谱、精选归档、本地Embedding五层协同。
- **插件集成**：启用后自动提供memory_recall、memory_store、memory_forget三个工具，深度集成Agent工作流。

付费版适合需要语义搜索、自动召回、完整记忆管理工具的高级本地AI Agent应用场景。

## 差异化优势

### 与同类方案对比

1. **手动操作**：与其他需要手动维护记忆系统的方案相比，"本地向量记忆免费版"通过自动化处理显著减少了开发者的工作量。例如，手动管理记忆文件需要频繁更新和维护，而本技能通过会话记忆协议和自动迁移机制，使得Agent在会话结束后自动更新记忆，无需开发者手动干预。

2. **其他记忆管理工具**：与其他依赖外部服务或工具的记忆管理方案（如某些云服务或专门的记忆管理平台）相比，本技能纯本地运行，无需外部依赖，保障了数据隐私和安全性。同时，它不依赖于Ollama或Node.js等外部API，简化了部署和维护流程。

3. **通用方法**：与其他通用的AI Agent记忆管理方法相比，本技能专注于提供两层记忆能力：热内存和文件归档，这使其更适合需要简单记忆管理功能的场景，避免了功能过载和性能问题。

### 独特功能

1. **WAL协议保障**：通过WAL协议确保先写状态再回复，有效防止了因系统崩溃导致的上下文丢失，提升了数据的安全性和可靠性。

2. **会话记忆协议**：标准化了Agent的内存行为，确保在会话开始、进行和结束时，Agent能够正确地读取、更新和迁移记忆，提高了用户体验的一致性。

3. **精选归档**：提供了人类可读的长期记忆和每日日志，便于开发者理解Agent的行为和决策过程。

4. **本地运行**：纯本地运行，无需外部依赖，保障了数据隐私和安全性。

5. **易于集成**：支持SKILL.md的任意AI Agent，易于集成到现有的开发环境中。

### 效率提升

使用"本地向量记忆免费版"可以节省大量时间，尤其是在需要频繁维护记忆文件的情况下。例如，手动更新记忆文件可能需要几分钟到几小时，而本技能可以自动完成这一过程，节省了宝贵的开发时间。

### 应用场景创新

1. **个人助理**：为个人助理提供记忆能力，使其能够记住用户偏好、历史交互和重要信息，从而提供更加个性化和高效的助手服务。

2. **智能客服**：为智能客服提供记忆功能，使其能够记住与用户的对话历史，提供更加连贯和专业的服务。

3. **教育助手**：为教育助手提供记忆功能，使其能够记住学生的学习进度、问题和反馈，从而提供更加个性化的学习建议和资源。
