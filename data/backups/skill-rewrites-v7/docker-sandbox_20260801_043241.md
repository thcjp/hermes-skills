...
license: "MIT"
description: |-
  Create and manage Docker sandboxed VM environments for safe agent execution。Use when 需要数据库操作、SQL查询、数据存储管理时使用。不适用于数据库架构设计决策。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。
tags:
  - Operations
  - 容器
  - Docker
