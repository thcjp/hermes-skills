...
### 输入限制
- **消息长度**：单个消息的长度限制为4000个字符。
- **格式化限制**：虽然支持基础消息格式化，但格式化后的消息长度加上原始文本的总长度不得超过4000字符。
- **频率限制**：由于Slack API的限制，对单个工作区的请求频率有限制。超过限制可能导致请求被拒绝。

### 性能边界
- **并发处理**：技能设计为单线程处理，因此不能同时处理多个并发请求。
- **响应时间**：由于依赖于Slack API的响应时间，技能的响应时间可能受到外部因素影响。

### 兼容性约束
- **工作区类型**：仅支持公共频道，不支持私有频道。
- **Bot权限**：Bot需要具有`chat:write`和`channels:read`权限才能正常工作。
- **Slack版本**：技能可能需要与特定版本的Slack客户端兼容。

### 其他限制
- **文件上传**：不支持文件上传功能，需使用其他技能或工具实现。
- **错误处理**：技能在遇到错误时，可能无法提供详细的错误信息，需要开发者自行解析API返回的错误码。
