...
### 输入限制
- **输入长度**：单个输入字符串的长度不应超过1024个字符，以确保系统处理效率和稳定性。
- **文件系统访问**：输入中不应包含路径分隔符或特殊字符，以避免对本地文件系统的潜在破坏。
- **格式规范**：输入数据应遵循预定义的格式规范，如Markdown格式，以确保系统能正确解析和处理。

### 性能边界
- **处理速度**：对于单个请求，系统应在2秒内完成处理，以保证用户体验。
- **并发处理**：系统应能同时处理多个请求，但单个用户在短时间内不应发起超过10个并发请求。

### 兼容性约束
- **操作系统**：系统应在Windows、macOS和Linux操作系统上运行，但不保证在所有版本的操作系统上都能正常工作。
- **Agent平台**：系统应与支持SKILL.md的任何AI Agent兼容，但不保证与所有Agent的特定功能完全兼容。

### 数据存储限制
- **文件大小**：单个文件的大小不应超过10MB，以避免对文件系统造成压力。
- **目录结构**：系统对本地目录结构的访问应限于`./productivity/`目录及其子目录，避免访问其他目录。

### 功能限制
- **网络请求**：系统不执行任何网络请求，因此无法访问外部服务或API。
- **数据同步**：系统不支持数据同步功能，所有数据仅存储在本地目录中。

### 习惯追踪限制
- **习惯类型**：系统仅支持追踪每日习惯，不支持其他频率的习惯追踪。

### 周回顾限制
- **回顾频率**：系统仅支持每周回顾，不支持其他频率的回顾。
- **回顾内容**：回顾内容限于本周完成、未完成原因和下周聚焦，不支持其他回顾内容。

### 数据隐私
- **数据加密**：系统不提供数据加密功能，所有数据以明文形式存储在本地文件中。
- **数据访问**：系统不提供数据访问控制功能，所有数据对用户可完全访问。
