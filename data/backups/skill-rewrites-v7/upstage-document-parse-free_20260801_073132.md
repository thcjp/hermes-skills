...
### 输入限制
- **文件大小**：同步模式解析的文件大小限制为50MB，超过此限制的文件无法进行解析。
- **页数限制**：同步模式解析的页数限制为100页，超过此限制的文件需要分批处理。
- **格式限制**：支持的输入格式包括JPEG、PNG、BMP、PDF、TIFF，其他格式需要转换成支持格式。

### 性能边界
- **解析速度**：基础版文档处理时间约为3秒，处理复杂文档时可能需要更长时间。
- **并发处理**：同步模式下，技能不支持并发处理多个文件。

### 兼容性约束
- **操作系统**：技能在Windows、macOS和Linux操作系统上运行。
- **Agent平台**：技能支持SKILL.md的任意AI Agent，如Claude Code、Cursor、Codex、Gemini CLI等。
- **API Key**：需要配置有效的API Key，且API Key应妥善保管，避免泄露。

### 输出限制
- **格式限制**：仅支持Markdown格式输出，不支持HTML和text格式。
- **元素类别**：仅支持基础版面元素识别，不支持高级元素类别如chart、equation等。
- **OCR限制**：仅支持自动OCR，不支持强制OCR。

### 其他限制
- **异步模式**：免费版仅支持同步模式，不支持异步模式。
- **enhanced模式**：免费版不支持enhanced模式，不支持复杂表格精确解析。
- **force OCR**：免费版不支持force OCR，不支持对扫描PDF或图片执行OCR。
