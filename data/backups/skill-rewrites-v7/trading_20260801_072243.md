...
edition: "pro"
description: |-
  面向专业交易者的多策略交易分析系统。支持维加斯通道、MACD、布林带 等多种技术策略组合，提供批量标的分析、历史回测验证、自动信号触发 与风险控制功能。Use when 用户需要trading相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。
tags:
  - 交易分析
  - 自动化
  - 分析
