...
summary_zh: "企业级视频分析工具,支持批量处理、自定义模型、结构化报告,适配商业内容审核与生产。通义千问视频分析专业版,面向企业团队与专业用户的高级AI视频内容理解工具。核心能力: - 批量视频分析,支"
license: "MIT"
edition: "pro"
description: |-
...
|---|---|---|
| 基础功能 | 支持 | 支持 |
| 通义千问视频分析-专业版企业级视频分析 | 不支持 | 支持 |
| 通义千问视频分析-专业版支持批量处理 | 不支持 | 支持 |
| 高清分辨率与无损输出 | 不支持 | 支持 |
| 批量生成与风格预设 | 不支持 | 支持 |
| 自定义模型微调 | 不支持 | 支持 |
...
针对本地视频,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供本地视频分析相关的配置参数、输入数据和处理选项.
**输出**: 返回本地视频分析的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`本地视频分析`的配置文档进行参数调优
### 远程 URL 分析

针对远程 URL ,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
