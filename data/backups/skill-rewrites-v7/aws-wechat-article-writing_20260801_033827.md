...
用户供图分支(`image_source: user`):用户图片须先放入本篇 `imgs/`;由智能体读图分析并生成或补全 `img_analysis.md`(可用 `user_image_prepare.py` 先生成模板再填写);未落盘 `img_analysis.md` 不得调用 `write.py`;`img_analysis.md` 中推荐用途为封面必须且只能有 1 处;写稿时直接使用用户图片路径 `imgs/未指定.png`,不再输出 `placeholder`。`image_source` 只允许 `generated` 或 `user`,禁止写 `user_provided`.
