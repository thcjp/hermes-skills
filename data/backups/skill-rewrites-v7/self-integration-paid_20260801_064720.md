...
summary_zh: "企业级外部应用集成方案，含自定义连接器、批量动作、工作流编排与审计日志。自集成工具（专业版）为团队与企业提供完整的外部应用集成治理方案，支持连接任意外部应用并编排自动化工作流。核心能力：自"
license: "MIT"
edition: "pro"
description: |-
...
针对预置连接器,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供预置连接器相关的配置参数、输入数据和处理选项.
**输出**: 返回预置连接器的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`预置连接器`的配置文档进行参数调优
### 自定义连接器构建

针对自定义连接器,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供自定义连接器构建相关的配置参数、输入数据和处理选项.
**输出**: 返回自定义连接器构建的处理结果。- 验证返回数据的完整性和格式正确性
- 参考`自定义连接器构建`的配置文档进行参数调优
### OAuth授权连接

针对OAuth授权连接,自动解析输入参数、调度任务队列、格式化输出,返回结构化响应.
**输入**: 用户提供OAuth授权连接相关的配置参数、输入数据和处理选项.
