...
### 输入限制
- **议题创建与更新**：创建议题时，项目key、issuetype.name、priority等必填字段必须符合Jira实例的配置。更新议题时，只能修改允许的字段。
- **批量操作**：批量操作时，单次操作最多支持100个议题。超过100个议题的操作需要分批次进行。
- **用户搜索与分配**：搜索用户时，必须提供至少一个搜索关键字。分配议题时，必须提供有效的accountId。

### 性能边界
- **请求频率限制**：Jira API对请求频率有限制，超过限制可能导致请求失败。建议在批量操作时合理分配请求间隔。
- **数据量限制**：Jira API对返回的数据量有限制，超过限制可能导致数据截断。建议在处理大量数据时，使用分页查询。

### 兼容性约束
- **Jira版本兼容性**：Jira工具箱(专业版)支持Jira Cloud 7.13及以上版本。旧版本可能不支持所有功能。
- **操作系统兼容性**：Jira工具箱(专业版)支持Windows、macOS和Linux操作系统。不同操作系统可能需要安装不同的依赖项。
- **Node.js版本**：Jira工具箱(专业版)要求Node.js版本为16及以上。较低版本可能存在兼容性问题。

### 其他限制
- **OAuth授权范围**：部分功能需要额外的OAuth授权范围。申请授权范围前，请确保了解Jira API的使用限制。
- **云资源费用**：使用Jira工具箱(专业版)可能产生云资源费用，如API调用费用、存储费用等。使用前请确认计费方式。
