...
| 错误场景 | 错误信息 | 原因分析 | 处理方式 |
|:---:|:---:|:---:|:---:|
| token 缺失 | `{"error":"token_required"}` | 请求未携带token | 使用带`?token=`的完整URL访问 |
| token 无效 | `{"error":"invalid_token"}` | token与服务端不匹配 | 重启服务获取新token |
| 端口被占用 | `EADDRINUSE: address already in use :::19195` | 端口19195被占用 | 停止占用进程；付费版可自定义端口 |
...
| session.json 格式错误 | 会话区显示异常 | 文件非合法JSON | 检查浏览器认证写入逻辑 |
