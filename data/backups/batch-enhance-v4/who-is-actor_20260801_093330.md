...
summary: "Git仓库参与者识别技能。Git repository actor identification skill。核心能力: - 开发工具领域的专业化AI辅助工具 - Skil"
summary_zh: "Git仓库参与者识别技能。Git repository actor identification skill。核心能力: - 开发工具领域的专业化AI辅助工具 - Skil"
license: "MIT"
description: |-
...
### 输入限制
- **输入内容长度**：Who Is Actor技能对输入内容的长度有限制，通常单次输入不应超过10,000个字符，以避免性能问题和处理延迟。
- **文件类型**：技能仅支持处理文本内容，不支持图片、视频等多媒体文件。
- **编码格式**：输入文本应使用UTF-8编码，以确保正确处理特殊字符。

### 性能边界
- **处理速度**：对于大量数据或复杂分析，技能的处理速度可能会受到影响，建议分批次处理以优化性能。
- **并发处理**：Who Is Actor技能可能不支持高并发请求，过多同时请求可能导致服务不可用。

### 兼容性约束
- **操作系统**：Who Is Actor技能在Windows、macOS和Linux操作系统上运行，但不保证在其他操作系统上的兼容性。
- **浏览器兼容性**：如果技能通过Web界面访问，可能需要特定的浏览器版本或插件支持。
- **API版本兼容性**：Who Is Actor技能可能对API版本有特定要求，使用前请确保API版本与技能版本兼容。

### 数据隐私
- **敏感信息**：Who Is Actor技能不处理包含个人敏感信息的输入，如身份证号码、银行账户信息等。
- **数据存储**：技能处理的数据不会在本地永久存储，确保用户数据的安全性和隐私性。

### 依赖性
- **第三方服务**：Who Is Actor技能可能依赖于第三方服务，如LLM API，若第三方服务不可用，技能可能无法正常工作。
- **网络连接**：技能的正常运行依赖于稳定的网络连接，网络中断可能导致服务不可用。

## 功能详解与边界条件

### 核心功能详解

1. **Who Is ActorGit仓库参与者识别**:
   - **输入参数**: `content` (必填) - 包含Git仓库提交历史信息的文本内容。
   - **处理逻辑**: 分析提交历史，识别出参与者的贡献情况，包括参与者ID、提交次数、提交日期等。
   - **输出结果**: 包含参与者列表、每个参与者的ID、提交次数、首次和最后提交日期等详细信息。

2. **代码静态分析与质量评分**:
   - **输入参数**: `content` (必填) - 包含Git仓库代码的文本内容。
   - **处理逻辑**: 对代码进行静态分析，评估代码质量，包括代码复杂度、代码重复率、潜在错误等。
   - **输出结果**: 提供代码质量评分和详细分析报告。

3. **依赖漏洞检测与升级建议**:
   - **输入参数**: `content` (必填) - 包含项目依赖信息的文本内容。
   - **处理逻辑**: 检测项目依赖中存在的已知漏洞，并提供相应的升级建议。
   - **输出结果**: 列出存在漏洞的依赖项和升级建议。

4. **批量代码审查与报告生成**:
   - **输入参数**: `content` (必填) - 包含代码审查请求的文本内容。
   - **处理逻辑**: 执行批量代码审查，生成审查报告。
   - **输出结果**: 提供审查报告，包括代码审查结果、问题分类、优先级等。

### 边界条件

1. **输入内容长度**: 单次输入内容不超过10,000个字符。
2. **文件类型**: 仅支持处理文本内容。
3. **编码格式**: 输入文本必须使用UTF-8编码。
4. **并发处理**: 不支持高并发请求。
5. **数据大小**: 处理的数据大小不超过5GB。
6. **网络连接**: 需要稳定的网络连接。
7. **API版本兼容性**: 必须使用与技能版本兼容的API版本。
8. **操作系统兼容性**: 仅支持Windows、macOS和Linux操作系统。
...
4. **输入内容格式不正确**: 检查输入是否符合技能使用说明中的格式要求。
5. **执行结果与预期不符**: 提供更详细的指令描述，补充必要的上下文信息。

### 性能指标

1. **处理速度**: 对于大量数据或复杂分析，处理速度可能会受到影响。
2. **并发处理能力**: 不支持高并发请求。
3. **数据存储容量**: 处理的数据大小不超过5GB。
4. **网络带宽**: 需要稳定的网络连接，以保证数据传输的稳定性。
