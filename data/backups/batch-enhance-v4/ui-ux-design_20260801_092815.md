...
license: "MIT"
description: |-
  现代UI/UX设计原则、模式与优选实践指南.
  涵盖Mobile-First响应式设计、配色系统、排版、WCAG 2.2无障碍.
  包含Shadcn/ui + Tailwind CSS技术栈集成与微交互设计.
  适用于Web与移动应用的界面设计、布局选择与设计审查。Use when 需要设计创作、UI设计、海报制作、品牌视觉时使用。不适用于3D建模和动画制作。
tools:
  - read
  - exec
  - write
...
**灵感来源：**
- Linear (linear.app) — 优选键盘优先UI，微妙动画
- Stripe Dashboard — 干净数据可视化，完善间距
- Vercel — 极简，快速，现代渐变
- Notion — 直观拖放，清晰层次
