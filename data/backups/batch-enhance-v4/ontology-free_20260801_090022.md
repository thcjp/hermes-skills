...
license: "MIT"
description: "类型化知识图谱引擎基础版,将知识表示为可验证的实体-关系图谱.每个实体拥有类型、属性和关系,所有变更在提交前根据类型约束进行验证.基础能力包括实体创建、关系绑定、属性设置与图谱查询.适用于需要结构化知识存储与推理的Agent场景,确保图谱一致性与完整历史追溯,支持追加式变更日志记录与图谱版本回溯查询."
