...
### 输入限制
- **主题名称**: 必须是内置的10+预设主题之一，或有效的自定义主题名称。
- **品牌色**: 必须是有效的HEX颜色代码。
- **输出格式**: 支持的输出格式包括theme.json、variables.css、preview.png等。
- **文件类型**: 支持的文件类型包括PPT、DOCX、HTML、Markdown、PDF等。

### 性能边界
- **处理时间**: 对于单个主题应用，处理时间通常在几秒到几十秒之间，取决于输出格式和文件大小。
- **并发处理**: 同时处理多个主题应用时，性能可能会受到影响，建议单次处理不超过5个请求。

### 兼容性约束
- **操作系统**: 支持Windows、macOS和Linux操作系统。
- **浏览器**: 支持主流的现代浏览器，如Chrome、Firefox、Safari等。
- **字体**: 内置字体支持有限，对于非内置字体，需要确保系统已安装或通过Web字体CDN提供。

### 色彩空间
- **色彩模式**: 主要支持RGB色彩空间，对于印刷品设计，需要转换为CMYK色彩空间。
- **色彩对比度**: 确保生成的色彩对比度符合WCAG AA标准。

### 字体使用
- **字体版权**: 必须遵守所使用字体的版权规定。
- **字体替换**: 当系统缺少指定字体时，将自动使用备用字体栈。

### 主题一致性
- **主题映射**: 确保所有输出物的一致性，包括色彩、字体和间距。
- **自定义主题**: 自定义主题生成时，需要确保色彩和谐，避免视觉混乱。

### 输出文件
- **theme.json**: 包含主题配置，可用于其他应用场景。
- **variables.css**: 包含CSS变量，适用于Web开发。
- **preview.png**: 主题预览图，用于视觉检查。

### 主题应用
- **主题应用映射**: 确保主题在所有支持的输出格式中正确应用。
- **间距规范**: 间距规范适用于所有输出格式，确保视觉一致性。

### 自定义品牌主题
- **品牌色**: 必须是品牌指定的主色，确保品牌一致性。
- **色板生成**: 基于主色生成完整色板，包括辅色、强调色、功能色和中性色。

### 一致性检查
- **色彩使用**: 检查色彩使用是否符合60-30-10比例。
- **字体使用**: 检查字体使用是否符合规定。
- **间距一致性**: 检查间距是否统一。
- **图表配色**: 检查图表配色是否与主题一致。

### 输出与交付
- **输出文件**: 生成可复用的主题配置文件和预览图。
- **应用示例**: 提供应用主题的示例文件，如PPT、文档、HTML等。
