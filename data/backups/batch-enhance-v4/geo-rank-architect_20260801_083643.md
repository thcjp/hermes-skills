...
license: MIT
description: "GEO搜索占位架构师——AI搜索时代品牌霸屏利器,5维GEO评分+JSON-LD注入+评分门控,提升AI搜索引擎引用率。核心能力: JSON-LD结构化数据注入。Use when 需要SEO优化、关键词分析、排名提升、搜索流量优化时使用。不适用于黑帽SEO手段。适用于独立开发者、企业团队和自动化工作流场景。"
