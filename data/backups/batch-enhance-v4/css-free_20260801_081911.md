...
license: "MIT"
description: "CSS样式与布局辅助引擎,覆盖现代CSS布局技术(含Flexbox与Grid)、动画效果设计、响应式适配方案与组件级样式生成.支持Tailwind工具类映射与自定义样式输出,提供结构化工作流程、可复用模板与配置指引.适用于前端页面开发、组件库构建与样式规范化场景,帮助开发者快速产出符合设计规范的CSS代码并保持代码质量."
