...
### 输入限制
- **文本长度**：发送的文本消息长度限制为1024个字符。
- **图片尺寸**：支持的图片尺寸最大为2048x2048像素。
- **媒体格式**：支持的图片格式为PNG和JPEG。
- **URL格式**：发送的图片URL必须是HTTPS格式。

### 性能边界
- **并发请求**：免费版支持的单个会话中的并发请求有限制，超过限制可能导致请求失败。
- **频率限制**：发送消息的频率受到限制，频繁发送可能导致账号受限。

### 兼容性约束
- **操作系统**：该技能在Windows、macOS和Linux操作系统上均兼容。
- **浏览器**：免费版不支持在浏览器中直接使用，需要通过支持SKILL.md的AI Agent平台进行操作。
- **WhatsApp版本**：仅支持通过WhatsApp Business API发送消息，不支持普通WhatsApp账号。

### 其他限制
- **消息类型**：免费版仅支持文本消息和图片消息，不支持模板消息、交互按钮、列表消息等。
- **时间限制**：自由文本与图片消息受24小时客服窗口限制，窗口外无法发送。
- **手机号格式**：发送消息的手机号必须包含国家代码，且格式正确。

### 安全与隐私
- **数据存储**：使用该技能时，用户数据将被存储在ClawLink的服务器上，ClawLink将采取适当措施保护用户数据安全。
- **API访问**：所有API调用均通过ClawLink进行鉴权，确保API访问的安全性。
...
| --- | --- | --- | --- | --- |
| 发送单条文本消息 | 30秒 | 5秒 | 25秒 | 100% |
| 发送单条图片消息 | 1分钟 | 10秒 | 50秒 | 100% |
| 查询手机号列表 | 10分钟 | 1分钟 | 9分钟 | 100% |
| 批量发送消息 | 1小时 | 10分钟 | 50分钟 | 100% |
| 消息跟踪与统计 | 2小时 | 30分钟 | 1.5小时 | 100% |
...
| --- | --- | --- | --- | --- |
| 手动发送消息效率低 | 每次发送消息都需要手动操作，耗时且易出错 | 影响工作效率和用户体验 | 自动化发送消息 | 时间节约25% |
| 消息跟踪困难 | 难以跟踪消息发送状态和效果 | 影响营销效果和客户满意度 | 消息跟踪与统计功能 | 准确率提升100% |
| 手机号查询不便 | 手动查询手机号耗时且容易遗漏 | 影响业务拓展和客户管理 | 手机号查询功能 | 时间节约90% |

## 故障排查指南
| 错误现象 | 可能原因 | 诊断步骤 | 解决方案 |
| --- | --- | --- | --- |
| 无法发送消息 | API访问权限不足 | 检查ClawLink连接状态和API令牌 | 重新连接ClawLink或更新API令牌 |
| 消息发送失败 | 网络连接问题 | 检查网络连接状态 | 确保网络连接正常 |
| 手机号查询失败 | 手机号不存在或权限不足 | 检查手机号是否正确 | 确保手机号正确且账户有查询权限 |
| 无法获取phone_number_id | 数据库连接问题 | 检查数据库连接状态 | 确保数据库连接正常 |
| 消息发送延迟 | API响应时间过长 | 检查API响应时间 | 调整API请求频率或联系API提供商 |
