...
以下是部分关键参数的说明：
- content: 字符串，输入要处理的内容，可选值为json/text/markdown。
- style: 字符串，输出风格，参考 `references/style.md`。
- rate_limit_per_sec: 整数，每秒消息发送速率限制。
- max_retries: 整数，消息发送失败时的最大重试次数。
- retry_backoff: 字符串，重试退避策略，支持'linear'和'exponential'。
...
针对常见的错误情况，我们提供了以下处理方案：
- Webhook无响应：检查证书有效期和防火墙设置。
- 大量429错误：检查速率限制配置和队列缓冲状态。
- 支付回调丢失：确保handler响应时间小于10秒。
- 状态机卡死：检查Redis连接和定时任务。
- 群组消息遗漏：调用getChat验证成员状态，重新邀请。
- 模板渲染失败：检查模板变量注入逻辑，添加默认值。
...
批量推送功能通过队列引擎实现，支持异步发送消息，并具备以下特性：
- 支持自定义速率限制，防止触发Telegram的429错误。
- 实现了消息发送失败的重试机制，最多重试3次。
- 支持追踪消息送达状态，包括成功、失败和被屏蔽的情况。
- 提供了详细的日志记录，方便问题排查。
