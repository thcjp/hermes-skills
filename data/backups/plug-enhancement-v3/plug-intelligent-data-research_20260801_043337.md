...
description: |-
  ...
  定价方案: 月付￥399/月 | 年付￥3999/年 | 买断￥5999

  包含技能: internet-search-pro-2, google-search-4, free-web-search-4, web-crawler-engine
tools:
  - read
  - exec
  - write
...
| 异常场景 | 原因 | 处理方式 |
|:---------|:-----|:---------|
| AI搜索LLM调用失败 | LLM服务不可用或超时 | 返回success=false，建议检查LLM_API_KEY配置 |
| AI搜索查询构造失败 | LLM返回格式异常 | 使用原始查询直接搜索，标注warning |
| Google CSE API配额耗尽 | 每日免费100次查询用尽 | 自动降级到free-web-search-4（Bing/DuckDuckGo） |
| Google CSE API不可用 | API Key未配置或网络不通 | 降级到free-web-search-4，标注warning |
| Bing API不可用 | API Key未配置或服务异常 | 自动切换到DuckDuckGo引擎 |
| DuckDuckGo搜索失败 | 网络不通或服务异常 | 返回success=false，建议检查网络连接 |
| 网页抓取目标URL无效 | URL格式错误或页面不存在 | 跳过该URL，记录错误日志，继续抓取其他URL |
| 网页抓取被目标网站拦截 | 反爬虫机制检测到自动化 | 降低并发数，增加请求间隔，设置User-Agent |
| 网页抓取超时 | 目标网站响应缓慢 | 单URL超时30秒，超时后跳过，记录到告警 |
| 增量同步失败 | 数据源API变更或网络中断 | 记录失败任务，下次同步自动重试，发送告警通知 |
| SQL分析执行失败 | SQL语法错误或表不存在 | 返回错误详情，建议检查表结构和SQL语法 |
...
| 错误码 | 说明 | 处理建议 |
|:-------|:-----|:---------|
| 400 | 参数错误 | 检查输入格式，确认查询关键词已提供 |
| 401 | 未授权 | 检查GOOGLE_API_KEY/BING_API_KEY配置 |
| 403 | API配额耗尽 | Google CSE每日免费100次，降级到free-web-search-4 |
| 429 | 限流 | 降低请求频率，增加请求间隔 |
| 500 | 服务异常 | 检查网络连接和LLM服务状态 |
| GOOGLE_QUOTA_EXCEEDED | Google CSE配额耗尽 | 已自动降级到Bing/DuckDuckGo免费搜索 |
| BING_UNAVAILABLE | Bing API不可用 | 已自动切换到DuckDuckGo引擎 |
| CRAWL_URL_INVALID | URL无效 | 跳过该URL，继续抓取其他URL |
| CRAWL_BLOCKED | 被目标网站拦截 | 降低并发数，增加请求间隔，设置User-Agent |
| CRAWL_TIMEOUT | 抓取超时 | 单URL超时30秒已跳过，检查目标网站可用性 |
| SYNC_FAILED | 增量同步失败 | 记录失败任务，下次同步自动重试 |
| SQL_ERROR | SQL分析错误 | 检查表结构和SQL语法 |
...
### 解决痛点
- 信息分散找不到？多引擎聚合搜索一步到位，AI优化查询构造
- 搜索结果不够精准？Google CSE精准搜索 + 类目路由 + 多步搜索
- 搜索API太贵？free-web-search-4免费方案（Bing+DuckDuckGo），零API Key需求
- 需要批量抓取数据？增量同步+SQL分析+批量调度一体化，数据不遗漏

### 核心价值
- 4个搜索/抓取技能协同工作，覆盖从信息发现到数据归档到分析洞察的全链路
- 支持Google、Bing、DuckDuckGo等多引擎，免费方案零成本起步
- 增量同步+SQL分析，数据不遗漏，洞察自动生成
- 多格式导出（CSV/JSON/Excel），监控告警（飞书/钉钉/邮件），适合团队协作

## 定价方案

月付399元/月 | 年付3999元/年 | 买断5999元

| 方案 | 单买总价 | Plug价格 | 节省比例 |
|:-----|:---------|:---------|:---------|
| 月付 | ~1200元/月 | 月付399元/月 | 60%+ |
| 年付 | ~12000元/年 | 年付3999元/年 | 60%+ |
| 买断 | ~20000元 | 买断5999元 | 70%+ |
...
| 风险项 | 等级 | 防护措施 | 验证方法 |
|:------|:------|:------|:------|
| 数据泄露 | 高 | 数据加密传输 | 定期安全审计 |
| 系统漏洞 | 中 | 定期更新安全补丁 | 安全漏洞扫描 |
| 未授权访问 | 中 | 访问控制 | 安全日志审计 |
| 网络攻击 | 高 | 防火墙和入侵检测系统 | 实时监控 |
| 物理安全 | 中 | 设备安全措施 | 定期检查 |
