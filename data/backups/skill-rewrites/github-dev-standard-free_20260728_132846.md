...
- **需求描述清晰度**：技能要求用户提供清晰的需求描述，模糊或过于宽泛的需求可能导致流程执行错误或效率低下。
- **代码库结构**：技能假定代码库具有一定的结构，如存在大量无组织或无注释的代码，可能影响流程的执行。
- **依赖版本**：技能依赖的运行时环境（如Python、Node.js）需要与技能版本兼容，不兼容的版本可能导致执行失败。

### 性能边界

- **并发处理**：技能设计为单线程执行，高并发场景下可能需要等待当前任务完成。
- **数据处理量**：对于大量数据的处理，技能可能需要较长时间，具体时间取决于数据量和系统资源。

### 兼容性约束

- **操作系统**：技能主要在Windows、macOS和Linux操作系统上测试，其他操作系统可能存在兼容性问题。
- **浏览器兼容性**：如果技能涉及Web界面，可能存在不同浏览器之间的兼容性问题。

### 其他限制

- **API调用频率**：如果技能涉及外部API调用，可能受到API调用频率限制。
- **外部工具依赖**：技能依赖的外部工具（如Git、LLM API）需要正确配置和可用，否则技能无法正常执行。
