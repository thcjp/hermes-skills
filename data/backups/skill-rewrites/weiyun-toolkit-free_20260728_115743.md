...
summary: "微云网盘文件管理免费版：目录浏览、文件下载与上传，内置快速脚本与基础错误码指南.。微云工具箱（免费版）面向个人用户与独立开发者，封装微云网盘的日常文件操作：目录浏览、文件下载、文件上传。通过"
license: MIT
edition: free
...
微云网盘是腾讯提供的个人云存储服务。该工具箱通过 MCP工具协议与微云网盘交互，把日常的"列目录、下载、上传"三件事封装成可直接复制的工作流。免费版聚焦于"能用、够用"，专业版补齐分享、删除、重命名、移动、分类查询等进阶操作.
...
| 快速上传脚本 | `upload_to_weiyun.py` 自动计算参数 | 是 |
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：微云网盘文件管理、免费版、目录浏览、文件下载与上传、内置快速脚本与基、础错误码指南、微云工具箱、面向个人用户与独、立开发者、封装微云网盘的日、常文件操作、文件下载、文件上传、MCP、工具协议与微云网、盘交互、所有文件哈希计算、在本地完成、避免敏感数据外泄、Use、when、需要数据分析、报表生成、统计洞察、数据可视化时使用、不适用于实时流数、据处理、适用于独立开发者、企业团队和自动化、工作流场景等.
## 使用场景
...
### 场景二：上传本地文件到网盘备份
用户说"把这个项目压缩包传到微云"。Agent 应直接调用快速脚本 `upload_to_weiyun.py`，传入文件路径与目标目录 `pdir_key`，无需手动计算分片哈希.
### 场景三：浏览网盘结构
...
### 4. 服务繁忙重试
遇到 `retcode=50000` 时，等待 2-5 秒后重试，最多 3 次。快速脚本已内置此逻辑.
### 5. 安全披露
...
### Q3：分片上传通道 len=0？
A：每轮上传完一片后，通道列表可能全部 len=0，需要重新预上传获取下一批通道。快速脚本已自动处理.
### Q4：SHA1 不匹配？
A：确保分块 SHA 值使用流式 SHA1 内部状态（小端序），而非独立分块 SHA1。直接用快速脚本可避免此问题.
### Q5：file_sha 被覆盖？
A：服务端用最后一个 block 的 SHA 覆盖 file_sha，两者必须相等。快速脚本会自动保证这一点.
### Q6：通道 ID 不匹配？
