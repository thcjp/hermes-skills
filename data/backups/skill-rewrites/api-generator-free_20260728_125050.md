...
API 代码生成器免费版在设计和使用过程中存在一些边界条件和限制，以下列举了其中一些关键点：

### 输入限制
- **输入数据格式**: 输入数据必须符合预期的格式，否则可能导致生成失败或错误。
- **资源名称长度**: 资源名称的长度通常有限制，过长可能导致生成错误。
- **参数数量**: 输入参数的数量和复杂性可能影响生成效率和结果。

### 性能边界
- **处理速度**: 对于复杂的API结构或大量数据，生成过程可能需要较长时间。
- **内存使用**: 生成大量代码时，内存使用可能会增加，影响其他应用程序的性能。

### 兼容性约束
- **操作系统**: 免费版支持Windows、macOS和Linux操作系统，但可能存在特定版本的兼容性问题。
- **Agent平台**: 需要使用支持SKILL.md的AI Agent，如Claude Code、Cursor、Codex或Gemini CLI等。
- **LLM API**: 必须由Agent内置LLM提供，不支持自定义LLM。

### 输出限制
- **代码输出**: 生成的代码输出到stdout，不支持直接写入文件。
- **高级功能**: 免费版不支持OpenAPI文档、Python客户端、Mock服务器、认证代码和速率限制器等高级功能。

### 使用限制
- **免费版专享**: 免费版仅提供基础功能，高级功能需升级付费版。
- **API Key**: 使用免费版时，需要配置API Key，并确保其安全性。

### 其他限制
- **版本兼容性**: 免费版可能不兼容某些特定版本的库或框架。
- **社区支持**: 免费版用户可能无法获得与付费版用户相同的社区支持。
