...
- **日期范围**: 输入的日期范围不能超过Google日历API允许的查询范围，通常为一年。
- **事件数量**: 单次查询或操作的事件数量不能超过API的单次请求限制，通常为1000个事件。
- **事件详情**: 创建事件时，标题、时间、时长等必要信息必须提供，否则事件无法创建。

### 性能边界

- **并发请求**: FREE版本限制为单次一个任务，不支持并发请求。
- **处理时间**: 处理大量事件或复杂查询时，可能需要较长的处理时间。

### 兼容性约束

- **操作系统**: 支持Windows、macOS和Linux操作系统。
- **运行时**: 支持 Python 3.8+ 或 Node.js 18+。
- **网络连接**: 部分功能需要网络连接，如API调用。
- **API Key**: 必须配置有效的Google Calendar API Key或CalDAV服务器信息。

### 其他限制

- **批量操作**: FREE版本不支持批量操作，每次只能处理一个任务。
- **团队协作**: FREE版本不支持团队协作功能，仅限个人使用。
- **高级数据分析**: FREE版本不支持高级数据分析功能。
