...
license: MIT
edition: free
description: "核心能力: 效率管控领域的专业化 AI 辅助工具,包含核心基础功能兼容. 适用于需要prod improve tool相关能力的开发场景,包含结构化的工作流程和可复用的模板,帮助用户快速完成任务并保持代码质量. 适用于需要prod improve tool相关能力的开发场景,包含结构化的工作流程和配置指引."
tags:
  - 效率追踪
  - prod
  - improve
...
- **输入长度**：自然语言指令的长度不应超过 1024 个字符，以避免处理错误。
- **参数格式**：输入参数必须符合 JSON 格式，否则技能将无法正确解析。
- **敏感词过滤**：输入中包含的敏感词将被自动过滤，以保护用户隐私。

### 性能边界

- **并发处理**：由于 FREE 版本限制，每次只能处理一个任务，不支持并发执行。
- **数据处理量**：对于大量数据的处理，技能可能需要较长时间完成，建议分批处理。

### 兼容性约束

- **操作系统**：支持 Windows、macOS 和 Linux 操作系统。
- **AI Agent**：支持 Claude Code、Cursor、Codex、Gemini CLI 等支持 SKILL.md 格式的 AI Agent。
- **网络连接**：部分功能需要网络连接访问外部 API，确保网络畅通。

### 数据存储

- **本地存储**：所有数据本地存储在 `data/` 目录，不提供云同步功能。
- **数据备份**：建议定期备份 `data/` 目录，以防数据丢失。

### 安全限制

- **API Key**：FREE 版本支持单一 API Key 配置，需妥善保管，避免泄露。
- **数据安全**：所有数据本地存储，不上传云端，确保隐私安全。
