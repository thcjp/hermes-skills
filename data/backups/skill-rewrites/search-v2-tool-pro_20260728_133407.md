...
### 输入限制
- **查询长度**：单个查询字符串长度不应超过255个字符，以避免API限制和性能问题。
- **批量查询数量**：一次批量查询的查询数量上限为100个，超过此数量可能导致API限制或性能问题。
- **结果数量**：单次搜索请求的结果数量上限为20条，超过此数量可能导致API限制或性能问题。
- **缓存大小**：本地缓存的大小受限于系统可用内存，建议定期清理过期缓存以避免内存不足。

### 性能边界
- **搜索深度**：不同搜索深度（ultra-fast, fast, basic, advanced）对应不同的延迟和精度，应根据具体需求选择合适的深度。
- **全文内容提取**：全文内容提取会增加响应大小和API消耗，建议仅在需要深入分析内容时使用。
- **图片搜索**：图片搜索结果数量受`max_results`参数限制，且图片URL、标题和描述信息可能不如文本结果丰富。

### 兼容性约束
- **操作系统**：工具支持Windows、macOS和Linux操作系统，但某些功能可能在不同操作系统之间存在差异。
- **Python版本**：需要Python 3.8或更高版本，不支持Python 2.x。
- **网络环境**：需要稳定的网络连接，且网络环境应允许访问Tavily API和LLM API。
- **API Key**：需要有效的Tavily API Key，且该Key应具有足够的权限使用所需功能。

### 其他限制
- **API调用频率**：Tavily API对调用频率有限制，超过限制可能导致请求被拒绝。
- **数据格式**：输出结果的数据格式为JSON，不支持其他格式。
- **错误处理**：工具提供基本的错误处理机制，但对于某些特定错误可能需要手动干预。
