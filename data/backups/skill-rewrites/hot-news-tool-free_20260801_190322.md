...
license: "MIT"
edition: free
description: "轻量级新闻聚合工具，自发搜索筛选国内外科技、军事、社会新闻要点，适合个人用户快速了解时事动态。核心能力:。可自发提升工作效率.该技能适用于相关开发场景,包含结构化的工作流程和配置指引.经过深度差异化处置,针对用户反馈和使用痛点进行了改进,提升了实用性和可操作性."
  - 聚合国内外主流科技、军事、社会新闻源
...
**处理**: 解析单次聚合最多覆盖 10 个新闻的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回单次聚合最多覆盖 10 个新闻的响应数据,包含返回码、数据和处理记录.
- 使用`input_params`进行配置,支持创建/查询/导出操作

### 不支持定时自动更新
不支持定时自动更新

**处理**: 解析不支持定时自动更新的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回不支持定时自动更新的响应数据,包含返回码、数据和处理记录.
- 使用`input_params`进行配置,支持创建/查询/导出操作

### 不支持自定义新闻源
不支持自定义新闻源

**处理**: 解析不支持自定义新闻源的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回不支持自定义新闻源的响应数据,包含返回码、数据和处理记录.
- 使用`input_params`进行配置,支持创建/查询/导出操作

### 不支持实时推送通知
不支持实时推送通知

**处理**: 解析不支持实时推送通知的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回不支持实时推送通知的响应数据,包含返回码、数据和处理记录.
- 使用`input_params`进行配置,支持创建/查询/导出操作

### 不支持舆情趋势分析
不支持舆情趋势分析

**处理**: 解析不支持舆情趋势分析的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回不支持舆情趋势分析的响应数据,包含返回码、数据和处理记录.
- 使用`input_params`进行配置,支持创建/查询/导出操作

**处理**: 解析已知限制的输入参数,完成核心逻辑,返回格式化结果.
**输出**: 返回已知限制的响应数据,包含返回码、数据和处理记录.
**能力覆盖范围**：核心能力涵盖以下关键词：轻量级新闻聚合工、自动搜索筛选国内、外科技、社会新闻要点、适合个人用户快速、了解时事动态、核心能力、聚合国内外主流科、社会新闻源、自动筛选与去重、过滤低质量内容、按类别整理、每条含标题、生成结构化、Markdown、汇总报告等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
