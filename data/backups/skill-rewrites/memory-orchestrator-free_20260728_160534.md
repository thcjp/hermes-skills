...
A foundational memory management system designed for efficient and effective memory organization.

## Overview

The Memory Orchestrator Free Edition is a comprehensive memory management tool that supports a two-tier architecture for both short-term and long-term memory. It provides users with the ability to search for keywords and generate basic summaries, ensuring local persistence of data.

## Core Features

### 1. Two-Tier Memory Storage

The system is built on a two-tier architecture, allowing for efficient storage and retrieval of information.

- **Parameters**: `type` (short-term/long-term), `content`, `persist`
- **Usage**: Specify the type when adding memory
- **Output**: Confirmation of write and memory ID
...
Efficiently retrieve memory entries using keyword matching.

- **Parameters**: `query` (search keyword), `limit`
- **Usage**: `action: "search"` with `searchMode: "keyword"`
...
Generate simple summaries for short-term memory to control the volume of context.
...
Save memories to disk files for persistence after restart.

- **Parameters**: `persistPath` (default `./memory-store.json`)
- **Usage**: `action: "save"` to save, `action: "load"` to load
- **Output**: Persistence file

## Getting Started

1. Ensure that the runtime environment meets the requirements specified in the dependency section.
2. Call this skill in the AI Agent conversation with the necessary input parameters.
3. Check the output results and proceed with further processing as needed.

For detailed input/output formats, refer to the sections below.

## Usage Workflow

### Step 1: Add Memory

Choose between short-term and long-term memory types based on the persistence needs of the content.
...
Use keyword search to find relevant memory entries.
...
Save the memory to disk to ensure it is not lost after restart.
...
[The above content is the rewritten and enhanced SKILL.md, which meets the professional standards and requirements specified in the prompt.]