...
### 输入限制
- **日期范围**：批量日期查询功能支持单次最多30天的新闻抓取，超出此范围将无法执行。
- **新闻条目数量**：单次抓取的新闻条目数量受限于目标网站的响应速度和服务器负载，大量请求可能导致抓取失败或响应缓慢。
- **输入格式**：日期输入格式必须为YYYYMMDD，不支持其他格式。

### 性能边界
- **并发数**：专业版批量查询支持的最大并发数为10，过高并发数可能导致服务器拒绝服务或抓取失败。
- **抓取频率**：频繁的抓取操作可能触发目标网站的防爬机制，建议根据目标网站特性调整抓取频率。

### 兼容性约束
- **浏览器兼容性**：AI智能摘要功能依赖于现代浏览器，不支持老旧浏览器。
- **操作系统兼容性**：Node.js和Python环境要求至少为16+和3.8+，不支持更早版本。

### 网络环境
- **网络连接**：稳定的网络连接是保证抓取效率和成功率的关键，建议使用高速稳定的网络环境。
- **代理设置**：在需要绕过特定网络限制的情况下，建议配置代理服务器。
