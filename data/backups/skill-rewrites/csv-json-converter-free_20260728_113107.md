...
license: MIT
edition: free
description: "CSV转JSON免费版是一款面向独立开发者与数据工程师的轻量级表格数据格式变换工具。围绕\"表头推断—类型识别—单文件变换—结果校验\"四件事，包含可复制即用的Python/Node。Use. 适用于需要csv json converter相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - agent
  - csv
  - json
...
支持。免费版默认识别逗号分隔，但提供 `--delimiter '\t'` 参数切换为制表符。Agent会自动检测分隔符，若领先行含制表符多于逗号，则判定为TSV.
