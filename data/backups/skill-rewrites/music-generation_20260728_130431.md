...
### 输入限制
- **文本长度**：输入文本长度不宜过长，建议不超过500个字符，以确保AI能够准确理解和生成音乐。
- **复杂性**：避免过于复杂的音乐风格描述，以免AI难以理解并生成符合要求的结果。
- **格式规范**：输入文本应遵循正确的格式，如使用逗号分隔不同的音乐元素描述。

### 性能边界
- **实时性**：由于音乐生成过程涉及复杂的算法处理，实时生成可能存在延迟，建议用户耐心等待。
- **生成质量**：AI生成的音乐质量受限于训练数据和算法，可能无法达到专业音乐制作水平。

### 兼容性约束
- **操作系统**：Skill.md技能在支持SKILL.md的AI Agent平台上运行，如Claude Code、Cursor、Codex、Gemini CLI等。
- **网络环境**：网络连接稳定是确保技能正常运行的关键，建议在高速网络环境下使用。
- **外部API**：部分音乐生成服务可能需要外部API支持，确保已正确配置API Key和相关参数。

### 其他限制
- **版权问题**：AI生成的音乐可能存在版权问题，请确保在使用前了解相关法律法规。
- **个性化定制**：AI生成的音乐风格和元素可能有限，无法满足所有个性化需求。
- **语言支持**：目前音乐生成技能主要支持英文输入，其他语言可能存在限制。
