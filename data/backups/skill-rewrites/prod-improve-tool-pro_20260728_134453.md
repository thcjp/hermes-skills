...
summary: "企业级效率追踪系统,支持团队效率分析、自动数据采集与 AI 效率教练。核心能力: 效率管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 效率管控领域的专业化 AI 辅助工具,包含企业级高级功能兼容. 适用于需要prod improve tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 效率追踪
  - 时间管理
  - 活动分析
...
本工具是 **效率管理** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析团队效率分析与基准对比的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回团队效率分析与基准对比的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自动数据采集
自动数据采集:从日历/Git/任务系统提取

**处理**: 解析自动数据采集的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自动数据采集的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### AI 效率教练
AI 效率教练:个性化改进建议

**处理**: 解析AI 效率教练的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回AI 效率教练的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 峰值效率时段识别与任务匹配
峰值效率时段识别与任务匹配

**处理**: 解析峰值效率时段识别与任务匹配的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回峰值效率时段识别与任务匹配的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 工作生活平衡指标与预警
工作生活平衡指标与预警

**处理**: 解析工作生活平衡指标与预警的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回工作生活平衡指标与预警的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级效率追踪系、支持团队效率分析、自动数据采集与、核心能力、效率管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 效率追踪器专业版处理的输入数据或指令 |
...
- **输入长度**：自然语言指令的长度限制为1000个字符，超过此长度将无法正确解析。
- **数据格式**：输入数据必须符合规定的JSON格式，否则可能导致解析错误。
- **敏感信息**：输入中不应包含敏感信息，如个人隐私数据，否则可能导致数据泄露。

### 性能边界

- **并发处理**：同时处理的任务数量上限为50个，超过此数量可能导致性能下降或超时。
- **数据处理速度**：对于大规模数据集，数据处理速度可能受到系统资源限制，建议分批处理。

### 兼容性约束

- **操作系统**：目前仅支持Windows、macOS和Linux操作系统。
- **运行时**：Python 3.8+ 或 Node.js 18+ 运行时环境，其他版本可能不支持。
- **API Key**：PRO版本需要有效的API Key才能访问某些高级功能。

### 数据存储

- **本地存储**：数据存储在本地，不提供云存储选项。
- **数据迁移**：数据迁移仅在版本升级时支持，不支持跨版本迁移。

### 安全限制

- **操作审计**：仅支持记录操作日志，不提供实时监控或报警功能。
- **权限管理**：团队协作模式下，权限管理由企业内部系统负责。
