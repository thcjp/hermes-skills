...
version: 1.0.0
displayName: 店铺文化助手专业版
summary: "企业级店铺运营平台，支持多店铺管理、数据分析、团队培训与商业授权，面向连锁品牌、电商运营公司与代运营机构。"
license: Proprietary
edition: pro
---
...
店铺文化助手专业版是一款专为连锁品牌、电商运营公司与代运营机构设计的企业级店铺运营平台。它不仅继承了免费版的文案生成功能，还扩展了多店铺管理、销售数据分析、团队培训、批量生成、商业授权、电商平台对接等企业级能力。专业版旨在提升工作效率，为用户提供结构化的工作流程和配置指引，通过深度差异化处理和用户反馈优化，显著提升了实用性和可操作性。
...
使用 `input_params` 参数进行配置。该参数用于解析核心功能执行的输入参数，完成核心逻辑，并返回结构化响应。支持创建、查询和导出操作。

### 参数配置与调用

使用 `config_options` 参数进行配置。该参数用于解析参数配置与调用的输入参数，完成核心逻辑，并返回结构化响应。支持修改、重置和导入操作。

### 结果处理与输出

使用 `output_format` 参数进行配置。该参数用于解析结果处理与输出的输入参数，完成核心逻辑，并返回结构化响应。支持导出、保存和转换操作。
