...
summary: "企业级中文演示文稿系统,支持中文排版引擎、本地化品牌模板与多地区适配。核心能力: 文档处理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 文档处置领域的专业化 AI 辅助工具,包含企业级高级功能兼容. 适用于需要pptx cn tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - PowerPoint
  - PPTX
  - 中文
...
**处理**: 解析中文排版引擎的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回中文排版引擎的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 本地化品牌模板库与多地区适配
本地化品牌模板库与多地区适配

**处理**: 解析本地化品牌模板库与多地区适配的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回本地化品牌模板库与多地区适配的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 中英混排自动调整与字体配对
中英混排自动调整与字体配对

**处理**: 解析中英混排自动调整与字体配对的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回中英混排自动调整与字体配对的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 批量中文演示文稿生成
批量中文演示文稿生成

**处理**: 解析批量中文演示文稿生成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量中文演示文稿生成的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 中文可读性评分与内容优化建议
中文可读性评分与内容优化建议

**处理**: 解析中文可读性评分与内容优化建议的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回中文可读性评分与内容优化建议的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级中文演示文、稿系统、支持中文排版引擎、本地化品牌模板与、核心能力、文档处理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | PPT 中文工具专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "PPT 中文工具专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pptx cn pro"
    }
  },
