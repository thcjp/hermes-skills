...
license: MIT
edition: free
description: "面向个人开发者的K8s YAML清单产出工具。兼容Deployment/Service/. 适用于需要k8s devops tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性。Use when 需要代码生成、编程辅助、调试测试、开发部署时使用。不适用于无明确技术栈的模糊需求。"
tags:
  - Operations
  - Kubernetes
  - DevOps
...
- **资源类型限制**：免费版仅支持Deployment、Service、ConfigMap、Secret、Ingress、Job/CronJob、PVC、Namespace等基础资源的清单生成和校验，不支持Helm Chart和Kustomize。
- **模板限制**：免费版提供预置模板，不支持用户自定义模板。
- **参数限制**：输入参数需符合预定义的格式和类型，如资源名称、镜像名称、端口等，否则可能导致生成失败。

### 性能边界

- **并发处理**：免费版在处理大量并发请求时可能存在性能瓶颈，建议在资源较少的情况下使用。
- **数据量限制**：对于生成的清单文件，免费版没有明确的数据量限制，但过大的数据量可能导致处理速度变慢。

### 兼容性约束

- **操作系统兼容性**：免费版在Windows、macOS和Linux操作系统上均可运行，但可能存在细微差异。
- **Python版本兼容性**：免费版要求Python版本为3.8及以上，不支持Python 2.x版本。
- **kubectl兼容性**：免费版在执行dry-run校验时需要kubectl环境，且kubectl版本需与免费版兼容。

### 其他限制

- **API Key**：免费版无需API Key，所有操作均在本地完成。
- **版本控制**：免费版不提供版本控制功能，建议用户将清单文件纳入Git管理。
- **复杂业务场景**：对于复杂的业务场景，免费版可能无法满足所有需求，建议结合人工经验判断。
...
- **自动化执行**: Kubernetes YAML清单生成工具，支持常用资源模板与基础校验.。面向个人开发者的K8s YAML清单生成工具。
