...
### 输入限制
- **评估项目成员数量**：单个评估项目建议不超过30人，以保证投票决策的效率和质量。
- **工具数据库工具数量**：虽然内置200+常用工具，但手动扩展工具库时，建议每次添加的工具数量不宜过多，以避免数据库性能下降。
- **评估维度与权重**：自定义评估模型时，建议维度数量不宜过多，以免影响评估效率和结果的清晰度。

### 性能边界
- **并发处理能力**：协作评估功能在多人同时投票时，系统可能会出现短暂的响应延迟，建议在高峰时段合理分配投票任务。
- **报告生成速度**：生成决策报告时，PDF格式的生成速度可能会比Markdown格式慢，因为PDF需要额外的渲染过程。

### 兼容性约束
- **操作系统兼容性**：虽然支持Windows、macOS和Linux，但某些特定功能可能在不同的操作系统上表现不一致。
- **LLM支持**：需要依赖LLM（大型语言模型）的支持，若LLM服务不可用或响应缓慢，可能会影响技能的执行效率。
- **第三方依赖**：某些功能依赖于第三方工具，如pandoc和Mermaid CLI，这些工具的安装和配置可能会增加使用难度。
