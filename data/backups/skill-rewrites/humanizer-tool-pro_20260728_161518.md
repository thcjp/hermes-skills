...
### 输入限制
- **文档格式**：仅支持处理纯文本文件，如 `.md`、`.txt` 等，不支持图片、PDF等非文本格式。
- **文档大小**：单个文档大小限制取决于运行环境，通常建议不超过100MB。
- **文本长度**：单个文档的文本长度限制取决于处理能力，通常建议不超过100,000个字符。

### 性能边界
- **处理速度**：批量处理速度取决于文档数量和文本长度，建议每分钟处理文档数量不超过100篇。
- **并发处理**：单次调用处理能力受限于运行环境，建议单次调用处理文档数量不超过10篇。

### 兼容性约束
- **操作系统**：支持Windows、macOS和Linux操作系统。
- **Python版本**：要求Python版本为3.9或更高。
- **LLM API**：需要接入支持SKILL.md的AI Agent，如Claude Code、Cursor、Codex、Gemini CLI等。

### 其他限制
- **多语言支持**：虽然支持多语言处理，但对于非英语文本的处理效果可能不如英语文本。
- **AI痕迹识别**：AI痕迹识别能力受限于当前技术，可能无法完全识别所有AI痕迹。
- **品牌语气风格库**：品牌语气风格库的定制化程度受限于用户自身的能力和资源。
