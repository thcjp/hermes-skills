...
- **输入数据格式**：输入数据必须是有效的JSON格式，否则技能将无法正确解析和处理。
- **数据长度**：输入数据长度不应超过预设的最大长度限制，否则可能导致技能处理失败。
- **参数类型**：输入参数的类型必须与技能定义的类型相匹配，否则技能将无法执行。

### 性能边界

- **并发处理**：技能支持一定数量的并发请求，但超过限制可能导致性能下降或服务不可用。
- **响应时间**：技能的响应时间受限于服务器性能和网络延迟，可能无法满足所有场景下的实时性要求。
- **资源消耗**：技能运行过程中会消耗服务器资源，如CPU和内存，长时间大量运行可能导致资源耗尽。

### 兼容性约束

- **浏览器支持**：技能主要针对现代浏览器，如Chrome和Edge，不支持旧版浏览器。
- **操作系统**：技能在Windows、macOS和Linux操作系统上运行，但可能存在兼容性问题。
- **JavaScript环境**：技能依赖于JavaScript环境，不支持纯静态页面或无JavaScript的网站。

### 反自动化检测限制

- **检测强度**：技能的反自动化检测能力针对常见检测策略，但对于高级或定制化的检测可能无效。
- **检测范围**：技能的反自动化检测主要针对浏览器行为，对于服务器端的检测可能无法生效。
- **检测效果**：反自动化检测可能无法完全避免被检测到，但可以降低被检测到的概率。

### 平台踩坑指南限制

- **平台支持**：技能的踩坑指南针对部分热门平台，对于其他平台可能不适用。
- **数据准确性**：技能提供的平台数据仅供参考，实际效果可能因平台更新而有所差异。
- **操作复杂性**：部分平台的操作可能较为复杂，需要根据实际情况进行调整。

### SPA内部导航策略限制

- **平台支持**：技能的SPA导航策略主要针对Next.js等常见SPA框架，对于其他框架可能不适用。
- **路由规则**：技能的SPA导航策略基于常见的路由规则，对于复杂的路由结构可能需要定制化处理。
- **性能影响**：SPA导航策略可能对页面性能产生一定影响，需要根据实际情况进行优化。

### 连接管理器限制

- **连接数量**：技能的连接管理器支持一定数量的连接，超过限制可能导致性能下降或服务不可用。
- **连接类型**：技能的连接管理器主要针对WebSocket连接，对于其他类型的连接可能不适用。
- **连接稳定性**：连接管理器无法保证连接的稳定性，可能存在连接断开的情况。

### 多标签页管理限制

- **标签页数量**：技能的多标签页管理支持一定数量的标签页，超过限制可能导致性能下降或服务不可用。
- **标签页切换**：技能的标签页切换可能存在延迟，对于实时性要求较高的场景可能不适用。
- **标签页关闭**：关闭标签页时，技能无法保证所有资源都被正确释放。
