...
license: "Proprietary"
edition: "pro"
description: "|-. 适用于需要dlazy gen tool相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 工作流
  - prompt
  - dlazy
...
- **输入数据格式**：输入数据必须符合规定的JSON格式，否则可能导致处理失败。
- **字符长度**：输入文本的长度应控制在合理范围内，过长的文本可能导致处理速度变慢或失败。
- **引用数量**：管道链接中引用的输出数量有限制，过多引用可能导致处理错误。
- **模型参数**：某些模型可能对输入参数有特定的要求，如分辨率、时长等，超出范围可能导致生成失败。

### 性能边界

- **并发处理**：单次请求处理多个任务时，系统性能可能受到影响，建议分批处理。
- **处理速度**：复杂任务的处理速度可能较慢，请耐心等待。
- **API限制**：根据API Key的等级，每月有处理任务的次数限制。

### 兼容性约束

- **操作系统**：支持Windows、macOS和Linux操作系统。
- **Node.js版本**：要求Node.js版本为16或更高。
- **第三方依赖**：部分功能可能需要FFmpeg等第三方工具的支持。

### 其他限制

- **版权内容**：不适用于版权受保护的媒体内容处理。
- **批量任务**：单批任务建议不超过50个，超大批量请分批执行。
- **API Key**：API Key可随时轮换或撤销，请定期检查并更新。
- **余额**：视频生成消耗较高，建议定期检查余额。
