...
- **关键词数量**：批量查询功能支持单次最多执行50个关键词的搜索任务，超过此数量可能导致性能下降或任务失败。
- **关键词长度**：单个关键词长度建议不超过100个字符，过长的关键词可能导致搜索结果不准确或任务执行失败。
- **结果数量**：单次搜索任务返回的结果数量上限为20条，超过此数量可能导致结果加载缓慢或内存不足。

### 性能边界

- **并发搜索**：系统支持并行执行搜索任务，但过高的并发数可能导致服务器负载过高，建议根据服务器配置调整并发数。
- **网络延迟**：搜索结果加载速度受网络延迟影响，建议使用高速网络环境以提高搜索效率。
- **数据处理能力**：对于大量数据的结果处理，可能需要较长的处理时间，建议根据实际情况调整任务执行时间。

### 兼容性约束

- **操作系统**：目前支持Windows、macOS和Linux操作系统，其他操作系统可能需要额外配置。
- **Python版本**：要求Python版本为3.8及以上，不同版本的Python可能需要调整依赖库。
- **浏览器引擎**：依赖Playwright库进行网页抓取，需要安装Chromium浏览器引擎。

### 其他限制

- **API Key**：PRO版核心功能无需API Key，但如需启用邮件告警，需要配置SMTP信息。
- **LLM支持**：需要LLM支持，无LLM环境不可用。
- **复杂场景**：对于复杂业务场景，建议结合人工经验判断，以避免误判或漏判。
