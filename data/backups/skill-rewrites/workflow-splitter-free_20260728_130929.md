...
summary: "将复杂任务拆解为可执行子步骤，为每步匹配最合适模型并跟踪进度.。工作流分解器免费版为知识工作者提供轻量级的任务拆解能力，聚焦将复杂工作任务分解为详细、可执行的子步骤，并为每个步骤匹配最合适的"
license: MIT
edition: free
description: "工作流分解器免费版为知识工作者包含轻量级的任务拆解能力，聚焦将复杂工作任务分解为详细、可执行的子步骤，并为每个步骤匹配最合适的执行模型。解决\"面对复杂任务不知从何下手\"的核心痛点。Use. 适用于需要workflow splitter相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 任务拆解
  - 工作流分解
  - 模型编排
...
**处理**: 解析任务接收与分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回任务接收与分析的响应数据,包含状态码、结果和日志.
### 2. 步骤拆解
...
**处理**: 解析步骤拆解的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回步骤拆解的响应数据,包含状态码、结果和日志.
### 3. 模型匹配
...
**处理**: 解析模型匹配的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回模型匹配的响应数据,包含状态码、结果和日志.
### 4. 进度跟踪
...
**处理**: 解析进度跟踪的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回进度跟踪的响应数据,包含状态码、结果和日志.
...
**处理**: 解析问题诊断的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回问题诊断的响应数据,包含状态码、结果和日志.
