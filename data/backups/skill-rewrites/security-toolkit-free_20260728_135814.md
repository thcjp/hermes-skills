...
### 输入限制
- **文件大小限制**：由于内存和性能限制，单个文件的大小不应超过100MB。
- **目录深度限制**：扫描目录的深度不应超过10层，以避免递归调用过深导致的栈溢出。
- **文件类型限制**：免费版仅支持扫描JavaScript、TypeScript、Python、Solidity、Markdown等特定文件类型。

### 性能边界
- **扫描速度**：对于包含大量文件的目录，扫描速度可能会降低，因为每个文件都需要逐行分析。
- **并发扫描**：免费版不支持并发扫描，即同一时间只能扫描一个目录。

### 兼容性约束
- **操作系统兼容性**：虽然理论上可以在Windows、macOS和Linux上运行，但最佳性能和稳定性可能在Linux系统上。
- **Python版本兼容性**：Python版本应至少为3.8，因为某些依赖库可能不支持更早的版本。
- **Node.js版本兼容性**：Node.js版本应至少为16，以确保所有依赖项都能正常工作。

### 其他限制
- **无API Key配置**：免费版无需API Key，因此无法访问远程服务或数据库。
- **无运行时监控**：免费版不提供运行时监控功能，因此无法检测动态运行时的安全风险。
- **无可视化报告**：免费版不提供可视化报告，所有输出均为文本格式。
