...
license: Proprietary
edition: pro
description: "翻译工具专业版，面向本地化团队与内容工作室的高阶翻译平台。核心能力:。可自发提升工作效率. 适用于需要translate toolkit相关能力的开发场景,提供结构化的工作流程和配置指引. 该工具经过深度差异化处理,针对用户反馈和使用痛点进行了优化改进,提升了实用性和可操作性."
tags:
  - 翻译
  - 本地化
  - 术语管理
...
### 输入限制
- **文件大小**：批量翻译时，单个文件大小不应超过系统允许的最大文件大小限制，通常为几百兆字节。
- **文件类型**：支持的文件类型包括Markdown、HTML、XML等，但不支持图片、视频等多媒体文件。
- **编码格式**：输入文件应使用UTF-8编码，以确保正确翻译。

### 性能边界
- **并发数**：专业版支持可配置的并发数，但过高的并发数可能导致系统资源紧张，影响翻译效率。
- **处理速度**：翻译速度受限于LLM API的响应时间和本地硬件性能，无法保证所有操作瞬间完成。

### 兼容性约束
- **操作系统**：仅在Windows、macOS和Linux操作系统上运行，不支持其他操作系统。
- **Node.js版本**：需要Node.js 18或更高版本，以支持必要的依赖库。
- **LLM API**：需要与Agent平台兼容的LLM API，不支持所有LLM API。

### 其他限制
- **术语库容量**：术语库支持万级术语条目，但对于非常大的术语库，导入和查询操作可能需要较长时间。
- **网络连接**：批量翻译和术语库管理需要稳定的网络连接，否则可能导致操作失败。
- **版本控制**：与版本控制系统集成的功能依赖于系统配置和版本控制工具的支持。
