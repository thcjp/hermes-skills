...
WhatsApp图片发送-免费版在设计和实现时考虑了以下边界条件和限制：

- **输入文件大小限制**：由于网络传输和平台限制，免费版对单次发送的图片大小有限制。建议图片大小不超过5MB，以确保发送效率和成功率。

- **并发发送限制**：免费版不支持并发发送图片，即同一时间只能发送一张图片。这避免了因并发操作导致的性能瓶颈。

- **图片格式限制**：免费版仅支持JPG、PNG和GIF三种图片格式。其他格式如PDF、TIFF等无法直接发送。

- **网络稳定性要求**：发送图片需要稳定的网络连接。在网络不稳定或连接中断的情况下，发送操作可能会失败。

- **平台兼容性**：免费版在支持SKILL.md规范的AI Agent平台上运行，如Claude Code、Cursor、Codex、Gemini CLI等。在其他平台上可能无法正常使用。

- **工作区空间限制**：免费版使用工作区存储临时文件，工作区空间有限。频繁发送大文件可能会导致空间不足。

- **发送频率限制**：为防止滥用，免费版对发送频率有限制。连续发送过多图片可能导致发送操作被限制。

- **发送目标限制**：免费版仅支持发送至单个联系人，不支持群组发送。

- **功能限制**：免费版不支持视频、音频、文档等文件类型的发送，不支持定时发送、模板消息等高级功能。

## 性能边界

WhatsApp图片发送-免费版在性能方面有以下边界：

- **图片下载速度**：受网络带宽和服务器性能影响，图片下载速度可能不稳定。

- **发送速度**：发送图片的速度取决于网络连接和WhatsApp服务器的响应速度。

- **并发处理能力**：由于免费版不支持并发发送，同时处理的图片数量有限。

- **处理延迟**：从用户输入到图片发送完成，可能会有一定的延迟。

- **资源消耗**：免费版在运行过程中会消耗一定的系统资源，如CPU、内存等。

## 兼容性约束

WhatsApp图片发送-免费版在兼容性方面有以下约束：

- **操作系统**：支持Windows、macOS和Linux操作系统。

- **网络连接**：需要稳定的网络连接，包括Wi-Fi或移动数据。

- **AI Agent平台**：支持SKILL.md规范的AI Agent平台，如Claude Code、Cursor、Codex、Gemini CLI等。

- **Skill平台**：需要Skill平台支持，并安装了`message`工具。

- **WhatsApp账号**：需要有效的WhatsApp账号，且账号状态正常。

- **图片格式**：仅支持JPG、PNG和GIF三种图片格式。

## 输入限制

WhatsApp图片发送-免费版对输入有以下限制：

- **图片URL或路径**：输入的图片URL或路径必须有效，且图片格式为JPG、PNG或GIF。

- **联系人手机号**：输入的手机号必须包含国家代码，且为有效的WhatsApp账号。

- **说明文字**：说明文字长度有限制，建议不超过200个字符。

- **工作区路径**：工作区路径必须存在，且Agent有权限访问。

- **配置选项**：可选配置选项需符合平台规范，否则可能导致发送失败。
