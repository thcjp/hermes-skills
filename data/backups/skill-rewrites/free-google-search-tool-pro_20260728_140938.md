...
### 输入限制
- **查询长度**：单个查询的长度不应超过500个字符，以避免搜索性能下降和潜在的错误。
- **并发数**：批量搜索时，最大并发数不应超过5，以符合Google的反爬虫策略。
- **结果数量**：单次搜索结果数量限制为100条，批量搜索时，单次请求结果数量限制为20条。

### 性能边界
- **搜索延迟**：搜索响应时间受网络条件和Google服务状态影响，通常在几秒到几十秒之间。
- **缓存容量**：结果缓存容量受限于可用存储空间，默认缓存24小时，可配置最长为168小时。

### 兼容性约束
- **浏览器自动化**：支持主流浏览器，如Chrome和Firefox，但不支持移动浏览器。
- **操作系统**：支持Windows、macOS和Linux操作系统。
- **语言支持**：支持中、英、日、韩、西、法、德七种语言搜索，但翻译功能可能不支持所有语言。

### 其他限制
- **代理使用**：使用代理时，应确保代理服务器稳定可靠，并遵守相关法律法规。
- **API Key**：搜索功能无需API Key，但告警推送和LLM模型路由可能需要配置相关API Key。
- **网络环境**：需要稳定的网络连接，访问Google服务可能需要代理或VPN。
