...
### 输入限制
- **文件数量**: 当知识库包含大量`.md`文件时，solo audit可能需要较长时间完成审计，因为每个文件都需要被检查。
- **文件大小**: 单个文件的大小不应超过操作系统或文件系统的限制，否则可能导致审计失败。
- **路径长度**: 操作系统对路径长度有限制，过长的路径可能导致审计工具无法正确访问文件。

### 性能边界
- **处理速度**: 对于包含大量文件和复杂结构的知识库，solo audit的性能可能会受到影响，处理速度可能较慢。
- **资源消耗**: 审计过程可能消耗大量CPU和内存资源，特别是在检查大量文件时。

### 兼容性约束
- **操作系统**: solo audit需要运行在支持SKILL.md的AI Agent平台上，如Claude Code、Cursor、Codex、Gemini CLI等。
- **文件格式**: 仅支持`.md`格式的文件，对于其他格式的文件，审计工具无法进行检查。
- **外部依赖**: 如果项目中有外部链接或脚本，solo audit可能无法正确处理，需要确保所有依赖项都符合预期。
