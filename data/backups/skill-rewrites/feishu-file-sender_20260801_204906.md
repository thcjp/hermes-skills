...
edition: "pro"
description: |-
  |- 功能涵盖: feishu,。Use when 用户需要feishu-file-sender相关功能时使用。不适用于超出本技能能力范围的复杂需求。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。提供结构化输出和错误处理机制。

  飞书文件发送专业版面向企业用户与高效能个人用户。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。输出结果可直接使用，减少二次加工成本。支持多场景应用和灵活配置。
tags:
  - 飞书
  - 文件发送
  - 分析
...
```python
# 示例1: 使用飞书文件发送专业版上传文件并发送给指定用户
from feishu import Client, File
...
# 示例2: 使用飞书文件发送专业版上传并发送图片
from feishu import Client, Image
...
## 功能概览
- **自动化执行**: 企业级文件批量分发与稳定投递，支持审计与重试机制。飞书文件发送专业版面向企业用户与高效能个人用户。Use when 需要
