...
* **文件路径**：文章发布功能要求用户必须提供位于当前工作目录下的 `.md` 文件路径，或直接粘贴 Markdown 正文。AI 不得主动猜测或代填路径，也不得处理诸如 `[REDACTED_SSH_PATH]` 这类敏感路径。
* **文章内容**：文章内容长度有限制，通常不超过掘金平台的限制。AI 应确保发布的内容符合平台规范。
* **图片大小**：文章中插入的图片大小有限制，通常不超过 2MB。AI 应在发布前检查图片大小，避免超限。
* **Cookie 过期**：保存的掘金会话 Cookie 有效期有限，过期后需要用户重新登录。

### 性能边界

* **并发请求**：掘金 API 对并发请求有限制，AI 应避免短时间内发起过多请求，以免触发 API 限制。
* **批量下载**：批量下载功能有硬上限，默认 20 篇，硬上限 50 篇。AI 应确保在用户同意的情况下才执行批量下载。

### 兼容性约束

* **操作系统**：技能在 Windows、macOS 和 Linux 操作系统上运行，但具体兼容性取决于 Python 和 Playwright 的版本。
* **浏览器**：浏览器登录功能依赖于 Playwright，因此需要确保 Playwright 支持用户使用的浏览器。
* **网络环境**：技能需要稳定的网络连接，否则可能导致请求失败或响应延迟。

### 其他限制

* **安全性**：技能在处理用户数据时，需要确保数据的安全性和隐私性，避免数据泄露。
* **合规性**：技能需要遵守掘金平台的规则和政策，避免违规操作。
* **可维护性**：技能代码应具有良好的可读性和可维护性，以便后续维护和升级。
