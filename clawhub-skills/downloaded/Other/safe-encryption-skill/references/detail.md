...
```bash
echo "LS0tLS1CRUdJTi..." | echo "[REDACTED_BASE64]" | safe decrypt -k ~/.safe/keys/id.x25519.key
...
The SAFE CLI automatically discovers and uses SSH private keys from `[REDACTED_SSH_PATH]
...
2. `[REDACTED_SSH_PATH] - All SSH private keys in `[REDACTED_SSH_PATH] directory
   * Ed25519 keys → converted to X25519
   * P-256 ECDSA keys → used directly
...
safe: using SSH key [REDACTED_SSH_PATH]
...
* Private keys can be in `~/.safe/keys/` (SAFE format) **OR** `[REDACTED_SSH_PATH] (OpenSSH format)
...
ssh-keygen -t ed25519 -f [REDACTED_SSH_PATH] -N "" -C "safe-agent-key"
gh ssh-key add [REDACTED_SSH_PATH] --title "SAFE Agent Key"
...
4. Click **ADD** → **KEY** → paste your SSH private key (from `[REDACTED_SSH_PATH] or `[REDACTED_SSH_PATH]
...
* **CLI is simpler for decryption** — it auto-discovers SSH keys from `[REDACTED_SSH_PATH] browser requires pasting the private key once
...
| SSH key auto-discovery | ✅ Yes (`[REDACTED_SSH_PATH] | ❌ No (manual paste) |
...
Personal keys are stored in `~/.safe/` (similar to `[REDACTED_SSH_PATH] The CLI manages this directory automatically:
...
6. **Always pipe instead of writing temp files.** When decrypting inline/embedded content (base64, pasted data), pipe directly: `echo "..." | echo "[REDACTED_BASE64]" | safe decrypt -k key.key`. Never write intermediate `.safe` files to `/tmp` or elsewhere.
...
> 详细内容已移至 `references/detail.md` - ## Installation
...
git clone <https://[SkillHub仓库].git> ~/.claude/skills/safe-encryption
