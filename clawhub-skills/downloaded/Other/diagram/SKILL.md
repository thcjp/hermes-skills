...
### 输入限制
- **描述长度**：输入描述的长度应控制在合理范围内，过长可能导致解析困难或错误。
- **格式规范**：输入描述应遵循一定的格式规范，如使用正确的关键词和语法结构。
- **复杂度**：对于过于复杂的系统或流程，可能需要分解成多个部分逐步生成图表。

### 性能边界
- **并发处理**：同一时间处理多个请求时，性能可能受到影响，建议合理分配请求。
- **响应时间**：在高峰时段，响应时间可能有所延迟，请耐心等待。

### 兼容性约束
- **平台兼容**：目前支持Windows、macOS和Linux操作系统，其他平台可能存在兼容性问题。
- **浏览器兼容**：Mermaid和PlantUML的渲染效果可能在不同浏览器中存在差异。
- **API版本**：依赖的LLM API版本可能影响技能的功能和性能。
