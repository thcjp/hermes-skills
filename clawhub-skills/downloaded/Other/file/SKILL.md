...
license: MIT
description: |-
  A comprehensive AI agent skill for managing the files, folders, and
  documents that accumulate acr。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。适用于独立开发者、企业团队和自动化工作流场景。
tags:
- Other
tools:
  - - read
...
- A comprehensive AI agent skill for managing the files, folders, and
  documents that accumulate acr
- 触发关键词: files, comprehensive, file, agent, managing, skill
...
### 输入限制
- **文件大小限制**：技能可能对单个文件的大小有限制，超出限制的文件可能无法处理。
- **文件格式限制**：并非所有文件格式都支持，如某些加密格式或特殊格式文件可能无法识别和处理。
- **文件数量限制**：对于大量文件的批量处理，技能可能存在处理数量上限。

### 性能边界
- **处理速度**：对于大量文件或复杂文件结构，处理速度可能会受到影响，可能需要较长时间完成。
- **资源消耗**：处理大量文件或复杂操作时，可能会消耗较多的系统资源，如CPU和内存。

### 兼容性约束
- **操作系统兼容性**：技能可能在某些操作系统版本上表现不佳或不支持。
- **软件版本兼容性**：技能可能需要特定版本的软件支持，如特定的AI Agent平台版本。

### 其他限制
- **隐私和安全**：技能不适用于处理包含敏感信息的文件，如个人隐私数据或公司机密。
- **人工干预**：某些操作可能需要用户的人工干预，如文件重命名或分类。
- **网络依赖**：技能可能依赖于网络连接，如访问云端存储或使用外部API服务。
