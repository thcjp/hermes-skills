...
license: MIT-0
description: |-
  长篇网文写作。从大纲到正文，辅助长篇网络小说的创作，包括世界观、人物、情节线管理。触发方式：/story-long-write、/写长篇、「帮我开书」「写大纲」「日更」「续写」「继续写」「修改第...
...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/story-long-analyze` Stage 3+ 或重新 `/story-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」——选段以主对标剧情单元为单位，按「类型/桥段标签」圈同类（详见该节步骤 1），选中的剧情单元写入剧情单元卡「对标剧情参照」。无对标书则按八节点占比自排。
