...
- **输入格式**：技能仅接受符合预定义触发关键词（如“sessions”, “slides”, “powerpoint”, “control”, “notes”）的输入，否则无法识别和处理。
- **复杂指令**：对于涉及复杂逻辑或需要人工判断的指令，技能无法处理，需用户手动操作。

### 性能边界

- **并发处理**：技能不支持同时处理多个PowerPoint会话，每个会话需独立操作。
- **响应时间**：技能的响应时间取决于底层模型能力和执行环境的性能。

### 兼容性约束

- **操作系统**：仅支持在Windows、macOS和Linux操作系统上运行。
- **PowerPoint版本**：需要安装与技能版本兼容的Microsoft PowerPoint应用程序。
- **macOS功能**：macOS系统需具备`osascript`功能，以便技能通过脚本控制PowerPoint。

### 功能限制

- **自动化程度**：技能无法处理需要用户交互或复杂逻辑的PowerPoint功能。
- **数据存储**：技能无法访问或修改PowerPoint之外的文件或应用程序。
- **安全性**：技能不支持绕过PowerPoint的安全提示或执行可能损害数据安全的操作。

### 资源限制

- **内存使用**：技能的内存使用量取决于处理任务的大小和复杂性。
- **磁盘空间**：技能可能需要临时存储一些数据，如临时文件或日志文件，这可能会占用一定的磁盘空间。
