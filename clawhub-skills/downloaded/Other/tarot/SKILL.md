...
### Q1: 如何开始使用Tarot from Univoice？
A: 请先阅读使用流程章节，确认环境满足依赖说明中的要求。

### Q2: 遇到错误怎么办？
A: 请参考错误处理章节，按照表格中的处理方式操作。

### Q3: Tarot from Univoice有什么限制？
A: 请参考已知限制章节了解具体限制。
...
### 输入限制
- **问题内容**: 用户输入的问题或请求应与情感支持、内心梳理相关，避免涉及医疗、法律、财务等专业领域。
- **语言限制**: 支持的语言限于英文和中文，其他语言输入可能导致错误或不准确的结果。

### 性能边界
- **并发处理**: 同时处理多个tarot抽取请求的能力有限，高并发情况下可能需要排队等待。
- **卡牌抽取**: 卡牌抽取基于随机算法，每次抽取结果可能不同，但长期来看卡牌分布将趋于均匀。

### 兼容性约束
- **操作系统**: 仅支持Windows、macOS和Linux操作系统。
- **Agent平台**: 必须支持SKILL.md的AI Agent，如Claude Code、Cursor、Codex或Gemini CLI等。
- **网络环境**: 需要稳定的网络连接，以保证API调用的成功率和响应速度。

### 数据隐私
- **用户数据**: 用户的tarot抽取记录仅用于提供情感支持，不会用于其他目的，且保证用户隐私不被泄露。

### 依赖性
- **LLM API**: 必须有LLM API的支持，否则tarot抽取功能无法正常工作。
- **外部API**: 部分功能可能依赖于外部API，如翻译服务、图像处理等，这些API的可用性可能会影响技能的表现。

### 人工干预
- **复杂决策**: 对于需要人工判断的复杂决策场景，tarot抽取技能无法提供直接帮助，需要用户自行决策。
- **紧急情况**: 对于用户表达出的明确自伤意图，技能将暂停tarot抽取，并转向提供安全支持。
