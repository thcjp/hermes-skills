...
- **设计文件格式**：UI Audit技能仅支持特定格式的设计文件，如Figma、Sketch等，不支持3D建模和动画制作。
- **设计复杂度**：对于过于复杂的设计，技能可能无法完全覆盖所有细节，需要人工辅助进行更深入的审计。
- **交互逻辑**：技能主要针对视觉和交互一致性进行审计，对于复杂的交互逻辑和业务规则，可能需要额外的说明或人工验证。

### 性能边界

- **处理速度**：对于大型设计文件，技能的处理速度可能会受到影响，建议分批处理或优化设计文件结构以提高效率。
- **并发处理**：技能的并发处理能力有限，不建议同时处理大量设计文件，以免影响性能。

### 兼容性约束

- **操作系统**：技能在Windows、macOS和Linux操作系统上运行，但可能存在兼容性问题，具体取决于操作系统版本和配置。
- **浏览器**：如果技能通过Web界面访问，建议使用主流浏览器，如Chrome、Firefox等，以保证最佳体验。

### 其他限制

- **语言支持**：技能主要支持英语，对于其他语言的设计文件，可能需要人工翻译或使用第三方工具。
- **数据隐私**：技能在处理设计文件时，会收集部分设计数据，但会严格遵守数据隐私保护法规，确保用户数据安全。
