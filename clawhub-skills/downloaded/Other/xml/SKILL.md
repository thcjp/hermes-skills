...
### 输入限制
- XML输入文档大小限制：由于内存和性能考虑，XML文档的大小不应超过Skill所能处理的限制。具体大小限制取决于运行环境和底层模型的能力。
- 输入格式限制：输入的XML文档必须遵循XML规范，包括正确的命名空间处理和编码声明。不规范的XML文档可能导致Skill无法正确解析。
- 输入内容限制：Skill不支持解析包含非法字符或特殊结构的XML文档，例如，XML声明中的编码声明必须正确匹配文件的实际编码。

### 性能边界
- 处理速度限制：对于大型XML文档，Skill的解析和转换操作可能需要较长时间，具体时间取决于文档的大小和复杂性。
- 并发处理限制：Skill不支持同时处理多个XML文档，每次只能处理一个文档。

### 兼容性约束
- XML版本兼容性：Skill主要针对XML 1.0版本进行优化，对于XML 1.1或其他版本的兼容性可能有限。
- 命名空间处理兼容性：Skill对XML命名空间的支持可能与其他XML解析器有所不同，特别是在处理默认命名空间和属性命名空间时。
- 编码兼容性：Skill对UTF-8编码的支持最为全面，对于其他编码（如UTF-16、UTF-32等）的支持可能有限。

### 其他限制
- Skill不支持XML模式（XML Schema）的复杂约束，如类型继承、复杂类型等。
- Skill不支持XML命名空间的动态注册，所有命名空间必须在XML文档中显式声明。
- Skill不支持XML文档的验证功能，如DTD或XSD验证。
---
