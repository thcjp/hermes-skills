...
### 输入限制
- **文本长度限制**：Skill对输入文本的长度有限制，通常不超过2048个字符，以确保处理效率和准确性。
- **格式要求**：输入文本应遵循Markdown格式，否则Skill可能无法正确解析。
- **敏感信息**：Skill不处理包含敏感信息的输入，如个人隐私数据、信用卡信息等。

### 性能边界
- **处理速度**：Skill在处理大量数据或复杂任务时，可能需要较长时间完成。
- **并发处理**：Skill不支持同时处理多个并发请求，每次只能处理一个请求。

### 兼容性约束
- **操作系统**：Skill在Windows、macOS和Linux操作系统上运行，但不保证在其他操作系统上的兼容性。
- **浏览器兼容性**：Skill在主流浏览器上运行，但不保证在所有浏览器上的兼容性。
- **网络环境**：Skill需要稳定的网络连接，否则可能无法正常工作。

### 数据存储限制
- **文件大小**：Skill存储的数据文件大小有限制，超过限制可能导致Skill无法正常工作。
- **存储空间**：Skill需要足够的存储空间来存储数据和日志文件。

### 功能限制
- **无网络功能**：Skill不支持网络请求，无法访问外部API或服务。
- **无外部文件读写**：Skill无法读取或写入外部文件，所有数据存储在本地目录中。
- **无复杂逻辑处理**：Skill不支持复杂的逻辑处理，仅适用于简单的数据处理和文本分析任务。
