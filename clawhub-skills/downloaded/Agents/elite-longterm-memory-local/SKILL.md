...
* **输入长度**：单条输入的长度限制为1024个字符，超过此长度将被自动截断。
* **数据格式**：输入数据必须符合JSON格式，否则技能将无法正确解析。
* **敏感信息**：输入中不应包含敏感信息，如个人隐私数据，技能不提供加密存储。

### 性能边界

* **并发处理**：技能支持一定程度的并发处理，但高并发情况下可能存在性能下降。
* **内存使用**：由于技能使用本地存储，内存使用量与存储的数据量成正比，大量数据可能导致内存不足。

### 兼容性约束

* **操作系统**：技能在Windows、macOS和Linux操作系统上均能运行。
* **JavaScript环境**：技能依赖于Node.js环境，因此需要在支持Node.js的环境中运行。
* **依赖版本**：技能的依赖项有特定版本要求，运行前请确保所有依赖项版本符合要求。

### 数据持久性

* **本地存储**：技能使用本地存储，数据不会自动同步到其他设备。
* **备份与恢复**：技能提供备份和恢复功能，但需要手动操作。

### API Key限制

* **外部API**：技能中部分功能需要外部API支持，如LLM API，需要配置相应的API Key。
* **API Key管理**：API Key应妥善保管，避免泄露。

### 功能限制

* **复杂决策**：技能不适用于需要人工判断的复杂决策场景。
* **数据结构**：技能不支持复杂的数据结构，如嵌套JSON对象。

### 系统限制

* **技能版本**：技能版本更新可能引入新的限制或更改现有功能。
* **平台兼容性**：技能可能在不同平台上存在兼容性问题。
