...
### 输入限制
1. **内容类型**：该技能仅适用于文本内容，如文章、声明、视频脚本等。对于非文本内容（如图片、音频）或代码，技能无法执行。
2. **内容长度**：对于非常长的文本内容，技能可能无法完整处理，建议用户提供关键部分或摘要。
3. **语言支持**：技能支持多种语言，但需确保输入内容与所选事实核查服务支持的语言相匹配。

### 性能边界
1. **响应时间**：对于非常复杂或模糊的声明，技能可能需要较长时间来获取和整合信息。
2. **结果准确性**：技能依赖于外部事实核查服务的准确性，因此结果可能存在误差。

### 兼容性约束
1. **技术平台**：技能需在支持SKILL.md框架的技术平台上运行。
2. **API访问**：技能可能需要访问外部API以获取事实核查服务的数据，因此需确保API可用性和权限。
3. **数据隐私**：技能处理的数据需符合相关隐私法规，确保用户数据安全。

### 事实核查服务限制
1. **服务可用性**：技能依赖于外部事实核查服务，若服务不可用或出现故障，技能将无法执行。
2. **服务范围**：并非所有声明都能在现有事实核查服务中找到相关信息，技能可能无法提供验证结果。
3. **服务更新**：技能使用的服务列表可能随时间更新，技能需定期同步以保持最新状态。

### 结果呈现限制
1. **结果复杂性**：对于涉及多个子声明或复杂情况的声明，技能可能无法提供详细的分解分析。
2. **结果多样性**：技能可能无法呈现所有可能的结果，如部分正确、部分错误等。
3. **结果解释**：技能提供的结果可能需要用户具备一定的背景知识才能正确理解。
...
1. **手动操作**：手动验证声明需要大量时间和精力，涉及搜索、阅读、对比和分析多个来源。本技能通过自动化的方式，能够快速识别并筛选出相关事实核查服务，显著减少人工操作时间，提高效率。
   
2. **其他工具**：虽然市场上存在一些自动化的事实核查工具，但它们往往局限于特定语言或地区，覆盖面有限。本技能支持多语言和跨区域的事实核查，能够满足更广泛的用户需求。

3. **通用方法**：通用方法可能无法提供专业的事实核查服务，且缺乏对声明真实性的深度分析。本技能依托专业的核查服务，提供更加准确和可靠的验证结果。

### 独特功能

1. **多源验证**：本技能从多个事实核查机构获取信息，确保结果的全面性和准确性。
2. **区域相关性**：根据内容上下文，优先选择与内容相关的区域核查机构，提高核查的针对性。
3. **语言匹配**：尽可能使用内容原始语言的核查机构，确保核查的准确性和可靠性。
4. **可信源筛选**：严格筛选核查机构，确保仅使用可信和可靠的服务。
5. **平衡呈现**：公平呈现所有核查结果，包括支持性和反对性观点。

### 效率提升

使用本技能，用户可以节省大量时间，减少验证声明所需的步骤。例如，对于一个包含多个声明的复杂文章，本技能可以在几分钟内完成核查，而手动操作可能需要数小时甚至数天。

### 应用场景创新

1. **新闻媒体**：新闻媒体可以利用本技能快速验证新闻来源的真实性，提高报道的准确性和可信度。
2. **社交平台**：社交平台可以集成本技能，帮助用户识别和过滤虚假信息，维护平台生态健康。
3. **教育机构**：教育机构可以利用本技能培养学生的批判性思维，提高对信息真伪的判断能力。
