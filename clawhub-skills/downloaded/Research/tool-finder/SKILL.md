...
- **关键词长度**：tool-finder技能对搜索关键词的长度有限制，通常建议关键词长度在2-50个字符之间，过长的关键词可能会导致搜索结果不准确或无结果。
- **特殊字符**：关键词中不应包含特殊字符，如`<`, `>`, `|`, `&`等，这些字符可能会影响搜索结果的准确性。
- **重复关键词**：同一搜索请求中不应包含重复的关键词，这可能会导致搜索结果重复或不准确。

### 性能边界

- **搜索结果数量**：tool-finder技能默认返回的搜索结果数量有限，通常为10条。如果需要更多结果，可以通过增加`--limit`参数来调整。
- **搜索响应时间**：在高并发情况下，搜索响应时间可能会增加，建议在系统负载较低时进行搜索操作。

### 兼容性约束

- **SkillHub和Smithery版本**：tool-finder技能需要与SkillHub和Smithery的特定版本兼容，请确保使用的SkillHub和Smithery版本与tool-finder技能的版本相匹配。
- **操作系统**：tool-finder技能在Windows、macOS和Linux操作系统上均支持，但可能存在一些细微的兼容性问题，如路径配置等。

### 其他限制

- **评分限制**：tool-finder技能默认隐藏评分低于1.0的技能，因为这些技能可能不相关或质量较低。
- **限流处理**：在高并发情况下，SkillHub API可能会出现限流，tool-finder技能会自动回退，但可能会影响搜索结果的完整性。
- **MCP安装**：Smithery MCP的安装需要指定客户端，如claude-code、cursor或vscode等，tool-finder技能会输出指引让用户手动安装。
...
1. **手动搜索与SkillHub直接搜索**：
   - **手动搜索**：用户需要分别访问SkillHub和Smithery网站，手动搜索和筛选工具，耗时且效率低。
   - **SkillHub直接搜索**：虽然SkillHub提供搜索功能，但仅限于SkillHub内部，无法同时搜索Smithery MCP，且缺乏评分排序和推荐规则。
   - **Tool Finder优势**：统一搜索ClawHub skills和Smithery MCP servers，节省用户时间，并提供评分排序和推荐规则，提升搜索效率和准确性。

2. **其他工具如SEO工具**：
   - **SEO工具**：虽然SEO工具可以帮助用户进行关键词分析和排名提升，但它们通常不专注于技能和MCP的搜索，且可能无法提供来源标识。
   - **Tool Finder优势**：专注于技能和MCP的搜索，提供来源标识，帮助用户快速找到高质量的工具，同时避免黑帽SEO手段。

### 独特功能

1. **评分排序**：根据SkillHub评分对搜索结果进行排序，优先展示高评分技能，帮助用户快速找到优质工具。
2. **推荐规则**：基于评分和名称匹配度，提供推荐等级，让用户一目了然地了解工具的受欢迎程度。
3. **来源标识**：明确标识搜索结果的来源（SkillHub或Smithery），帮助用户了解工具的背景信息。
4. **自动触发**：通过预设的关键词触发搜索，无需用户手动输入，提高使用便捷性。
5. **扩展补充**：在原词搜索结果不足时，自动使用同义词进行扩展搜索，确保用户找到尽可能多的相关工具。

### 效率提升

- **节省时间**：统一搜索和推荐功能，用户无需在多个平台间切换，节省大量搜索时间。
- **减少步骤**：自动触发和评分排序功能，用户无需手动筛选和排序，减少操作步骤。

### 应用场景创新

1. **自动化工作流**：将Tool Finder集成到自动化工作流中，自动搜索和安装所需工具，提高工作效率。
2. **开发者工具箱**：为开发者提供一个便捷的技能和MCP搜索工具，快速找到所需工具，提升开发效率。
3. **企业团队协作**：企业团队可以使用Tool Finder快速找到适合团队协作的工具，提高团队效率。
