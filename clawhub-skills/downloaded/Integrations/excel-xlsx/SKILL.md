...
### 输入限制
- **文件大小**: Excel / XLSX 技能可以处理大型工作簿，但存在性能限制。对于超过一定大小的文件（例如，超过100MB），可能需要使用流式读取或分块处理来避免内存溢出。
- **公式复杂性**: 技能可以处理复杂的公式，但存在一定的复杂性限制。过于复杂的公式或嵌套公式可能导致性能下降或错误。
- **数据类型**: 对于非标准数据类型（如日期、时间、货币等），技能可能需要额外的配置或转换来正确处理。

### 性能边界
- **处理速度**: 对于大型工作簿，处理速度可能会受到影响。技能的性能取决于工作簿的大小、复杂性和机器的硬件配置。
- **并发处理**: 技能不支持并发处理多个工作簿。每次只能处理一个工作簿。

### 兼容性约束
- **Excel版本**: 技能主要针对较新的Excel版本（如Excel 2007及以上），对于旧版本（如Excel 2003及以下）可能存在兼容性问题。
- **操作系统**: 技能支持Windows、macOS和Linux操作系统，但可能存在特定于操作系统的限制。
- **宏和VBA**: 技能不支持处理包含宏或VBA代码的工作簿，因为这些功能通常需要用户交互或额外的软件支持。

### API Key限制
- 技能依赖于LLM API，因此需要有效的API Key才能运行。在没有API Key的情况下，技能无法执行任何操作。

### 其他限制
- **加密文件**: 技能不支持处理加密的Excel文件，如受密码保护的`.xls`或`.xlsx`文件。
- **外部依赖**: 技能可能依赖于其他外部库或工具，如`openpyxl`，这些依赖项的可用性和版本可能影响技能的功能和性能。
