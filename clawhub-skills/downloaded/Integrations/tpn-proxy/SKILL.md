...
summary: "经Tao Private Network去中心化SOCKS5代理发Web请求"
  Network (TPN). This sk...
license: MIT
...
TPN Proxy 技能的边界条件与限制如下：

### 输入限制
- **地理位置 (geo)**: 必须是有效的 ISO 3166-1 alpha-2 国家代码，且长度为2个大写字母。
- **租期 (minutes)**: 必须是介于1到1440之间的正整数，表示代理的租用时间（分钟）。
- **连接类型 (connection_type)**: 必须是 `any`、`datacenter` 或 `residential` 中的一个，表示所需的代理类型。
- **格式 (format)**: 必须是 `text` 或 `json` 中的一个，表示返回结果的格式。

### 性能边界
- **租期限制**: 最长租期为1440分钟，即24小时。
- **并发连接**: 单个技能实例同时处理的代理请求不应超过系统限制。
- **API 调用频率**: 遵循 TPN API 的调用频率限制，避免因调用频率过高而被限制。

### 兼容性约束
- **API 版本**: 技能必须使用 TPN API 的最新版本，以避免因 API 变更导致的兼容性问题。
- **环境变量**: `TPN_API_KEY` 环境变量必须正确设置，否则技能无法正常工作。
- **工具依赖**: 技能依赖于 `curl`、`jq` 等工具，这些工具必须在执行环境中可用。

### 结果限制
- **代理有效期**: 生成的代理凭证有效期最长为24小时。
- **代理类型**: 可用的代理类型包括商业和住宅出口节点。
- **响应格式**: 默认返回文本格式的代理配置，可选 JSON 格式。

### 安全限制
- **输入验证**: 所有用户输入都必须经过验证，防止注入攻击。
- **环境变量**: `TPN_API_KEY` 环境变量仅用于身份验证，不应在日志或输出中显示其值。
- **网络连接**: 仅与 `api.taoprivatenetwork.com` 和用户指定的 URL 进行网络连接。
