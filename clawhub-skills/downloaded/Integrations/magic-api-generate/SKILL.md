...
license: MIT-0
description: |-
  magic-api 国产接口快速开发框架。通过 Web UI 编写脚本自动映射为 HTTP 接口，无需 Controller/Service/Dao。当用户提到
  magic-api、接口快速开发...
  ...
  触发关键词: 通过, controller, api, generate, 国产接口快速, 映射为, http, magic-api-generate
tags:
- Integrations
tools:
- read
...
var token="[REDACTED]" + user.id + "_" + Date.now();
