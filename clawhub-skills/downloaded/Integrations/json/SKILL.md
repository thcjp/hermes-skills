...
### 输入限制
- JSON技能接受的数据输入应为有效的JSON格式，不支持的格式可能导致解析错误。
- 输入数据的大小应限制在合理的范围内，过大的数据量可能影响性能和响应时间。

### 性能边界
- 对于大规模数据处理，技能的性能可能会受到影响，建议分批次处理或优化数据结构。
- 由于技能的执行依赖于Agent平台的资源，高并发请求可能导致资源竞争，影响性能。

### 兼容性约束
- JSON技能可能不完全支持所有JSON扩展特性，如自定义类型、特殊序列化规则等。
- 不同版本的SkillHub平台可能对JSON技能的兼容性有所不同，请确保SkillHub版本与技能版本匹配。

### 安全限制
- 为了避免安全风险，技能不处理包含潜在恶意代码的JSON输入。
- 对敏感数据进行序列化时，应确保敏感数据被正确脱敏，避免数据泄露。

### 功能限制
- 由于SkillHub平台的限制，某些高级功能可能无法直接实现，如复杂的条件判断、循环等。
- 技能不提供对外部库或工具的直接调用，所有功能必须通过SkillHub平台提供的API实现。
