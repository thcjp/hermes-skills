...
summary: "处理PowerPoint/pptx,布局/模板/图表/编辑"
---
...
### 输入限制
- **文件格式限制**：该技能仅支持Microsoft PowerPoint presentations和`.pptx`文件格式的处理。
- **文件大小限制**：由于性能和资源限制，处理大型或高分辨率的`.pptx`文件可能需要较长时间，且可能不支持所有功能。
- **内容复杂度限制**：对于包含大量复杂图表、动画或特殊效果的`.pptx`文件，技能可能无法完全正确处理所有细节。

### 性能边界
- **处理速度**：处理速度取决于文件大小和复杂度，以及运行环境的性能。
- **并发处理**：该技能不支持同时处理多个`.pptx`文件，每次只能处理一个文件。

### 兼容性约束
- **软件兼容性**：该技能依赖于支持SKILL.md的AI Agent，因此需要Agent平台和操作系统兼容。
- **格式兼容性**：可能无法完美处理来自不同PowerPoint版本或不同软件的`.pptx`文件，因为格式和功能可能有所不同。
- **字体和媒体文件兼容性**：某些字体或媒体文件可能无法在所有环境中正确显示或渲染。
