...
- **URL格式**：输入的URL必须符合有效的HTTP或HTTPS格式，否则技能将无法识别并执行。
- **字符长度**：URL长度不应超过Chrome或Edge浏览器的最大URL长度限制（通常为2000个字符）。
- **特殊字符**：URL中不应包含特殊字符，除非这些字符是URL的一部分（如查询参数）。

### 性能边界

- **并发连接**：同一时间只能有一个WebSocket连接到浏览器的调试端口，因此不能同时执行多个CDP任务。
- **页面加载时间**：CDP自动化可能需要等待页面加载完成，对于复杂的JS渲染页面，等待时间可能较长。
- **内存限制**：浏览器和CDP自动化脚本可能受到系统内存限制的影响，导致性能下降或任务失败。

### 兼容性约束

- **浏览器版本**：CDP自动化依赖于特定版本的Chrome或Edge浏览器，旧版本可能不支持某些功能。
- **操作系统**：CDP自动化在Windows、macOS和Linux操作系统上均支持，但可能存在特定版本的兼容性问题。
- **浏览器配置**：远程调试功能必须在浏览器中手动开启，且浏览器配置可能影响CDP自动化脚本的执行。

### 交互限制

- **JavaScript执行**：CDP自动化只能执行页面中的JavaScript代码，无法直接与服务器端代码交互。
- **DOM操作**：CDP自动化只能操作页面DOM元素，无法访问或修改浏览器的其他部分，如系统设置或插件。
- **安全性**：CDP自动化可能受到浏览器的安全限制，如同源策略，这可能导致某些操作无法执行。

### 其他限制

- **反爬虫机制**：某些网站可能具有反爬虫机制，CDP自动化可能无法绕过这些机制。
- **网络稳定性**：CDP自动化依赖于稳定的网络连接，网络中断可能导致任务失败。
- **浏览器资源占用**：长时间运行CDP自动化可能导致浏览器资源占用过高，影响其他应用程序的性能。
