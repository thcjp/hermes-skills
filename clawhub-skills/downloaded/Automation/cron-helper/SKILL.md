...
summary: "免写cron语法为AI Agent排定时任务"
---
# Cron Helper
...
### 输入限制
- **任务描述长度**: 单个任务描述不应超过255个字符，以避免命令行处理错误。
- **时间间隔精度**: 时间间隔设置需为整数，例如 "1h", "30m"，不支持小数或非整数间隔。
- **任务ID**: 每个任务在系统中有一个唯一的ID，用户在管理任务时需要正确引用该ID。

### 性能边界
- **任务并发执行**: 同时执行的任务数量受限于Agent的并发处理能力，过多并发任务可能导致性能下降。
- **执行时间限制**: 单个任务执行时间不应超过30分钟，以避免长时间运行占用系统资源。

### 兼容性约束
- **操作系统兼容性**: Cron Helper技能在Windows、macOS和Linux操作系统上均支持，但某些特定功能可能在不同的操作系统上有所差异。
- **LLM API版本**: 必须使用兼容的LLM API版本，否则可能导致技能无法正常工作。

### 其他限制
- **外部API调用**: 如果任务涉及外部API调用，需要确保API调用频率符合外部API的使用限制。
- **资源限制**: 在资源受限的环境中，Cron Helper技能可能无法按预期执行，因为任务执行依赖于Agent的可用资源。
