...
### 输入限制
- **issue 描述长度**：技能无法处理超过特定长度的 issue 描述，例如超过 1000 字符的描述可能会导致处理错误。
- **代码文件大小**：对于非常大的代码文件，技能可能无法正确执行 diff 或语法检查，建议代码文件大小不超过 10MB。
- **环境变量**：技能依赖于特定的环境变量，如 `GITHUB_TOKEN` 用于认证 GitHub API，如果环境变量未设置或配置错误，技能将无法正常工作。

### 性能边界
- **并发处理**：技能设计为单线程，对于高并发请求，可能需要排队等待处理，建议在高峰时段合理分配请求。
- **响应时间**：技能的响应时间受限于网络延迟和服务器负载，建议用户在等待响应时保持耐心。

### 兼容性约束
- **操作系统**：技能在 Windows、macOS 和 Linux 系统上均应能运行，但可能存在细微的兼容性问题，如路径分隔符在不同系统中的差异。
- **GitHub 仓库**：技能仅支持公共仓库和私有仓库中具有适当权限的仓库，对于没有权限的仓库，技能将无法访问或执行操作。

### 资源限制
- **API 调用频率**：由于 GitHub API 的限制，技能可能存在 API 调用频率限制，超过限制可能导致请求失败。
- **存储空间**：技能在处理代码时可能需要临时存储文件，存储空间不足可能导致处理失败或数据丢失。
