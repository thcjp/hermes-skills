...
version: "1.0.1"
displayName: Git cli
summary: "用Git CLI检查/暂存/提交/分支/同步代码变更(社区下载版)"
  code changes. Use...
license: MIT-0
description: |-
...
### 输入限制
- **非Git仓库路径**: 当用户在非Git仓库的路径下输入命令时，技能将无法执行，并提示用户当前目录不是Git仓库。
- **无效命令参数**: 如果用户输入的命令参数无效或格式错误，技能将无法正确执行，并提示用户正确的命令格式或参数。
- **文件路径不存在**: 当用户输入的文件路径不存在时，对于需要文件路径的命令（如`git add`、`git restore`等），技能将无法执行，并提示用户文件路径不存在。

### 性能边界
- **命令执行时间**: 对于一些耗时的命令（如`git fetch`、`git pull`等），技能的响应时间可能会较长，用户需要耐心等待。
- **命令输出长度**: 对于输出结果较长的命令（如`git log`、`git blame`等），技能可能无法一次性完整输出，用户可能需要分批次查看。

### 兼容性约束
- **操作系统兼容性**: 虽然技能在Windows、macOS和Linux操作系统上均能运行，但对于某些特定命令，不同操作系统之间可能存在兼容性问题。
- **Git版本兼容性**: 技能可能无法在旧版本的Git上正常工作，建议用户使用较新版本的Git。

### 其他限制
- **外部依赖**: 技能依赖于LLM支持，无LLM环境无法使用。
- **复杂场景处理**: 对于需要人工辅助判断的复杂决策场景，技能可能无法提供准确的指导。
- **性能依赖**: 技能的性能取决于底层模型能力，可能受到模型复杂度和计算资源的影响。
