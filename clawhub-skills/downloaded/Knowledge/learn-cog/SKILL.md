...
- **任务长度**: 单个任务输入的文本长度应控制在一定范围内，以避免性能问题。建议不超过5000字符。
- **复杂度**: 对于过于复杂的任务，例如涉及大量数据处理的任务，CellCog可能无法提供准确或完整的答案。
- **格式规范**: 输入的文本格式应规范，避免使用特殊字符或格式错误，以免影响处理结果。

### 性能边界

- **响应时间**: CellCog的响应时间取决于任务复杂度和系统负载，通常在几秒到几十秒之间。
- **并发处理**: CellCog支持一定程度的并发处理，但过多并发请求可能导致响应时间延长。

### 兼容性约束

- **操作系统**: CellCog在Windows、macOS和Linux操作系统上均支持运行，但某些特定功能可能在不同操作系统上有所差异。
- **编程语言**: CellCog的SDK支持多种编程语言，但部分语言可能需要额外的依赖或配置。
- **LLM支持**: CellCog依赖于底层LLM（大型语言模型）的支持，无LLM环境无法使用。

### 数据隐私

- **用户数据**: CellCog在处理任务时可能会收集用户数据，包括输入文本和输出结果。请确保用户数据符合相关隐私法规。
- **数据存储**: CellCog不会长期存储用户数据，所有数据在任务完成后将被清除。

### 其他限制

- **3D建模和动画制作**: CellCog不适用于3D建模和动画制作。
- **复杂决策场景**: CellCog不适用于需要人工判断的复杂决策场景。
- **模型能力**: CellCog的性能取决于底层模型能力，可能无法处理所有类型的问题。
