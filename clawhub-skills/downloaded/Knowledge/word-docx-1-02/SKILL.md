...
summary: "处理Word/docx,修订/批注/页眉/编号/表格/模板(社区下载版)"
---
# Word Docx 1.02
...
- 触发关键词: edit, create, microsoft, inspect, docx, word, 1
...
### 输入限制
- **文件格式**: 仅支持处理Microsoft Word文档（.docx）格式，不支持其他格式如.doc、.pdf等。
- **文件大小**: 输入文件大小受限于Agent平台的处理能力，通常不超过一定大小的限制。
- **内容复杂性**: 对于包含大量复杂格式、宏、VBA代码或特殊布局的文档，处理效果可能不理想。

### 性能边界
- **处理速度**: 处理速度受限于底层模型能力和系统资源，对于大型或复杂文档，处理时间可能较长。
- **并发处理**: 同时处理多个文档的能力受限于Agent平台的并发处理能力。

### 兼容性约束
- **软件版本**: 与不同版本的Microsoft Word和Office套件兼容性可能存在差异，建议使用较新版本的软件。
- **字体和格式**: 特定字体和复杂格式可能在不同编辑器中显示不一致。
- **跨平台兼容性**: 虽然基于OOXML标准，但不同编辑器（如Word、LibreOffice、Google Docs）之间的兼容性可能存在差异。
