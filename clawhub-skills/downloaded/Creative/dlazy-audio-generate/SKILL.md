...
以下列出Dlazy Audio Generate技能的特定功能边界条件，以表格形式呈现。

| 边界条件 | 描述 | 示例 |
      |----------|------|------|
      | 文本长度限制 | 输入文本长度超过模型处理能力时，可能导致处理失败或输出质量下降。 | 文本长度超过1000个字符 |
      | 音频模型适用性 | 某些音频模型可能不适合处理特定类型的音频内容。 | 使用`dlazy elevenlabs-music`生成超过300秒的音乐 |
      | API调用频率限制 | 超过API调用频率限制可能导致服务暂时不可用。 | 在短时间内连续发起大量请求 |
      | 输出文件格式限制 | 某些音频格式可能因为技术限制而无法生成。 | 生成AAC格式的音频 |
      | 版权内容限制 | 不适用于处理版权受保护的媒体内容。 | 生成包含版权音乐片段的音频 |
      | 操作系统兼容性 | 需要满足操作系统要求，包括Windows、macOS和Linux。 | 在不支持的操作系统中运行 |
      | 网络连接要求 | 需要稳定的网络连接，特别是在上传和下载大文件时。 | 在网络不稳定的环境中运行 |
...
以下列出Dlazy Audio Generate技能的详细错误处理方案表。

| 错误码 | 场景描述 | 可能原因 | 处理方式 | 恢复策略 |
      |:-------|:---------|:---------|:---------|:---------|
      | 401 | 身份验证失败 | Key未设置/已过期/格式错 | 确认环境变量，重新获取Key |  |
      | 501 | 缺失必填参数 | 缺失必填项或值超范围 | 检查参数表，修正后重试 |  |
      | 502 | 本地文件读取错误 | 文件不存在或路径错误 | 检查文件路径，确保文件存在 |  |
      | 503 | API请求失败（无余额） | 余额不足 | 指导用户充值 |  |
      | 503 | API请求失败（服务器错误） | 服务器内部错误 | 等待1-2分钟后重试 |  |
      | 504 | 异步任务执行失败 | 任务超时或失败 | 检查任务状态，重试或联系支持 |  |
...
以下列出Dlazy Audio Generate技能的完整输入输出参数说明表格。

| 参数名 | 类型 | 必填 | 默认值 | 取值范围 | 示例值 |
      |:-------|:-----|:---------|:---------|:---------|:---------|
      | prompt | 字符串 | 是 | 无 | 无 | "Hello, world!" |
      | model | 字符串 | 否 | dlazy doubao-tts | 无 | dlazy elevenlabs-dialogue |
      | speed | 浮点数 | 否 | 1.0 | 0.5-2.0 | 1.5 |
      | pitch | 浮点数 | 否 | 1.0 | 0.5-2.0 | 0.8 |
      | voice | 字符串 | 否 | 无 | 无 | "male1" |
      | output_format | 字符串 | 否 | mp3 | mp3, wav,ogg | mp3 |
      | text | 字符串 | 是 | 无 | 无 | "Welcome to the coast." |
      | audio_file | 字符串 | 否 | 无 | 无 | "path/to/audio/file.mp3" |
...
以下列出Dlazy Audio Generate技能的多种使用场景，包含输入输出示例。

| 场景 | 输入 | 输出 |
      |------|------|------|
      | 生成配音 | 文本描述 "Narrate the story of Little Red Riding Hood." | 下载配音文件 |
      | 创建背景音乐 | 文本描述 "Create a calm background music for meditation." | 下载音乐文件 |
      | 生成声音效果 | 文本描述 "Create a sound effect of a door creaking." | 下载声音效果文件 |
      | 生成语音克隆 | 上传声音样本 "Upload a voice sample for cloning." | 下载克隆语音文件 |
