...
* **任务描述长度**: CellCog技能可以处理较长的任务描述，但过长的描述可能会影响生成效率和质量。建议将描述控制在1000字符以内。
* **格式规范**: 提交的任务描述应遵循一定的格式规范，如使用明确的标题、分段描述，避免使用过于模糊或歧义性的词汇。
* **技术栈兼容性**: CellCog生成的资产需要与用户指定的游戏引擎或平台兼容。例如，GLB格式的3D模型适用于Unity、Unreal、Godot等游戏引擎。

### 性能边界

* **并发处理**: CellCog技能支持多任务并发处理，但受限于服务器资源，同时处理的任务数量有限。
* **响应时间**: 在高负载情况下，技能的响应时间可能会延长，建议用户在提交任务时预留足够的时间。
* **生成效率**: CellCog的生成效率取决于任务复杂度和资源需求，简单任务可能迅速完成，复杂任务可能需要较长时间。

### 兼容性约束

* **操作系统**: CellCog技能支持Windows、macOS和Linux操作系统。
* **游戏引擎**: CellCog生成的资产适用于Unity、Unreal、Godot、Three.js和Blender等游戏引擎。
* **网络连接**: 使用CellCog技能需要稳定的网络连接，以保证数据传输的可靠性和效率。

### 其他限制

* **内容创作限制**: CellCog技能生成的资产内容受到版权和知识产权的限制，用户应确保其使用方式符合相关法律法规。
* **技术实现限制**: CellCog技能的生成能力受限于底层模型和算法，某些特定需求可能无法满足。
