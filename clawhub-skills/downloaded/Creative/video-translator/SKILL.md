...
### 输入限制
- 视频文件大小限制：单次提交的视频文件大小不超过2GB。
- 视频格式限制：支持的视频格式包括MP4、AVI、MOV、MKV等常见格式，具体支持格式以API文档为准。

### 性能边界
- 翻译速度限制：根据视频长度和复杂度，翻译速度可能有所不同，通常情况下，完整视频的翻译时间在几分钟到几十分钟不等。
- 并发处理限制：同一时间只能处理一个翻译任务，若需要同时处理多个任务，请依次提交。

### 兼容性约束
- 翻译语言限制：目前仅支持中文和英文之间的互译，其他语言的翻译功能尚未开放。
- 字幕显示限制：字幕显示功能仅支持在视频播放时显示，不支持导出带字幕的视频文件。

### 网络环境
- 确保网络连接稳定，避免因网络问题导致翻译任务失败。

### API Key
- API Key有效期为一年，过期后需重新申请。

### 其他限制
- 不支持对视频内容进行深度分析，如人脸识别、物体检测等。
- 不支持对视频进行剪辑、拼接等编辑操作。
