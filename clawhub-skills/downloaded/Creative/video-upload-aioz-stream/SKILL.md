...
summary: "上传视频到AIOZ Stream API,3步完成"
---
...
### 输入限制
* **视频文件大小**：单个视频文件大小上限取决于AIOZ Stream API的配置，但通常建议不超过50GB。
* **视频分辨率**：上传的视频分辨率不应超过API支持的分辨率上限，否则可能导致上传失败或视频质量下降。
* **视频格式**：支持的视频格式包括MP4、MOV、AVI等，具体格式请参考AIOZ Stream API文档。

### 性能边界
* **并发上传**：同时上传多个视频文件时，建议单个视频文件上传完成后再上传下一个，以避免性能瓶颈。
* **API调用频率**：频繁调用API可能导致API限制，请遵守AIOZ Stream API的使用限制。

### 兼容性约束
* **操作系统**：AIOZ Stream API支持Windows、macOS和Linux操作系统。
* **浏览器**：上传视频时，建议使用现代浏览器，如Chrome、Firefox或Safari。
* **网络环境**：稳定的网络环境是保证上传成功的关键，建议使用高速宽带网络。

### 其他限制
* **API Key**：需要有效的API Key才能使用该技能，请确保API Key配置正确。
* **视频内容**：上传的视频内容需符合AIOZ Stream API的服务条款，不得包含违规内容。
* **费用**：根据AIOZ Stream API的定价模型，上传视频可能产生费用，请提前了解相关费用信息。

## 边界条件与限制

### 输入限制
* **视频文件大小**：单个视频文件大小上限为50GB。
* **视频分辨率**：上传的视频分辨率不应超过4320p（8K）。
* **视频格式**：支持的格式包括MP4、MOV、AVI等，具体格式请参考AIOZ Stream API文档。

### 性能边界
* **并发上传**：同时上传多个视频文件时，建议单个视频文件上传完成后再上传下一个。
* **API调用频率**：频繁调用API可能导致API限制，请遵守AIOZ Stream API的使用限制。

### 兼容性约束
* **操作系统**：AIOZ Stream API支持Windows、macOS和Linux操作系统。
* **浏览器**：建议使用Chrome、Firefox或Safari等现代浏览器。
* **网络环境**：稳定的网络环境是保证上传成功的关键。

### 其他限制
* **API Key**：需要有效的API Key才能使用该技能。
* **视频内容**：上传的视频内容需符合AIOZ Stream API的服务条款。
* **费用**：根据AIOZ Stream API的定价模型，上传视频可能产生费用。
