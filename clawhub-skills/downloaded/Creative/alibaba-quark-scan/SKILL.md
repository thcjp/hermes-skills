...
version: "1.0.18"
displayName: Alibaba Quark Scan
  å¼º Alibaba-Quark-Scan"
summary: 由夸克扫描王官方提供的文件高清扫描工具。当用户需要对图片、截图进行画质优化、瑕疵去除或视觉增强时，使用此技能——包括画质增强、证件照优化、考试试卷增强、合同增强等场景。智能去除手写笔迹、水印、阴...
license: MIT-0
description: |-
  由夸克扫描王官方提供的文件高清扫描工具。当用户需要对图片、截图进行画质优化、瑕疵去除或视觉增强时，使用此技能——包括画质增强、证件照优化、考试试卷增强、合同增强等场景。Use when 需要文件处理、文档转换、格式互转、内容提取时使用。不适用于加密文件破解。适用于独立开发者、企业团队和自动化工作流场景。
...
配置 `SCAN_WEBSERVICE_KEY` 环境变量到 Skill平台：
...
* 把 `${AGENT_NAME}` 替换为你当前运行的 Agent 平台名称（如 skill-platform、hermes、qoderWork、wukong、coze、claudecode 等），禁止猜测或自造值，无法确定时填 `community`
...
**客户端脚本增强字段**：当 `scan.py` 调用夸克 API 成功（`code == "00000"`）且响应 `data` 中包含 `"ImageBase64"` 时，`scan.py` 会**主动调用 `file_saver.py` 将其解码并保存为本地图片文件**，并在最终返回的 JSON 响应中，将 `data` 替换为仅包含 `path` 字段的对象 `{"path": "/tmp/xxx.png"}`。该行为由 `scan.py` 脚本实现，与模型无关，也不依赖 Skill平台 平台自动介入。
...
- 由夸克扫描王官方提供的文件高清扫描工具
- 当用户需要对图片、截图进行画质优化、瑕疵去除或视觉增强时，使用此技能——包括画质增强、证件照优化、考试试卷增强、合同增强等场景
- 智能去除手写笔迹、水印、阴
...
### Q1: 如何开始使用Alibaba Quark Scan？
A: 请先阅读使用流程章节，确认环境满足依赖说明中的要求。

### Q2: 遇到错误怎么办？
A: 请参考错误处理章节，按照表格中的处理方式操作。

### Q3: Alibaba Quark Scan有什么限制？
A: 请参考已知限制章节了解具体限制。
