...
### 输入限制
- **时间窗口限制**：技能配置中`intervalHours`参数定义了时间窗口，超过此时间窗口的数据将不会被处理。
- **推文数量限制**：`fetchLimitForYou`和`fetchLimitFollowing`参数分别限制了从“为你推荐”和“关注的人”时间线中获取的推文数量。
- **相似度阈值限制**：`similarityThreshold`参数设置了近重复文本的相似度阈值，低于此阈值的文本将被视为重复并去重。

### 性能边界
- **处理速度**：技能的性能受限于bird工具的处理速度和系统资源。
- **数据量**：对于大量数据，技能可能需要较长时间来完成处理。

### 兼容性约束
- **bird工具要求**：技能依赖于bird工具，因此需要确保bird已正确安装并配置。
- **操作系统兼容性**：技能在Windows、macOS和Linux操作系统上运行，但可能需要根据具体操作系统进行配置调整。

### 其他限制
- **非交互式**：技能不支持交互式输入，所有输入必须在启动时提供。
- **无通知功能**：技能本身不处理通知，用户需要通过其他工作流来接收结果通知。
