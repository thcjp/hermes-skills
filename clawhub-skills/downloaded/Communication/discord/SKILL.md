...
### 输入限制
- **消息内容长度**：`sendMessage`、`editMessage`等操作的消息内容长度受限于Discord平台，通常不超过2000个字符。
- **emoji大小**：上传的emoji图片大小限制为256KB，超过此大小将无法上传。
- **sticker大小**：上传的sticker图片大小限制为512KB，超过此大小将无法上传。
- **poll答案数量**：创建poll时，答案数量限制在2到10个之间。

### 性能边界
- **并发请求**：由于Discord API的限制，同时进行的请求数量有限，大量并发请求可能导致API限制或超时。
- **响应时间**：某些操作（如上传文件）可能需要较长时间完成，具体时间取决于网络条件和Discord服务器的响应速度。

### 兼容性约束
- **Discord版本**：技能可能不完全兼容所有Discord版本，特别是旧版本可能不支持某些新功能。
- **平台限制**：某些功能可能仅在Discord桌面版或移动版中可用，而在网页版中不可用。
- **角色限制**：某些操作（如上传emoji或sticker）可能需要特定的角色权限，如管理员权限。
