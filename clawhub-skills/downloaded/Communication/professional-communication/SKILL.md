...
### 输入限制
- **输入格式**：技能仅接受纯文本输入，不支持HTML、图片或其他非文本格式。
- **长度限制**：输入文本长度不应超过1000字符，以确保处理效率和准确性。
- **内容类型**：输入内容应与专业沟通相关，如邮件草稿、会议纪要等。

### 性能边界
- **响应时间**：在正常网络条件下，技能的响应时间通常在几秒之内。
- **并发处理**：技能支持一定程度的并发请求，但高并发情况下可能需要等待。

### 兼容性约束
- **操作系统**：技能在Windows、macOS和Linux操作系统上均能运行，但可能需要根据具体操作系统调整配置。
- **浏览器兼容性**：技能在主流现代浏览器上表现良好，但在旧版浏览器上可能存在兼容性问题。
- **语言支持**：技能主要支持英语，对于其他语言的支持可能有限。

### 其他限制
- **外部API依赖**：技能依赖于外部LLM API，因此在LLM API不可用或响应缓慢时，技能可能无法正常工作。
- **复杂场景处理**：对于需要人工判断的复杂决策场景，技能可能无法提供满意的解决方案，需要人工介入。
