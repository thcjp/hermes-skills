...
### 输入限制
- **邮件数量限制**：当处理大量邮件时，例如超过1000封邮件，可能需要更长的时间来检索和操作。
- **邮件大小限制**：单个邮件的大小不应超过服务提供商的限制，通常Gmail和Outlook的附件大小限制在25MB左右。
- **特殊字符限制**：邮件地址或主题中不应包含特殊字符，这些字符可能会影响命令的执行。

### 性能边界
- **并发处理限制**：同时处理多个账户或多个邮件操作时，性能可能会下降，因为每个操作都需要与邮件服务提供商进行交互。
- **网络延迟影响**：网络延迟可能会影响命令的执行时间，特别是在发送邮件或检索大量邮件时。

### 兼容性约束
- **服务提供商限制**：目前仅支持Gmail、Outlook和Exchange账户，不支持其他邮件服务。
- **浏览器兼容性**：虽然推荐使用浏览器登录，但某些浏览器或浏览器扩展可能会影响登录过程。
- **操作系统兼容性**：虽然技能在Windows、macOS和Linux上运行，但某些命令行工具可能在不同操作系统上的行为略有差异。
