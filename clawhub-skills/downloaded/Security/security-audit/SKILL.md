...
### 输入限制
- **凭证扫描**：不支持扫描非环境变量中的凭证，如文件系统中的明文存储。
- **端口扫描**：仅限于扫描本地网络环境中的开放端口，不支持远程网络扫描。
- **配置检查**：仅检查Clawdbot配置文件，不涉及第三方应用或服务的配置。

### 性能边界
- **扫描范围**：大规模部署的扫描可能需要较长时间完成。
- **资源消耗**：全量扫描可能会对系统资源造成一定压力，建议在低峰时段运行。

### 兼容性约束
- **操作系统**：仅支持Windows、macOS和Linux操作系统。
- **Node.js版本**：需要Node.js环境，推荐使用Node.js 14或更高版本。
- **Docker支持**：如果使用Docker，需要确保Docker版本兼容。

### 其他限制
- **自动化修复**：自动修复功能仅针对常见安全问题，复杂问题可能需要手动处理。
- **外部依赖**：不依赖于外部服务或工具，但可能需要本地环境中的某些工具（如`git`）。
- **安全性**：不适用于渗透测试未授权目标，仅用于安全审计和加固。
